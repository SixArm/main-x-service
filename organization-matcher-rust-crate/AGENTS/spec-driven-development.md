...
- When the public surface of the matcher changes (re-exports from
  ...
  test in
  [`../organization-service-rust-crate/tests/duplicate_detection.rs`](../../organization-service-rust-crate/tests/duplicate_detection.rs)
  in the same PR.

## When To Update Which Section

The organization-matcher spec uses the §1–§25 matcher-crate shape (distinct
...
| The list of `Organization` fields | §6 Domain model |
...
| How `organization-service` consumes the crate | §20 |
...
| §3 Glossary | `src/organization.rs` types, `src/scoring.rs` enums |
| §4 Research basis | `AGENTS/matching-algorithm.md` |
| §5 Algorithm overview | `src/matcher.rs` |
| §6 Domain model | `src/organization.rs` |
...
- **Adding a deterministic-identifier scheme without bridge-test
  coverage** in `organization-service`. A false positive at score 1.0 is
  the worst-case bug.
