# Main Worker Service (MPI)

A high-performance, enterprise-grade Main Worker Service system built with Rust for healthcare organizations.
...
The Main Worker Service (MPI) is a critical healthcare system that maintains a centralized registry of worker identities across multiple healthcare facilities. This production-ready implementation provides:
...
```bash
# Clone repository
git clone https://github.com/sixarm/main-worker-service-rust-crate.git
cd main-worker-service-rust-crate
...
2. Copy binary: `cp target/release/main-worker-service /opt/main-worker-service/`
3. Set up environment: `cp .env.production.example /opt/main-worker-service/.env`
4. Run migrations: `sea-orm-cli migrate up`
5. Start service: `./main-worker-service`
...
```
main-worker-service-rust-crate/
├── src/
│   ├── api/
...
- **Issues**: [GitHub Issues](https://github.com/sixarm/main-worker-service-rust-crate/issues)
- **Discussions**: [GitHub Discussions](https://github.com/sixarm/main-worker-service-rust-crate/discussions)
