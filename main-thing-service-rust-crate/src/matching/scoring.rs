...
/// Compute match score between two things using weighted components.
pub fn compute_match(a: &Thing, b: &Thing, weights: &MatchWeights) -> MatchResult {
    ...
    let thing_type_score = match (&a.thing_type, &b.thing_type) {
    ...
    if a.thing_type.is_some() && b.thing_type.is_some() {
        total += weights.thing_type * thing_type_score;
        weight_sum += weights.thing_type;
    }
...
mod tests {
    ...
    use crate::models::thing_type::ThingType;

    fn central_park() -> Thing {
        let mut p = Thing::new("Central Park");
        p.thing_type = Some(ThingType::Park);
    ...
    fn test_different_things_low_score() {
        let a = central_park();
        let mut b = Thing::new("Buckingham Palace");
        b.thing_type = Some(ThingType::CivicStructure);
    ...
    fn test_gln_deterministic_match() {
        let mut a = Thing::new("Store A");
        a.identifiers = vec![ThingIdentifier::gln("1234567890123")];
        let mut b = Thing::new("Store B");
    ...
    fn test_default_weights_sum_to_one() {
        ...
        let sum = w.name + w.geo + w.address + w.thing_type + w.identifier;
