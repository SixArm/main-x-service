use main_thing_service::matching::name::name_similarity;
use main_thing_service::matching::address::address_similarity;
use main_thing_service::matching::geo::{geo_similarity, geo_similarity_with_reference, within_radius};
use main_thing_service::matching::identifier::{identifier_similarity, has_gln_match};
use main_thing_service::matching::phonetic::{soundex, soundex_match};
...
use main_thing_service::models::identifier::{IdentifierType, ThingIdentifier};
...
fn test_name_similarity_very_long_names() {
    let a = "The Very Long Name of an Incredibly Important Historical Thing in Downtown";
    let b = "The Very Long Name of an Incredibly Important Historical Thing in Downtown";
...
fn test_identifier_no_match_different_values() {
    let a = vec![ThingIdentifier::new(IdentifierType::Fips, "36061")];
    let b = vec![ThingIdentifier::new(IdentifierType::Fips, "06037")];
    let score = identifier_similarity(&a, &b);
    assert!((score - 0.0).abs() < f64::EPSILON);
}
...
fn test_match_with_custom_weights() {
    let mut a = Thing::new("Test Thing");
    a.geo = Some(GeoCoordinates::new(40.7829, -73.9654));

    let mut b = Thing::new("Test Thing");
...
fn test_match_with_all_components() {
    ...
    a.identifiers = vec![ThingIdentifier::new(IdentifierType::Gnis, "975772")];
    ...
    assert!((result.breakdown.thing_type_score - 1.0).abs() < f64::EPSILON);
    assert!((result.breakdown.identifier_score - 1.0).abs() < f64::EPSILON);
}

#[test]
fn test_batch_matching_sorted_by_relevance() {
    let target = Thing::new("Central Park");
    let candidates = [
        Thing::new("Central Park"),       // exact
        Thing::new("Central Gardens"),     // partial
        Thing::new("Buckingham Palace"),   // different
    ];
    let weights = MatchWeights::default();
