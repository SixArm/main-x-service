...
describe("verify load", () => {
  it("returns missingToken when no token is present", async () => {
    ...
    expect(result).toEqual({ error: "missingToken", title: "Sign-in link — Workforce Planning Management" });
  });
  ...
  it("returns invalidToken when the upstream responds not-ok", async () => {
    ...
    expect(result).toEqual({ error: "invalidToken", title: "Sign-in link — Workforce Planning Management" });
  });
});
