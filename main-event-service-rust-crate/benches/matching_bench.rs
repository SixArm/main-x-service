//! Benchmarks for event matching algorithms

use criterion::{criterion_group, criterion_main, Criterion, black_box};
use chrono::{NaiveDate, Utc};
use uuid::Uuid;

use main_event_service::models::*;
use main_event_service::matching::*;
use main_event_service::matching::algorithms::{
    name_matching, dob_matching, gender_matching,
    address_matching,
    tax_id_matching, document_matching,
};
use main_event_service::matching::phonetic;
use main_event_service::config::MatchingConfig;
...
fn create_test_event_with_address(
    family: &str,
    given: &str,
    ...
    let mut event = create_test_event(family, given, birth_date);
...
fn bench_full_event_matching(c: &mut Criterion) {
    let config = create_matching_config();
    let matcher = ProbabilisticMatcher::new(config);

    let dob = NaiveDate::from_ymd_opt(1980, 1, 15);
    let event = create_test_event_with_address(
        "Smith", "John", dob,
        "Springfield", "IL", "62701",
    );

    c.bench_function("match_events_pair", |b| {
        let candidate = create_test_event("Smyth", "Jon", dob);
        b.iter(|| {
            matcher.match_events(black_box(&event), black_box(&candidate)).unwrap()
        })
    });

    let candidates_10: Vec<Event> = (0..10)
        .map(|i| create_test_event(&format!("Event{}", i), &format!("Given{}", i), None))
        .collect();

    c.bench_function("find_matches_10_candidates", |b| {
        b.iter(|| {
            matcher.find_matches(black_box(&event), black_box(&candidates_10)).unwrap()
        })
    });

    let candidates_100: Vec<Event> = (0..100)
        .map(|i| create_test_event(&format!("Event{}", i), &format!("Given{}", i), None))
        .collect();

    c.bench_function("find_matches_100_candidates", |b| {
        b.iter(|| {
            matcher.find_matches(black_box(&event), black_box(&candidates_100)).unwrap()
        })
    });

    let candidates_1000: Vec<Event> = (0..1000)
        .map(|i| create_test_event(&format!("Event{}", i), &format!("Given{}", i), None))
        .collect();

    c.bench_function("find_matches_1000_candidates", |b| {
        b.iter(|| {
            matcher.find_matches(black_box(&event), black_box(&candidates_1000)).unwrap()
        })
    });
}
...
fn bench_deterministic_matching(c: &mut Criterion) {
    ...
    let candidate = create_test_event("Smith", "John", dob);
...
fn bench_tax_id_matching(c: &mut Criterion) {
    let mut p1 = create_test_event("Smith", "John", None);
    p1.tax_id = Some("123-45-6789".to_string());

    let mut p2 = create_test_event("Smyth", "Jon", None);
    ...
    let p3 = create_test_event("Jones", "Bob", None);
