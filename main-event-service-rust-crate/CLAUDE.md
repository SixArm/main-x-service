# Main Event Service (MPI)

The Main Event Service (MPI) is a critical healthcare system that maintains a
centralized registry of event identities across healthcare providers.
...
- Event identifier management (MRN, SSN, national IDs)
...
- Multiple names and addresses per event
...
```bash
# Clone repository
git clone https://github.com/sixarm/main-event-service-rust-crate.git
cd main-event-service-rust-crate
...
6. Search Engine `index_event()` -> Tantivy Index
7. Event Publisher -> EventCreated Event
...
5. Event Records -> JSON Serialization
6. HTTP Response -> Client (with pagination)

## Project Structure

```
main-event-service-rust-crate/
├── src/
│   ├── api/
...
│   │   └── mod.rs         # Event types
│   ├── models/
│   │   ├── event.rs     # Event model (with tax_id, documents, emergency_contacts)
...
**Create Event (with duplicate detection):**
...
```bash
curl -X POST http://localhost:8080/api/events/check-duplicates \
  -H "Content-Type: application/json" \
  -d '{ "name": { "family": "Smith", "given": ["John"] }, "birth_date": "1980-01-15", "gender": "male" }'
```

**Search Events (with pagination and masking):**

```bash
curl "http://localhost:8080/api/events/search?q=Smith&limit=10&offset=0&fuzzy=true&mask_sensitive=true"
```
...
```bash
curl -X POST http://localhost:8080/api/events/merge \
  -H "Content-Type: application/json" \
  -d '{ "main_event_id": "uuid-main", "duplicate_event_id": "uuid-dup", "merge_reason": "Confirmed duplicate" }'
```

**Batch Deduplication:**

```bash
curl -X POST http://localhost:8080/api/events/deduplicate \
  -H "Content-Type: application/json" \
...
```bash
curl "http://localhost:8080/api/events/{id}/export"
```

**Masked Event View:**

```bash
curl "http://localhost:8080/api/events/{id}/masked"
```
...
cargo test --lib test_event_matching        # Specific test
...
- Models (event, document, emergency contact): 8 tests
...
- GDPR Data Export: Full event data export endpoint
...
- **HL7 FHIR**: Partial compliance (Event resource)

## Performance

### Benchmarks

- **Event Create**: ~50ms (includes DB + search index + duplicate check)
- **Event Read**: ~5ms
