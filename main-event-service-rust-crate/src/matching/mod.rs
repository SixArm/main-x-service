//! Event matching algorithms and scoring
...
/// Match result containing a event and their match score
#[derive(Debug, Clone)]
...
pub trait EventMatcher: Send + Sync {
    /// Match a event against a candidate
    fn match_events(&self, event: &Event, candidate: &Event) -> Result<MatchResult>;

    /// Find potential matches for a event
...
impl EventMatcher for ProbabilisticMatcher {
    ...
    fn find_matches(&self, event: &Event, candidates: &[Event]) -> Result<Vec<MatchResult>> {
        let mut matches: Vec<MatchResult> = candidates
            .iter()
            .map(|candidate| self.scorer.calculate_score(event, candidate))
...
mod tests {
    ...
    fn create_test_event(family: &str, given: &str, dob: Option<NaiveDate>) -> Event {
        Event {
            id: uuid::Uuid::new_v4(),
            identifiers: vec![],
            active: true,
    ...
    fn test_probabilistic_find_matches() {
        ...
        let candidates = vec![
            create_test_event("Smith", "John", dob), // Exact match
            create_test_event("Smyth", "John", dob), // Close match
            create_test_event("Johnson", "Bob", NaiveDate::from_ymd_opt(1990, 5, 20)), // No match
        ];

        let matches = matcher.find_matches(&event, &candidates).unwrap();
    ...
    fn test_deterministic_matcher() {
        ...
        let dob = NaiveDate::from_ymd_opt(1980, 1, 15);
        let event1 = create_test_event("Smith", "John", dob);
        let event2 = create_test_event("Smith", "John", dob);

        let result = matcher.match_events(&event1, &event2).unwrap();
    ...
    fn test_probabilistic_matcher_with_threshold() {
        ...
        let result = matcher.match_events(&event, &candidate).unwrap();
    ...
    fn test_match_result_ordering_by_score() {
        ...
        let candidates = vec![
            create_test_event("Johnson", "Bob", NaiveDate::from_ymd_opt(1995, 5, 20)), // Low match
            create_test_event("Smith", "John", dob), // Exact match
            create_test_event("Smyth", "John", dob), // Close match
        ];
    ...
    fn test_empty_candidates_list() {
        ...
        let matches = matcher.find_matches(&event, &[]).unwrap();
