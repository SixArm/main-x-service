...
//!
//! This module implements various matching algorithms for comparing event records:
...
pub mod tax_id_matching {
    ...
    /// Match events by tax ID (exact match after normalization).
    /// Returns 1.0 for exact match, 0.0 otherwise.
    pub fn match_tax_ids(event: &Event, candidate: &Event) -> f64 {
        let tid1 = event.effective_tax_id();
...
pub mod document_matching {
    ...
    /// Match identity documents between two events.
