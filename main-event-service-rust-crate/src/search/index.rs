...
/// Fields in the event search index
#[derive(Clone)]
...
impl EventIndex {
    ...
    pub fn create<P: AsRef<Path>>(index_path: P) -> Result<Self> {
        let schema_def = EventIndexSchema::new();
...
mod tests {
    ...
    fn test_create_index() {
        ...
        let index = EventIndex::create(temp_dir.path()).unwrap();
    ...
    fn test_schema_fields() {
        let schema = EventIndexSchema::new();
    ...
    fn test_create_or_open() {
        ...
        // First call creates
        let index1 = EventIndex::create_or_open(temp_dir.path()).unwrap();
        assert_eq!(index1.stats().unwrap().num_docs, 0);

        // Second call opens
        let index2 = EventIndex::create_or_open(temp_dir.path()).unwrap();
        assert_eq!(index2.stats().unwrap().num_docs, 0);
    }

    #[test]
    fn test_index_event_and_retrieve() {
        let temp_dir = TempDir::new().unwrap();
        let event_index = EventIndex::create(temp_dir.path()).unwrap();
        let schema = event_index.schema();

        let mut writer = event_index.writer(50).unwrap();
        let event_id = uuid::Uuid::new_v4().to_string();

        let mut doc = tantivy::TantivyDocument::default();
        doc.add_text(schema.id, &event_id);
        ...
        event_index.reload().unwrap();

        let stats = event_index.stats().unwrap();
    ...
    fn test_fuzzy_search_typo() {
        ...
        let searcher = event_index.reader().searcher();
    ...
    #[test]
    fn test_delete_event_from_index() {
        use tantivy::schema::Term;
        ...
        event_index.reload().unwrap();
        assert_eq!(event_index.stats().unwrap().num_docs, 1);
        ...
        event_index.reload().unwrap();
        assert_eq!(event_index.stats().unwrap().num_docs, 0, "Document should be deleted");
    }
    ...
    fn test_search_by_name_and_year_filter() {
        ...
        // Add two events: same name, different birth years
        ...
        assert_eq!(event_index.stats().unwrap().num_docs, 2);
        ...
        let top_docs = searcher.search(&query, &TopDocs::with_limit(10)).unwrap();
        assert_eq!(top_docs.len(), 1, "Should find exactly 1 event with matching name+DOB");
    }
}
