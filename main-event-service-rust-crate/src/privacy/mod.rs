...
/// Mask sensitive fields in a event record for display.
/// Returns a new event with masked data.
...
/// Check whether a event has active consent for a given purpose.
...
/// Generate a GDPR data export for a event (right of access).
/// Returns a JSON value containing all stored event data.
...
mod tests {
    ...
    fn test_mask_event() {
        ...
        event.identifiers.push(Identifier::ssn("123-45-6789".into()));
    ...
    #[test]
    fn test_export_event_data_includes_all_fields() {
        use crate::models::*;
        ...
        event.tax_id = Some("987-65-4321".into());
        event.birth_date = Some(chrono::NaiveDate::from_ymd_opt(1990, 5, 20).unwrap());

        let export = export_event_data(&event);
