...
Main Event Service (MPI) - A healthcare event identification and matching system built with Rust. Production-ready with 15 API endpoints, 34 unit tests, and comprehensive feature set.
...
- Methods: search, fuzzy_search, search_by_name_and_year, index_event, index_events, delete_event
...
- Health: GET /health
- Event CRUD: POST /events, GET /events/{id}, PUT /events/{id}, DELETE /events/{id}
- Search: GET /events/search (pagination, fuzzy, mask_sensitive)
- Matching: POST /events/match
- Dedup: POST /events/check-duplicates, POST /events/merge, POST /events/deduplicate
- Privacy: GET /events/{id}/export, GET /events/{id}/masked
- Audit: GET /events/{id}/audit, GET /audit/recent, GET /audit/user
- OpenAPI/Swagger at /swagger-ui

## Phase 6: FHIR R5 Support (Partial)

- FhirEvent resource model with all standard fields
- Bidirectional conversion (to_fhir_event / from_fhir_event)
...
- DieselEventRepository: full CRUD with transactions
...
- `tax_id` field on Event (CPF, SSN, TIN)
...
- Real-time during POST /events (returns 409 Conflict with matches)
- POST /events/check-duplicates (explicit check)
...
- mask_event(): masks SSN, tax ID, passport, DL, phone (shows last 4)
- GET /events/{id}/export: GDPR data export (full JSON)
- GET /events/{id}/masked: masked event view
