...
`Accept: application/vnd.case-tracking.v1+json` mediatype once
...
createdb case_tracking_development

# 2) Set DATABASE_URL (or accept the default)
export DATABASE_URL=postgres://postgres:postgres@localhost:5432/case_tracking_development
...
DATABASE_URL=postgres://postgres@localhost:5432/case_tracking_test \
  cargo test                   # 6 unit (nhs) + 29 request tests
```

The request tests need a Postgres database (`case_tracking_test`
by default) for Loco's boot, but no migrations run. Create it once
with `createdb case_tracking_test`. Tests run serially
...
```
case-tracker-service-with-rust/
...
- [Sibling Svelte app](../case-tracker-front-end-with-svelte) — same domain, different stack
