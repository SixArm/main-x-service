...
//! `PATIENT_FLOW_EVENT_TRANSPORT` (default `memory`):
//!
...
//!
//! Unlike the single-entity registries, Patient Flow emits for several
//! record kinds, so `entity` and `kind` are `String`s: `ward`/`bay`/
...
//! `bed_request_created`/`bed_request_allocated`/`bed_request_cancelled`/
//! `infection_flagged`/`infection_cleared`/`red_green_recorded`.
...
pub struct Envelope {
    ...
    /// Human-oriented label (bed number, patient display name — already
...
pub fn transport() -> EventTransport {
    ...
    *TRANSPORT.get_or_init(|| {
        EventTransport::parse(&std::env::var("PATIENT_FLOW_EVENT_TRANSPORT").unwrap_or_default())
    })
}
...
mod tests {
    ...
    fn envelope_and_view_shape() {
        let a = envelope("bed", "bed_state_changed", "pid-1", "Bed 1", Some("u1"), None);
        let b = envelope("bed", "bed_state_changed", "pid-1", "Bed 1", None, None);
        assert_eq!(a.schema_version, SCHEMA_VERSION);
        assert_ne!(a.event_id, b.event_id);
        assert!(b.seq > a.seq);
        let view = EventView::from(&a);
        assert_eq!(view.kind, "bed_state_changed");
