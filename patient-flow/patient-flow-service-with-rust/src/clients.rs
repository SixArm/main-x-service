//! Upstream service clients (PF-D11): **stub-first** display-name
//! resolution for `person:` / `worker:` `EntityRefs`.
//!
//! Patient Flow references identities by URN and needs only a display
//! label for whiteboards. Lookups are read-only, cached, best-effort,
//! and **never block writes**: an unreachable upstream degrades the
//! board to showing the URN.
//!
//! Selection is per-process via `PATIENT_FLOW_UPSTREAM_MODE`:
...
//!   `PATIENT_FLOW_PERSON_SERVICE_URL` / `PATIENT_FLOW_WORKER_SERVICE_URL`,
...
/// The process-wide mode, from `PATIENT_FLOW_UPSTREAM_MODE`.
#[must_use]
pub fn mode() -> Mode {
    static MODE: OnceLock<Mode> = OnceLock::new();
    *MODE.get_or_init(|| {
        match std::env::var("PATIENT_FLOW_UPSTREAM_MODE")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase()
            .as_str()
...
/// are logged at debug and swallowed — upstream reads never fail a
