...
//! slice, the **ward-scoped handover query** (spec `audit.md`), and
...
/// `GET /api/audits?ward=<pid>&since=<rfc3339>` — the clinical
/// handover query: everything that changed on a ward since a
/// timestamp (cap 200).
...
async fn handover(
    ...
    Query(params): Query<HandoverParams>,
) -> Result<Response> {
    let ward_pid = records::parse_pid(&params.ward)?;
    format::json(Audit::for_ward_since(&ctx.db, ward_pid, params.since, 200).await?)
}
