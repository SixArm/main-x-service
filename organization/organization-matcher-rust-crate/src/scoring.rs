...
/// The outcome of matching two organizations.
...
impl Confidence {
    ...
    ///
    /// ```
    /// use organization_matcher::Confidence;
    ///
...
pub struct MatchBreakdown {
    /// Name similarity (Jaro-Winkler over name / legalName / aliases).
    pub name_score: Option<f64>,
    /// Postal-address similarity (weighted field-by-field).
    pub address_score: Option<f64>,
    /// URL / domain similarity.
    pub url_score: Option<f64>,
    /// Jurisdiction (country) similarity.
    pub jurisdiction_score: Option<f64>,
    /// Founding-date similarity (by year).
