//! Error type for the `organization-matcher` crate.

use thiserror::Error;

/// Errors produced by organization matching and normalization.
#[derive(Debug, Error)]
pub enum Error {
    ...
    /// (ORGM-T1): a negative/`NaN`/infinite weight, or a threshold
    /// outside `[0.0, 1.0]`.
