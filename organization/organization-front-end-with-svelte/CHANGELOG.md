...
`x-large` scale) arrays from `+layout.svelte`; `PickerBar` now gets no
`themes`/`sizes` prop at all, so it uses its own `DEFAULT_THEMES` (45,
alphabetical, UK/US government themes grouped at the end) and
`DEFAULT_SIZES` (the 7-step `largest`…`smallest` scale) directly — a
superset of what was offered before, not a behavioural narrowing.
