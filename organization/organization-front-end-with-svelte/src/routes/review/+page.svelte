<!--
  ...
  three ways over one selection:
    ...
    3. An inline comparison panel below both, fetching each side of the
       pair with its own `GET /api/organizations/{pid}` (there is no
       combined "fetch the pair" endpoint) and rendering a **live**
  ...
  merge. The panel therefore deep-links to `/merge` with both ids
...
<script lang="ts">
    ...
    async function runScan() {
        ...
        try {
            ...
            scanned = report.organizations_scanned;
    ...
    let pair = $state<{ a: Organization | null; b: Organization | null } | null>(
        null,
    );
    ...
    // 404) and a half-rendered comparison still beats none. Once both
    // sides are in hand, follow up with a live `/match` call for the
    // score breakdown (see the file header comment for why).
    async function loadPair(item: ReviewQueueItem) {
        ...
        const [a, b] = await Promise.allSettled([
            repo.get(item.organization_id_a),
            repo.get(item.organization_id_b),
        ]);
        if (token !== pairToken) return;
        const orgA = a.status === "fulfilled" ? a.value : null;
        const orgB = b.status === "fulfilled" ? b.value : null;
        pair = { a: orgA, b: orgB };
        pairPartial = a.status === "rejected" || b.status === "rejected";
        pairLoading = false;
        if (orgA && orgB) await loadBreakdown(token, orgA, orgB);
    }
    ...
    async function loadBreakdown(
        ...
        try {
            const ranked = await repo.matchAgainst(orgA, [orgB]);
            if (token !== pairToken) return;
            breakdown = ranked[0]?.[1]?.breakdown ?? null;
        } catch {
            // A failed breakdown call is not fatal to the comparison — the
            // panel simply shows no breakdown, same as a missing one.
            if (token === pairToken) breakdown = null;
        } finally {
            if (token === pairToken) breakdownLoading = false;
        }
    }
    ...
    const PROVENANCE_LABEL: Record<string, StringKey> = {
        operator: "review.provenance.operator",
        import: "review.provenance.import",
        matcher_suggested: "review.provenance.matcherSuggested",
    };
    ...
    // Falls back to the raw token: the column is free text server-side, so
    // an unknown value is real data and must not be swallowed.
    function provenanceLabel(provenance: string): string {
        const key = PROVENANCE_LABEL[provenance];
        return key ? t(key) : provenance;
    }
    ...
    function pairLabel(item: ReviewQueueItem): string {
        return `${shortId(item.organization_id_a)} ↔ ${shortId(item.organization_id_b)}`;
    }

    /** First postal address, flattened; `null` when there is none. */
    function addressLine(org: Organization | null): string | null {
    ...
    /** Comma-joined keywords; `null` when there are none. */
    function keywordsLine(org: Organization | null): string | null {
    ...
    // The comparison rows, so the table body stays declarative. `id` reads
    // off the review item, not the fetched `Organization` — the DTO
    // carries no id field of its own (`OrgRef.pid` is the create/list
    // shape; the review item already names both pids).
    const compareRows = $derived(
        pair && selectedItem
            ? [
                  {
                      labelKey: "detail.id" as StringKey,
                      a: selectedItem.organization_id_a,
                      b: selectedItem.organization_id_b,
                  } as { labelKey: StringKey; a: string | null; b: string | null },
                  {
                      labelKey: "review.component.name" as StringKey,
                      a: pair.a?.name ?? null,
                      b: pair.b?.name ?? null,
                  },
                  {
                      labelKey: "review.field.legalName" as StringKey,
                      a: pair.a?.legal_name ?? null,
                      b: pair.b?.legal_name ?? null,
                  },
                  {
                      labelKey: "review.component.address" as StringKey,
                      a: addressLine(pair.a),
                      b: addressLine(pair.b),
                  },
                  {
                      labelKey: "review.component.url" as StringKey,
                      a: pair.a?.url ?? null,
                      b: pair.b?.url ?? null,
                  },
                  {
                      labelKey: "review.component.jurisdiction" as StringKey,
                      a: pair.a?.jurisdiction ?? null,
                      b: pair.b?.jurisdiction ?? null,
                  },
                  {
                      labelKey: "review.component.foundingDate" as StringKey,
                      a: pair.a?.founding_date ?? null,
                      b: pair.b?.founding_date ?? null,
                  },
                  {
                      labelKey: "review.component.keywords" as StringKey,
                      a: keywordsLine(pair.a),
                      b: keywordsLine(pair.b),
                  },
              ]
            : [],
    );

    const breakdownItem = $derived(breakdownRows(breakdown));
    ...
    const cards = $derived(
        (items ?? []).map((item) => ({
            ...
            description: `${item.match_quality} · ${item.match_score.toFixed(2)} · ${item.detection_method} · ${provenanceLabel(item.provenance)}`,
            status: item.status,
        })),
    );
        ...
        <table class="queue" data-testid="review-list">
            <thead>
                <tr>
                    ...
                    <th scope="col">{t("review.col.provenance")}</th>
...
{#if selectedItem}
    ...
    >
        ...
        <dl class="kv">
            ...
            <dt>{t("review.field.provenance")}</dt>
            <dd>{provenanceLabel(item.provenance)}</dd>
            ...
            <table class="compare" data-testid="review-compare-table">
                <thead>
                    <tr>
                        ...
                        <th scope="col">
                            {t("review.compare.a")} · {shortId(item.organization_id_a)}
                        </th>
                        <th scope="col">
                            {t("review.compare.b")} · {shortId(item.organization_id_b)}
                        </th>
                    </tr>
                </thead>
                <tbody>
        ...
        {#if breakdownLoading}
            <p class="muted" data-testid="review-breakdown-loading">
                {t("review.breakdown.loading")}
            </p>
        {:else if breakdownItem.length === 0}
        ...
        {:else}
            <table class="compare" data-testid="review-breakdown">
                ...
                <tbody>
                    {#each breakdownItem as row (row.key)}
                        <tr>
                            <th scope="row">{t(row.labelKey)}</th>
        ...
        {#if item.status === "confirmed"}
            ...
            <div class="row">
                <a
                    class="button"
                    href={mergeHref(item.organization_id_a, item.organization_id_b)}
                >
                    {t("review.merge.keepA")}
                </a>
                <a
                    class="button"
                    href={mergeHref(item.organization_id_b, item.organization_id_a)}
                >
