<!--
  ...
  Kanban columns (Pending / Confirmed / Rejected / AutoMerged),
  mirroring the person/worker/place/thing boards. The queue loads on
  mount (a safe GET); the batch scan runs only on the button (POST
  /deduplicate is a destructive-classed action, never a page-load side
  effect) and upserts into the same stored queue with stable item ids.
  Dragging a pending card into Confirmed / Rejected records the
  decision through the decision endpoint; the service's
...
<script lang="ts">
    ...
    // Column ids are the wire tokens (the service serializes the
    ...
    const cards = $derived(
        (items ?? []).map((item) => ({
            id: item.id,
            label: `${item.organization_id_a.slice(0, 8)} ↔ ${item.organization_id_b.slice(0, 8)}`,
