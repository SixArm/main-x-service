...
//! asynchronous bulk import/export operations (BLK-5).
//!
//! Adopts the family schema in
//! [`agents/share/bulk-import-export.md`](../../../../agents/share/bulk-import-export.md)
//! §3, matching the person service's reference table (and care-pathway's
//! loco-idiomatic migration shape, which this one is cribbed from) so the
//! family does not drift. Scope for this crate (BLK-5) is **JSONL and
//! CSV only** — no Parquet — so `format` only ever holds `jsonl`/`csv`
//! tokens, but the column itself is a plain string so a later format
//! needs no migration.
...
enum BulkJobs {
    /// The table itself.
    Table,
    /// Job id (application-assigned, also the status-path segment).
    Id,
    /// `import` | `export`.
    Kind,
    /// The entity the job operates on (`organization`).
    Entity,
...
impl MigrationTrait for Migration {
    ...
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        ...
        // second import/export of the same data (shared doc §3). Partial,
        // so the many jobs with no key do not collide with each other.
        m.create_index(
            Index::create()
