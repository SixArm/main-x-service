...
//! (`ORGANIZATION_EVENT_TRANSPORT=outbox`) **and** `ORGANIZATION_EVENT_RELAY`
//! is truthy — so it is a no-op by default, matching the family's
//! flag-gated posture. With those on, [`spawn`] additionally selects
//! `FluvioSink` over [`LoggingSink`] when `ORGANIZATION_FLUVIO_ENDPOINT`
//! (§7) is configured — see `spawn`'s and `build_sink`'s doc comments in
//! this file for the "no silent fallback" rule that governs an endpoint
//! configured without the `fluvio` feature.
...
/// Whether the relay background loop should run: `ORGANIZATION_EVENT_RELAY`
/// truthy (`1`/`true`/`yes`/`on`). Off by default.
#[must_use]
pub fn relay_enabled() -> bool {
    matches!(
        std::env::var("ORGANIZATION_EVENT_RELAY")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase()
            .as_str(),
...
/// Relay poll interval in seconds (`ORGANIZATION_EVENT_RELAY_INTERVAL_SECS`,
/// default 5, floored at 1).
#[must_use]
pub fn interval_secs() -> u64 {
    std::env::var("ORGANIZATION_EVENT_RELAY_INTERVAL_SECS")
        .ok()
...
/// Outbox retention in days (`ORGANIZATION_EVENT_RETENTION_DAYS`, default 7).
#[must_use]
pub fn retention_days() -> i64 {
    std::env::var("ORGANIZATION_EVENT_RETENTION_DAYS")
        .ok()
        .and_then(|v| v.trim().parse::<i64>().ok())
        .unwrap_or(7)
}

/// Fluvio broker endpoint (`ORGANIZATION_FLUVIO_ENDPOINT`, §7). Unset/blank
/// ⇒ no broker configured, so the relay uses [`LoggingSink`] — the
/// documented no-broker dev/CI sink — even under the `outbox` transport.
#[must_use]
pub fn fluvio_endpoint() -> Option<String> {
    std::env::var("ORGANIZATION_FLUVIO_ENDPOINT")
        .ok()
        .filter(|s| !s.trim().is_empty())
}

/// The topic events publish to (`ORGANIZATION_EVENT_TOPIC`, default
/// `mxi.organization.events` per §7's `mxi.<entity>.events` convention).
#[must_use]
pub fn event_topic() -> String {
    std::env::var("ORGANIZATION_EVENT_TOPIC")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| "mxi.organization.events".to_string())
}
...
///
/// Sink selection: no `ORGANIZATION_FLUVIO_ENDPOINT` ⇒ [`LoggingSink`]. An
...
pub fn spawn(db: DatabaseConnection) {
    ...
    if endpoint.is_some() && !cfg!(feature = "fluvio") {
        tracing::error!(
            "ORGANIZATION_FLUVIO_ENDPOINT is set but this binary was built without the \
             `fluvio` cargo feature; the relay will NOT start — rebuild with \
...
mod tests {
    ...
    fn capturing_sink_records_entity_and_key() {
        ...
        rt.block_on(async {
            sink.send("organization", "pid-9", &serde_json::json!({}))
                .await
                .unwrap();
        });
        assert_eq!(
            sink.0.lock().unwrap().as_slice(),
            &[("organization".to_string(), "pid-9".to_string())]
        );
    }
    ...
    fn config_defaults_are_safe() {
        ...
        if std::env::var("ORGANIZATION_EVENT_RELAY").is_err() {
            assert!(!relay_enabled());
        }
        if std::env::var("ORGANIZATION_EVENT_RELAY_INTERVAL_SECS").is_err() {
            assert_eq!(interval_secs(), 5);
        }
        if std::env::var("ORGANIZATION_EVENT_RETENTION_DAYS").is_err() {
