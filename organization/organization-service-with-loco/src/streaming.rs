...
//! ring buffer via the [`EventPublisher`] seam. This is Phase 1 of the
//! family's durable event-bus design (see
//! `agents/share/event-bus.md`): the versioned envelope plus the
//! publisher trait, with [`InMemoryPublisher`] wired as the default.
//! Behaviour is identical to the previous free-function ring buffer.
...
//! wire shape consumed by the front-end — unchanged by this refactor.
//!
//! In loco there is no per-request shared state for this, so the
//! publisher is a `OnceLock`-initialised global.
...
/// capped at `limit`. Drives the frozen `/events/recent` endpoint.
#[must_use]
