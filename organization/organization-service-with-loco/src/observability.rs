...
//! OTLP export** (repo `tasks.md` PRO-H12).
//!
//! Ported from [`course-service`]'s `src/observability.rs` — itself a port
//! of [`person-service`]'s, itself a port of [`link-graph-service`]'s, the
...
//! and is the **first of the four remaining loco-idiomatic registries**
//! (organization, care-pathway, case, portfolio — `src/controllers/`, not
//! `src/api/rest/`) to carry it: PRO-H12 slices 1–3 (course, place,
//! thing) were all person-style crates with a hand-rolled second router
//! surface; this crate has none (see "What differs from the person-style
//! ports" below).
//!
...
//! | `OTLP_SERVICE_NAME` | `organization-service` | `service.name` resource attribute |
//! | `OTLP_ENDPOINT` | `http://localhost:4317` | OTLP/gRPC collector endpoint |
//!
//! These three variables are **deliberately not `ORGANIZATION_`-prefixed**,
...
//! `App::after_routes` in `src/app.rs`; even the request-level test suite
//! boots the real `App` via loco's own testing harness rather than a
//! second hand-rolled router. `trace_mw` is therefore layered once, in
//! `after_routes`. This crate also declares no `tonic` dependency of its
//! own (no gRPC stub — `agents/share/overview.md`'s capability matrix), so
//! the in-process OTLP collector tests below need no `package = "…"`
//! rename, exactly as course's did not.
...
pub const DEFAULT_SERVICE_NAME: &str = "organization-service";
...
fn http_metrics() -> &'static HttpMetrics {
    ...
    METRICS.get_or_init(|| {
        let meter = global::meter("organization-service");
        HttpMetrics {
            duration: meter
...
/// embed record UUIDs (`/api/organizations/{pid}`,
/// `/api/organizations/{pid}/masked`, …), so a per-path label would be
/// unbounded cardinality — the same reasoning link-graph-service applied
/// to its own `EntityRef`-shaped paths, here for plain UUID path segments
/// instead.
///
...
mod tests {
    ...
    fn exporter_diagnostics_are_added_to_a_default_filter() {
        ...
        let base = || tracing_subscriber::EnvFilter::new("organization_service=info");
    ...
    fn an_operator_supplied_filter_is_left_alone() {
        ...
        assert_eq!(untouched.to_string(), "organization_service=info");
    }
    ...
    fn explicit_service_name_and_endpoint_are_honoured() {
        let config = TelemetryConfig::resolve(
            Some("organization-service-canary"),
            Some("http://collector.internal:4317"),
        );
        assert_eq!(config.service_name, "organization-service-canary");
        assert_eq!(
    ...
    async fn building_an_exporter_does_not_require_a_live_collector() {
        let config = TelemetryConfig {
            service_name: "organization-service".to_string(),
    ...
    fn resource_carries_service_name_and_version() {
        ...
        let name = resource
            ...
            .expect("service.name");
        assert_eq!(name.as_str(), "organization-service");
