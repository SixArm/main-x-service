...
const CONFIG: KeyConfig = KeyConfig::new(env!("CARGO_PKG_NAME"), "ORGANIZATION");

/// Environment variable holding the active MAC **root** key, hex-encoded.
pub const KEY_ENV: &str = "ORGANIZATION_INTEGRITY_MAC_KEY";
...
pub const KEY_FILE_ENV: &str = "ORGANIZATION_INTEGRITY_MAC_KEY_FILE";

/// Environment variable naming the active key, for the stored prefix.
pub const KEY_ID_ENV: &str = "ORGANIZATION_INTEGRITY_MAC_KEY_ID";

/// Environment variable holding retired keys still needed for
/// verification, as `id:hex` pairs separated by commas.
pub const RETIRED_KEYS_ENV: &str = "ORGANIZATION_INTEGRITY_MAC_KEYS_RETIRED";
