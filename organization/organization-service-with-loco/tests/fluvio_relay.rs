...
//! ORGANIZATION_FLUVIO_ENDPOINT=127.0.0.1:9103 \
...
use organization_service::relay::{self, FluvioSink};
...
async fn fluvio_relay_publishes_an_outbox_row_to_a_real_topic() {
    let endpoint = std::env::var("ORGANIZATION_FLUVIO_ENDPOINT").expect(
        "set ORGANIZATION_FLUVIO_ENDPOINT to the broker's SC address (e.g. 127.0.0.1:9103)",
    );
    ...
    request::<App, _, _>(|_request, ctx| async move {
        let org = Organization::new("Fluvio Relay Round Trip");
        let model = streaming::create_and_emit(&ctx.db, &org, None)
            .await
            .expect("create_and_emit under outbox transport");

        let sink = FluvioSink::connect(&endpoint, "mxi.organization.events.test")
            .await
