...
//! with `ORGANIZATION_REQUIRE_AUTH` **on** — the "activation proof"
//! (AU-2).
//!
...
/// A minimal valid organization payload with a unique name, so a create
/// that reaches the handler is not turned away as a duplicate.
fn organization_body() -> Value {
    let unique = uuid::Uuid::new_v4().simple().to_string();
    json!({ "name": format!("{unique} Enforcement Org") })
}
...
async fn enforcement_on_gates_the_real_router() {
    ...
    // `ORGANIZATION_ABAC_POLICY` ⇒ the built-in default policy (read
    ...
    unsafe {
        ...
        std::env::set_var(
            ...
            std::env::temp_dir().join(format!(
                "organization-enforcement-index-{}",
                std::process::id()
            )),
        );
    ...
    request::<App, _, _>(|request, _ctx| async move {
        ...
        assert_eq!(
            request.get("/api/organizations").await.status_code(),
            401,
