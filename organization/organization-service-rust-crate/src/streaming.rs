...
//!
//! Every CRUD action publishes a typed [`OrgEvent`] to a process-wide
...
pub struct OrgEvent {
    ...
    pub pid: String,
    /// The organization's name at the time of the event.
    pub name: String,
...
fn buffer() -> &'static Mutex<VecDeque<OrgEvent>> {
    static BUF: OnceLock<Mutex<VecDeque<OrgEvent>>> = OnceLock::new();
...
#[must_use]
pub fn recent(limit: usize) -> Vec<OrgEvent> {
