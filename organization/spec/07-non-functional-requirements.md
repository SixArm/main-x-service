...
| NFR-5 | Security | SSO via the [authentication entity](../../authentication/) — service verifies RS256 JWTs offline via JWKS; TLS at the edge; least-privilege DB roles | Unauthenticated (MVP); JWT middleware queued (§13); audit `actor` is always `null` until auth lands |
