...
| PASETO v4.public verification (auth-service Ed25519 key) + bearer wiring — see [`authentication-sessions.md`](../../agents/share/authentication-sessions.md) (supersedes RS256-JWT + JWKS) | Done (service `src/auth.rs` offline verification; front-end BFF); paseto-keys-over-HTTP fetch at boot still deferred | service + front-end |
| Search box, audit views | Deferred | front-end |
