...
The `PersonRepository` trait uses `async fn` methods. All `match state.person_repository.*()` and `state.audit_log.*()` calls need `.await`. Also fix `DbAuditLog` → `crate::db::models::audit_log::Model` (3 occurrences) and cast audit `limit` from `i64` to `u64`.

- [ ] **Step 1:** Add `.await` to every `state.person_repository.create/get_by_id/update/delete/list_active()` call
...
Same pattern — all `state.person_repository.*()` calls need `.await`.

- [ ] **Step 1:** Add `.await` to every `state.person_repository.create/get_by_id/update/delete()` call
...
- Modify: `src/models/person.rs` — add model construction tests
...
- [ ] **Step 1:** Create matching benchmarks (name matching, full person matching, scoring)
- [ ] **Step 2:** Create search benchmarks (indexing, searching, fuzzy search)
- [ ] **Step 3:** Create validation benchmarks (validate_person, normalize)
