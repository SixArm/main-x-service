...
Completed Phase 2 of the Main Person Index (MPI) implementation: Database Schema & Models. This phase establishes the complete database architecture for storing and managing person and organization data at scale.

## Goals Achieved

1. **Database Schema Design**: Created comprehensive PostgreSQL schema documentation
2. **Person Tables**: Designed normalized tables for person records and related data
...
- **Scalability**: Handle millions of person records efficiently
...
- **Flexibility**: Support multiple names, addresses, identifiers per person
...
- `persons` - Primary person records
- `person_names` - Multiple names per person
- `person_identifiers` - MRN, SSN, and other identifiers
- `person_addresses` - Multiple addresses
- `person_contacts` - Phone, email, etc.
- `person_links` - Links between duplicate/merged records
- `person_match_scores` - Calculated match scores
...
- **person_names**: family, given (array), prefix (array), suffix (array), use_type, is_primary
- **person_identifiers**: type (MRN/SSN/DL/etc.), system, value, assigner
- **person_addresses**: line1, line2, city, state, postal_code, country, use_type, is_primary
- **person_contacts**: system (phone/email/etc.), value, use_type, is_primary
- **person_links**: other_person_id, link_type (replaced_by/replaces/refer/seealso)

#### person_match_scores table

Stores calculated match scores for person matching:

```sql
- person_id, candidate_id (FKs)
...
- `audit_person_changes()` - Tracks all person modifications
...
- Establishes foundation (must exist before persons reference it)
- Enables pgcrypto extension for UUID generation
- 63 lines of SQL (up), 5 lines (down)

#### Migration 2: Persons (2024122800000002)
...
#### Migration 3: Person Related Tables (2024122800000003)

- Creates all person child tables:
  - person_names, person_identifiers
  - person_addresses, person_contacts
  - person_links, person_match_scores
...
- **Triggers**:
  ...
  - `audit_person_changes()` function
  - `audit_organization_changes()` function
- **Full-text search**:
  - pg_trgm extension for fuzzy matching
  - Trigram indexes on person names
- **Composite indexes**:
...
- `idx_persons_birth_date` - Date range searches
- `idx_persons_gender` - Gender filtering
- `idx_persons_active` - Active person filtering
- `idx_persons_organization` - Organization queries
- `idx_persons_deleted_at` - Excluding deleted records
- `idx_persons_active_gender` - Composite for filtered searches
- `idx_persons_birth_date_gender` - Composite for matching

**Person Names** (for matching):

- `idx_person_names_family` - Family name searches
- `idx_person_names_family_trgm` - Fuzzy family name matching
- `idx_person_names_given_trgm` - Fuzzy given name matching

**Person Identifiers**:

- `idx_person_identifiers_type` - Search by identifier type
- `idx_person_identifiers_value` - Search by value
- `idx_person_identifiers_system_value` - Unique identifier lookup

**Person Addresses**:

- `idx_person_addresses_postal_code` - Zip code searches
- `idx_person_addresses_city_state` - Location searches
...
1. **Queryable** models - For reading from database (e.g., `DbPerson`)
2. **Insertable** models - For creating new records (e.g., `NewDbPerson`)
3. **Changeset** models - For updates (e.g., `UpdateDbPerson`)

**Implemented Models**:

- `DbPerson`, `NewDbPerson`, `UpdateDbPerson`
- `DbPersonName`, `NewDbPersonName`
- `DbPersonIdentifier`, `NewDbPersonIdentifier`
- `DbPersonAddress`, `NewDbPersonAddress`
- `DbPersonContact`, `NewDbPersonContact`
- `DbPersonLink`, `NewDbPersonLink`
- `DbOrganization`, `NewDbOrganization`
- `DbPersonMatchScore`, `NewDbPersonMatchScore`
...
- person_addresses → persons
- person_contacts → persons
- person_identifiers → persons
- person_links → persons
- person_match_scores → persons
...
- `audit_person_changes()` - Logs all person CRUD operations
...
- Partitioning for person_match_scores (if storing all scores)
- Regular ANALYZE for query planner statistics

### 12. Capacity Planning

Estimated storage for 10 million persons:

| Component              | Size      |
| ---------------------- | --------- |
| persons table         | 5 GB      |
| person_names          | 4.5 GB    |
| person_identifiers    | 6 GB      |
| person_addresses      | 5 GB      |
| person_contacts       | 6 GB      |
| **Data Total**         | ~27 GB    |
...
- `migrations/2024122800000002_create_persons/up.sql`
- `migrations/2024122800000002_create_persons/down.sql`
- `migrations/2024122800000003_create_person_related_tables/up.sql`
- `migrations/2024122800000003_create_person_related_tables/down.sql`
...
echo "DATABASE_URL=postgres://username:password@localhost:5432/main_person_index" > .env
...
-- Insert test person
INSERT INTO persons (gender, birth_date, active)
VALUES ('male', '1980-01-15', true);

-- Insert person name
INSERT INTO person_names (person_id, family, given, is_primary)
VALUES ('...person-uuid...', 'Smith', ARRAY['John', 'Robert'], true);
...
| ----------------------------- | ------------- |
| Person lookup by ID          | < 1ms         |
| Person search by name        | < 10ms        |
| Person search by identifier  | < 5ms         |
| Matching query (with scoring) | < 100ms       |
| Audit log query (by entity)   | < 10ms        |
| Bulk insert (1000 persons)   | < 1 second    |
...
1. Implement person matching algorithms
2. Implement probabilistic matching scoring
3. Implement deterministic matching rules
4. Create person merge functionality
5. Create person link/unlink functionality
6. Implement person search functionality
7. Add conflict resolution logic
8. Implement person identifier management
...
Phase 2 successfully established a comprehensive, enterprise-grade database architecture for the Main Person Index system. The schema is:

- **Normalized**: Proper 3NF with strategic denormalization
- **Scalable**: Designed for millions of persons
...
- **Flexible**: Multiple names, addresses, identifiers per person
...
This foundation supports the complex person matching and management operations required for a production Main Person Index system serving millions of persons across thousands of healthcare facilities.
