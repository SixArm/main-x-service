...
Completed Phase 3 of the Main Person Index (MPI) implementation: Core MPI Logic. This phase implements the sophisticated person matching algorithms and scoring systems that form the heart of the MPI system.
...
- **Identify Duplicates**: Find potential duplicate person records across facilities
...
```
Person A: John Smith, 1980-01-15, Male, 123 Main St, 12345, MRN:12345
Person B: John Smith, 1980-01-15, Male, 123 Main St, 12345, MRN:12345
...
```
Person A: John Smith, 1980-01-15, Male, (no address), (no identifier)
Person B: John Smith, 1980-01-15, Male, (no address), (no identifier)
...
```
Person A: William Smith, 1980-01-15, Male
Person B: Bill Smyth, 1980-01-16, Male
...
- If both persons have addresses:
...
pub trait PersonMatcher {
    fn match_persons(&self, person: &Person, candidate: &Person)
        -> Result<MatchResult>;

    fn find_matches(&self, person: &Person, candidates: &[Person])
...
- `match_persons()`: Compare two persons
...
pub struct MatchResult {
    pub person: Person,       // The matched person
...
Matchers don't modify person records:
...
- Typically small (1-3 identifiers per person)
- Space: O(n) for normalization

**Overall Person Match**:
...
4. **Caching**: Cache frequently compared person pairs
...
- Match time: < 1 second per person

## Usage Examples

### Example 1: Basic Person Matching

```rust
use main_person_index::matching::{ProbabilisticMatcher, PersonMatcher};
use main_person_index::config::MatchingConfig;
use main_person_index::models::{Person, HumanName, Gender};
...
// Match persons
let result = matcher.match_persons(&person1, &person2)?;
...
let new_person = create_person("John Smith", "1980-01-15");

// Get candidates from database (pseudo-code)
let candidates: Vec<Person> = database
    .search_by_name_soundex("Smith")
    .limit(1000)
    .collect()?;

// Find matches
let matches = matcher.find_matches(&new_person, &candidates)?;

for (idx, match_result) in matches.iter().enumerate() {
    println!("Match #{}: {} (score: {:.3})",
        idx + 1,
        match_result.person.full_name(),
        match_result.score
    );
...
```rust
use main_person_index::matching::DeterministicMatcher;
...
if matcher.is_match(result.score) {
    ...
    flag_for_review(&person1, &person2, result.score)?;
}
```
...
if auto_mode {
    let matches = auto_matcher.find_matches(&person, &candidates)?;
    // Only highest confidence matches
} else {
    let matches = search_matcher.find_matches(&person, &candidates)?;
...
impl PersonRepository {
    fn find_duplicates(&self, person: &Person) -> Result<Vec<MatchResult>> {
        // 1. Use blocking strategy to narrow candidates
        let soundex = calculate_soundex(&person.name.family);
        ...
        matcher.find_matches(person, &candidates)
    }
}
```
...
```rust
impl SearchEngine {
    fn find_potential_matches(&self, person: &Person) -> Result<Vec<Person>> {
        // Use Tantivy for initial filtering
        let query = format!(
            "name:{} AND birth_year:{}",
            person.name.family,
            person.birth_date.map(|d| d.year()).unwrap_or(0)
        );
...
async fn match_person(
    ...
    Extension(repo): Extension<Arc<dyn PersonRepository>>,
) -> Result<Json<Vec<MatchResult>>> {
    let candidates = repo.find_potential_duplicates(&person)?;
...
4. **Multi-person Clustering**: Identify clusters of related records
...
Phase 3 successfully implemented a sophisticated person matching system with:
...
- Identify potential duplicate persons
- Support manual review workflows
- Enable automated record linking
- Scale to millions of persons (with future optimizations)

This foundation enables the MPI system to fulfill its core purpose: maintaining a unified, accurate view of person identities across healthcare organizations.
