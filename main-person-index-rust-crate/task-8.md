...
This phase implements a comprehensive event streaming and audit logging system for the Main Person Index (MPI), providing critical infrastructure for production healthcare environments. The implementation enables real-time event notification, complete audit trails for compliance, and integration capabilities with downstream systems.

## Task Description

Implement event-driven architecture and audit logging to capture, publish, and persist all person data lifecycle events within the MPI system. This includes:

1. **Event Publisher Infrastructure**: Create a thread-safe event publishing system that broadcasts person lifecycle events
2. **Audit Log Repository**: Implement database-backed audit logging for compliance and forensic analysis
3. **Repository Integration**: Enhance the person repository to automatically publish events and log audits
...
1. **Event-Driven Architecture**: Enable real-time notification of person data changes to downstream systems
...
4. **System Integration**: Allow other systems to react to person data changes
...
Healthcare systems must maintain complete audit trails of all person data modifications:
...
The MPI acts as the authoritative source for person identity. Other systems need to know when:

- New persons are registered
- Person demographics are updated
- Persons are merged (duplicate resolution)
- Person records are linked/unlinked
...
- **Debugging**: Trace the history of person record changes
...
Defined comprehensive person lifecycle events:
...
- Event-specific data (person record or IDs)
- Timestamp for temporal ordering
- Helper methods: `timestamp()`, `person_id()`
...
- `get_logs_for_entity()`: Retrieve audit history for a specific person
...
Enhanced `DieselPersonRepository` with event and audit capabilities:

**Builder Pattern:**

```rust
let repository = DieselPersonRepository::new(pool)
...
fn create(&self, person: &Person) -> Result<Person> {
    ...
    self.publish_event(PersonEvent::Created {
        person: result.clone(),
        timestamp: Utc::now(),
    });
    ...
    if let Ok(person_json) = serde_json::to_value(&result) {
        self.log_audit("CREATE", result.id, None, Some(person_json), &AuditContext::default());
    }
...
fn update(&self, person: &Person) -> Result<Person> {
    ...
    let old_person = self.get_by_id(&person.id)?;

    // 2. Execute database transaction
    let result = conn.transaction(|conn| { ... })?;

    // 3. Publish event
    self.publish_event(PersonEvent::Updated { ... });
...
fn delete(&self, id: &Uuid) -> Result<()> {
    // 1. Get old values for audit
    let old_person = self.get_by_id(id)?;
    ...
    self.publish_event(PersonEvent::Deleted {
        person_id: *id,
        timestamp: Utc::now(),
    });
...
impl AppState {
    pub fn new(db_pool, search_engine, matcher, config) -> Self {
        ...
        // 3. Create person repository with dependencies
...
- Person data mutations should succeed even if audit/events fail
...
```rust
fn publish_event(&self, event: PersonEvent) {
...
4. **Event Compression**: Compress large person records in events
...
- Must be secured with same rigor as person data
...
**Symptom**: Slow person operations
...
Phase 8 establishes production-grade observability infrastructure for the Main Person Index:

✓ **Event Streaming**: Real-time person lifecycle event publication
