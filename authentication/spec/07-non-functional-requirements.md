...
| NFR-6 | Key-rotation operability | `kid` stamped in every token footer and key-set entry; verifier selects by `kid`, so old + new keys can serve side by side. Multi-key `paseto-keys` set + grace window *(delivered — §13 T-5)* |
| NFR-7 | Key custody | Asymmetric only — no shared secret ever leaves this service. An **Ed25519** PASETO keypair (was RS256); the private key signs `POST /token`, only public keys are published. Production seed injected via `TOKEN_PRIVATE_KEY_SEED` / `TOKEN_PRIVATE_KEY_FILE`; the built-in dev seed is dev-only (no committed key files) |
...
| NFR-11 | Determinism in dev | Built-in dev seed keeps the published key set stable across restarts; no SMTP needed (links logged) |
| NFR-12 | Verifier footprint | Dependency-light (`rusty_paseto` in place of the removed `jsonwebtoken`; `serde`, `thiserror`; `reqwest` optional behind `fetch`), `#![forbid(unsafe_code)]`, allocation-light `verify` safe to share behind an `Arc` |
