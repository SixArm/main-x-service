...
Material changes to this spec — the JWT claim set, the JWKS shape or
location, the magic-link protocol, the verifier's public API, token
lifetimes, compliance scope — MUST land in the same commit as the
corresponding code change, alongside the affected subproject's own
spec edit where one exists. Three-part PRs: spec + code + test.
...
| Integration contract (claims, JWKS, magic-link surface, verifier API) | **This spec** |
