...
/// Pins (DB-free) the localized magic-link email bridge that
/// `Emailer::send_magic_link` uses: a request-body `locale` is normalised
/// by `i18n::select_locale`, the chosen catalog entry renders the
/// `{frontend}/verify?token={token}` link into both bodies, and the
/// subject/body language tracks the locale. This pins the
/// `params.locale → select_locale → magic_link_email → render` path at the
/// exact expression the mailer evaluates, without SMTP or a database.
#[test]
fn selected_locale_renders_the_mailer_email_copy() {
    use authentication_service::i18n;

    // The mailer builds this link, then renders the locale's copy onto it.
    let frontend = "https://example.test";
    let token = "magic-token-abc";
    let link = format!("{frontend}/verify?token={token}");

    // A `cy` request renders the Welsh subject + the substituted link.
    let cy_locale = i18n::select_locale(Some("cy-GB"));
    let (cy_subject, cy_text, cy_html) = i18n::magic_link_email(&cy_locale).render(&link);
    assert_eq!(cy_subject, "Eich dolen hud i fewngofnodi");
    assert!(cy_text.contains(&link), "link must appear in the text body");
    assert!(
        cy_html.contains(&format!("href=\"{link}\"")),
        "link must appear in the html body"
    );
    assert!(!cy_text.contains("{link}"), "placeholder must be consumed");
    assert!(cy_text.contains("Cliciwch y ddolen"));

    // An unknown locale falls back to the English subject (still localised
    // at the catalog, never an error or an empty body).
    let fallback_locale = i18n::select_locale(Some("zz"));
    let (en_subject, en_text, _en_html) = i18n::magic_link_email(&fallback_locale).render(&link);
    assert_eq!(en_subject, "Your magic sign-in link");
    assert!(en_text.contains(&link));

    // Absent `locale` (the common request) is English.
    let default_locale = i18n::select_locale(None);
    assert_eq!(
        i18n::magic_link_email(&default_locale).subject,
