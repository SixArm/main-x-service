...
//! ([`select_locale`]) normalises the caller's optional request-body
//! `locale` field to a supported tag, falling back to `en` for anything
//! unknown. (There is no `Accept-Language` header parsing today; the
//! request-body field is the only wired input — see service spec §6.)
//!
...
mod tests {
    ...
    #[test]
    fn selected_locale_drives_the_rendered_email_copy() {
        // Pins the connected path the handlers take:
        // `params.locale -> select_locale -> magic_link_email -> render`.
        // A `cy` preference must yield the Welsh subject + a substituted
        // link; an unknown preference must yield the English subject.
        let link = "https://example.test/verify?token=xyz";

        let cy_locale = select_locale(Some("cy-GB"));
        let (cy_subject, cy_text, _cy_html) = magic_link_email(&cy_locale).render(link);
        assert_eq!(cy_subject, "Eich dolen hud i fewngofnodi");
        assert!(cy_text.contains(link));
        assert!(cy_text.contains("Cliciwch y ddolen"));

        let fallback_locale = select_locale(Some("fr"));
        let (en_subject, en_text, _en_html) = magic_link_email(&fallback_locale).render(link);
        assert_eq!(en_subject, "Your magic sign-in link");
        assert!(en_text.contains(link));

        // Absent preference (the common case) is English.
        let default_locale = select_locale(None);
        assert_eq!(
            magic_link_email(&default_locale).subject,
