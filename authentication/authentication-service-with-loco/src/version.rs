...
//! (`/api/auth/…`, `/api/compliance/…`, with no version segment in the
//! path), and a client selects the representation version with
//! `Accepts-version: 1.0`. This crate's routes were already version-free
//! but carried no negotiation helper at all (spec §13 T-13) — this
//! mirrors the reference implementation in the event service (and the
//! `case`/`care-pathway` ports), layered here in `app.rs`.
//!
...
//!   `Accepts-version` for `/api/*` requests (`/api/auth/…` and
//!   `/api/compliance/…` alike; `/.well-known/paseto-keys`, `/api-docs/*`,
//!   and `/metrics.prom` are exempt), returns `406 Not Acceptable` on an
//!   unsupported explicit version, and stamps the resolved version onto
//!   the response as `Accepts-version`. It is a near-noop when the
//!   header is absent (defaults to the current version).
...
/// (`/api` and `/api/...` — covers both `/api/auth/…` and
/// `/api/compliance/…`). `/.well-known/paseto-keys`, `/api-docs/*`, and
/// `/metrics.prom` are exempt.
...
mod tests {
    ...
    fn only_api_paths_are_versioned() {
        ...
        assert!(is_versioned_path("/api/auth"));
        assert!(is_versioned_path("/api/auth/whoami"));
        assert!(is_versioned_path("/api/compliance"));
        assert!(!is_versioned_path("/.well-known/paseto-keys"));
