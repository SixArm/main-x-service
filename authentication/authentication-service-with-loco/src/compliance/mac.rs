...
const CONFIG: KeyConfig = KeyConfig::new(env!("CARGO_PKG_NAME"), "AUTH");

/// Environment variable holding the active MAC **root** key, hex-encoded.
pub const KEY_ENV: &str = "AUTH_INTEGRITY_MAC_KEY";
...
pub enum Domain {
    ...
    /// not here.
    ///
    /// The only domain this service has. `sessions` are ephemeral and
    /// already expire; `users` rows change through ordinary account
    /// operations. The `auth_events` trail is the record that must not be
    /// quietly rewritten, because it is what says who logged in and who
    /// was granted which authorization attributes.
    AuditRow,
}
