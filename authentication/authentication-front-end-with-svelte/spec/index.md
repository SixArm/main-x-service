...
- [x] **T-13 (2026-09-19): IdP-initiated "Sign in with SSO" link (EV-2, `agents/share/authentication-sessions.md` §7a rollout step 3).** The auth service's OIDC relying-party backend (behind its own `oidc` Cargo feature) had no front-end entry point — the only way to start a federated sign-in was to hand-navigate to `{AUTH_API_URL}/api/auth/oidc/login`.
  - **Why not a BFF `fetch` call, unlike every other auth action this app performs.** The OIDC authorization-code flow needs the **browser itself** to navigate to the identity provider and back — a server-to-server `fetch` from this app's SvelteKit server cannot do that hop. This is the one auth action in this app that is a plain `<a href>` browser navigation rather than a form action or a `+page.server.ts` `load`.
  - **`src/routes/signin/sso/+server.ts`** — a new `GET` route: 404s when `PUBLIC_OIDC_SIGNIN_ENABLED` is unset (this app's own opt-in, so a deployment that has not configured OIDC on the paired auth service shows no dead link — the auth service's own `/api/auth/oidc/login` independently 404s when unconfigured too, so this is belt-and-suspenders, not the only guard); otherwise 303-redirects to `oidcLoginUrl(url.origin)` (`src/lib/server/auth.ts`), which builds `{AUTH_API_URL}/api/auth/oidc/login?return_url={this app's own origin}` — the same allow-listed per-app `return_url` knob the auth service's magic-link endpoints already support, so the post-federation bridge (see the auth service's own `spec/index.md` §13 EV-2 follow-up entry) lands back on *this* app.
  - **`src/routes/signin/+page.svelte`** — a "Sign in with SSO" link (`t("signin.sso")`, all 14 locale entries in `src/lib/i18n.svelte.ts`) shown only when `PUBLIC_OIDC_SIGNIN_ENABLED === "true"` (`$env/dynamic/public`), placed between the magic-link form and the "no account yet?" prompt.
  - **`PUBLIC_OIDC_SIGNIN_ENABLED`** is a new env var (`.env.example`) — deliberately a boolean flag, not a URL: `AUTH_API_URL` itself stays server-only per Ground rule 6 (no `PUBLIC_API_BASE_URL`), and this flag does not change that — it never reaches the client as anything more than "show the link or don't."
  - **Acceptance:** `tests/unit/auth.test.ts` gained two pure `oidcLoginUrl` tests (points at `/api/auth/oidc/login`; carries the caller's origin as `return_url`); a new `tests/unit/signin-sso.test.ts` (2 tests, mocking `$env/dynamic/public`) pins the 404-when-disabled and 303-when-enabled behavior of the new route by catching the thrown `HttpError`/`Redirect` (`isHttpError`/`isRedirect` from `@sveltejs/kit`) rather than exercising a real server. `pnpm run check` (0 errors), `pnpm exec vitest run` (32/32, +4), `pnpm run build`, `pnpm run lint` all clean. Verified live in a real browser (Playwright, since this app is CSR-only — `ssr = false` in `+layout.ts`, so `curl` cannot see rendered content): the link renders with the correct text/href on `/signin`, and clicking it drives the 303 chain to the auth service's login endpoint with the correctly URL-encoded `return_url`.
