...
`THEMES`/`THEME_LABELS` (39 slugs — a subset of Lily's own 45, missing
...
government themes grouped at the end) and `DEFAULT_SIZES` (the
...
This restores a locale picker to the chrome: there was previously no
locale-switcher UI at all (the stored locale was read on boot with no
way to change it in-page), and `PickerBar` bundles all four
...
`labels.locale` reuses the existing `nav.locale` ("Language") key,
already present in every locale's catalog.
...
`en`, since only the primary subtag was ever checked). The layout's
`$effect` that mirrors the locale onto `<html lang>` now converts the
underscore to a hyphen (`en_US` → `en-US`) for BCP 47 validity,
matching what `LocalePicker` itself writes via its own
`bcp47LocaleTag`.

`pnpm install` / `pnpm run check` (0 errors) / `pnpm test` (28/28, was
27) / `pnpm run build` all verified clean.
