...
```bash
DATABASE_URL=postgresql://test_user:test_password@localhost:5432/main_worker_index_test
...
```bash
kubectl rollout undo deployment/main_worker_index-server
```
...
```yaml
kubectl set image deployment/main_worker_index-server mpi-server=${{ env.REGISTRY }}/${{ env.IMAGE_NAME }}:${{ github.ref_name }}
```
...
# Pull image
docker pull ghcr.io/username/main-worker-index-rust-crate:latest
```
...
# Kubernetes
kubectl rollout undo deployment/main_worker_index-server
kubectl rollout undo deployment/main_worker_index-server --to-revision=3
...
Phase 13 completes the Main Worker Index project with a production-ready CI/CD pipeline. The system now has automated testing, code quality checks, security scanning, and deployment automation, making it ready for production use in healthcare environments.
