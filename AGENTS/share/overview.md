...
| [organization-service](../../organization/organization-service-rust-crate) | Organization | loco.rs registry for schema.org/Organization — CRUD + matching (embeds organization-matcher; API DTO is the matcher's Organization type) + name search + audit log + event streaming + OpenAPI/Swagger + record merge. Deferred: Tantivy full-text, privacy, JWT verification. |
