...
Route ordering note: the literal segments (`/review-queue`, `/review-queue/kanban`, `/deduplicate`, `/compare`, `/import`, `/calendar`, `/map`, `/search`, `/search/partial`) are registered **before** the dynamic `/:id` so they take precedence in the matchit router. The per-record action paths (`/:id/edit`, `/:id/audit`, etc.) sit as siblings of `/:id` and disambiguate by suffix.
