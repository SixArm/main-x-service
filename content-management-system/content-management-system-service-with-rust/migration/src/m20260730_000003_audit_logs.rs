...
//! Every mutation **and every sensitive read** (an unpublished revision,
//! a preview-token issue or use, an audit query) writes one row here
//! recording the entity kind, the action, the (optional) actor, and a
...
impl MigrationTrait for Migration {
    ...
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(
            ...
            &[
                ("id", ColType::PkAuto),
                // Which record kind the entry concerns: site | template
                // | content_type (and, as later phases land, entry |
                // variant | revision | asset | route | ...).
                ("entity", ColType::String),
                // The record pid the entry concerns.
                ("entity_pid", ColType::Uuid),
                // created / updated / deleted / schema_changed /
                // published / unpublished / restored / redacted /
                // sensitive reads (revision_read, preview_issued).
        ...
        .await?;
        // The per-record trail is the common query; the owner-scoped
        // query rides the snapshot's `owner` and is time-bounded first.
        m.get_connection()
            .execute_unprepared(
