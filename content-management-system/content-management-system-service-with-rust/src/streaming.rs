...
//! `CMS_EVENT_TRANSPORT` (default `memory`):
//!
...
//!
//! Unlike the single-entity registries, CMS emits for several record
//! kinds, so `entity` and `kind` are `String`s: `site` / `template` /
//! `content_type` (and, as later phases land, `entry` / `variant` /
//! `revision` / `asset` / `route`) × `created` / `updated` /
//! `deleted` / `site_configured` / `content_type_changed` /
//! `variant_published` / … (the full kind list is spec
//! `domain-model.md`).
...
pub struct Envelope {
    ...
    /// The record kind: `site`, `content_type`, `variant`, ….
    pub entity: String,
    /// The change kind: `created`, `variant_published`, ….
    pub kind: String,
    ...
    /// Human-oriented label (a site or content-type name — already
...
pub fn transport() -> EventTransport {
    ...
    *TRANSPORT.get_or_init(|| {
        EventTransport::parse(&std::env::var("CMS_EVENT_TRANSPORT").unwrap_or_default())
    })
}
...
mod tests {
    ...
    fn envelope_and_view_shape() {
        ...
        assert_eq!(view.kind, "content_type_changed");
