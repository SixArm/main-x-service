//! Artifact storage for asset bytes (CMS-D9), reusing the family
//! `ArtifactStore` seam (`agents/share/bulk-import-export.md` §3, §12)
//! rather than inventing a second storage abstraction.
//!
//! The asset library stores opaque bytes — images, video, audio,
//! documents — referenced from `assets.storage_ref`. Keys are the
//! **SHA-256 of the content** (CMS-R6), so identical bytes deduplicate
//! to one stored object no matter how many times they are uploaded, and
//! a key can never be chosen by an uploader. Storage sits behind the
//! [`ArtifactStore`] trait so the concrete backend is config-driven.
//!
//! Two backends ship:
//!
//! - [`LocalFsArtifactStore`] — dev and test. Writes under a configurable
//!   base directory (`CMS_ARTIFACT_DIR`) and returns `file://`
...
//! trait is therefore `async_trait`, and the call sites (the
//! upload and download handlers) are already async.
//!
...
/// A place to `put` and `get` opaque byte artifacts.
///
/// `put` stores `bytes` under a caller-chosen `key` and returns an opaque
/// **reference** string (persisted in `assets.storage_ref`); `get` resolves
...
///
/// `CMS_ARTIFACT_BACKEND` selects it: `local` (default) or
...
/// writing a deployment's asset bytes to the local disk of
...
pub async fn from_env() -> Result<Box<dyn ArtifactStore>> {
    let backend = std::env::var("CMS_ARTIFACT_BACKEND").unwrap_or_default();
    from_backend(&backend).await
}

/// Build the store named by `backend`, the value
/// `CMS_ARTIFACT_BACKEND` would hold.
///
...
pub async fn from_backend(backend: &str) -> Result<Box<dyn ArtifactStore>> {
    match backend.trim().to_ascii_lowercase().as_str() {
        ...
        other => {
            tracing::warn!(
                backend = other,
                "unknown CMS_ARTIFACT_BACKEND; falling back to the local store"
            );
...
async fn s3_from_env() -> Result<Box<dyn ArtifactStore>> {
    Err(Error::Message(
        "CMS_ARTIFACT_BACKEND=s3 but this binary was built without the `s3` \
         cargo feature; rebuild with `--features s3` (falling back to local storage would \
         silently write asset bytes to an ephemeral container disk, and a site would \
         lose its images on the next restart)"
            .to_string(),
    ))
}
...
impl LocalFsArtifactStore {
    ...
    /// Build a store from the environment: `CMS_ARTIFACT_DIR`, or
    /// a `content-management-system-assets` directory under the system temp dir when
    /// unset/blank.
    #[must_use]
    pub fn from_env() -> Self {
        let base = std::env::var("CMS_ARTIFACT_DIR")
            .ok()
            .filter(|s| !s.trim().is_empty())
            .map_or_else(
                || std::env::temp_dir().join("content-management-system-assets"),
                PathBuf::from,
            );
        Self::new(base)
    }
...
/// Cloudflare R2) by pointing `CMS_S3_ENDPOINT` at it.
...
impl S3ArtifactStore {
    ...
    /// | Variable | Meaning |
    /// |---|---|
    /// | `CMS_S3_BUCKET` | **required** — the bucket |
    /// | `CMS_S3_ENDPOINT` | custom endpoint for a non-AWS store |
    /// | `CMS_S3_REGION` | region (default `us-east-1`) |
    /// | `CMS_S3_FORCE_PATH_STYLE` | path-style addressing (default **on**) |
    ///
    ...
    pub async fn from_env() -> Result<Self> {
        let bucket = std::env::var("CMS_S3_BUCKET")
            .ok()
            .filter(|s| !s.trim().is_empty())
            .ok_or_else(|| {
                Error::Message(
                    "CMS_ARTIFACT_BACKEND=s3 requires \
                     CMS_S3_BUCKET"
                        .to_string(),
                )
            })?;

        let region = std::env::var("CMS_S3_REGION")
            .ok()
            .filter(|s| !s.trim().is_empty())
        ...
        if let Some(endpoint) = std::env::var("CMS_S3_ENDPOINT")
            .ok()
            .filter(|s| !s.trim().is_empty())
        {
    ...
    /// fetched: the reference comes from an `assets` row, and honouring
...
fn force_path_style_from_env() -> bool {
    std::env::var("CMS_S3_FORCE_PATH_STYLE")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .is_none_or(|v| {
            matches!(
...
impl ArtifactStore for S3ArtifactStore {
    ...
    async fn presigned_get(&self, reference: &str, ttl_secs: u64) -> Result<Option<String>> {
        let (_, key) = self.split_reference(reference)?;
        // Bounded: an unbounded TTL on a URL to a stored asset is a
        // long-lived unauthenticated read of personal data. One hour is the
        // ceiling, matching the family's artifact retention posture.
...
mod tests {
    ...
    async fn put_then_get_round_trips() {
        ...
        let reference = store.put("sha256/abc/def", b"hello asset").await.unwrap();
    ...
    /// asset bytes to an ephemeral container disk while
    ...
    ///
    /// ```sh
    /// CMS_S3_ENDPOINT=http://127.0.0.1:9000 \
    /// CMS_S3_BUCKET=care-pathway-test \
    ...
    /// A reference naming a different bucket is refused rather than
    /// fetched. The reference comes from an `assets` row, so honouring
    /// an arbitrary bucket in it would turn a row edit into a read of any
    /// bucket this service's credentials can reach.
    #[cfg(feature = "s3")]
    #[test]
    ...
    fn path_style_defaults_on() {
        ...
        if std::env::var("CMS_S3_FORCE_PATH_STYLE").is_err() {
