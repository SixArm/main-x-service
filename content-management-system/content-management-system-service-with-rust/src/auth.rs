//! Bearer-token authentication for the content-management-system API.
//!
...
//!
//! - `CMS_PASETO_KEYS_URL` — optional URL of the auth service's
...
//!   wins over `CMS_PASETO_KEYS` (`tracing::info!`), on failure the
...
//!   up without a restart (interval `CMS_PASETO_KEYS_REFRESH_SECS`,
//!   default 1 h; `0` disables; keeps the current keys on a failed
//!   fetch).
//! - `CMS_PASETO_KEYS` — the Ed25519 key set (JSON, OKP/Ed25519
...
//!   still boots).
//! - `CMS_TOKEN_ISSUER` — expected `iss` (default
//!   `authentication-service`).
//! - `CMS_TOKEN_AUDIENCE` — expected `aud` (default
//!   `main-x-service`).
//!
//! ## Blanket enforcement
//!
//! When `CMS_REQUIRE_AUTH` is truthy (`1`/`true`/`yes`/`on`,
...
//! ## Authorization (ABAC)
//!
//! Inside the same guard — so it applies only when `CMS_REQUIRE_AUTH`
...
//! ([`policy`], built by [`policy_from_env`]) from `CMS_ABAC_POLICY`
//! (inline JSON) or `CMS_ABAC_POLICY_FILE` (path); unset or unparsable
...
//! (body carries the deciding rule). The five CMS personas — author,
//! editor, translator, admin, and the machine delivery peer — are
//! policy profiles over that same `attrs` claim, not a role enum in
//! this code (spec `auth.md`).
//!
//! ## The public delivery surface
//!
//! CMS is the family's first service whose *purpose* includes serving
//! anonymous readers. That is **not** handled by loosening the guard:
//! published delivery reads become anonymous only for sites whose
//! `visibility` is `public`, only for `GET`/`HEAD`, and only for
//! published revisions (CMS-D7). Because visibility is per-site state,
//! the decision needs a database read and therefore cannot live in the
//! pure [`enforce`]: [`delivery_site_key`] is the pure half — it
//! recognises a delivery request and names the site it addresses — and
//! [`enforce`] defers exactly those requests to the delivery
//! controller, which reads the site's visibility on **every** request
//! (so flipping a site to `restricted` takes effect immediately) and
//! refuses anything it should not answer.

use std::collections::BTreeMap;
use std::sync::OnceLock;

use crate::models::_entities::{content_types, entries, entry_variants, sites};
use authentication_verifier::{Claims, Policy, ReloadablePolicy, ReloadableVerifier, Verifier};
// Re-exported so a controller asks `auth` for the action vocabulary
// rather than reaching into the verifier crate directly — one import
// to change if the engine moves.
pub use authentication_verifier::Action;
...
pub const ENTITY: &str = "cms";

/// Path suffixes of this crate's **destructive named POSTs** (per
/// `authorization-attributes.md` §2). CMS has no record merge; what is
/// irreversible here is bulk import and **redaction** — the erasure
/// path that blanks a revision body while preserving the row (spec
/// `audit.md`). Both are listed ahead of their features (CMS-T24 /
/// the erasure work), so the guard is already correct when they land
/// rather than being widened afterwards. A POST whose path ends with
/// one of these derives [`Action::Destructive`] instead of
/// [`Action::Write`].
pub const DESTRUCTIVE_POST_SUFFIXES: [&str; 2] = ["/import", "/redact"];
...
/// (called from `App::after_routes`). When `CMS_PASETO_KEYS_URL` is set
...
pub async fn init() {
    if let Some(url) = std::env::var("CMS_PASETO_KEYS_URL")
        .ok()
        .filter(|s| !s.trim().is_empty())
    {
        let issuer = env_or("CMS_TOKEN_ISSUER", DEFAULT_ISSUER);
        let audience = env_or("CMS_TOKEN_AUDIENCE", DEFAULT_AUDIENCE);
...
/// `CMS_PASETO_KEYS_REFRESH_SECS` is unset. One hour — key rotation is
...
/// key set from `CMS_PASETO_KEYS_URL` and swaps it into the live
...
/// out). Interval from `CMS_PASETO_KEYS_REFRESH_SECS` (default
/// [`KEY_REFRESH_DEFAULT_SECS`]); **`0` disables** the loop.
///
/// A no-op when `CMS_PASETO_KEYS_URL` is unset (env-supplied keys have
/// nothing to re-fetch). Call once at boot (`app.rs::after_routes`).
pub fn spawn_key_refresh() {
    let Some(url) = std::env::var("CMS_PASETO_KEYS_URL")
        .ok()
    ...
    let secs = std::env::var("CMS_PASETO_KEYS_REFRESH_SECS")
        .ok()
    ...
    tracing::info!(secs, "polling CMS_PASETO_KEYS_URL for key rotation");
}
...
/// `CMS_REQUIRE_AUTH` and cached. Off by default — see the
...
pub fn require_auth() -> bool {
    ...
    *REQUIRE_AUTH.get_or_init(|| parse_bool(&std::env::var("CMS_REQUIRE_AUTH").unwrap_or_default()))
}
...
/// scraper needs no bearer token). `GET`/`HEAD` on `/delivery/*` is
/// deferred to the delivery controller (see [`enforce`]); everything
...
/// Whether `path` is a preview render — `/delivery/{site}/preview/…`.
///
/// These carry **their own credential** (a scoped, short-lived token
/// checked by the preview controller), so the blanket guard defers them
/// exactly as it defers public delivery reads. The token, not the
/// site's visibility, is what authorizes the read.
#[must_use]
pub fn is_preview_path(method: &Method, path: &str) -> bool {
    matches!(*method, Method::GET | Method::HEAD)
        && delivery_site_key(method, path)
            .is_some_and(|_| path.split('/').nth(3) == Some("preview"))
}

/// The site key a delivery request addresses, if the request is one:
/// a `GET`/`HEAD` of `/delivery/{site_key}/…`. This is the **pure
/// half** of the public-delivery allow-list (CMS-D7): recognising the
/// request and naming its site takes no I/O, while deciding whether
/// that site is `public` needs a database read and so belongs in the
/// delivery controller (CMS-T17/T21).
///
/// Returning the key rather than a boolean is deliberate: a caller
/// cannot act on this without going and looking the site up, so there
/// is no shape of the answer that accidentally means "let it through".
/// A non-`GET`/`HEAD` method, or a path with no site segment, is
/// `None`.
#[must_use]
pub fn delivery_site_key<'a>(method: &Method, path: &'a str) -> Option<&'a str> {
    if !matches!(*method, Method::GET | Method::HEAD) {
        return None;
    }
    let rest = path.strip_prefix("/delivery/")?;
...
pub fn derive_action(method: &Method, path: &str) -> Action {
    ...
    // check, so `POST /api/entries/import/` (and `//`) is still classified as
...
/// Load the ABAC policy: `CMS_ABAC_POLICY` (inline JSON) wins, then
/// `CMS_ABAC_POLICY_FILE` (path to a JSON file), else the built-in
...
pub fn policy_from_env() -> Policy {
    let source = std::env::var("CMS_ABAC_POLICY")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .or_else(|| {
            let path = std::env::var("CMS_ABAC_POLICY_FILE")
                .ok()
                .filter(|v| !v.trim().is_empty())?;
...
/// `CMS_ABAC_POLICY` / `CMS_ABAC_POLICY_FILE` (else the built-in
...
/// Re-read the ABAC policy from the environment (`CMS_ABAC_POLICY` /
/// `CMS_ABAC_POLICY_FILE`, same rules and default-fallback as
...
/// `CMS_ABAC_POLICY_FILE` changes on disk — so operators can edit the
...
///
/// A no-op when `CMS_ABAC_POLICY_FILE` is unset — an inline
/// `CMS_ABAC_POLICY` (or the built-in default) has nothing to watch,
/// and env vars do not change at runtime. Call once at boot
/// (`app.rs::after_routes`).
pub fn spawn_policy_watcher() {
    let Some(path) = std::env::var("CMS_ABAC_POLICY_FILE")
        .ok()
    ...
    tracing::info!(
        ...
        "watching CMS_ABAC_POLICY_FILE for changes"
    );
}
...
pub fn enforce(
    ...
    // The public delivery surface (CMS-D7). The guard cannot decide
    // this alone — whether a site is `public` is a database read — so
    // it defers `GET`/`HEAD` on `/delivery/*` to the delivery
    // controller, which checks the site's visibility on **every**
    // request and refuses a restricted site itself. The narrowness is
    // in `delivery_site_key`: no other method, no other prefix, and the
    // composer it defers to can only reach published revisions.
    if delivery_site_key(method, path).is_some() {
        return Ok(());
    }
...
/// Derive the **record-level resource attributes** of a stored site
...
/// |---|---|---|
/// | `resource.site` | `Site::key` | `handbook` |
/// | `resource.visibility` | `Site::visibility` | `public`, `restricted` |
/// | `resource.owner` | `Site::owner_ref` | the bare organization uuid (enables `$sub` ownership) |
/// | `resource.owner_ref` | `Site::owner_ref` | the full `organization:` URN |
///
/// A deployment can then scope an editor to their own sites
/// ("allow write when `resource.site` is one of theirs") purely as
/// policy, with no code change.
#[must_use]
pub fn site_resource_attrs(site: &sites::Model) -> BTreeMap<String, Vec<String>> {
    let mut attrs = BTreeMap::new();
    attrs.insert("site".to_string(), vec![site.key.clone()]);
    attrs.insert("visibility".to_string(), vec![site.visibility.clone()]);
    if let Some(owner_ref) = &site.owner_ref {
        let owner = owner_ref
            .split_once(':')
...
/// Record-level resource attributes of an **entry variant** — the
/// record most policies actually care about
/// (`authorization-attributes.md` §9). Keys a policy matches with
/// `resource.<key>`:
///
/// | Resource key | From | Example tokens |
/// |---|---|---|
/// | `resource.owner` | the entry's `owner_ref` | the bare worker uuid (enables `$sub` ownership) |
/// | `resource.owner_ref` | the entry's `owner_ref` | the full `worker:` URN |
/// | `resource.reviewer` | the variant's `reviewer_ref` | the bare worker uuid |
/// | `resource.status` | the variant | `draft`, `in_review`, `approved`, `published`, `archived` |
/// | `resource.locale` | the variant | `en`, `fr-CA` |
/// | `resource.content_type` | the entry | `article` |
/// | `resource.published` | the variant | `true` / `false` |
///
/// This is what makes the five personas expressible as policy rather
/// than code: "authors write only where `resource.owner = $sub`",
/// "translators write only their locales", "nobody but admin touches
/// `resource.content_type = policy-notice`".
#[must_use]
pub fn variant_resource_attrs(
    entry: &entries::Model,
    ...
    let bare = |urn: &str| {
        urn.split_once(':')
            .map_or_else(|| urn.to_string(), |(_, id)| id.to_string())
    };
    if let Some(owner_ref) = &entry.owner_ref {
        attrs.insert("owner".to_string(), vec![bare(owner_ref)]);
        attrs.insert("owner_ref".to_string(), vec![owner_ref.clone()]);
    }
    if let Some(reviewer_ref) = &variant.reviewer_ref {
        attrs.insert("reviewer".to_string(), vec![bare(reviewer_ref)]);
    }
    attrs.insert("status".to_string(), vec![variant.status.clone()]);
    attrs.insert("locale".to_string(), vec![variant.locale.clone()]);
    attrs.insert(
        "content_type".to_string(),
        vec![entry.content_type_key.clone()],
    );
    attrs.insert(
        "published".to_string(),
        vec![variant.published_revision_pid.is_some().to_string()],
    );
    attrs
}

/// Resource attributes for a variant that does **not exist yet** — the
/// locale a caller is asking to create.
///
/// A locale-scoped persona (a translator) has to be gated on the
/// locale being *requested*, since there is no record to describe. The
/// entry's own attributes carry over; `status` is `draft`, because
/// that is what a new variant is, and `published` is false.
#[must_use]
pub fn proposed_variant_attrs(
    entry: &entries::Model,
    locale: &str,
) -> BTreeMap<String, Vec<String>> {
    let mut attrs = BTreeMap::new();
    if let Some(owner_ref) = &entry.owner_ref {
        let bare = owner_ref
            .split_once(':')
            .map_or(owner_ref.as_str(), |(_, id)| id);
        attrs.insert("owner".to_string(), vec![bare.to_string()]);
        attrs.insert("owner_ref".to_string(), vec![owner_ref.clone()]);
    }
    attrs.insert("status".to_string(), vec!["draft".to_string()]);
    attrs.insert("locale".to_string(), vec![locale.to_string()]);
    attrs.insert(
        "content_type".to_string(),
        vec![entry.content_type_key.clone()],
    );
    attrs.insert("published".to_string(), vec!["false".to_string()]);
    attrs
}

/// The keys the `mask` obligation redacts on a variant or revision
/// read: the unpublished body and the newsroom's internals.
///
/// Structure stays visible — that an entry exists, its type, its
/// status — which is the shape a partner needs without the editorial
/// detail (spec `auth.md`).
pub const MASKED_CONTENT_KEYS: [&str; 4] = ["blocks", "fields", "note", "reviewer_ref"];

/// Record-level resource attributes of a **content type**:
/// `resource.content_type` (its key) and `resource.routable`. A
/// deployment can then reserve a type for admins ("deny write when
/// `resource.content_type` is `policy-notice` unless `access=admin`")
/// as policy.
#[must_use]
pub fn content_type_resource_attrs(
    content_type: &content_types::Model,
) -> BTreeMap<String, Vec<String>> {
    let mut attrs = BTreeMap::new();
    attrs.insert("content_type".to_string(), vec![content_type.key.clone()]);
    attrs.insert(
        "routable".to_string(),
        vec![content_type.routable.to_string()],
    );
    attrs
}
...
/// Apply the `mask` obligation to a JSON value: set each named
/// top-level key to `null` (CMS-R22 masks **unpublished bodies,
/// revision notes, and reviewer identity** while the structure — that
/// an entry exists, its type, its published state — stays visible).
/// Setting to `null` rather than an empty string keeps the family's
/// honesty rule: masked content is *absent*, never a plausible-looking
/// blank an editor might mistake for real emptiness.
#[must_use]
pub fn mask_json(mut value: serde_json::Value, keys: &[&str]) -> serde_json::Value {
...
/// Apply the `mask` obligation, if the decision carried one.
///
/// The pairing of [`authorize_record`]'s obligations with
/// [`MASKED_CONTENT_KEYS`], in one call — because the two halves being
/// separate is exactly how a handler ends up computing the obligation
/// and then not honouring it. Every read that can expose an
/// unpublished body goes through here.
///
/// An obligation this build does not implement is **refused, not
/// ignored**: silently serving the full record because the policy
/// asked for something unrecognised would turn a stricter policy into
/// a weaker one. The caller sees the unhonoured obligation named.
///
/// # Errors
///
/// `500` naming the obligation, when the decision carries one this
/// build cannot honour.
pub fn mask_if_required_checked(
    ...
    for obligation in obligations {
        match obligation.as_str() {
            "mask" => masked = mask_json(masked, &MASKED_CONTENT_KEYS),
            // `audit` is honoured by the handler's own audit row.
            "audit" => {}
            other => {
                return Err((
                    StatusCode::INTERNAL_SERVER_ERROR,
                    format!(
                        "the policy attached the obligation `{other}`, which this build cannot \
                         honour; refusing rather than serving the record unmasked"
                    ),
                ));
            }
        }
...
/// [`mask_if_required_checked`], for the common case where the caller
/// has no way to report the error and the safe answer is to mask.
///
/// An unknown obligation masks rather than refusing here, on the same
/// principle: when in doubt, show less.
#[must_use]
pub fn mask_if_required(value: serde_json::Value, obligations: &[String]) -> serde_json::Value {
    mask_if_required_checked(value, obligations)
        .unwrap_or_else(|_| mask_json(serde_json::Value::Null, &MASKED_CONTENT_KEYS))
}
...
/// attributes ([`site_resource_attrs`] /
/// [`content_type_resource_attrs`]) and
...
/// context.
///
/// Gated on the same `CMS_REQUIRE_AUTH` flag as the blanket guard:
...
fn build_from_env() -> Verifier {
    let issuer = env_or("CMS_TOKEN_ISSUER", DEFAULT_ISSUER);
    let audience = env_or("CMS_TOKEN_AUDIENCE", DEFAULT_AUDIENCE);
    let keys = std::env::var("CMS_PASETO_KEYS")
        .ok()
        .filter(|s| !s.trim().is_empty())
...
mod tests {
    ...
    /// SEC-G8 — the default-off **exposure pin**. With `CMS_REQUIRE_AUTH`
    /// off (the shipped default), the most sensitive reads — the audit
    /// trail, a site's whole configuration, and (once they exist)
    /// unpublished revisions — are **open without a token**. For a CMS
    /// that means *every* draft is world-readable, which is the
    /// pre-publication disclosure this project calls its signature harm
    /// (spec `auth.md`). This is by design
    ...
    fn default_off_exposes_sensitive_reads_activation_is_a_release_gate() {
        ...
        for path in [
            "/api/sites/0c4f1e2a-0000-4000-8000-000000000001", // a site's whole config
            "/api/content-types/0c4f1e2a-0000-4000-8000-000000000002", // its schemas
            "/api/audits/recent",                              // system-wide audit
            "/api/events/recent",                              // the change stream
        ] {
            assert!(
    ...
    fn derive_action_matrix() {
        for method in [Method::GET, Method::HEAD, Method::OPTIONS] {
            assert_eq!(derive_action(&method, "/api/sites"), Action::Read);
        }
        assert_eq!(
            derive_action(&Method::DELETE, "/api/sites/1"),
            Action::Delete
        );
        for path in [
            "/api/sites/import",
            "/api/content-types/1/redact",
            "/api/entries/import",
        ] {
            assert_eq!(derive_action(&Method::POST, path), Action::Destructive);
        }
        assert_eq!(derive_action(&Method::POST, "/api/sites"), Action::Write);
        assert_eq!(
            derive_action(&Method::POST, "/api/content-types/1/compatibility"),
            Action::Write
        );
        assert_eq!(derive_action(&Method::PUT, "/api/sites/1"), Action::Write);
        assert_eq!(derive_action(&Method::PATCH, "/api/sites/1"), Action::Write);
        // GET on a destructive-suffixed path is still a read — only
        // POST consults the suffix list.
        assert_eq!(
            derive_action(&Method::GET, "/api/sites/import"),
            Action::Read
        );
    ...
    fn abac_access_admin_allows_destructive() {
        ...
        for path in ["/api/sites/import", "/api/content-types/1/redact"] {
            assert!(
                enforce(
                    true,
                    &Method::POST,
    ...
    fn abac_svc_true_allows_everything() {
        ...
        for (method, path) in [
            (Method::GET, "/api/sites"),
            (Method::POST, "/api/sites"),
            (Method::PUT, "/api/sites/1"),
            (Method::DELETE, "/api/sites/1"),
            (Method::POST, "/api/sites/import"),
            (Method::POST, "/api/content-types/1/redact"),
        ] {
            assert!(
    ...
    /// A site model for the resource-attribute tests.
    fn a_site(
        owner: Option<uuid::Uuid>,
        visibility: &str,
    ) -> crate::models::_entities::sites::Model {
        crate::models::_entities::sites::Model {
            created_at: chrono::Utc::now().into(),
            updated_at: chrono::Utc::now().into(),
            id: 1,
            pid: uuid::Uuid::new_v4(),
            key: "handbook".to_string(),
            name: "Staff handbook".to_string(),
            owner_ref: owner.map(|o| format!("organization:{o}")),
            default_locale: "en".to_string(),
            locales: serde_json::json!(["en"]),
            fallback_chains: serde_json::json!({}),
            strict_locales: serde_json::json!([]),
            visibility: visibility.to_string(),
            base_url: None,
            robots_default: "index,follow".to_string(),
            require_distinct_approver: true,
            deleted_at: None,
        }
    }

    /// The site resource attributes map key, visibility, and owner
    /// (`authorization-attributes.md` §9).
    #[test]
    fn site_resource_attrs_map_key_visibility_and_owner() {
        let owner = uuid::Uuid::new_v4();
        let attrs = site_resource_attrs(&a_site(Some(owner), "public"));
        assert_eq!(attrs["site"], vec!["handbook".to_string()]);
        assert_eq!(attrs["visibility"], vec!["public".to_string()]);
        assert_eq!(attrs["owner"], vec![owner.to_string()]);
        assert_eq!(attrs["owner_ref"], vec![format!("organization:{owner}")]);
        assert!(!site_resource_attrs(&a_site(None, "restricted")).contains_key("owner"));
    }

    /// The attributes drive site scoping through the shared engine: an
    /// editor scoped to `handbook` writes it, and falls through to a
    /// masked read elsewhere.
    #[test]
    fn site_attrs_drive_scoping_and_masking() {
        let policy = Policy::from_json(
            r#"{ "rules": [
                { "effect": "allow", "actions": ["write"], "when": { "resource.site": ["handbook"] } },
                { "effect": "allow", "actions": ["read"], "when": {}, "obligations": ["mask"] }
            ] }"#,
        )
        .expect("policy parses");
        let editor = claims_with_attrs(&[]);
        let in_scope = policy.evaluate_with_resource(
            &editor,
            Action::Write,
            ENTITY,
            &site_resource_attrs(&a_site(None, "restricted")),
        );
        assert!(in_scope.allowed);
        assert!(in_scope.obligations.is_empty());

        let mut elsewhere = a_site(None, "restricted");
        elsewhere.key = "intranet".to_string();
        let denied = policy.evaluate_with_resource(
            &editor,
            Action::Write,
            ENTITY,
            &site_resource_attrs(&elsewhere),
        );
        assert!(!denied.allowed, "another site is out of scope");
    ...
    /// A content type's key and routability are policy-visible, so a
    /// deployment can reserve a type for admins.
    #[test]
    fn content_type_resource_attrs_map_key_and_routable() {
        let row = crate::models::_entities::content_types::Model {
            created_at: chrono::Utc::now().into(),
            updated_at: chrono::Utc::now().into(),
            id: 1,
            pid: uuid::Uuid::new_v4(),
            site_pid: uuid::Uuid::new_v4(),
            key: "policy-notice".to_string(),
            name: "Policy notice".to_string(),
            description: None,
            fields: serde_json::json!([]),
            routable: true,
            template_key: None,
            schema_version: 1,
            unpublish_on_stale: false,
            deleted_at: None,
        };
        let attrs = content_type_resource_attrs(&row);
        assert_eq!(attrs["content_type"], vec!["policy-notice".to_string()]);
        assert_eq!(attrs["routable"], vec!["true".to_string()]);
    }

    /// A preview render is recognised as such: it is deferred by the
    /// guard like a public delivery read, but for a different reason —
    /// it carries its own scoped, short-lived credential, which the
    /// preview controller checks. Distinguishing the two matters when
    /// reading the guard: one is authorized by site visibility, the
    /// other by a token.
    #[test]
    fn preview_paths_are_recognised_as_carrying_their_own_credential() {
        assert!(is_preview_path(
            &Method::GET,
            "/delivery/handbook/preview/abc123"
        ));
        assert!(is_preview_path(
            &Method::HEAD,
            "/delivery/handbook/preview/abc123"
        ));
        // An ordinary page read is a delivery read, not a preview.
        assert!(!is_preview_path(
            &Method::GET,
            "/delivery/handbook/en/about"
        ));
        // A mutating method is neither.
        assert!(!is_preview_path(
            &Method::POST,
            "/delivery/handbook/preview/abc123"
        ));
        // And nothing outside the prefix.
        assert!(!is_preview_path(&Method::GET, "/api/preview-tokens/x"));
    }

    /// The pure half of the public-delivery allow-list: it recognises a
    /// delivery read and names its site, and refuses everything else —
    /// so no non-`GET` and no non-delivery path can be mistaken for a
    /// candidate (CMS-D7).
    #[test]
    fn delivery_site_key_recognises_only_delivery_reads() {
        assert_eq!(
            delivery_site_key(&Method::GET, "/delivery/handbook/en/about"),
            Some("handbook")
        );
        assert_eq!(
            delivery_site_key(&Method::HEAD, "/delivery/handbook/sitemap.xml"),
            Some("handbook")
        );
        assert_eq!(
            delivery_site_key(&Method::GET, "/delivery/handbook"),
            Some("handbook")
        );
        // Mutating methods are never delivery reads.
        for method in [Method::POST, Method::PUT, Method::DELETE, Method::PATCH] {
            assert_eq!(
                delivery_site_key(&method, "/delivery/handbook/en/about"),
                None
            );
        }
        // Neither is anything outside the prefix, or a missing site.
        assert_eq!(delivery_site_key(&Method::GET, "/api/sites"), None);
        assert_eq!(delivery_site_key(&Method::GET, "/delivery/"), None);
        assert_eq!(delivery_site_key(&Method::GET, "/delivery"), None);
    }

    /// The guard defers delivery **reads** to the controller (which
    /// checks the site's visibility), and defers nothing else: a
    /// mutating method on the same prefix is still refused here, so the
    /// public hole cannot widen by verb.
    #[test]
    fn the_guard_defers_delivery_reads_and_nothing_else() {
        let (keys, _) = test_keys_and_kid();
        let verifier = Verifier::from_paseto_keys_value(&keys, ISSUER, AUDIENCE).unwrap();
        let policy = policy();
        let no_token = HeaderMap::new();
        ...
        for method in [Method::GET, Method::HEAD] {
            assert!(
                ...
                .is_ok(),
                "{method} on /delivery is deferred to the controller"
            );
        }
        for method in [Method::POST, Method::PUT, Method::DELETE, Method::PATCH] {
            assert_eq!(
                ...
                .0,
                StatusCode::UNAUTHORIZED,
                "{method} on /delivery is not deferred"
            );
        }
    ...
    /// `mask_json` nulls the named keys and leaves the structure —
    /// a masked draft body is absent, not an empty document an editor
    /// could mistake for real emptiness.
    #[test]
    fn mask_json_nulls_unpublished_content_keeps_structure() {
        let masked = mask_json(
            serde_json::json!({
                "key": "about",
                "status": "in_review",
                "blocks": [{ "kind": "paragraph", "text": "embargoed" }],
                "note": "hold until Friday",
            }),
            &["blocks", "note"],
        );
        assert_eq!(masked["key"], "about");
        assert_eq!(masked["status"], "in_review");
        assert!(masked["blocks"].is_null());
        assert!(masked["note"].is_null());
        // Non-object values pass through untouched.
        let passthrough = mask_json(serde_json::json!([1, 2]), &["blocks"]);
        assert_eq!(passthrough, serde_json::json!([1, 2]));
    }

    /// The pairing that makes the obligation real.
    ///
    /// `authorize_record` computing an obligation nobody applies is
    /// exactly the failure this helper exists to prevent — and was the
    /// state of the code until the read handlers were wired to it.
    #[test]
    fn the_mask_obligation_redacts_the_body_and_keeps_the_structure() {
        let record = serde_json::json!({
            "pid": "abc", "number": 3, "title": "Draft",
            "blocks": [{ "kind": "paragraph", "text": "embargoed" }],
            "fields": { "summary": "s" },
            "note": "hold until Friday",
        });

        let unmasked = mask_if_required(record.clone(), &[]);
        assert_eq!(unmasked, record, "no obligation, no change");

        let masked = mask_if_required(record.clone(), &["mask".to_string()]);
        assert!(masked["blocks"].is_null());
        assert!(masked["fields"].is_null());
        assert!(masked["note"].is_null());
        // The structure survives: that the revision exists, its number
        // and title, is what a partner legitimately sees.
        assert_eq!(masked["number"], 3);
        assert_eq!(masked["title"], "Draft");
        assert!(!masked.to_string().contains("embargoed"));
    }

    /// An obligation this build cannot honour must not be ignored.
    #[test]
    fn an_unknown_obligation_is_refused_rather_than_served_unmasked() {
        let record = serde_json::json!({ "blocks": ["secret"], "number": 1 });
        let refused = mask_if_required_checked(record.clone(), &["redact-pii".to_string()]);
        let (status, reason) = refused.expect_err("an unhonoured obligation must refuse");
        assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);
        assert!(
            reason.contains("redact-pii"),
            "the refusal names it: {reason}"
        );

        // `audit` is honoured elsewhere (the handler's audit row), so
        // it passes through without masking.
        let audited = mask_if_required_checked(record.clone(), &["audit".to_string()]).unwrap();
        assert_eq!(audited, record);

        // The infallible wrapper errs toward showing less.
        let safe = mask_if_required(record, &["redact-pii".to_string()]);
        assert!(!safe.to_string().contains("secret"));
    }

    /// A locale-scoped persona is gated on the locale being *asked
    /// for*, since the variant does not exist yet.
    #[test]
    fn proposed_variant_attributes_describe_the_variant_being_created() {
        let entry = entries::Model {
            pid: uuid::Uuid::new_v4(),
            site_pid: uuid::Uuid::new_v4(),
            content_type_key: "article".to_string(),
            type_schema_version: 1,
            key: "k".to_string(),
            source_locale: "en".to_string(),
            owner_ref: Some("worker:11111111-1111-4111-8111-111111111111".to_string()),
            archived_at: None,
            deleted_at: None,
            id: 1,
            created_at: chrono::Utc::now().into(),
        ...
        let attrs = proposed_variant_attrs(&entry, "fr-CA");
        assert_eq!(attrs["locale"], vec!["fr-CA".to_string()]);
        assert_eq!(attrs["status"], vec!["draft".to_string()]);
        assert_eq!(attrs["published"], vec!["false".to_string()]);
        assert_eq!(attrs["content_type"], vec!["article".to_string()]);
        // The bare uuid is what `$sub` ownership compares against.
        assert_eq!(
            attrs["owner"],
