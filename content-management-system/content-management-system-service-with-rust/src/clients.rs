//! Upstream service clients (CMS-D16): **stub-first** display-name
//! resolution for `worker:` / `organization:` `EntityRefs` (authors,
//! editors, translators, and a site's publishing body), plus whatever
//! entity type an `entity_ref` content field points at.
//!
//! CMS references identities by URN and needs only display labels.
//! Lookups are read-only, cached, best-effort, and **never block
//! writes**: an unreachable upstream degrades a view to showing the
//! URN.
//!
//! Selection is per-process via `CMS_UPSTREAM_MODE`:
...
//!   `CMS_WORKER_SERVICE_URL` / `CMS_ORGANIZATION_SERVICE_URL` (and
//!   the same pattern for any other referenced type), reading a
//!   best-effort `name`-ish field. (Thin by design; the family
//!   front-ends do the rich rendering.)
...
/// The process-wide mode, from `CMS_UPSTREAM_MODE`.
#[must_use]
pub fn mode() -> Mode {
    static MODE: OnceLock<Mode> = OnceLock::new();
    *MODE.get_or_init(|| {
        match std::env::var("CMS_UPSTREAM_MODE")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase()
            .as_str()
...
/// are logged at debug and swallowed — an unresolvable display name
/// never fails a CMS request, and never blocks a write.
...
mod tests {
    ...
    async fn stub_mode_misses_and_cache_hits() {
        let urn = format!("worker:{}", uuid::Uuid::new_v4());
