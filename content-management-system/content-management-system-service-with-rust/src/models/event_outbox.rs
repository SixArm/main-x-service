...
pub struct OutboxInsert {
    /// Envelope dedup id.
    pub event_id: Uuid,
    /// Entity name (`site` / `content_type` / …).
    pub entity: String,
    /// Record pid (bus partition key).
    pub entity_pid: Uuid,
    /// Change kind token (`created`, `content_type_changed`, …).
    pub kind: String,
