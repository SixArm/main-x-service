...
//!
//! Every mutation **and every sensitive read** (an unpublished
//! revision, a preview-token issue or use) writes one row. The
//! `snapshot` JSON carries the action detail (old/new state, the reason
//! a reasoned action required, the `owner` where relevant — which is
//! what the owner-scoped query filters on).
