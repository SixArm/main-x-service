...
//! [`PersonEvent`](super::PersonEvent) / [`EventProducer`](super::EventProducer)
...
//! `PERSON_EVENT_TRANSPORT=outbox` is set.
//!
//! [`Envelope::for_event`] builds the envelope from a domain
//! [`Person`](crate::models::Person); [`EventView`] is the flat operator
...
pub struct Envelope {
    ...
    /// The entity name (`snake_case`), e.g. `"person"`.
    ///
    ...
    /// The record's denormalised label (its full name) at event time.
    pub name: String,
...
impl Envelope {
    /// Build an envelope describing `kind` applied to `person`, minting a
    ...
    #[must_use]
    pub fn for_event(person: &Person, kind: EventKind) -> Self {
        Self {
            event_id: Uuid::new_v4(),
            schema_version: SCHEMA_VERSION,
            entity: ENTITY,
            kind,
            pid: person.id.to_string(),
            seq: next_seq(),
            actor: None,
    ...
    #[must_use]
    pub fn for_merge(survivor: &Person, merged_from: &Uuid) -> Self {
        Self {
    ...
    /// the envelope `pid` is the originating person (the edge's `from`
...
impl EventTransport {
    /// Parse the `PERSON_EVENT_TRANSPORT` value. `outbox` selects the
...
/// The process-wide transport, read once from `PERSON_EVENT_TRANSPORT` and
...
pub fn transport() -> EventTransport {
    ...
    *TRANSPORT.get_or_init(|| {
        std::env::var("PERSON_EVENT_TRANSPORT")
...
mod tests {
    ...
    fn for_event_populates_the_envelope_from_the_record() {
        let person = a_person();
        let env = super::Envelope::for_event(&person, EventKind::Created);
        assert_eq!(env.entity, ENTITY);
        assert_eq!(env.schema_version, SCHEMA_VERSION);
        assert_eq!(env.kind, EventKind::Created);
        assert_eq!(env.pid, person.id.to_string());
        assert_eq!(env.name, "John Smith");
        assert!(env.actor.is_none());
    }

    #[test]
    fn for_merge_sets_kind_merged_and_merged_from() {
        let survivor = a_person();
    ...
    fn for_event_leaves_merged_from_none() {
        let env = super::Envelope::for_event(&a_person(), EventKind::Updated);
    ...
    fn seq_is_monotonic_across_envelopes() {
        let person = a_person();
        let a = super::Envelope::for_event(&person, EventKind::Created);
        let b = super::Envelope::for_event(&person, EventKind::Updated);
    ...
    fn event_view_projects_the_frozen_shape() {
        let person = a_person();
        let env = super::Envelope::for_event(&person, EventKind::Deleted);
    ...
    fn crud_envelope_omits_data_on_the_wire() {
        let env = super::Envelope::for_event(&a_person(), EventKind::Created);
    ...
    fn for_link_carries_edge_detail_data() {
        ...
        assert_eq!(
            json["data"]["to_ref"],
            "worker:0c4f1e2a-0000-4000-8000-000000000002"
        );
    }
