...
//! periodically purges old published rows. Delivery is **at-least-once**
//! (a crash between the sink send and the `published_at` stamp re-sends),
//! so sinks/consumers dedupe on the envelope `event_id` (see
...
//! `FluvioSink` (BUS-3; ported from case-service's BUS-1 reference; only
//! compiled under this crate's `fluvio` cargo feature, off by default, so a
//! default build's dependency tree and behaviour are unchanged) is the
...
//! (`PERSON_EVENT_TRANSPORT=outbox`) **and** `PERSON_EVENT_RELAY` is truthy
...
//! [`LoggingSink`] when `PERSON_FLUVIO_ENDPOINT` (§7) is configured — see
...
/// The real-broker sink (BUS-3, ported from case-service's BUS-1
/// reference; behind the `fluvio` cargo feature): ships each event to a
/// [Fluvio](https://www.fluvio.io/) topic, partitioned by record `pid` per
/// `agents/share/event-bus.md` §7. One producer per topic, held for the
/// sink's lifetime — `TopicProducerPool` is designed to be built once and
/// reused across sends rather than reconnected per event.
#[cfg(feature = "fluvio")]
...
impl FluvioSink {
    ...
    pub async fn connect(endpoint: &str, topic: &str) -> std::result::Result<Self, SinkError> {
...
pub async fn purge_published(db: &DatabaseConnection, retention_days: i64) -> Result<u64> {
    let cutoff = time::OffsetDateTime::now_utc() - time::Duration::days(retention_days.max(0));
    let res = event_outbox::Entity::delete_many()
        .filter(OutboxColumn::PublishedAt.is_not_null())
        .filter(OutboxColumn::PublishedAt.lt(cutoff))
        .exec(db)
        .await?;
    Ok(res.rows_affected)
}

/// Whether the relay background loop should run: `PERSON_EVENT_RELAY`
/// truthy (`1`/`true`/`yes`/`on`). Off by default.
#[must_use]
pub fn relay_enabled() -> bool {
    matches!(
        std::env::var("PERSON_EVENT_RELAY")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase()
            .as_str(),
...
/// Relay poll interval in seconds (`PERSON_EVENT_RELAY_INTERVAL_SECS`,
/// default 5, floored at 1).
#[must_use]
pub fn interval_secs() -> u64 {
    std::env::var("PERSON_EVENT_RELAY_INTERVAL_SECS")
        .ok()
...
/// Outbox retention in days (`PERSON_EVENT_RETENTION_DAYS`, default 7).
#[must_use]
pub fn retention_days() -> i64 {
    std::env::var("PERSON_EVENT_RETENTION_DAYS")
        .ok()
        .and_then(|v| v.trim().parse::<i64>().ok())
        .unwrap_or(7)
}

/// Fluvio broker endpoint (`PERSON_FLUVIO_ENDPOINT`, §7). Unset/blank ⇒ no
...
pub fn fluvio_endpoint() -> Option<String> {
    std::env::var("PERSON_FLUVIO_ENDPOINT")
        .ok()
        .filter(|s| !s.trim().is_empty())
}

/// The topic events publish to (`PERSON_EVENT_TOPIC`, default
/// `mxi.person.events` per §7's `mxi.<entity>.events` convention).
#[must_use]
pub fn event_topic() -> String {
    std::env::var("PERSON_EVENT_TOPIC")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| "mxi.person.events".to_string())
}
...
///
/// Sink selection: no `PERSON_FLUVIO_ENDPOINT` ⇒ [`LoggingSink`]. An
...
pub fn spawn(db: DatabaseConnection) {
    ...
    if endpoint.is_some() && !cfg!(feature = "fluvio") {
        tracing::error!(
            "PERSON_FLUVIO_ENDPOINT is set but this binary was built without the `fluvio` \
...
mod tests {
    ...
    fn logging_sink_sends_ok() {
        ...
        let out = rt.block_on(async {
            LoggingSink
                .send("person", "pid-1", &serde_json::json!({"kind": "created"}))
                .await
        });
        assert!(out.is_ok());
    }
    ...
    fn capturing_sink_records_entity_and_key() {
        ...
        rt.block_on(async {
            sink.send("person", "pid-9", &serde_json::json!({}))
                .await
                .unwrap();
        });
        assert_eq!(
            sink.0.lock().unwrap().as_slice(),
            &[("person".to_string(), "pid-9".to_string())]
        );
    }
    ...
    fn config_defaults_are_safe() {
        ...
        if std::env::var("PERSON_EVENT_RELAY").is_err() {
            assert!(!relay_enabled());
        }
        if std::env::var("PERSON_EVENT_RELAY_INTERVAL_SECS").is_err() {
            assert_eq!(interval_secs(), 5);
        }
        if std::env::var("PERSON_EVENT_RETENTION_DAYS").is_err() {
