...
pub struct Checkpoint {
    /// Format version.
    pub version: String,
    /// The anchor row's `seq` — the position this checkpoint describes.
    ///
    /// `seq` rather than `id`, because this table's primary key is an
    /// application-assigned UUID that carries no ordering.
    pub anchor_id: i64,
...
pub enum CheckpointVerdict {
    ...
    AnchorMissing {
        /// The `seq` that should have been present.
        anchor_id: i64,
    },
...
impl Checkpoint {
    ...
    pub async fn take(
        ...
        let Some(anchor) = audit_log::Entity::find()
            .filter(audit_log::Column::Hash.is_not_null())
        ...
        let rows_at_or_before = audit_log::Entity::find()
            .filter(audit_log::Column::Seq.lte(anchor.seq))
            .count(db)
            .await?;

        let mut checkpoint = Self {
            version: CHECKPOINT_VERSION.to_string(),
            anchor_id: anchor.seq,
            head,
            rows_at_or_before,
            taken_at_micros,
            mac: None,
    ...
    pub async fn verify_against(
        &self,
        db: &sea_orm::DatabaseConnection,
    ) -> crate::Result<CheckpointVerdict> {
        ...
        let Some(anchor) = audit_log::Entity::find()
            .filter(audit_log::Column::Seq.eq(self.anchor_id))
            .one(db)
            .await?
        else {
        ...
        let now = audit_log::Entity::find()
            .filter(audit_log::Column::Seq.lte(self.anchor_id))
            .count(db)
            .await?;
