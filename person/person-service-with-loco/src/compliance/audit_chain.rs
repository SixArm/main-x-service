...
//! Person is the identity spine of the family and its records are
...
//!
//! Person's `audit_log` predates the loco-style services and is shaped
...
mod tests {
    ...
    fn row(seq: i64, prev: Option<&str>, action: &str) -> audit_log::Model {
        let mut model = audit_log::Model {
            ...
            hash_sha3: None,
            // No key in unit tests: `mac_absent`, not a mismatch.
            mac: None,
            context: None,
            disclosure: false,
            redacted_at: None,
        ...
        model.hash_sha3 = Some(row_hash_sha3(&input_for(&model, prev)));
        model
    }
