//! Row-level integrity hashing over person records.
//!
//! The audit chain ([`super::audit_chain`]) proves the **trail** was not
//! rewritten. It says nothing about the person records themselves: an
...
//!
//! Each `persons` row carries a `content_hash` — a SHA-256 over the
...
//! column, so hashing "the record" is hashing one field. A person is
//! **relational**: names, identifiers, addresses, contacts, documents,
//! and links live in their own tables, and those are exactly where the
//! data worth tampering with lives — a surname, a national identifier, a
//! home address. Hashing only the `persons` parent row would repeat the
//! precise narrowness that made the database triggers worthless: covering
//! the parent while missing everything that matters.
//!
//! So the digest is taken over the **assembled domain [`Person`]** — the
...
//! ## Not a chain
//!
//! Unlike `audit_log`, person rows are *mutable by design*: an update
...
pub const RECORD_HASH_VERSION: &str = "p-r1";
...
/// The canonical JSON of a person for hashing: the domain model with the
...
#[must_use]
fn canonical_json(person: &Person) -> Option<String> {
    let mut value = serde_json::to_value(person).ok()?;
...
///
/// As [`record_hash`].
pub fn hash_of_live(person: &Person) -> crate::Result<String> {
...
#[must_use]
pub fn verify(rows: &[(Person, Option<String>, Option<i64>)]) -> RecordIntegrityReport {
    ...
    for (person, stored, deleted_at) in rows {
        let Some(stored) = stored.as_deref() else {
            report.unhashed += 1;
            continue;
        };
        match hash_with_deleted_at(person, *deleted_at) {
...
mod tests {
    ...
    fn the_version_tag_is_bound() {
        let p = person("Versioned");
        let json = canonical_json(&p).expect("serializable");
        assert!(!json.contains("\"created_at\""));
        assert!(!json.contains("\"updated_at\""));
        assert!(RECORD_HASH_VERSION.starts_with("p-"));
    }
