...
/// `PERSON_AUDIT_FAIL_CLOSED` (read once and cached).
///
...
/// under `PERSON_EVENT_TRANSPORT=outbox`, where the audit row shares
...
pub fn fail_closed() -> bool {
    ...
    *FAIL_CLOSED.get_or_init(|| {
        crate::api::rest::auth::parse_bool(
            &std::env::var("PERSON_AUDIT_FAIL_CLOSED").unwrap_or_default(),
        )
    })
}

/// Record one read/disclosure audit row — a **no-op unless**
/// `PERSON_AUDIT_READS` is on, so adopting this module changes nothing
...
pub async fn record_access(
    ...
    let audit_ctx = crate::db::AuditContext {
        user_id: actor.map(ToString::to_string),
        ip_address: None,
        user_agent: None,
    };
    let Err(error) = audit
        .log_access(entity_type, entity_id, action, &audit_ctx, ctx)
        .await
    else {
        return Ok(());
    };
    if fail_closed() {
        tracing::error!(
            %error,
            action,
            "read-access audit write failed and PERSON_AUDIT_FAIL_CLOSED is on; refusing the \
    ...
    tracing::warn!(
        ...
        "failed to write read-access audit row; the read proceeds (fail-open) and the chain \
         will show a gap indistinguishable from a deletion — set PERSON_AUDIT_FAIL_CLOSED to \
         refuse instead"
    );
    Ok(())
}
