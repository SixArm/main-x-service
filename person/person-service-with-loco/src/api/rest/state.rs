...
/// key set (spec §13 T-1c). Reads:
///
/// - `PERSON_PASETO_KEYS_URL` — URL of the auth-service published key
...
///   `PERSON_PASETO_KEYS`; on failure (network/HTTP/parse) log a
///   warning and fall back to the env path — the service always boots,
///   auth-service downtime never prevents startup.
/// - `PERSON_TOKEN_ISSUER` / `PERSON_TOKEN_AUDIENCE` — expected `iss` /
...
pub async fn verifier_from_env_or_fetch() -> Verifier {
    let issuer = env_or("PERSON_TOKEN_ISSUER", DEFAULT_ISSUER);
    let audience = env_or("PERSON_TOKEN_AUDIENCE", DEFAULT_AUDIENCE);
    let url = std::env::var("PERSON_PASETO_KEYS_URL")
        .ok()
        .filter(|s| !s.trim().is_empty());
...
/// `None` ⇒ the `PERSON_PASETO_KEYS` env path; `Some(url)` ⇒ fetch via
    ...
    match Verifier::from_paseto_keys_url(url, issuer, audience).await {
        ...
        Err(error) => {
            tracing::warn!(
                url,
                error = %error,
                "PASETO key set fetch failed; falling back to the PERSON_PASETO_KEYS env path"
            );
            env_keys_verifier(issuer, audience)
        }
    }
