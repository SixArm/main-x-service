...
//! with `PERSON_REQUIRE_AUTH` **on** — the "activation proof" (AU-1).
//!
...
/// A minimal valid person payload with a unique family name, so a create
/// that reaches the handler is not turned away as a duplicate.
fn person_body() -> Value {
    json!({
        "name": { "family": common::unique_person_name("Enforcement"), "given": ["Ada"] },
        "gender": "female"
    })
}
...
async fn post_person(app: &axum::Router, token: Option<&str>) -> StatusCode {
    ...
    app.clone()
        .oneshot(
            builder
                .body(Body::from(serde_json::to_vec(&person_body()).unwrap()))
                .unwrap(),
        )
        .await
        .unwrap()
...
async fn enforcement_on_gates_the_real_router() {
    ...
    unsafe {
        ...
        // No PERSON_ABAC_POLICY ⇒ the built-in default policy: read
    ...
    assert_eq!(
        get(&app, "/api/persons/search?q=anyone", None).await,
        StatusCode::UNAUTHORIZED,
        "a protected read without a token is 401"
    );
    assert_eq!(
        post_person(&app, None).await,
        StatusCode::UNAUTHORIZED,
    ...
    assert_eq!(
        get(&app, "/api/persons/search?q=anyone", Some("not-a-token")).await,
        StatusCode::UNAUTHORIZED,
    ...
    let reader = mint(&kid, &[]);
    assert_eq!(
        get(&app, "/api/persons/search?q=anyone", Some(&reader)).await,
        StatusCode::OK,
        "any authenticated caller may read"
    );
    assert_eq!(
        post_person(&app, Some(&reader)).await,
    ...
    assert_eq!(
        post_person(&app, Some(&writer)).await,
        StatusCode::CREATED,
