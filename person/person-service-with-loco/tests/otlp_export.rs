...
//!
//! Ported from link-graph-service's `tests/otlp_export.rs` — the family's
//! first working exporter, and the reference for this one
//! (`agents/share/rust-tracing-opentelemetry-stack.md`). worker / event
//! still carry the commented-out stub this crate had until this change; a
//! test asserting only "it compiles and does not panic" would prove
...
use person_service::observability::{TelemetryConfig, build_meter_provider, build_tracer_provider};
...
async fn a_tracing_span_reaches_a_real_otlp_collector() {
    ...
    let config = TelemetryConfig {
        service_name: "person-service-otlp-test".to_string(),
    ...
    {
        let layer = tracing_opentelemetry::layer().with_tracer(provider.tracer("person-service"));
        let subscriber = tracing_subscriber::registry().with(layer);
        tracing::subscriber::with_default(subscriber, || {
            let span = tracing::info_span!("persons.get", person.id = "0c4f1e2a-test");
            let _entered = span.enter();
            tracing::info!("serving a person read");
        });
    }
    ...
    assert!(
        service_names.contains(&"person-service-otlp-test".to_string()),
    ...
    let span = spans
        .iter()
        .find(|span| span.name == "persons.get")
        .unwrap_or_else(|| {
            panic!(
                "exported spans did not include `persons.get`: {:?}",
                spans.iter().map(|span| &span.name).collect::<Vec<_>>()
            )
        });
    assert!(
        span.attributes.iter().any(|kv| kv.key == "person.id"),
...
async fn a_metric_reaches_a_real_otlp_collector() {
    ...
    let meter = provider.meter("person-service");
    let histogram = meter
