...
//!
//! Ported from link-graph-service's `tests/otlp_middleware.rs`.
//! `tests/otlp_export.rs` proves the export pipeline; this proves the
//! thing the service actually runs — the exact
//! [`observability::trace_mw`](person_service::observability::trace_mw)
...
use person_service::observability::{TelemetryConfig, build_tracer_provider, trace_mw};
...
async fn a_served_request_exports_a_span_and_returns_its_traceparent() {
    ...
    let config = TelemetryConfig {
        service_name: "person-service-mw-test".to_string(),
    ...
    tracing_subscriber::registry()
        .with(tracing_opentelemetry::layer().with_tracer(provider.tracer("person-service")))
        .with(tracing_subscriber::EnvFilter::new(
            "person_service=trace,otlp_middleware=trace",
        ))
        .init();
