...
|---|---|
| `Person` field list or types | §5.1 |
| Supporting types (`Organization`, `Consent`, `MergeRecord`, …) | §5.2 |
| Domain invariants | §5.3 |
| New capability surface (e.g. a new endpoint family) | §6.x |
| Default match weights or thresholds | §6.2 |
| Validation / normalisation rules | §6.5 |
| Privacy / consent behaviour | §6.6 |
| FHIR coverage | §6.8 |
...
A `spec-drift` CI check (tracked as §13 T-7) will fail any pull
request that modifies `src/matching/**` or `src/models/person.rs`
