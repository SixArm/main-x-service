...
This guide covers deploying the Person Service using Docker and Docker Compose.
...
```bash
git clone https://github.com/your-org/person-service-rust-crate.git
...
# Tag for registry
docker tag person-server:latest your-registry.com/person_service-server:v1.0.0
```
...
# Push image
docker push your-registry.com/person_service-server:v1.0.0
```
...
docker inspect person-server --format='{{json .State.Health}}'
```
...
```bash
# Backup search index
docker cp person-server:/app/data/search_index ./search_index_backup

# Restore search index
docker cp ./search_index_backup person-server:/app/data/search_index
podman compose restart person-server
```
