...
This is an identity-adjacent library. Treat every line of code as if a person's care might depend on it (because it might).
...
2. **No global state.** No `static mut`, no `OnceCell` holding person data, no thread-local caches.
...
- ❌ Adding `serde_json::to_writer(File::create(...)?, &person)` "for diagnostics." That writes PII to disk.
