...
**T-38 — Pin OQ-7: phonetic bonus's participation in `total_weight` (§22 OQ-7 / §12.3).**
- [ ] Add a unit test that directly asserts whether `total_weight` (the probabilistic-average denominator) includes the phonetic bonus's weight only when the bonus actually applied, vs. always — pinning today's implemented behaviour byte-for-byte.
- [ ] Update §22 OQ-7 from "Open" to "Resolved (T-38)" with a one-line statement of the pinned rule, cross-referenced from `agents/matching-algorithm.md`.
- **Acceptance:** the new test fails if a future change flips the current `total_weight` semantics without an explicit spec update; OQ-7 marked resolved; `cargo test` clean (verified: §22 lists OQ-7 as still "Open" — "current behaviour is judged correct" but not pinned by any test — and `grep -n "OQ-7\|phonetic_bonus" src/ tests/` finds no reference outside the spec itself; the closest existing test, `phonetic_match_is_a_bonus_not_a_penalty` at `src/matcher.rs:2568`, checks direction, not `total_weight` inclusion).
