# Person matcher Rust Crate

A comprehensive Rust library for matching person records for identity exchanges.
...
This crate implements both **deterministic** and **probabilistic** person matcher algorithms based on research from:
...
- ✅ **Batch API**: `match_one_to_many` scores one query against many candidates (output parallel to input); `rank_one_to_many` returns the same scores sorted by descending score with a deterministic tiebreak — the building block for screening against a main person index
...
- ✅ **Place of Birth Matching**: `Person::birth_place` reuses the existing `Address` type (FHIR `Patient.birthPlace` parity); dedicated city + country sub-score (`0.7 × Jaro-Winkler(city) + 0.3 × exact(country)` blend) is diacritic-tolerant and ignores street / postcode fields that aren't meaningful for a place of birth.
- ✅ **Multiple-Birth (Twin) Disambiguation**: `Person::multiple_birth` carries FHIR `Patient.multipleBirth` (1-indexed birth order) — the canonical fix for identical-twin records that otherwise share name, DOB, address, and demographic data and would otherwise be wrongly merged.
- ✅ **Date of Death and Place of Death**: `Person::death_date` (FHIR `Patient.deceasedDateTime`) and `Person::death_place` for deceased-person records. Death date uses the same DD/MM ↔ MM/DD transposition heuristic as date of birth; place of death shares the `0.7 × city + 0.3 × country` blend with place of birth via a shared `score_named_place` helper.
...
- **United Kingdom National Health Service Number**: 10-digit national personal identifier with Modulus-11 check digit
...
`MatchingEngine::match_persons` uses the E.164 form first and falls back to the legacy form when either input cannot be parsed. The default country is configured via `MatchConfig::phone_default_country` (defaults to `Some("GB")`):

```rust
use person_matcher::{MatchConfig, MatchingEngine, Normalizer, Person};
...
- Any shared `(country, number)` pair across the two persons' lists is sufficient for a deterministic match. A multi-country person matches another record that carries any one of their books.
...
For main-person-index workflows, screen one query against many candidates:
...
let candidates: Vec<Person> = vec![
    Person::builder().given_name("Grace").family_name("Hopper").build(),
    Person::builder().given_name("Ada").family_name("Lovelace").build(),   // best match
...
```rust
use person_matcher::{MatchConfig, MatchingEngine, Person};

let a = Person::builder().given_name("Alice").family_name("Anderson")
    .email("  Alice@Example.ORG  ").build();
let b = Person::builder().given_name("Alice").family_name("Anderson")
    .email("alice@example.org").build();
...
let a = Person::builder().given_name("X").family_name("Y")
    .email("j.smith@gmail.com").build();
let b = Person::builder().given_name("X").family_name("Y")
    .email("jsmith+work@gmail.com").build();
assert_eq!(engine.match_persons(&a, &b).breakdown.email_score, Some(1.0));
```

Note that `local_id` is **not** scored: different organisations may issue colliding values (different persons' MRNs from sites A and B can be byte-equal), so positional matching would produce false positives.
...
```rust
use person_matcher::{MatchConfig, MatchingEngine, NicknameTable, Person};
...
let a = Person::builder().given_name("Michael").family_name("Jones").build();
let b = Person::builder().given_name("Mike").family_name("Jones").build();
let r = engine.match_persons(&a, &b);
...
```rust
use person_matcher::{Normalizer, ParsedAddressLine};
