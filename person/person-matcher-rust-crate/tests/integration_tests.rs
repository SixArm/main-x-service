...
//! [`Person`] records, constructing a [`MatchingEngine`], and inspecting
//! the [`person_matcher::MatchResult`] / [`person_matcher::MatchBreakdown`].
//!
...
/// Builds a chrono `NaiveDate` from year/month/day, keeping the fixtures terse.
fn dob(y: i32, m: u32, d: u32) -> NaiveDate {
    NaiveDate::from_ymd_opt(y, m, d).unwrap()
}

/// A self-consistent United Kingdom National Health Service Number-format
/// check-digit identifier used across fixtures. This is the same illustrative
/// value reused in the AGENTS guides; it is not allocated to any real
/// subscriber.
const UNITED_KINGDOM_NATIONAL_HEALTH_SERVICE_NUMBER_A: &str = "9434765919";
/// A second self-consistent United Kingdom National Health Service Number-format
/// identifier for "different person" cases.
const UNITED_KINGDOM_NATIONAL_HEALTH_SERVICE_NUMBER_B: &str = "9434765870";
...
fn test_unparseable_united_kingdom_national_health_service_numbers_score_none_not_zero() {
    let p1 = Person::builder()
        .united_kingdom_national_health_service_number("ABCDEFGHIJ")
        .given_name("Ada")
        .family_name("Lovelace")
        .date_of_birth(dob(1815, 12, 10))
        .build();
    let p2 = Person::builder()
        .united_kingdom_national_health_service_number("not-valid")
        .given_name("Ada")
        .family_name("Lovelace")
        .date_of_birth(dob(1815, 12, 10))
        .build();
...
fn test_common_name_typo_under_lenient_config() {
    ...
    let p2 = Person::builder()
        ...
        .gender(Gender::Female)
        .build();
    let r = MatchingEngine::new(MatchConfig::lenient()).match_persons(&p1, &p2);
    assert!(r.score > 0.75);
}
...
fn test_middle_name_lifts_score_when_given_names_differ() {
    ...
    let p2 = Person::builder()
        ...
        .family_name("Smith")
        .build();
    let without_middle = MatchingEngine::default_config().match_persons(
        &Person::builder()
            .given_name("Jon")
            .family_name("Smith")
            .build(),
    ...
    let with_middle = MatchingEngine::default_config().match_persons(&p1, &p2);
...
fn test_previous_addresses_does_not_lower_a_strong_current_match() {
    ...
    let p1 = Person::builder()
        ...
        .build();
    let p2_no_history = Person::builder()
        .given_name("X")
        .family_name("Y")
        .address(current.clone())
        .build();
    let p2_with_history = Person::builder()
        .given_name("X")
        .family_name("Y")
        .address(current)
    ...
    let without = engine
        .match_persons(&p1, &p2_no_history)
        .breakdown
        .address_score
        .expect("scored");
    let with = engine
        .match_persons(&p1, &p2_with_history)
        .breakdown
        .address_score
        .expect("scored");
    assert!(
...
fn test_strict_mode_accepts_deterministic_match() {
    ...
    let p = Person::builder()
        ...
        .gender(Gender::Male)
        .build();
    let r = MatchingEngine::new(MatchConfig::strict()).match_persons(&p, &p.clone());
    assert!(r.is_match);
...
fn test_strict_mode_rejects_nickname() {
    ...
    let p2 = Person::builder()
        ...
        .date_of_birth(dob(1978, 9, 25))
        .build();
    let r = MatchingEngine::new(MatchConfig::strict()).match_persons(&p1, &p2);
    assert!(!r.is_match);
}
...
fn test_lenient_mode_more_forgiving_than_strict() {
    ...
    let p2 = Person::builder()
        ...
        .date_of_birth(dob(1965, 7, 14))
        .build();
    let strict = MatchingEngine::new(MatchConfig::strict()).match_persons(&p1, &p2);
    let lenient = MatchingEngine::new(MatchConfig::lenient()).match_persons(&p1, &p2);
...
fn test_custom_weights_can_swing_outcome() {
    ...
    let cfg = MatchConfig {
        ...
        united_kingdom_national_health_service_number_weight: 0.0,
        fr_nir_weight: 0.0,
        es_tsi_weight: 0.0,
        ie_ihi_weight: 0.0,
    ...
    let custom = MatchingEngine::new(cfg).match_persons(&p1, &p2);
    let default = MatchingEngine::default_config().match_persons(&p1, &p2);
...
fn test_engine_is_symmetric() {
    ...
    let p2 = Person::builder()
        ...
        .build();
    let engine = MatchingEngine::default_config();
    let r1 = engine.match_persons(&p1, &p2);
    let r2 = engine.match_persons(&p2, &p1);
    assert!(
...
fn test_public_types_are_send_and_sync() {
    ...
    assert_send_sync::<person_matcher::MatchResult>();
    assert_send_sync::<person_matcher::MatchBreakdown>();
    assert_send_sync::<person_matcher::MatchingError>();
    assert_send_sync::<person_matcher::Confidence>();
    assert_send_sync::<person_matcher::NicknameTable>();
...
fn test_engine_usable_across_threads() {
    ...
    let handles: Vec<_> = (0..4)
        .map(|_| {
            ...
            let p = Arc::clone(&p);
            thread::spawn(move || engine.match_persons(&p, &p).score)
        })
        .collect();
...
fn test_score_is_in_unit_interval() {
    ...
    for a in &persons {
        for b in &persons {
            let s = engine.match_persons(a, b).score;
...
fn test_fr_nir_probabilistic_score_breakdown() {
    let same1 = Person::builder()
        ...
        .family_name("Dupont")
        .build();
    let same2 = Person::builder()
        .fr_nir(NIR_A_FR)
        .given_name("Jean")
        .family_name("Dupont")
        .build();
    let r = MatchingEngine::default_config().match_persons(&same1, &same2);
    assert_eq!(r.breakdown.fr_nir_score, Some(1.0));
    assert!(r.is_match);

    let diff = Person::builder()
        .fr_nir(NIR_B_FR)
        .given_name("Jean")
        .family_name("Dupont")
        .build();
    let r = MatchingEngine::default_config().match_persons(&same1, &diff);
...
fn test_ie_ihi_whitespace_layouts_match_deterministically() {
    ...
    let p2 = Person::builder().ie_ihi(IHI_A_IE).build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_se_personnummer_separator_variants_match_deterministically() {
    ...
    let p2 = Person::builder().se_personnummer(PNR_A_SE).build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_se_personnummer_ten_and_twelve_digit_forms_do_not_cross_match() {
    ...
    let p1 = Person::builder().se_personnummer("460324-3850").build();
    let p2 = Person::builder().se_personnummer("19460324-3850").build();
    let r = MatchingEngine::default_config().match_persons(&p1, &p2);
...
fn test_au_ihi_whitespace_variants_match_deterministically() {
    ...
    let p2 = Person::builder().au_ihi(IHI_A_AU).build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_au_ihi_does_not_cross_match_ie_ihi() {
    ...
    let p1 = Person::builder().au_ihi(IHI_A_AU).build();
    let p2 = Person::builder().ie_ihi("1234567").build();
    assert!(!MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_uk_chi_number_does_not_cross_match_united_kingdom_national_health_service_number() {
    ...
    // recorded as a CHI Number on one side and a United Kingdom National
...
fn test_uk_chi_number_does_not_cross_match_uk_hc_number() {
    ...
    // (united_kingdom_national_health_service_number, uk_hc_number, uk_chi_number) but none of them
    // ever cross-match: provenance lives on the field name.
    let p1 = Person::builder().uk_chi_number(CHI_A_UK).build();
    let p2 = Person::builder().uk_hc_number(CHI_A_UK).build();
    assert!(!MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_validate_accepts_any_single_national_identifier() {
    ...
    assert!(Person::builder().it_cf(CF_A_IT).build().validate().is_ok());
    assert!(
        Person::builder()
            .nl_bsn(BSN_A_NL)
            .build()
...
fn test_address_parsing_penalises_different_house_numbers() {
    ...
    let same = engine.match_persons(&make(a_match1), &make(a_match2));
    let diff = engine.match_persons(&make(a_diff1), &make(a_diff2));
    assert!(
...
fn test_nicknames_default_config_does_not_change_existing_scores() {
    ...
    let p2 = Person::builder()
        ...
        .date_of_birth(dob(1978, 9, 25))
        .build();
    let baseline = MatchingEngine::default_config().match_persons(&p1, &p2);
...
fn test_nicknames_english_table_lifts_given_name_to_at_least_0_9() {
    ...
    for (a, b) in [
        ...
        let r = MatchingEngine::new(cfg.clone()).match_persons(&p1, &p2);
        let s = r.breakdown.given_name_score.unwrap();
        assert!(
            s >= 0.9,
...
fn test_nicknames_boost_is_one_way_lift_never_drop() {
    ...
    let p = Person::builder()
        ...
        .family_name("Smith")
        .build();
    let r = MatchingEngine::new(cfg).match_persons(&p, &p.clone());
...
fn test_nicknames_unrelated_names_are_untouched() {
    ...
    let p1 = Person::builder()
        ...
        .family_name("S")
        .build();
    let p2 = Person::builder().given_name("Bob").family_name("S").build();
    let r = MatchingEngine::new(cfg).match_persons(&p1, &p2);
...
fn test_nicknames_promote_overall_match_from_under_to_over_threshold() {
    ...
    let with_nicknames = MatchingEngine::new(cfg).match_persons(&p1, &p2);
    assert!(
...
fn test_email_gmail_dot_folding_opt_in() {
    ...
    let folded = MatchingEngine::new(cfg).match_persons(&p1, &p2);
...
fn test_confidence_legacy_json_payload_without_field_defaults_to_low() {
    ...
    let legacy = r#"{
        ...
        "is_match": true,
        "breakdown": {
            "united_kingdom_national_health_service_number_score": null,
            "fr_nir_score": null,
            "es_tsi_score": null,
            "ie_ihi_score": null,
    ...
    let r: person_matcher::MatchResult = serde_json::from_str(legacy).unwrap();
    assert_eq!(r.confidence, Confidence::Low);
}
...
fn test_config_loaded_from_json_drives_engine_behaviour_unchanged() {
    ...
    let r1 = MatchingEngine::new(original).match_persons(&p1, &p2);
    let r2 = MatchingEngine::new(loaded).match_persons(&p1, &p2);
...
fn test_dob_transposition_lifts_overall_score_relative_to_zero_dob() {
    ...
    let p1 = Person::builder()
        ...
        .phone("07700 900111")
        .build();
    let p2_transposed = Person::builder()
        .given_name("Thomas")
        .family_name("Price")
        .date_of_birth(dob(1995, 10, 1))
        .gender(Gender::Male)
        .phone("07700 900111")
        .build();
    let p2_unrelated = Person::builder()
        .given_name("Thomas")
        .family_name("Price")
        .date_of_birth(dob(1980, 6, 30))
        .gender(Gender::Male)
        ...
        .build();
    let engine = MatchingEngine::default_config();
    let transposed = engine.match_persons(&p1, &p2_transposed);
    let unrelated = engine.match_persons(&p1, &p2_unrelated);
    assert!(
...
fn test_batch_match_one_to_many_filtered_for_is_match() {
    ...
    // Index 0 deterministically matches (same United Kingdom National Health
    // Service Number) and index 2 is a clone. Index 1 has a different United
    // Kingdom National Health Service Number and no other
...
fn test_batch_rank_one_to_many_with_empty_candidates() {
    let engine = MatchingEngine::default_config();
    let query = Person::builder().given_name("Solo").build();
...
fn test_batch_match_one_to_many_threadsafe_under_arc() {
    ...
    let candidates: Arc<Vec<Person>> = Arc::new(vec![
        query.as_ref().clone(),
        Person::builder()
            .given_name("Grace")
            .family_name("Hopper")
...
fn test_be_nn_deterministic_match() {
    ...
    let p2 = Person::builder().be_nn("80.01.01-001.07").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
    assert!(
        Person::builder()
            .be_nn("80010100107")
...
fn test_bg_egn_deterministic_match() {
    ...
    let p2 = Person::builder().bg_egn("8001010013").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
    let p3 = Person::builder()
        ...
        .family_name("B")
        .build();
    let p4 = Person::builder()
        .bg_egn("0123456789")
        .given_name("A")
        .family_name("B")
        .build();
    let r = MatchingEngine::default_config().match_persons(&p3, &p4);
...
fn test_cz_rc_deterministic_match() {
    ...
    let p2 = Person::builder().cz_rc("8001150014").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_dk_cpr_format_only_match() {
    ...
    let p2 = Person::builder().dk_cpr("150180-1234").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_ee_ik_deterministic_match() {
    ...
    let p2 = Person::builder().ee_ik("48001150011").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_es_dni_deterministic_match() {
    ...
    let p2 = Person::builder().es_dni("12345678-z").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_fi_hetu_deterministic_match() {
    ...
    let p2 = Person::builder().fi_hetu("150180-999B").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_hr_oib_deterministic_match() {
    ...
    let p2 = Person::builder().hr_oib("12345678903").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_is_kt_deterministic_match() {
    ...
    let p2 = Person::builder().is_kt("1501802529").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_lt_ak_deterministic_match() {
    ...
    let p2 = Person::builder().lt_ak("48001150011").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_lv_pk_deterministic_match() {
    ...
    let p2 = Person::builder().lv_pk("15018010007").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_mt_id_deterministic_match() {
    ...
    let p2 = Person::builder().mt_id("1234567m").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_no_fnr_deterministic_match() {
    ...
    let p2 = Person::builder().no_fnr("15018012399").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_pl_pesel_deterministic_match() {
    ...
    let p2 = Person::builder().pl_pesel("80011500014").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_ro_cnp_deterministic_match() {
    ...
    let p2 = Person::builder().ro_cnp("1800115400012").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_si_emso_deterministic_match() {
    ...
    let p2 = Person::builder().si_emso("1501980500015").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_sk_rc_deterministic_match() {
    ...
    let p2 = Person::builder().sk_rc("8051150019").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_uk_nino_deterministic_match() {
    ...
    let p2 = Person::builder().uk_nino("ab 12 34 56 a").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_gr_dss_deterministic_match() {
    ...
    let p2 = Person::builder().gr_dss("12-34-56 78 90").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
    assert!(
        Person::builder()
            .gr_dss("1234567890")
...
fn test_li_id_deterministic_match_with_8_digit_form() {
    ...
    let p2 = Person::builder().li_id("id 1234 5678").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_li_id_deterministic_match_with_9_digit_form() {
    ...
    let p1 = Person::builder().li_id("ID022143586").build();
    let p2 = Person::builder().li_id("ID022143586").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_nl_id_deterministic_match() {
    ...
    let p2 = Person::builder().nl_id("ab 12 34 567").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
    assert!(
        Person::builder()
            .nl_id("AB1234567")
...
fn test_nl_id_distinct_from_nl_bsn() {
    ...
    let p1 = Person::builder().nl_bsn("111222333").build();
    let p2 = Person::builder().nl_id("AB1234567").build();
    assert!(!MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_pl_nip_deterministic_match() {
    ...
    let p2 = Person::builder().pl_nip("123-456-78-02").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_pt_nif_deterministic_match() {
    ...
    let p2 = Person::builder().pt_nif("123 456 789").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_br_cpf_deterministic_match() {
    ...
    let p2 = Person::builder().br_cpf("123.456.789-09").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_br_cpf_rejects_mismatch() {
    let p1 = Person::builder().br_cpf("12345678909").build();
    let p2 = Person::builder().br_cpf("11144477735").build();
    assert!(!MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_cn_rrn_deterministic_match() {
    ...
    let p2 = Person::builder().cn_rrn("11010519491231002x").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_in_aadhaar_deterministic_match() {
    ...
    let p2 = Person::builder().in_aadhaar("2341 2341 2346").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_jp_my_number_deterministic_match() {
    ...
    let p2 = Person::builder().jp_my_number("1234-5678-9018").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_mx_curp_deterministic_match() {
    ...
    let p2 = Person::builder().mx_curp("hegg560427mvzrrl04").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_nz_nhi_deterministic_match() {
    ...
    let p2 = Person::builder().nz_nhi("zaa0083").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_za_id_deterministic_match() {
    ...
    let p2 = Person::builder().za_id("800101 5009 087").build();
    assert!(MatchingEngine::default_config().deterministic_match(&p1, &p2));
}
...
fn test_t17_1_scheme_local_isolation() {
    ...
    let cpf_record = Person::builder().br_cpf("12345678909").build();
    let ssn_record = Person::builder().us_ssn("123456789").build();
...
fn test_t17_1_schemes_each_validate_solo() {
    for builder in [
        Person::builder().br_cpf("12345678909"),
        Person::builder().cn_rrn("11010519491231002X"),
        Person::builder().in_aadhaar("234123412346"),
        Person::builder().jp_my_number("123456789018"),
        Person::builder().mx_curp("HEGG560427MVZRRL04"),
        Person::builder().nz_nhi("ZAA0083"),
        Person::builder().za_id("8001015009087"),
    ] {
        assert!(
...
fn test_t17_1_breakdown_carries_each_score() {
    let p1 = Person::builder()
        ...
        .build();
    let r = MatchingEngine::default_config().match_persons(&p1, &p1.clone());
...
fn test_t17_1_legacy_payload_deserialises_to_none() {
    ...
    let p: Person = serde_json::from_str(legacy).unwrap();
...
fn test_uk_chi_does_not_cross_match_uk_nino_or_united_kingdom_national_health_service_number_or_uk_hc()
 {
    ...
    assert!(!engine.deterministic_match(&uknhsn, &hc));
    // NINO has a different format entirely; cannot cross with the
    // 10-digit Mod-11 schemes.
    let nino = Person::builder().uk_nino("AB123456A").build();
...
// identifier as a US "AB123456". A person may hold passports from
...
fn test_passport_book_score_none_when_either_side_empty() {
    let p1 = Person::builder()
        ...
        .build();
    let p2 = Person::builder().given_name("A").family_name("B").build();
    let r = MatchingEngine::default_config().match_persons(&p1, &p2);
...
fn test_multiple_birth_disambiguates_identical_twins() {
    ...
    let twin2 = Person::builder()
        ...
        .multiple_birth(2)
        .build();
    let r = MatchingEngine::default_config().match_persons(&twin1, &twin2);
...
fn test_death_date_weight_contributes_to_composite_score() {
    ...
    // identifying overlap.
    let baseline_p1 = Person::builder()
        .given_name("Dim")
        .family_name("Match")
        .build();
    let baseline_p2 = Person::builder()
        .given_name("Dim")
        .family_name("Match")
        .build();
    let with_dod_p1 = Person::builder()
        .given_name("Dim")
        .family_name("Match")
        .death_date(dob(1990, 4, 4))
        .build();
    let with_dod_p2 = Person::builder()
        .given_name("Dim")
        .family_name("Match")
        .death_date(dob(1990, 4, 4))
        .build();
    let engine = MatchingEngine::default_config();
    let baseline = engine.match_persons(&baseline_p1, &baseline_p2);
    let with_dod = engine.match_persons(&with_dod_p1, &with_dod_p2);
    assert!(
