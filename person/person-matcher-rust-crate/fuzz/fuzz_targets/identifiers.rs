...
//! `match_persons` fuzz target and the crate's proptest suite), not a
//! property of an individual parser.
...
fuzz_target!(|data: &[u8]| {
    ...
    let _ = identifiers::parse_united_kingdom_national_health_service_number(s);
