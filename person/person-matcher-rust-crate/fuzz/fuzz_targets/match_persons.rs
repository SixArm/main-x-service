...
//!
//! Interprets the fuzz bytes as a JSON `[person_a, person_b]` tuple, then
//! runs `MatchingEngine::match_persons`. Pins the two load-bearing
...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok((a, b)) = serde_json::from_slice::<(Person, Person)>(data) else {
        return;
    };

    let engine = MatchingEngine::new(MatchConfig::default());
    // Run both orderings so either argument position is exercised.
    for result in [engine.match_persons(&a, &b), engine.match_persons(&b, &a)] {
        assert!(
