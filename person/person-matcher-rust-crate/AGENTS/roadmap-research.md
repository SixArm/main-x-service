# Roadmap Research — Closed Spike Outcomes

This file archives the long-form research-spike write-ups that were previously inline in `spec.md` §21.4. Each entry records the question, decision matrix, recommended action, and any concrete code that shipped. The corresponding tasks (`T-9`, `T-14`, `T-17`, `T-19`) are recorded in `AGENTS/delivered-tasks.md`.
...
**Question.** Which national personal identifier schemes should be added next, beyond the 35 schemes already shipped (UK United Kingdom National Health Service Number, FR NIR, ES TSI, IE IHI, UK NI H&C, US SSN, AU IHI, DE KVNR, IT CF, NL BSN, SE Personnummer, UK Scotland CHI, BE NN, BG EGN, CZ RČ, DK CPR, EE IK, ES DNI, FI HETU, HR OIB, IS KT, LT AK, LV PK, MT ID, NO FNR, PL PESEL, RO CNP, SI EMŠO, SK RČ, UK NINO, GR DSS, LI ID, NL ID, PL NIP, PT NIF)?
...
**This recommendation closes T-17.** The implementation of the 7-batch shipped as T-17.1 (see `AGENTS/delivered-tasks.md`).
...
**This recommendation closes T-9.** The follow-up implementation task is filed separately as **T-9.1** under `spec.md` §23.2.
...
| Depend on `phonenumber` crate (Rust port of Google's libphonenumber) | Universal + mobile/landline distinction + region inference | Adds ~10 transitive deps, ~MB binary, measurable compile-time cost | High (track upstream metadata releases) | Same as the 250-territory table for canonicalisation; the mobile/landline distinction doesn't help matching (see below) | **Declined.** Trade-off doesn't fit a minimal-API, data-safety-deterministic, `< 50 µs` library. |
...
- Adds network IO and PII egress (commercial APIs), violating the data-safety axiom; or
