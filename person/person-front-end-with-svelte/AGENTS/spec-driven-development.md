...
1. **Service spec** —
   [`../../person-service-rust-crate/spec.md`](../../person-service-rust-crate/spec/index.md)
   — describes the API contract. If `Person` loses a field server-side,
...
- **Changing the wire-format type to "fix" a server bug.** If
  `Person` doesn't match `person-service`'s `Person`, the fix is in
