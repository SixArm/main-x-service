...
Verified live: 45 theme options with the UK/US group last, 7 size
options, and selecting "Largest" actually scales the root font size.
...
### Fixed — `/verify` crashed with a raw 500 when the authentication service was unreachable (T-32)
...
pass with it restored. See spec §13 T-32.
...
A toggle button on `/persons/[id]` re-fetches through the existing
`PersonRepository.masked(id)` (`GET /api/persons/{id}/masked`) instead
