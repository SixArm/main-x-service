...
| FR-23 | `/expiry` MUST render every fetched person's identity documents that carry an `expiry_date` as an all-day event in the SVAR Calendar (month view, read-only), and selecting an event MUST navigate to `/persons/{id}`. Each all-day event's `end` MUST be strictly after its `start` (one calendar day past the expiry date) — `@svar-ui/calendar-store` silently drops any all-day event where this does not hold, so an equal `start`/`end` renders nothing. |
