        ...
        <ul class="results">
            {#each results as r}
                <li class="result">
                    <header>
                        <strong>
                            {r.person.name.given.join(" ")}
                            {r.person.name.family}
                        </strong>
                    ...
                    <div class="meta small muted">
                        {#if r.person.birth_date}{t("matchResults.dob")}
                            {r.person.birth_date}{/if}
                        {#if r.person.gender}
                            · {r.person.gender}{/if}
                        {#if r.person.id}
                            · <a href={`/persons/${r.person.id}`}
                                >{r.person.id.slice(0, 8)}…</a
                            >{/if}
                    </div>
                    {#if r.breakdown}
                        <details>
