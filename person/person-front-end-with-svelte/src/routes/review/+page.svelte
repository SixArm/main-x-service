<!--
  ...
  three ways over one selection:
    ...
    3. An inline comparison panel below both, fetching each side of the
       pair with its own `GET /api/persons/{id}` (there is no combined
  ...
  merge. The panel therefore deep-links to `/persons/merge` with both ids
...
<script lang="ts">
    ...
    let pair = $state<{ a: Person | null; b: Person | null } | null>(null);
    ...
    async function loadPair(item: ReviewQueueItem) {
        ...
        const [a, b] = await Promise.allSettled([
            repo.get(item.person_id_a),
            repo.get(item.person_id_b),
        ]);
    ...
    function pairLabel(item: ReviewQueueItem): string {
        return `${shortId(item.person_id_a)} ↔ ${shortId(item.person_id_b)}`;
    }

    /** Full name in reading order; `null` when the record is absent. */
    function fullName(person: Person | null): string | null {
    ...
    function addressLine(person: Person | null): string | null {
    ...
    /** First contact point as `channel: value`; `null` when there is none. */
    function contactLine(person: Person | null): string | null {
        const contact = person?.telecom?.[0];
        return contact ? `${contact.system}: ${contact.value}` : null;
    }
    ...
    const compareRows = $derived(
        pair
            ? [
                  ...
                  {
                      labelKey: "review.component.birthDate" as StringKey,
                      a: pair.a?.birth_date ?? null,
                      b: pair.b?.birth_date ?? null,
                  },
                  {
                      labelKey: "review.component.gender" as StringKey,
                      a: pair.a?.gender ?? null,
                      b: pair.b?.gender ?? null,
                  },
                  {
                  ...
                  {
                      labelKey: "review.field.contact" as StringKey,
                      a: contactLine(pair.a),
                      b: contactLine(pair.b),
                  },
              ]
            : [],
    );
    ...
    >
        ...
        {#if item.status === "confirmed"}
            ...
            <div class="row">
                <a
                    class="button"
                    href={mergeHref(item.person_id_a, item.person_id_b)}
                >
                    {t("review.merge.keepA")}
                </a>
                <a
                    class="button"
                    href={mergeHref(item.person_id_b, item.person_id_a)}
                >
