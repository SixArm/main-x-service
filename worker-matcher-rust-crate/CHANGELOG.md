...
- New `benches/match_pair.rs` exercising the hot paths a downstream MPI integrator will care about: single-pair `match_workers` (identical / fuzzy / unrelated), `deterministic_match`, `rank_one_to_many` (`n ∈ {10, 100, 1000}` with criterion throughput reporting), and a config-variant sweep (default vs strict vs nickname-table-loaded).
...
- Blood type is **not** part of `deterministic_match` (too weak as a positive signal alone) and **not** an identifying field for `Worker::validate`. The disagreement signal surfaces in the breakdown so downstream consumers can flag clinically suspicious mismatches even when the overall score remains high.
