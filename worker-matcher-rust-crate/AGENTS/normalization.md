...
## Detailed normalization specifications (extracted from spec.md §14)

The verbatim spec §14 algorithm definitions are archived here so the spec itself can hold only a summary. Update both surfaces in lockstep.
...
`local_id` is **not** scored. Different organisations may issue colliding values (a worker's MRN at site A and another worker's MRN at site B can be byte-equal), so positional matching would produce false positives. This resolves §22 OQ-2's second half.
