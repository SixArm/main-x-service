...
//! names, edge-case dates, sparse / dense `Worker` records, and so on.
//!
...
/// A bounded date so we never trip `NaiveDate::from_ymd_opt` on
/// implausible years.
...
/// A `Worker` carrying enough data to make `validate()` pass and the
...
proptest! {
    ...
    #[test]
    fn score_is_bounded_unit_interval(p1 in worker_strategy(), p2 in worker_strategy()) {
    ...
    /// Self-match MUST produce `is_match == true` for any `Worker` that
    ...
    #[test]
    fn self_match_is_true(p in worker_strategy()) {
        prop_assume!(p.validate().is_ok());
        let r = MatchingEngine::default_config().match_workers(&p, &p);
    ...
    #[test]
    fn self_match_confidence_is_high(p in worker_strategy()) {
        prop_assume!(p.validate().is_ok());
        let r = MatchingEngine::default_config().match_workers(&p, &p);
        prop_assert_eq!(r.confidence, Confidence::High);
    }

    /// `match_workers` MUST be symmetric: swapping the two workers
    ...
    #[test]
    fn matching_is_symmetric(p1 in worker_strategy(), p2 in worker_strategy()) {
        let engine = MatchingEngine::default_config();
        let forward = engine.match_workers(&p1, &p2);
        let reverse = engine.match_workers(&p2, &p1);
        prop_assert!(
    ...
    #[test]
    fn deterministic_match_is_symmetric(p1 in worker_strategy(), p2 in worker_strategy()) {
        let engine = MatchingEngine::default_config();
        prop_assert_eq!(
    ...
    /// `Worker` MUST survive a JSON round-trip. Legacy consumers stash
    /// records in JSON columns; field renames or new required fields
    /// would silently corrupt their archives.
    #[test]
    fn worker_round_trips_through_json(p in worker_strategy()) {
    ...
    fn dob_subscore_is_symmetric(
        ...
        let engine = MatchingEngine::default_config();
        let p1 = Worker::builder().given_name("X").date_of_birth(d1).build();
        let p2 = Worker::builder().given_name("X").date_of_birth(d2).build();
        let forward = engine.match_workers(&p1, &p2).breakdown.date_of_birth_score;
        let reverse = engine.match_workers(&p2, &p1).breakdown.date_of_birth_score;
