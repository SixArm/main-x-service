...
- The service's [`spec.md`](../person-service-rust-crate/spec.md) and [`AGENTS/`](../person-service-rust-crate/AGENTS/) describe the API contract. If a field disappears from `Person` in the service, fix `src/lib/api/types.ts` here — do not let the front-end drift.
