...
A comprehensive Rust library for matching person records in healthcare information exchanges.
...
Note that `local_id` is **not** scored: different organisations may issue colliding values (different persons' MRNs from hospitals A and B can be byte-equal), so positional matching would produce false positives.
