...
The `person-matcher` crate provides a reusable, transparent, auditable Rust library to determine whether two person demographic records refer to the same person. It targets healthcare information exchange (HIE) scenarios where demographic data and national-style identifiers from disparate source systems must be reconciled into a single best-guess decision.
...
- Identifier-to-identifier translation across schemes (e.g. resolving a UK NHS Number to a French NIR for the same person — that requires a registry which this library deliberately does not consult).
...
- Pairwise matching of two `Person` records.
...
- Persistent master person indices.
...
- Cross-scheme identity resolution (matching a French NIR against a UK NHS Number for the same person); only same-scheme identifiers are ever compared.
...
| Term | Definition |
|---|---|
| **HIE** | Health Information Exchange — system that shares person data across organisations. |
...
| **España TSI** | *Tarjeta Sanitaria Individual* — Spain's national healthcare identifier, with regionally-varying formats. The national-level form is the *Código de Identificación Personal del Sistema Nacional de Salud* (CIP-SNS). |
...
| **SE Personnummer** | Sweden personal identity number — `YYMMDD-NNNC` (10-digit) or `YYYYMMDD-NNNC` (12-digit), with a Luhn check digit. |
...
| **Jaro-Winkler** | String similarity favouring common prefixes; well-suited to person names. |
...
1. Grannis SJ, et al. *Person matcher within a Health Information Exchange.* AMIA Annu Symp Proc, 2014. (PMC4696093)
...
### 6.1 Person Model
- **FR-1** The library MUST expose a `Person` struct holding the fields listed in §8.1, including thirty-five national-identifier fields plus `passport_books: Vec<PassportBook>`: `uk_nhs_number`, `fr_nir`, `es_tsi`, `ie_ihi`, `uk_hc_number`, `us_ssn`, `au_ihi`, `de_kvnr`, `it_cf`, `nl_bsn`, `se_personnummer`, `uk_chi_number`, `be_nn`, `bg_egn`, `cz_rc`, `dk_cpr`, `ee_ik`, `es_dni`, `fi_hetu`, `hr_oib`, `is_kt`, `lt_ak`, `lv_pk`, `mt_id`, `no_fnr`, `pl_pesel`, `ro_cnp`, `si_emso`, `sk_rc`, `uk_nino`, `gr_dss`, `li_id`, `nl_id`, `pl_nip`, `pt_nif`.
- **FR-2** `Person` MUST be constructible via a fluent `PersonBuilder` with one setter per national identifier.
- **FR-3** `Person` MUST be cloneable, comparable for equality, debuggable, and serde-serializable.
- **FR-4** `Person::validate()` MUST require at least one of: a name (`given_name` or `family_name`), any of the thirty national identifiers, or a non-empty `passport_books` list. Otherwise it MUST return `MatchingError::MissingField`.

### 6.2 Matching Engine
- **FR-5** The library MUST expose a `MatchingEngine` configured by a `MatchConfig`.
- **FR-6** `MatchingEngine::match_persons(&p1, &p2)` MUST return a `MatchResult { score, is_match, confidence, breakdown }`.
...
- **FR-51** `Person` MUST carry a `passport_books: Vec<PassportBook>` field. `MatchBreakdown::passport_book_score` MUST be `Some(1.0)` when at least one `(country, number)` pair is shared across the two persons' books, `Some(0.0)` when both sides carry at least one book but no pair is shared, and `None` when either side has no books. `Person::validate()` MUST accept a person whose only identifying data is a non-empty `passport_books` list.
- **FR-52** `deterministic_match` MUST return `true` when the two persons share at least one `(country, number)` passport-book pair after canonicalisation. Cross-country values with the same `number` MUST NEVER cross-match; provenance lives on `PassportBook::country`, not on the field name.
- **FR-53** `Person` and `Address` MUST carry `#[non_exhaustive]`. External consumers MUST construct them via the builder (`Person::builder()`) or the constructor plus fluent setters (`Address::new().with_postcode(...)`) — direct struct-literal construction is reserved for the defining crate. This formalises the long-standing expectation that field additions are non-breaking.
...
2. **Multi-country.** A single person may hold passports from several countries simultaneously (dual / multiple citizenship). Each passport is recorded as a separate `PassportBook` entry.
3. **Time-varying.** When a passport is renewed, the new book has a different number. Records carry both current and historical book numbers as separate `PassportBook` entries; the matcher treats any shared `(country, number)` pair as evidence the records refer to the same person, regardless of issue date.

The `PassportBook` type (§8.6) and the `passport_books: Vec<PassportBook>` field on `Person` (§8.1) capture this model. The matcher's deterministic and probabilistic paths consume the books as a set of `(country, number)` keys; date metadata is carried for audit but is not used in matching (FR-50 / FR-51 / FR-52).
...
- **FR-28** UK Northern Ireland H&C Numbers MUST be parsed via `identifiers::parse_uk_hc_number`. The algorithm is identical to UK NHS Number parsing because H&C and NHS share the 10-digit Modulus-11 scheme, but the two parsers are intentionally exposed as distinct functions tied to distinct `Person` fields.
...
- **FR-43** Sweden *Personnummer* values MUST be parsed via `identifiers::parse_se_personnummer`. The parser MUST keep only ASCII digits, accept exactly 10 or 12 digits, validate the Luhn check digit computed over the 10-digit (year-truncated) form, and preserve the input length in the canonical output (10-digit and 12-digit forms therefore do NOT cross-match).
...
- **FR-64** Latvia *Personas kods* MUST be parsed via `identifiers::parse_lv_pk`. 11 digits; weights `[1,6,3,7,9,10,5,8,4,2]`; `check = ((1101 − Σ) mod 11) mod 10`.
...
- **FR-36** `MatchingEngine::match_persons` MUST score email equality using the canonical form. `MatchBreakdown::email_score` MUST be `Some(1.0)` for equal canonical forms, `Some(0.0)` for unequal forms when both parse, and `None` when either input is absent or fails to parse. `local_id` MUST NOT be scored: different organisations may issue colliding values, so a positional match would produce false positives.
...
- **FR-22** When `use_phonetic_matching` is `true` and both persons have given AND family names, the engine MUST compute a phonetic name score using Soundex.
...
| `se_personnummer` | `Option<String>` | At least one identifying field | Sweden *Personnummer*. Raw string; parsed at match time via `identifiers::parse_se_personnummer`. |
...
The struct is `Debug + Clone + PartialEq + Eq + Serialize + Deserialize` and is re-exported from the crate root as `person_matcher::PassportBook`.
...
National-identifier fields (one per scheme): `uk_nhs_number_score`, `fr_nir_score`, `es_tsi_score`, `ie_ihi_score`, `uk_hc_number_score`, `us_ssn_score`, `au_ihi_score`, `de_kvnr_score`, `it_cf_score`, `nl_bsn_score`, `se_personnummer_score`, `uk_chi_number_score`, `passport_book_score`.
...
├── models.rs       Person, PersonBuilder, Address, Gender.
...
│                   parse_nl_bsn, parse_se_personnummer, parse_uk_chi_number.
...
| `parse_se_personnummer` | Sweden | *Personnummer* | 10 or 12 ASCII digits; Luhn check over the 10-digit form. |
...
- `match_persons(&p1, &p2) -> MatchResult` — see §12.3.
- `deterministic_match(&p1, &p2) -> bool` — see §12.1.
- `match_one_to_many(&query, &[Person]) -> Vec<MatchResult>` — score a single query against many candidates; parallel to input slice; see §12.6.
- `rank_one_to_many(&query, &[Person]) -> Vec<(usize, MatchResult)>` — same as `match_one_to_many` but sorted by descending score with stable index tiebreak.
...
```rust
pub mod identifiers;  // 35 personal-identifier parsers + 9 passport-format
...
`MatchingEngine::score_death_place` consumes `Person::death_place: Option<Address>`. The algorithm is identical to the place-of-birth sub-score (§12.4a) — both go through the shared `score_named_place(&Address, &Address) -> Option<f64>` free helper. The same city/country blend (`0.7 × city + 0.3 × country`, single-signal fallbacks) applies. The sub-score is bounded `[0.0, 1.0]`, returns `None` when neither side has comparable sub-fields, and is independent from the `birth_place` and `address` sub-scores (FR-84).
...
`MatchingEngine::match_one_to_many(query, candidates)` iterates `candidates` and produces one `MatchResult` per candidate via the same `match_persons` pipeline (§12.3). The output `Vec<MatchResult>` is parallel to the input slice; index `i` in the output corresponds to index `i` in `candidates`. Empty candidates yield an empty `Vec`.
...
| `se_personnummer_weight` | 0.30 | 0.30 | 0.30 |
...
National-identifier weights all default to `0.30` and are renormalised against the participating fields, so a person with only one identifier scheme on both sides gets the same effective weighting from that scheme as the pre-multinational behaviour gave to NHS Number alone.
...
`local_id` is **not** scored. Different organisations may issue colliding values (a person's MRN at hospital A and another person's MRN at hospital B can be byte-equal), so positional matching would produce false positives. This resolves §22 OQ-2's second half.
...
| Performance | `< 50 µs` per `match_persons` on a 2024-era Mac. | `benches/match_pair.rs` (criterion, §23 T-5); single-pair fuzzy match measures ~4 µs. |
...
22.5. Sweden *Personnummer*: 10-digit and 12-digit textual layouts canonicalise distinctly; both validate the Luhn check over the 10-digit form; 10-digit form accepts `-` or `+` separators.
...
27. Completely different persons → low score, no match.
28. `Person::validate` rejects empty persons.
29. `Person::validate` accepts a person carrying only a single national identifier (any scheme).
30. `Person` and `MatchResult` round-trip via JSON, including all six national-identifier fields.
...
- `score ∈ [0.0, 1.0]` for arbitrary `Person` pairs (the unit-interval invariant downstream services rely on).
- `match_persons(p, p).is_match == true` for any `p` passing `validate()`.
- Self-match always lands in `Confidence::High`.
- `match_persons` is symmetric in its arguments (score, `is_match`, and `confidence` are all argument-order-independent).
...
- **No global state**: no thread-locals, no `static mut`, no lazy_statics carrying person data.
...
- Locale-aware phonetic encoder (e.g. Double Metaphone, NYSIIS, or a custom encoder) as a replacement for or augmentation of American Soundex. **Spike outcome (T-9, §21.4):** keep Soundex as default; expose a `MatchConfig::phonetic_encoder` opt-in enum (`Soundex` / `DoubleMetaphone` / `DaitchMokotoff`); defer the default-switch decision until an empirical multinational person corpus is available. Implementation tracked as T-9.1 in §23.2.
...
- ~~Optional integration with an external postal-address reference for address standardisation, behind a feature flag.~~ — **Declined per §21.4 (T-14)**: the person-matcher crate stays IO-free and dependency-light; consumers SHOULD standardise addresses in the ingest pipeline before scoring.
...
| Use a generic "national-id" parser dependency | Universal | Heavy dep | High — vendor data quality varies | **Declined** by the same reasoning as T-14 / T-19: the person-matcher crate is the wrong layer for a kitchen-sink dataset. |
...
parse_za_id("9001015009087") -> Option<String>
    ...
    4. (Optional secondary signal exposed to consumers, NOT used by person-matcher: chars 6..10 < 5000 → female, ≥ 5000 → male; char 10 = citizenship.)
    5. Return Some("9001015009087").
```

**Implementation note.** Per the project's per-scheme pattern (T-16 / T-23 / T-27 / T-28 / T-30): each scheme requires a parser + `Person` field + `PersonBuilder` setter + `MatchConfig::<cc>_<scheme>_weight` (default `0.30`) + `MatchBreakdown::<cc>_<scheme>_score` with `#[serde(default)]` + `MatchingEngine::calculate_breakdown` / `calculate_weighted_score` / `deterministic_match` wiring + `Person::validate` extension + per-scheme unit + integration tests. The 3 external `MatchConfig` literal sites must be updated. Demo in `examples/basic_usage.rs`.
...
**Sample size and corpus.** This spike does **not** include an empirical evaluation: a representative multinational person corpus with ground-truth (name, alt-spelling, is-same-person) triples is not in the repo and would need to be sourced under appropriate clinical-data governance before any default-encoder switch could be justified. The recommendation below makes the corpus requirement explicit and proposes the methodology a future empirical evaluation should use.
...
| **Locale-specific** (Kölner Phonetik for DE, Soundex-FR for FR, …) | Best per-locale recall | Each locale is its own ruleset; matrix grows with person population diversity; no single Rust crate covers them all | Various ad-hoc | Skip — fragments the API and the maintenance burden grows linearly with person base. |
...
2. **Add a `MatchConfig::phonetic_encoder: PhoneticEncoder` enum** with at least three variants:
   ...
   - `DaitchMokotoff` — opt-in via `rphonetic`; documented for Slavic / Ashkenazi person populations
...
| Option | Coverage | Build / runtime cost | Maintenance | Recall lift for person matcher | Verdict |
|---|---|---|---|---|---|
| Stay with 26 countries | Major person-mobility partners | Already shipped | Low | Baseline | Insufficient — leaves 13 of the 35 identifier-scheme jurisdictions on the legacy `normalize_phone` fallback. |
...
| Expand to all ~250 ITU-T E.164 territories | Universal | ~700 LOC of static rows + per-region NSN bounds research | Medium (some countries split / merge; mobile prefixes drift) | Marginal — most person bases concentrate in ≤ 40 jurisdictions | **Declined.** Returns are sub-linear vs the maintenance burden, and the consumer can already use the legacy fallback for the long tail. |
...
4. The same "standardise upstream" advice given for T-14 applies cleanly: consumers with truly global person bases can run `phonenumber` (or libphonenumber) in their ingest pipeline and write the canonical `+CC…` form into `Person::phone`, after which the matcher reads a number that already canonicalises identically on both sides.
...
- Tail-of-the-world country coverage (the remaining ~210 ITU-T territories): can be added incrementally as person-population demand surfaces. The table format is now extensible (`Option<&str>` trunk handles every documented trunk convention).

**This recommendation closes T-19.**

#### T-14 — External postal-address standardisation: **declined**.

**Question.** Should `person-matcher` add (behind a feature flag) an integration with an external postal-address reference for address standardisation?
...
**Consumers that need higher address-matching recall SHOULD standardise upstream**: run libpostal / a commercial API in the ingest pipeline, write the normalised values into the `Address` fields, and let `person-matcher` score the standardised forms. `Address` is a plain struct that the consumer fills however they like — the crate is already well-positioned for this pattern.
...
- **OQ-1 — Resolved (0.3.0).** `middle_name` participates in the given-name component score with weight `0.05` (blended as `0.95 × given + 0.05 × middle`) when both persons have a middle name (FR-49, §12.2). See task T-25.
...
- **OQ-3 — Resolved (0.3.0).** `Person` and `Address` carry `#[non_exhaustive]` (FR-53; task T-8).
...
| Cross-scheme identifier confusion (e.g. recording an H&C Number in `uk_nhs_number` because both are 10 digits) | Medium | High | Distinct `Person` fields per scheme; deterministic matching strictly scheme-local; spec §12.1 and FR-13 forbid cross-scheme equality. Consumer applications must record provenance at ingest. |
...
- [x] Multinational national-identifier support (T-16): UK NHS Number, France NIR, España TSI, Éire IHI, UK NI H&C Number. Country-prefixed naming (`<cc>_<scheme>`) applied across `Person` fields, `PersonBuilder` setters, `MatchConfig` weights, and `MatchBreakdown` scores. `identifiers` module exposes one parser per scheme.
...
- `Person`/`Address` are `#[non_exhaustive]` (T-8).
...
1. Grannis SJ, Overhage JM, Hui S, McDonald CJ. *Person matcher within a Health Information Exchange.* AMIA Annu Symp Proc, 2014. Local copy: `help/person-matcher-within-a-health-information-exchange.pdf`.
2. Reisman M. *Patient Identification Techniques — Approaches, Implications, and Findings.* NCVHS, 2020. Local copy: `help/healthit-person-matcher-aggregation-and-linking-2019-08-16.pdf`.
