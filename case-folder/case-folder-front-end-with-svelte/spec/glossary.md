...
| LocaleSelect | `lily-design-system-svelte-locale-select` — sets `<html lang>`/`<html dir>` from a radiogroup, persists to `localStorage`. |
| ThemeSelect  | `lily-design-system-svelte-theme-select` — swaps `<link>` href + `<html data-theme>` to load one of the shared Lily themes from `static/assets/themes/` (~41-theme catalogue). |
