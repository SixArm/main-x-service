...
const CONFIG: KeyConfig = KeyConfig::new(env!("CARGO_PKG_NAME"), "LINK_GRAPH");

/// Environment variable holding the active MAC **root** key, hex-encoded.
pub const KEY_ENV: &str = "LINK_GRAPH_INTEGRITY_MAC_KEY";
...
pub const KEY_FILE_ENV: &str = "LINK_GRAPH_INTEGRITY_MAC_KEY_FILE";

/// Environment variable naming the active key, for the stored prefix.
pub const KEY_ID_ENV: &str = "LINK_GRAPH_INTEGRITY_MAC_KEY_ID";

/// Environment variable holding retired keys still needed for
/// verification, as `id:hex` pairs separated by commas.
pub const RETIRED_KEYS_ENV: &str = "LINK_GRAPH_INTEGRITY_MAC_KEYS_RETIRED";
...
pub enum Domain {
    /// Per-row audit integrity.
    ///
    /// Named `audit-row` rather than `audit-chain`: this service MACs
    /// each audit row individually and has no chain linking them, so
    /// borrowing the chain's domain label would imply a control that is
    /// not here.
    ///
    /// The only domain this service has. Its `edges` table is a derived
    /// read-model rebuilt from the entity event streams, so a MAC there
    /// would attest to a projection rather than to a source of truth —
    /// the authoritative edges live in each originating service's
    /// `entity_links`, and that is where they are worth protecting.
    AuditRow,
}
...
impl Domain {
    ...
    /// than on first use.
    const ALL: [&'static str; 1] = ["audit-row"];
}
...
mod tests {
    ...
    fn every_domain_has_a_distinct_label() {
        let labels: Vec<&str> = [Domain::AuditRow].iter().map(|d| d.as_str()).collect();
    ...
    ///
    /// One domain today; written as a loop over the full list so adding a
    /// second cannot quietly skip it.
    #[test]
    fn all_lists_every_domain() {
        let every = [Domain::AuditRow];
        for domain in every {
            assert!(
        ...
        assert_eq!(Domain::ALL.len(), every.len());
    }
}
