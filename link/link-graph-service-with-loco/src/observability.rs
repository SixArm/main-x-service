...
//! OTLP export** (spec §13 T-22, repo `tasks.md` AU-3).
//!
//! This is the family's first working OTLP pipeline. Three older crates
//! (person, worker, event) carry an `src/observability/` module that builds
//! an `OTel` `Resource` and then installs a plain JSON `tracing` subscriber
//! with the exporter and the bridge layer commented out behind
//! `// TODO: Initialize OTLP exporter`; every other service, including this
//! one until now, had nothing at all. So there was no pattern to copy — the
//! shape below is new work, written against
...
//! | `OTLP_SERVICE_NAME` | `link-graph-service` | `service.name` resource attribute |
...
pub const DEFAULT_SERVICE_NAME: &str = "link-graph-service";
...
/// swallowed.
///
/// Found the hard way, by booting the service with no collector and watching
/// **nothing** appear in the log: loco's default filter is a fixed module
/// whitelist, and `opentelemetry*` is not on it, so every failed export was
/// invisible. A silent exporter is worse than no exporter — it looks like it
/// is working.
///
...
fn http_metrics() -> &'static HttpMetrics {
    ...
    METRICS.get_or_init(|| {
        let meter = global::meter("link-graph-service");
        HttpMetrics {
            duration: meter
...
/// embed `EntityRef`s (`/api/neighbors/case:{uuid}`), so a per-path label
/// would be unbounded cardinality — and, worse, would put governed
/// `case ↔ person` identifiers into the metrics stream, which has none of
/// the read-path authorization the graph handlers apply.
pub async fn trace_mw(req: Request, next: Next) -> Response {
    ...
    // paths embed `EntityRef`s, including governed `case:{uuid}` ones.
...
mod tests {
    ...
    fn exporter_diagnostics_are_added_to_a_default_filter() {
        ...
        let base = || tracing_subscriber::EnvFilter::new("link_graph_service=info");
    ...
    fn an_operator_supplied_filter_is_left_alone() {
        ...
        assert_eq!(untouched.to_string(), "link_graph_service=info");
    }
    ...
    fn explicit_service_name_and_endpoint_are_honoured() {
        let config = TelemetryConfig::resolve(
            Some("link-graph-canary"),
            Some("http://collector.internal:4317"),
        );
        assert_eq!(config.service_name, "link-graph-canary");
        assert_eq!(
    ...
    async fn building_an_exporter_does_not_require_a_live_collector() {
        let config = TelemetryConfig {
            service_name: "link-graph-service".to_string(),
    ...
    fn resource_carries_service_name_and_version() {
        ...
        let name = resource
            ...
            .expect("service.name");
        assert_eq!(name.as_str(), "link-graph-service");
