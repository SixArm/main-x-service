...
| **`IdentityProbe`** | The lean, cross-type identity projection both a `Person` and a `Worker` record map to for the LNK-4 suggestion comparator (`name`, `birth_date`, `gender`, coded `identifiers`); carries no conversion to/from `person_matcher::Person` or `worker_matcher::Worker` (§5.5, the partition rule). |
| **Candidate blocking** | Bounding the LNK-4 suggestion job's person×worker comparisons to same-block pairs (a shared coded identifier, else `Soundex(family)` + birth-year) so cost stays `O(n + m + Σ\|block\|²)` rather than `O(n·m)` (§6.8, OQ-9(d)). |
| **Suggestion job** | The periodic aggregator-hosted job (`src/suggest/job.rs`, LNK-4) that fetches person + worker records, blocks + scores candidate pairs, and `POST`s survivors to person's write API as `matcher_suggested` `same_identity` edges. Never writes to this service's own storage beyond its own `suggestion_runs` audit row (§5.5, §6.8, §10.7). |
| **`suggestion_runs`** | This service's durable, one-row-per-completed-pass audit table for the suggestion job (counts fetched/candidates/posted/dropped/failed, plus the caps in force) — distinct from `audit_log` (§12.5), which records governed-edge *access*, not job runs (§10.7). |
