//! The **mounted** middleware, end to end (spec §13 T-22).
//!
//! `tests/otlp_export.rs` proves the export pipeline; this proves the thing
//! the service actually runs. It layers
//! [`observability::trace_mw`](link_graph_service::observability::trace_mw)
//! — the exact layer `app.rs::after_routes` installs — onto a real Axum
//! router, serves it over a real socket, makes a real HTTP request, and then
//! asserts that
//!
...
//!    so the header genuinely points at the trace, rather than merely being
...
use link_graph_service::observability::{TelemetryConfig, build_tracer_provider, trace_mw};
...
async fn a_served_request_exports_a_span_and_returns_its_traceparent() {
    ...
    let config = TelemetryConfig {
        service_name: "link-graph-service-mw-test".to_string(),
    ...
    tracing_subscriber::registry()
        .with(tracing_opentelemetry::layer().with_tracer(provider.tracer("link-graph-service")))
        .with(tracing_subscriber::EnvFilter::new(
            "link_graph_service=trace,otlp_middleware=trace",
        ))
        .init();

    let router = Router::new()
        .route("/api/health/freshness", get(|| async { "ok" }))
    ...
    let response = reqwest::Client::new()
        .get(format!("http://{addr}/api/health/freshness"))
        .send()
        .await
