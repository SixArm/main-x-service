...
//! over OTLP/gRPC (spec §13 T-22, repo `tasks.md` AU-3).
//!
//! This is the test the task exists for. Nothing in the Main X Index family
//! had ever exported a span — person, worker and event carry an
//! `src/observability/` module whose exporter is commented out — so a test
//! asserting only "it compiles and does not panic" would prove exactly what
//! those stubs already prove. Instead each case stands up a **real OTLP/gRPC
//! collector in-process** ([`otlp_collector`]), points the exporter at it,
//! and asserts on the decoded protobuf that arrives over the socket.
//!
//! No external collector, nothing `#[ignore]`d, no database: these run in a
//! normal `cargo test`.
...
use link_graph_service::observability::{
    TelemetryConfig, build_meter_provider, build_tracer_provider,
};
...
async fn a_tracing_span_reaches_a_real_otlp_collector() {
    ...
    let config = TelemetryConfig {
        service_name: "link-graph-service-otlp-test".to_string(),
    ...
    {
        let layer =
            tracing_opentelemetry::layer().with_tracer(provider.tracer("link-graph-service"));
        let subscriber = tracing_subscriber::registry().with(layer);
        tracing::subscriber::with_default(subscriber, || {
            let span = tracing::info_span!("neighbors.read", edge.kind = "same_identity");
            let _entered = span.enter();
            tracing::info!("serving a neighbors read");
        });
    }
    ...
    assert!(
        service_names.contains(&"link-graph-service-otlp-test".to_string()),
    ...
    let span = spans
        .iter()
        .find(|span| span.name == "neighbors.read")
        .unwrap_or_else(|| {
            panic!(
                "exported spans did not include `neighbors.read`: {:?}",
                spans.iter().map(|span| &span.name).collect::<Vec<_>>()
            )
        });
    assert!(
        span.attributes.iter().any(|kv| kv.key == "edge.kind"),
...
async fn a_metric_reaches_a_real_otlp_collector() {
    ...
    let meter = provider.meter("link-graph-service");
    let histogram = meter
