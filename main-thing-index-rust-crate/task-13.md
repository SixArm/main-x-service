...
```bash
DATABASE_URL=postgresql://test_user:test_password@localhost:5432/main_thing_index_test
...
```bash
kubectl rollout undo deployment/main_thing_index-server
```
...
```yaml
kubectl set image deployment/main_thing_index-server mpi-server=${{ env.REGISTRY }}/${{ env.IMAGE_NAME }}:${{ github.ref_name }}
```
...
# Pull image
docker pull ghcr.io/username/main-thing-index-rust-crate:latest
```
...
## Security Best Practices for Place Management Applications
...
- No location data in logs or error messages
...
# or
curl https://raw.githubusercontent.com/nektos/act/main/install.sh | sudo bash
```
...
# Kubernetes
kubectl rollout undo deployment/main_thing_index-server
kubectl rollout undo deployment/main_thing_index-server --to-revision=3
...
Phase 13 completes the Main Thing Index project with a production-ready CI/CD pipeline. The system now has automated testing, code quality checks, security scanning, and deployment automation, making it ready for production use in geographic data management environments.
...
The CI/CD pipeline ensures that every code change is thoroughly tested, securely scanned, and safely deployed, meeting the high standards required for applications handling place and location data.
