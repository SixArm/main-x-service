...
| **Access control / authentication** (ISO 27001; GDPR) | Central SSO: passwordless magic-link, **RS256** JWT (asymmetric — peers hold only the public key), offline JWKS verification, and **blanket `/api/*` enforcement** middleware (default-off, env-gated per service). | `authentication-verifier` crate embedded in `src/auth.rs`; see [authentication §4, §7](../authentication/index.md). |
...
- **Actor attribution.** Every audit row and event envelope is stamped
  ...
  RS256 token was presented; `NULL` otherwise. The same actor flows into
...
| **Blanket `/api/*` JWT enforcement** | Implemented but **default-off**, gated per service (`ORGANIZATION_REQUIRE_AUTH`, `CARE_PATHWAY_REQUIRE_AUTH`, `CASE_REQUIRE_AUTH`); the older Axum services (person / worker / place) are a separate follow-up. Activation is an ops decision (turning it on before the front-end attaches a token would `401` every request). See [authentication §7](../authentication/index.md). |
...
| **Secrets management** | Production signing keys and DB credentials come from the environment edges (e.g. `JWT_PRIVATE_KEY_FILE` / `JWT_PRIVATE_KEY_PEM`); the committed `*_dev.pem` keys are **dev-only**. A production secrets manager is an infrastructure concern. See [authentication §3.2](../authentication/index.md). |
