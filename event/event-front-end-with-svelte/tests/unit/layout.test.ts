...
// The layout (via the i18n store) imports `browser` from $app/environment and
// guards a `document.documentElement.lang` effect on it; off-browser keeps the
// effect inert under jsdom.
...
// The top-bar navigation is always collapsed behind the always-visible
// hamburger (at every viewport width); the toggle's contract (driving the
// CSS) is `aria-expanded` on the button + the `open` class on the <nav>.
// (spec §5 "Layout shell & navigation".)
