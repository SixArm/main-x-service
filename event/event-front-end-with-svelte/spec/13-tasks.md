...
- [x] **T-30: `search()`'s `items` vs `events` field-name mismatch — the Things/Places live-walkthrough bug, cross-checked here.** *(2026-09-08.)* `EventRepository.search()` read `data.items` from the service's enveloped search response, but the real service's `SearchResponse` (`event-service-with-loco/src/api/rest/handlers.rs`) names the field `events` — `items` never matched anything the real service sends. Found by cross-checking every sibling front-end after thing-front-end's identical bug (its own spec §13 T-30) surfaced. Verified live: with the bug present, a real (empty) `event-service` made `/events` throw `Cannot read properties of undefined (reading 'map')` on load — reverted the fix, reproduced the crash, then restored it — and with the fix in place `/events` renders cleanly against the same real backend.
  - **Resolved.** Fixed `data.items` → `data.events` in `src/lib/api/events.ts::search()`. This crate's own tests had no enveloped-object search stub to correct (its one search test uses the bare-array response shape, which the fix does not touch).
  - **Acceptance:** `npm run check` (0/0), `npm test` (61/61, unchanged — no stub encoded the wrong shape), and a live walkthrough against a real service (including reproducing the pre-fix crash) confirmed the fix. Also corrected in the same pass: `AGENTS.md`'s stale "GDPR-export download UI out of scope" line — it landed already (WEB-5).
