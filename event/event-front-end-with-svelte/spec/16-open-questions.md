...
- **OQ-3**: When the service returns `403`/`401` (post-auth), how should the UI redirect? The BFF + httpOnly-cookie model itself is landed (§13 T-23a): the browser holds only the `__Host-mxi_session` cookie, the SvelteKit server holds the session and attaches a short-lived PASETO server-side; no token in JS, no `localStorage`. CSRF on mutating browser→BFF calls is now also landed (§13 T-23b, 2026-08-28: `__Host-mxi_csrf` double-submit cookie). Still open: the 401/403 redirect UX.
