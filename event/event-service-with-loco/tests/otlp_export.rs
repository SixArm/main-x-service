...
//! link-graph-service — the family's first working exporter, and the
//! reference for both `agents/share/rust-tracing-opentelemetry-stack.md`).
//! worker still carries the commented-out stub this crate had until this
//! change; a test asserting only "it compiles and does not panic" would
//! prove exactly what that stub already proves. Instead each case stands
//! up a **real OTLP/gRPC collector in-process** ([`otlp_collector`]),
...
use event_service::observability::{TelemetryConfig, build_meter_provider, build_tracer_provider};
...
async fn a_tracing_span_reaches_a_real_otlp_collector() {
    ...
    let config = TelemetryConfig {
        service_name: "event-service-otlp-test".to_string(),
    ...
    {
        let layer = tracing_opentelemetry::layer().with_tracer(provider.tracer("event-service"));
        let subscriber = tracing_subscriber::registry().with(layer);
        tracing::subscriber::with_default(subscriber, || {
            let span = tracing::info_span!("events.get", event.id = "0c4f1e2a-test");
            let _entered = span.enter();
            tracing::info!("serving an event read");
        });
    }
    ...
    assert!(
        service_names.contains(&"event-service-otlp-test".to_string()),
    ...
    let span = spans
        .iter()
        .find(|span| span.name == "events.get")
        .unwrap_or_else(|| {
            panic!(
                "exported spans did not include `events.get`: {:?}",
                spans.iter().map(|span| &span.name).collect::<Vec<_>>()
            )
        });
    assert!(
        span.attributes.iter().any(|kv| kv.key == "event.id"),
...
async fn a_metric_reaches_a_real_otlp_collector() {
    ...
    let meter = provider.meter("event-service");
    let histogram = meter
