...
This guide covers deploying the Event Service using Podman and Podman Compose. (The compose files keep their
...
```bash
# Build the event server image
podman compose build

# Start all services (PostgreSQL + Event Server)
podman compose up -d
...
```bash
# Access the event server container
podman compose exec event-server bash
...
- **Event API**: http://localhost:8080/api
...
podman build -t event-server:latest .

# Tag for registry
podman tag event-server:latest your-registry.com/event_service-server:v1.0.0
```
...
# Push image
podman push your-registry.com/event_service-server:v1.0.0
```
...
# Pull latest image
podman pull your-registry.com/event_service-server:v1.0.0
...
Starts only essential services (PostgreSQL + Event Server):
...
```bash
podman compose up event-migrations
```
...
The event server includes a health check endpoint:
...
podman inspect event-server --format='{{json .State.Health}}'
```
...
podman compose logs event-server
podman compose logs postgres

# View last 100 lines
podman compose logs --tail=100 event-server
```
...
- Port already in use: Change `EVENT_PORT` in `.env`
...
```bash
podman compose exec postgres psql -U event_user -d event_service -c "SELECT 1;"
```
...
podman compose exec postgres psql -U event_user -d event_service
# Inside psql:
...
```bash
podman compose exec event-server rm -rf /app/data/search_index/*
podman compose restart event-server
```
...
```bash
# Create backup
podman compose exec postgres pg_dump -U event_user event_service > backup-$(date +%Y%m%d).sql

# Restore from backup
podman compose exec -T postgres psql -U event_user event_service < backup-20231228.sql
```
...
```bash
# Backup search index
podman cp event-server:/app/data/search_index ./search_index_backup

# Restore search index
podman cp ./search_index_backup event-server:/app/data/search_index
podman compose restart event-server
```
...
6. **Run as Non-Root**: Container runs as `event` user (UID 1000)
...
1. **Load Balancer**: Use nginx or HAProxy in front of multiple event server instances
...
4. **Stateless Design**: The event server is stateless, scales horizontally

Example:

```bash
podman compose up -d --scale event-server=3
```
