...
| Duplicate detection | Real-time + explicit + batch; batch scan returns scored `ReviewQueueItem`s directly in the response (ephemeral — no `review_queue` table, no list/decision endpoints; see §6.4, §16 OQ-4) |
