...
bumps together broke `src/api/grpc/`: tonic 0.12's generated-code
shape does not compile against prost 0.14. tonic 0.14 split its prost
codec out of the core crate into `tonic-prost` (the runtime
`ProstCodec`, which generated server/client code now names directly)
+ `tonic-prost-build` (the `build.rs` codegen crate, replacing
`tonic-build`), so the real fix is moving the whole gRPC stub to
`tonic = "0.14"` + `tonic-prost = "0.14"` + `tonic-prost-build`
together, mirroring person-service's and worker-service's identical
fix.
...
plain `use tonic::{...}`. This crate carries no SOUP register (unlike
person's/worker's), so no register update was needed. See
`spec/13-tasks.md` for the task record and `AGENTS.md`'s OTLP/gRPC
sections for the updated adaptation notes.

Verified: `cargo build --all-targets`, `cargo test --lib` (167
passed) + `cargo test --test otlp_export --test otlp_middleware` (4
