...
//! [`EventRepository`](crate::db::EventRepository) trait (whose methods
...
impl OutboxInsert {
    ...
    /// Convenience: build the row for `kind` applied to `event`, stamping
    ...
    pub fn for_event(event: &crate::models::Event, kind: EventKind) -> Result<Self> {
        let env = Envelope::for_event(event, kind);
    ...
    ///
    /// As [`Self::from_envelope`].
    pub fn for_merge(survivor: &crate::models::Event, merged_from: &Uuid) -> Result<Self> {
...
mod tests {
    ...
    fn from_envelope_maps_every_column_and_keeps_the_full_payload() {
        ...
        assert_eq!(row.payload["name"], "Annual Conference");
        assert_eq!(row.payload["entity"], "event");
