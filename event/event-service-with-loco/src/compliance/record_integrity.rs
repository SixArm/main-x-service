//! Row-level integrity over the assembled `Event` record.
//!
//! ## Why the assembled record, not the root row
//!
//! A event's data lives across several tables — the root `events` row
...
//! the same value `GET /api/events/{id}` returns.
//!
...
pub const RECORD_HASH_VERSION: &str = "e-r1";
...
pub struct RecordInput<'a> {
    ...
    /// cancelled record is a content change.
    pub active: bool,
}
...
pub fn preimage(input: &RecordInput<'_>) -> Vec<u8> {
    ...
    field(
        &serde_json::to_string(input.event).unwrap_or_else(|_| "\u{0}unserializable".to_string()),
    );
...
/// simply had nowhere to go, and noevent pointed that out.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn event(name: &str) -> Event {
        // A fixed start date so the fixture is deterministic: a digest
        // test whose input moved with the clock would be untrustworthy.
        let start = chrono::DateTime::from_timestamp(1_700_000_000, 0).expect("valid timestamp");
        let mut t = Event::new(name, start);
        t.id = Uuid::from_u128(1);
        t
    }

    fn input(t: &Event) -> RecordInput<'_> {
        RecordInput {
            id: t.id,
            event: t,
            active: true,
    ...
    fn spec_documents_this_version_tag() {
        assert_eq!(RECORD_HASH_VERSION, "e-r1");
    }
    ...
    /// The live flag is bound in, so reactivating a cancelled record by
