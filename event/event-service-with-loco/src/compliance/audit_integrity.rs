...
//! Noevent in a row can attest to its own continued existence. Catching
...
//! a silent edit to holding the key; it does noevent about `DELETE`.
...
pub const AUDIT_MAC_VERSION: &str = "e-a1";
...
pub const CAVEAT: &str = "A verified result attests that no examined row's content was altered \
     without the key. It does NOT attest that no row was deleted: noevent in a row can \
...
mod tests {
    ...
    fn every_field_is_bound_into_the_preimage() {
        ...
        assert_ne!(mutate(&|i| i.ip_address = None), base, "ip");
