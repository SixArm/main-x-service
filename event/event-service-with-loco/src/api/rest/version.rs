...
//! (`/api/events`, never `/api/v1/events`), and a client selects the
//! representation version with `Accepts-version: 1.0`. This module is the
//! **reference implementation** copied by the other services.
//!
...
mod tests {
    ...
    fn only_api_paths_are_versioned() {
        ...
        assert!(is_versioned_path("/api/events"));
        assert!(!is_versioned_path("/fhir/Appointment"));
