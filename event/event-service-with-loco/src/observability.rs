...
//!
//! Ported from [`person-service`]'s `src/observability.rs` (itself a port
//! of [`link-graph-service`]'s, the family's first working OTLP pipeline —
//! `agents/share/rust-tracing-opentelemetry-stack.md`). This crate
//! previously carried a stub (`src/observability/mod.rs` +
//! `src/observability/{metrics,traces}.rs`) that built an `OTel`
//! [`Resource`] and then installed a plain JSON `tracing` subscriber, with
//! the exporter and the bridge layer commented out behind
//! `// TODO: Initialize OTLP exporter` — dead scaffolding that was never
//! wired into [`crate::app::App`]'s `Hooks` impl at all (`init_telemetry`/
//! `shutdown_telemetry` had no caller). This module replaces it outright
//! rather than filling in the stub in place, because the stub's shape (a
//! bare `tracing_subscriber::fmt` layer with no bridge) cannot be extended
//! into the real pipeline without discarding most of it.
//!
...
//! | `OTLP_SERVICE_NAME` | `event-service` | `service.name` resource attribute |
//! | `OTLP_ENDPOINT` | `http://localhost:4317` | OTLP/gRPC collector endpoint |
//!
//! These three variables are **deliberately not `EVENT_`-prefixed**,
//! matching link-graph-service's and person-service's own choice and
...
pub const DEFAULT_SERVICE_NAME: &str = "event-service";
...
fn http_metrics() -> &'static HttpMetrics {
    ...
    METRICS.get_or_init(|| {
        let meter = global::meter("event-service");
        HttpMetrics {
            duration: meter
...
/// embed record UUIDs (`/api/events/{id}`, FHIR `/fhir/Appointment/{id}`,
...
mod tests {
    ...
    fn exporter_diagnostics_are_added_to_a_default_filter() {
        ...
        let base = || tracing_subscriber::EnvFilter::new("event_service=info");
    ...
    fn an_operator_supplied_filter_is_left_alone() {
        ...
        assert_eq!(untouched.to_string(), "event_service=info");
    }
    ...
    fn explicit_service_name_and_endpoint_are_honoured() {
        let config = TelemetryConfig::resolve(
            Some("event-service-canary"),
            Some("http://collector.internal:4317"),
        );
        assert_eq!(config.service_name, "event-service-canary");
        assert_eq!(
    ...
    fn resource_carries_service_name_and_version() {
        ...
        let name = resource
            ...
            .expect("service.name");
        assert_eq!(name.as_str(), "event-service");
