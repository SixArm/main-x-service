...
//! (`EVENT_EVENT_TRANSPORT=outbox`) **and** `EVENT_EVENT_RELAY` is truthy
...
//! [`LoggingSink`] when `EVENT_FLUVIO_ENDPOINT` (§7) is configured — see
...
/// Whether the relay background loop should run: `EVENT_EVENT_RELAY`
/// truthy (`1`/`true`/`yes`/`on`). Off by default.
#[must_use]
pub fn relay_enabled() -> bool {
    matches!(
        std::env::var("EVENT_EVENT_RELAY")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase()
            .as_str(),
...
/// Relay poll interval in seconds (`EVENT_EVENT_RELAY_INTERVAL_SECS`,
/// default 5, floored at 1).
#[must_use]
pub fn interval_secs() -> u64 {
    std::env::var("EVENT_EVENT_RELAY_INTERVAL_SECS")
        .ok()
...
/// Outbox retention in days (`EVENT_EVENT_RETENTION_DAYS`, default 7).
#[must_use]
pub fn retention_days() -> i64 {
    std::env::var("EVENT_EVENT_RETENTION_DAYS")
        .ok()
        .and_then(|v| v.trim().parse::<i64>().ok())
        .unwrap_or(7)
}

/// Fluvio broker endpoint (`EVENT_FLUVIO_ENDPOINT`, §7). Unset/blank ⇒ no
...
pub fn fluvio_endpoint() -> Option<String> {
    std::env::var("EVENT_FLUVIO_ENDPOINT")
        .ok()
        .filter(|s| !s.trim().is_empty())
}

/// The topic events publish to (`EVENT_EVENT_TOPIC`, default
/// `mxi.event.events` per §7's `mxi.<entity>.events` convention).
#[must_use]
pub fn event_topic() -> String {
    std::env::var("EVENT_EVENT_TOPIC")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| "mxi.event.events".to_string())
}
...
///
/// Sink selection: no `EVENT_FLUVIO_ENDPOINT` ⇒ [`LoggingSink`]. An
...
pub fn spawn(db: DatabaseConnection) {
    ...
    if endpoint.is_some() && !cfg!(feature = "fluvio") {
        tracing::error!(
            "EVENT_FLUVIO_ENDPOINT is set but this binary was built without the `fluvio` \
...
mod tests {
    ...
    fn logging_sink_sends_ok() {
        ...
        let out = rt.block_on(async {
            LoggingSink
                .send("event", "pid-1", &serde_json::json!({"kind": "created"}))
                .await
        });
        assert!(out.is_ok());
    }
    ...
    fn capturing_sink_records_entity_and_key() {
        ...
        rt.block_on(async {
            sink.send("event", "pid-9", &serde_json::json!({}))
                .await
                .unwrap();
        });
        assert_eq!(
            sink.0.lock().unwrap().as_slice(),
            &[("event".to_string(), "pid-9".to_string())]
        );
    }
    ...
    fn config_defaults_are_safe() {
        ...
        if std::env::var("EVENT_EVENT_RELAY").is_err() {
            assert!(!relay_enabled());
        }
        if std::env::var("EVENT_EVENT_RELAY_INTERVAL_SECS").is_err() {
            assert_eq!(interval_secs(), 5);
        }
        if std::env::var("EVENT_EVENT_RETENTION_DAYS").is_err() {
