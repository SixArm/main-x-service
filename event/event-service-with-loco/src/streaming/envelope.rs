...
//! [`EventEvent`](super::EventEvent) / [`EventProducer`](super::EventProducer)
...
//! `EVENT_EVENT_TRANSPORT=outbox` is set.
//!
//! [`Envelope::for_event`] builds the envelope from a domain
//! [`Event`](crate::models::Event); [`EventView`] is the flat operator
...
pub struct Envelope {
    ...
    /// The entity name (`snake_case`), e.g. `"event"`.
    ///
...
impl Envelope {
    /// Build an envelope describing `kind` applied to `event`, minting a
    ...
    #[must_use]
    pub fn for_event(event: &Event, kind: EventKind) -> Self {
        Self {
            event_id: Uuid::new_v4(),
            schema_version: SCHEMA_VERSION,
            entity: ENTITY,
            kind,
            pid: event.id.to_string(),
            seq: next_seq(),
            actor: None,
    ...
    #[must_use]
    pub fn for_merge(survivor: &Event, merged_from: &Uuid) -> Self {
        Self {
...
impl EventTransport {
    /// Parse the `EVENT_EVENT_TRANSPORT` value. `outbox` selects the
...
/// The process-wide transport, read once from `EVENT_EVENT_TRANSPORT` and
...
pub fn transport() -> EventTransport {
    ...
    *TRANSPORT.get_or_init(|| {
        std::env::var("EVENT_EVENT_TRANSPORT")
...
mod tests {
    ...
    fn for_event_leaves_merged_from_none() {
        let env = super::Envelope::for_event(&an_event(), EventKind::Updated);
    ...
    fn seq_is_monotonic_across_envelopes() {
        let event = an_event();
        let a = super::Envelope::for_event(&event, EventKind::Created);
        let b = super::Envelope::for_event(&event, EventKind::Updated);
    ...
    fn event_view_projects_the_frozen_shape() {
        let event = an_event();
        let env = super::Envelope::for_event(&event, EventKind::Deleted);
