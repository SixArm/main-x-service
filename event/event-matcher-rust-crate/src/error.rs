//! Error types for Event-matcher operations.
//!
//! The crate uses a single sum-type, [`MatchingError`], for every fallible
//! operation, and a [`Result`] alias to keep call-sites concise.
//!
//! The matching engine itself is **infallible**: scoring two events always
//! produces a [`crate::MatchResult`]. Errors arise from explicit validation
//! steps such as [`crate::Event::validate`].
//!
//! ## Example
//!
//! ```
//! use event_matcher::{MatchingError, Event};
//!
//! let empty = Event::builder().build();
...
///
/// ```
/// use event_matcher::MatchingError;
///
...
pub enum MatchingError {
    /// A required field was absent. Returned by [`crate::Event::validate`].
