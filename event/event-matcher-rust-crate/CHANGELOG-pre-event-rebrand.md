# Pre-event-rebrand changelog

Historical "Unreleased" entries authored before the 0.5.0
event-matcher rebrand (this crate used to be `place-matcher`).
These changes never shipped under the `place-matcher` name; they
are preserved here for the audit trail.

For the post-rebrand history see [`CHANGELOG.md`](./CHANGELOG.md).
For released place-matcher versions see
