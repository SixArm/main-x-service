...
consumers (`event-service` via its `to_matcher_event` adapter) depend on. The
...
- `Event::builder()` + every fluent builder method (demographic, contact,
  identifier, address slots).
- EventBuilder full surface, EventCategory variant set (schema.org/Event subtypes), EventStatus + EventAttendanceMode variants, EventIdScheme variants (Eventbrite, Meetup, Wikidata, …), Location builder dispatch.
- `MatchingEngine::default_config`, `MatchingEngine::new`,
  `match_events`, `deterministic_match`, `match_one_to_many`.
...
silent. The corresponding service-side test, `event-service`'s
