...
//! phones, addresses, emails, and ISO-8601 timestamps — prime never-panic
...
fuzz_target!(|data: &[u8]| {
    ...
    // Each of these is a pure `&str -> ...` transform; the assertion is
    // implicit — the process must not abort.
    ...
    let _ = Normalizer::parse_iso8601_unix_seconds(s);
});
