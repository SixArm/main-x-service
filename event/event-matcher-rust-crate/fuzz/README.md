...
|---|---|
| `match_events` | Deserialize a JSON `[event_a, event_b]` tuple → `MatchingEngine::match_events`. The whole deserialize → normalize → score path; asserts finite score in `[0,1]` in both argument orders. |
| `normalizer` | The pure `Normalizer` string helpers (name / postcode / phone / E.164 / address / phonetic / email / ISO-8601 timestamp) over arbitrary UTF-8 — never-panic. |
...
cargo +nightly fuzz run match_events     # fuzz until a crash / Ctrl-C
