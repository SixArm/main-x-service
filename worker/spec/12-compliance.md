...
**One gap, stated plainly.** `worker_assessments.content_hash` carries a
SHA-256 digest only. Assessments were adopted between the record hash and
the second algorithm, and extending them is a follow-up rather than part
of this change. The verification report counts every assessment row under
`blake3_unhashed`, so the gap is visible rather than implied.
