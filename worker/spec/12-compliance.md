...
**`worker_assessments` carries both digests too.** It briefly did not —
assessments were adopted between the record hash and the second algorithm
— and the gap was closed the same day rather than left as a follow-up.
This is the crate's most sensitive table, so a SHA-256-only digest there
would have left the likeliest tampering target with the least agility.
...
The shared crate takes a `&str` domain because the sets differ — this
service has a fourth, `assessment`, for `worker_assessments` — and an
enum there would have to know all of them.
...
mxi/worker-service/assessment/d1
```
