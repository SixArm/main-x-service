...
//! over OTLP/gRPC (repo `tasks.md` PRO-H9).
//!
//! Ported from person-service's `tests/otlp_export.rs` (itself ported
//! from link-graph-service's — the family's first working exporter, and
//! the ultimate reference for this one,
//! `agents/share/rust-tracing-opentelemetry-stack.md`). The other
//! entity registries either carry no `src/observability.rs` at all, or
//! still carry the commented-out stub this crate had until this change;
//! a test asserting only "it compiles and does not panic" would prove
//! exactly what those stubs already prove. Instead each case stands up a
//! **real OTLP/gRPC collector in-process** ([`otlp_collector`]), points
//! the exporter at it, and asserts on the decoded protobuf that arrives
//! over the socket.
//!
...
use worker_service::observability::{TelemetryConfig, build_meter_provider, build_tracer_provider};
...
async fn a_tracing_span_reaches_a_real_otlp_collector() {
    ...
    let config = TelemetryConfig {
        service_name: "worker-service-otlp-test".to_string(),
    ...
    {
        let layer = tracing_opentelemetry::layer().with_tracer(provider.tracer("worker-service"));
        let subscriber = tracing_subscriber::registry().with(layer);
        tracing::subscriber::with_default(subscriber, || {
            let span = tracing::info_span!("workers.get", worker.id = "0c4f1e2a-test");
            let _entered = span.enter();
            tracing::info!("serving a worker read");
        });
    }
    ...
    assert!(
        service_names.contains(&"worker-service-otlp-test".to_string()),
    ...
    let span = spans
        .iter()
        .find(|span| span.name == "workers.get")
        .unwrap_or_else(|| {
            panic!(
                "exported spans did not include `workers.get`: {:?}",
                spans.iter().map(|span| &span.name).collect::<Vec<_>>()
            )
        });
    assert!(
        span.attributes.iter().any(|kv| kv.key == "worker.id"),
...
async fn a_metric_reaches_a_real_otlp_collector() {
    ...
    let meter = provider.meter("worker-service");
    let histogram = meter
