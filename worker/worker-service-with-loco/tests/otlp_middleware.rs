//! The **mounted** middleware, end to end (repo `tasks.md` PRO-H9).
//!
//! Ported from person-service's `tests/otlp_middleware.rs` (itself ported
//! from link-graph-service's). `tests/otlp_export.rs` proves the export
//! pipeline; this proves the thing the service actually runs — the exact
//! [`observability::trace_mw`](worker_service::observability::trace_mw)
//! layer both `App::after_routes` and `api::rest::create_router` install
//! (see that function's doc comment for why it is wired onto both of this
//! crate's router-construction surfaces, unlike link-graph-service's
//! single loco-native one). It layers `trace_mw` onto a minimal, real
//! Axum router — rather than booting the full application, which would
//! need a database — serves it over a real socket, makes a real HTTP
//! request, and then asserts that
//!
...
use worker_service::observability::{TelemetryConfig, build_tracer_provider, trace_mw};
...
async fn a_served_request_exports_a_span_and_returns_its_traceparent() {
    ...
    let config = TelemetryConfig {
        service_name: "worker-service-mw-test".to_string(),
    ...
    tracing_subscriber::registry()
        .with(tracing_opentelemetry::layer().with_tracer(provider.tracer("worker-service")))
        .with(tracing_subscriber::EnvFilter::new(
            "worker_service=trace,otlp_middleware=trace",
        ))
        .init();
