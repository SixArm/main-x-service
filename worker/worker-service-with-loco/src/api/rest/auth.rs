...
/// enforcement runs.
pub fn apply_enforcement(router: Router, require_auth: bool) -> Router {
    router.layer(axum::middleware::from_fn(
        move |req: Request, next: Next| async move {
            // Snapshots taken per request, so a rotated key set or a
            // reloaded policy reaches the guard as well as the handlers.
            // They used to be captured here at construction, which meant
            // the guard could never see either change.
...
///
/// Built lazily from `WORKER_PASETO_KEYS` on first use; [`init`] swaps in
...
/// boot). With `WORKER_PASETO_KEYS_URL` set, the published key set is
...
pub async fn init() {
    if std::env::var("WORKER_PASETO_KEYS_URL")
        .ok()
...
/// `WORKER_PASETO_KEYS_REFRESH_SECS` is unset. One hour: rotation is
...
pub fn spawn_key_refresh() {
    let Some(url) = std::env::var("WORKER_PASETO_KEYS_URL")
        .ok()
    ...
    let secs = std::env::var("WORKER_PASETO_KEYS_REFRESH_SECS")
        .ok()
    ...
    let issuer = super::state::env_or("WORKER_TOKEN_ISSUER", super::state::DEFAULT_ISSUER);
    let audience = super::state::env_or("WORKER_TOKEN_AUDIENCE", super::state::DEFAULT_AUDIENCE);
    ...
    tracing::info!(secs, "polling WORKER_PASETO_KEYS_URL for key rotation");
}
...
/// `WORKER_ABAC_POLICY_FILE` changes on disk, so an operator can edit
/// the policy and have it take effect without a restart. Polls the
/// file's mtime (not an OS notify — this stays dependency-light).
///
/// A no-op when the variable is unset: an inline `WORKER_ABAC_POLICY`
...
pub fn spawn_policy_watcher() {
    let Some(path) = std::env::var("WORKER_ABAC_POLICY_FILE")
        .ok()
    ...
    tracing::info!(
        ...
        "watching WORKER_ABAC_POLICY_FILE for changes"
    );
}
