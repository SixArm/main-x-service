...
//! [`WorkerRepository`](crate::db::WorkerRepository) trait (whose methods
...
impl OutboxInsert {
    ...
    /// Convenience: build the row for `kind` applied to `worker`, stamping
    ...
    pub fn for_event(worker: &crate::models::Worker, kind: EventKind) -> Result<Self> {
        let env = Envelope::for_event(worker, kind);
    ...
    ///
    /// As [`Self::from_envelope`].
    pub fn for_merge(survivor: &crate::models::Worker, merged_from: &Uuid) -> Result<Self> {
...
mod tests {
    ...
    fn from_envelope_maps_every_column_and_keeps_the_full_payload() {
        ...
        assert_eq!(row.payload["entity"], "worker");
