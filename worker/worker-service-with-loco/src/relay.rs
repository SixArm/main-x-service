...
//! `FluvioSink` (BUS-3, ported from the case-service BUS-1 reference; only
...
//! real-broker `impl EventSink`; the [`EventSink`] trait is the seam, so the
//! drain loop and retention never change between the two
//! (`agents/share/event-bus.md` §5).
//!
//! Activation: the relay only runs when the transport is `outbox`
//! (`WORKER_EVENT_TRANSPORT=outbox`) **and** `WORKER_EVENT_RELAY` is truthy
...
//! [`LoggingSink`] when `WORKER_FLUVIO_ENDPOINT` (§7) is configured — see
...
/// The real-broker sink (BUS-3, ported from the case-service BUS-1
...
/// Whether the relay background loop should run: `WORKER_EVENT_RELAY`
/// truthy (`1`/`true`/`yes`/`on`). Off by default.
#[must_use]
pub fn relay_enabled() -> bool {
    matches!(
        std::env::var("WORKER_EVENT_RELAY")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase()
            .as_str(),
...
/// Relay poll interval in seconds (`WORKER_EVENT_RELAY_INTERVAL_SECS`,
/// default 5, floored at 1).
#[must_use]
pub fn interval_secs() -> u64 {
    std::env::var("WORKER_EVENT_RELAY_INTERVAL_SECS")
        .ok()
...
/// Outbox retention in days (`WORKER_EVENT_RETENTION_DAYS`, default 7).
#[must_use]
pub fn retention_days() -> i64 {
    std::env::var("WORKER_EVENT_RETENTION_DAYS")
        .ok()
        .and_then(|v| v.trim().parse::<i64>().ok())
        .unwrap_or(7)
}

/// Fluvio broker endpoint (`WORKER_FLUVIO_ENDPOINT`, §7). Unset/blank ⇒ no
...
pub fn fluvio_endpoint() -> Option<String> {
    std::env::var("WORKER_FLUVIO_ENDPOINT")
        .ok()
        .filter(|s| !s.trim().is_empty())
}

/// The topic events publish to (`WORKER_EVENT_TOPIC`, default
/// `mxi.worker.events` per §7's `mxi.<entity>.events` convention).
#[must_use]
pub fn event_topic() -> String {
    std::env::var("WORKER_EVENT_TOPIC")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| "mxi.worker.events".to_string())
}
...
///
/// Sink selection: no `WORKER_FLUVIO_ENDPOINT` ⇒ [`LoggingSink`]. An
...
pub fn spawn(db: DatabaseConnection) {
    ...
    if endpoint.is_some() && !cfg!(feature = "fluvio") {
        tracing::error!(
            "WORKER_FLUVIO_ENDPOINT is set but this binary was built without the `fluvio` \
...
mod tests {
    ...
    fn logging_sink_sends_ok() {
        ...
        let out = rt.block_on(async {
            LoggingSink
                .send("worker", "pid-1", &serde_json::json!({"kind": "created"}))
                .await
        });
        assert!(out.is_ok());
    }
    ...
    fn capturing_sink_records_entity_and_key() {
        ...
        rt.block_on(async {
            sink.send("worker", "pid-9", &serde_json::json!({}))
                .await
                .unwrap();
        });
        assert_eq!(
            sink.0.lock().unwrap().as_slice(),
            &[("worker".to_string(), "pid-9".to_string())]
        );
    }
    ...
    fn config_defaults_are_safe() {
        ...
        if std::env::var("WORKER_EVENT_RELAY").is_err() {
            assert!(!relay_enabled());
        }
        if std::env::var("WORKER_EVENT_RELAY_INTERVAL_SECS").is_err() {
            assert_eq!(interval_secs(), 5);
        }
        if std::env::var("WORKER_EVENT_RETENTION_DAYS").is_err() {
