...
/// `WORKER_AUDIT_FAIL_CLOSED` (read once and cached).
///
...
/// under `WORKER_EVENT_TRANSPORT=outbox`, where the audit row shares
...
pub fn fail_closed() -> bool {
    ...
    *FAIL_CLOSED.get_or_init(|| {
        crate::api::rest::auth::parse_bool(
            &std::env::var("WORKER_AUDIT_FAIL_CLOSED").unwrap_or_default(),
        )
    })
}

/// Record one read/disclosure audit row — a **no-op unless**
/// `WORKER_AUDIT_READS` is on, so adopting this module changes nothing
/// until a deployment opts in.
///
/// `entity_id` is the record read, or [`Uuid::nil`] for a
...
/// # Errors
///
/// [`AuditWriteRefused`] when the write failed **and** [`fail_closed`] is
/// on — the caller must then refuse the read. With fail-closed off (the
/// default) a failure is logged and `Ok` is returned.
pub async fn record_access(
    ...
    // The verified caller is the actor. IP and user-agent are not plumbed
    // through to read handlers today, so they stay `None` rather than
    // being invented — the disclosure recipient and purpose, which *are*
    // declared, go into the row's `context` instead.
    let audit_actor = crate::db::audit::AuditActor {
        user_id: actor,
        ip_address: None,
        user_agent: None,
    };
    let Err(error) = audit
        .log_access(entity_type, entity_id, action, &audit_actor, ctx)
        .await
    else {
        return Ok(());
    };
    if fail_closed() {
        tracing::error!(
            %error,
            action,
            "read-access audit write failed and WORKER_AUDIT_FAIL_CLOSED is on; refusing the \
             read rather than disclosing data unaccounted for"
        );
    ...
    tracing::warn!(
        %error,
        action,
        "failed to write read-access audit row; the read proceeds (fail-open) and the chain \
         will show a gap indistinguishable from a deletion — set WORKER_AUDIT_FAIL_CLOSED to \
         refuse instead"
    );
    Ok(())
}
...
mod tests {
    ...
    fn context_json_carries_declaration_and_derivation() {
        ...
        // The GDPR standing declarations arrive with §8.5 steps 4-5;
        // emitting empty placeholders now would imply a posture this
        // deployment has not declared.
        for absent in ["residency", "lawful_basis", "art9_condition"] {
            assert!(json.get(absent).is_none(), "{absent} is not claimed yet");
        }
    }
    ...
    fn fail_closed_defaults_off() {
        ...
        assert!(
            !crate::api::rest::auth::parse_bool(""),
            "unset means fail-open"
        );
        assert!(!crate::api::rest::auth::parse_bool("0"));
        assert!(
            crate::api::rest::auth::parse_bool("1"),
            "opt-in turns fail-closed on"
        );
        assert!(crate::api::rest::auth::parse_bool("true"));
    }
