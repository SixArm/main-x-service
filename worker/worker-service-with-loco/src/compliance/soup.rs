...
//! and justified.
//!
//! ## Why a workforce registry keeps one
//!
//! This service holds personal data, serves a FHIR `Practitioner`
//! representation, and stores assessment results that can cost someone a
//! job. It is **not** a medical device: no output drives an individual's
//! treatment (the qualification caveat in
...
//! correctness and confidentiality of personal and assessment data, not
//! to physical safety.
//!
    ...
    Sbom {
        ...
        metadata: SbomMetadata {
            component: SbomComponent {
                ...
                description: Some("Workforce identity registry service".to_string()),
                properties: Vec::new(),
            },
            properties: vec![
                // No `iec62304-safety-class` property: serving FHIR
                // Practitioner does not make this a medical device, and
...
mod tests {
    ...
    fn sbom_metadata_carries_lifecycle_evidence() {
        ...
        // Deliberately absent: a workforce registry does not drive
        // anyone's treatment, so a medical-device safety class would be an
        // unsupportable claim even though the crate serves FHIR
        // Practitioner.
        assert!(
            !names.contains(&"mxi:iec62304-safety-class"),
            "a workforce registry must not declare a device safety class"
        );
    }
