...
//!
//! ```text
//! mxi/worker-service/audit-chain/d1
//! mxi/worker-service/record/d1
//! mxi/worker-service/checkpoint/d1
//! ```
//!
...
pub const KEY_ENV: &str = "WORKER_INTEGRITY_MAC_KEY";
...
pub const KEY_FILE_ENV: &str = "WORKER_INTEGRITY_MAC_KEY_FILE";

/// Environment variable naming the active key, for the stored prefix.
pub const KEY_ID_ENV: &str = "WORKER_INTEGRITY_MAC_KEY_ID";

/// Environment variable holding retired keys still needed for
/// verification, as `id:hex` pairs separated by commas.
pub const RETIRED_KEYS_ENV: &str = "WORKER_INTEGRITY_MAC_KEYS_RETIRED";
...
pub enum Domain {
    ...
    Checkpoint,
    /// Per-worker assessment results (`worker_assessments`).
    ///
    /// A separate table with its own pre-image, so it gets its own
    /// subkey: sharing [`Self::Record`]'s would put it back on
    /// separation-by-version-tag, which is exactly what this enum exists
    /// to stop relying on.
    Assessment,
}
...
impl KeyMaterial {
    ...
    fn derive_all(root: &mut Vec<u8>) -> Self {
        let material = Self {
            ...
            assessment: derive(root, Domain::Assessment),
    ...
    fn for_domain(&self, domain: Domain) -> &[u8] {
        match domain {
            ...
            Domain::Assessment => &self.assessment,
        }
    }
}
...
mod tests {
    ...
    fn derivation_matches_independent_hkdf_vectors() {
        ...
        let cases = [
            (
                Domain::AuditChain,
                "2e0563f6590c1c8f72ce1dbf8e66e12e9a393a676fa1b65fac0d9efe3b4ac9cd",
                "f36081d85e6c47bc12a25c6c464f76945d75ba8c6fa4ee921bf6974cd7e69c63",
            ),
            (
                Domain::Record,
                "4455777302bf96ff9e99bc88e915137effa9a31c280e1147bce0b2618c33c152",
                "c093cbce793cdf5750747542fabefebd9499af3b5881d7d47e976214c82c7186",
            ),
            (
                Domain::Checkpoint,
                "1f7eef9998d33ffd29acc04ceb2a4e3115a2962449639c11c06463403aa054da",
                "4c155a7effe04848ee6cdc91f9af1fbfb26b349005730914d980b67ab4d73b87",
            ),
        ];
