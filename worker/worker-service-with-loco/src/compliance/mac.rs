...
const CONFIG: KeyConfig = KeyConfig::new(env!("CARGO_PKG_NAME"), "WORKER");
...
pub enum Domain {
    ...
    /// separation-by-version-tag, which is what this enum exists to stop
    /// relying on.
    Assessment,
}
...
impl Domain {
    ...
    /// than on first use.
    const ALL: [&'static str; 4] = ["audit-chain", "record", "checkpoint", "assessment"];
}
