...
//! of [`link-graph-service`]'s `src/observability.rs`, the family's first
//! working OTLP pipeline
//! (`agents/share/rust-tracing-opentelemetry-stack.md`). Person already
//! solved the two adaptations this crate's shape needs (two
//! router-construction surfaces, and a renamed `tonic` dev-dependency —
//! see below), so this is a port of person's port, not a second
//! independent port of link-graph-service's. This module **replaces**
//! the earlier `src/observability/` stub outright (a JSON `tracing`
//! subscriber with the OTLP exporter commented out behind
//! `// TODO: Initialize OTLP exporter`, never wired into
//! [`crate::app::App`]'s `Hooks` impl at all) rather than filling the stub
//! in place — its shape (a bare `tracing_subscriber::fmt` layer with no
...
//! | `OTLP_SERVICE_NAME` | `worker-service` | `service.name` resource attribute |
//! | `OTLP_ENDPOINT` | `http://localhost:4317` | OTLP/gRPC collector endpoint |
//!
//! These three variables are **deliberately not `WORKER_`-prefixed**,
...
pub const DEFAULT_SERVICE_NAME: &str = "worker-service";
...
fn http_metrics() -> &'static HttpMetrics {
    ...
    METRICS.get_or_init(|| {
        let meter = global::meter("worker-service");
        HttpMetrics {
            duration: meter
...
/// embed record UUIDs (`/api/workers/{id}`, `/api/workers/{id}/links/{link_id}`,
/// FHIR `/fhir/Practitioner/{id}`, …), so a per-path label would be
...
mod tests {
    ...
    fn exporter_diagnostics_are_added_to_a_default_filter() {
        ...
        let base = || tracing_subscriber::EnvFilter::new("worker_service=info");
    ...
    fn an_operator_supplied_filter_is_left_alone() {
        ...
        assert_eq!(untouched.to_string(), "worker_service=info");
    }
    ...
    fn explicit_service_name_and_endpoint_are_honoured() {
        let config = TelemetryConfig::resolve(
            Some("worker-service-canary"),
            Some("http://collector.internal:4317"),
        );
        assert_eq!(config.service_name, "worker-service-canary");
        assert_eq!(
    ...
    fn resource_carries_service_name_and_version() {
        ...
        let name = resource
            ...
            .expect("service.name");
        assert_eq!(name.as_str(), "worker-service");
