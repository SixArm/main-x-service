...
scripts/test-db.sh up worker/worker-service-with-loco   # containerised Postgres (Podman)
```
...
scripts/test-db.sh up worker/worker-service-with-loco   # test database
podman build -t worker-server:v1.0.0 . && podman run ...  # Production
```
