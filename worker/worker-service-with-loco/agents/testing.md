...
cargo test --lib test_worker_matcher        # Tests matching "test_worker_matcher"
...
**Requires:** PostgreSQL running — `scripts/test-db.sh up worker/worker-service-with-loco` starts the containerised one (`compose.test.yaml`).
...
DATABASE_URL=postgres://user:pass@localhost/worker_service_test cargo test --test api_integration_test
```
...
| ----------------------------- | --------------------------------------------------------------- |
| `benches/matching_bench.rs`   | Name matching, full worker matcher, phonetic encoding          |
...
service-side domain model through [`matching::adapter::to_matcher_worker`]
and asserts on `MatchingEngine::match_workers` output. The suite pins
...
- The worker-matcher crate exposes a new scoring component the service
  needs to surface.
