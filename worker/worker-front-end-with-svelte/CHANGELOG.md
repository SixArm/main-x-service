...
`adobe-spectrum`, `mozilla-protocol`, and the non-NHS UK/US government
themes) and `SIZES`/`SIZE_LABELS` (a custom 4-step `small`/`medium`/
`large`/`x-large` scale) arrays from `+layout.svelte`; `PickerBar` now
gets no `themes`/`sizes` prop at all, so it uses its own
