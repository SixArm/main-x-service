<!--
  ...
  three ways over one selection:
    ...
    3. An inline comparison panel below both, fetching each side of the
       pair with its own `GET /api/workers/{id}` (there is no combined
  ...
  merge. The panel therefore deep-links to `/workers/merge` with both ids
...
<script lang="ts">
    ...
    import type { ReviewQueueOptions } from "$lib/api/workers";
    import type {
        Worker,
        ReviewDecision,
        ReviewQueueItem,
    ...
    async function runScan() {
        ...
        try {
            ...
            scanned = report.workers_scanned;
    ...
    let pair = $state<{ a: Worker | null; b: Worker | null } | null>(null);
    ...
    async function loadPair(item: ReviewQueueItem) {
        ...
        const [a, b] = await Promise.allSettled([
            repo.get(item.worker_id_a),
            repo.get(item.worker_id_b),
        ]);
    ...
    function pairLabel(item: ReviewQueueItem): string {
        return `${shortId(item.worker_id_a)} ↔ ${shortId(item.worker_id_b)}`;
    }

    /** Full name in reading order; `null` when the record is absent. */
    function fullName(worker: Worker | null): string | null {
    ...
    function addressLine(worker: Worker | null): string | null {
    ...
    function contactLine(worker: Worker | null): string | null {
        const contact = worker?.telecom?.[0];
    ...
    >
            ...
            <table class="compare" data-testid="review-compare-table">
                <thead>
                    <tr>
                        ...
                        <th scope="col">
                            {t("review.compare.a")} · {shortId(item.worker_id_a)}
                        </th>
                        <th scope="col">
                            {t("review.compare.b")} · {shortId(item.worker_id_b)}
                        </th>
                    </tr>
                </thead>
                <tbody>
        ...
        {#if item.status === "confirmed"}
            ...
            <div class="row">
                <a
                    class="button"
                    href={mergeHref(item.worker_id_a, item.worker_id_b)}
                >
                    {t("review.merge.keepA")}
                </a>
                <a
                    class="button"
                    href={mergeHref(item.worker_id_b, item.worker_id_a)}
                >
