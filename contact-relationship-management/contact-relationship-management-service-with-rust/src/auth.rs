//! Bearer-token authentication for the contact-relationship-management API.
//!
...
//!
//! - `CRM_PASETO_KEYS_URL` — optional URL of the auth service's
...
//!   wins over `CRM_PASETO_KEYS` (`tracing::info!`), on failure the
...
//!   up without a restart (interval `CRM_PASETO_KEYS_REFRESH_SECS`,
//!   default 1 h; `0` disables; keeps the current keys on a failed
//!   fetch).
//! - `CRM_PASETO_KEYS` — the Ed25519 key set (JSON, OKP/Ed25519
...
//!   still boots).
//! - `CRM_TOKEN_ISSUER` — expected `iss` (default
//!   `authentication-service`).
//! - `CRM_TOKEN_AUDIENCE` — expected `aud` (default
//!   `main-x-service`).
//!
//! ## Blanket enforcement
//!
//! When `CRM_REQUIRE_AUTH` is truthy (`1`/`true`/`yes`/`on`,
...
//! ## Authorization (ABAC)
//!
//! Inside the same guard — so it applies only when `CRM_REQUIRE_AUTH`
...
//! ([`policy`], built by [`policy_from_env`]) from `CRM_ABAC_POLICY`
//! (inline JSON) or `CRM_ABAC_POLICY_FILE` (path); unset or unparsable
...
//! (body carries the deciding rule). Relationship data is personal + commercial data:
...
/// (called from `App::after_routes`). When `CRM_PASETO_KEYS_URL` is set
...
pub async fn init() {
    if let Some(url) = std::env::var("CRM_PASETO_KEYS_URL")
        .ok()
        .filter(|s| !s.trim().is_empty())
    {
        let issuer = env_or("CRM_TOKEN_ISSUER", DEFAULT_ISSUER);
        let audience = env_or("CRM_TOKEN_AUDIENCE", DEFAULT_AUDIENCE);
...
/// `CRM_PASETO_KEYS_REFRESH_SECS` is unset. One hour — key rotation is
...
/// key set from `CRM_PASETO_KEYS_URL` and swaps it into the live
...
/// out). Interval from `CRM_PASETO_KEYS_REFRESH_SECS` (default
/// [`KEY_REFRESH_DEFAULT_SECS`]); **`0` disables** the loop.
///
/// A no-op when `CRM_PASETO_KEYS_URL` is unset (env-supplied keys have
/// nothing to re-fetch). Call once at boot (`app.rs::after_routes`).
pub fn spawn_key_refresh() {
    let Some(url) = std::env::var("CRM_PASETO_KEYS_URL")
        .ok()
    ...
    let secs = std::env::var("CRM_PASETO_KEYS_REFRESH_SECS")
        .ok()
    ...
    tracing::info!(secs, "polling CRM_PASETO_KEYS_URL for key rotation");
}
...
/// `CRM_REQUIRE_AUTH` and cached. Off by default — see the
...
/// Load the ABAC policy: `CRM_ABAC_POLICY` (inline JSON) wins, then
/// `CRM_ABAC_POLICY_FILE` (path to a JSON file), else the built-in
...
pub fn policy_from_env() -> Policy {
    let source = std::env::var("CRM_ABAC_POLICY")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .or_else(|| {
            let path = std::env::var("CRM_ABAC_POLICY_FILE")
                .ok()
                .filter(|v| !v.trim().is_empty())?;
...
/// `CRM_ABAC_POLICY` / `CRM_ABAC_POLICY_FILE` (else the built-in
...
/// Re-read the ABAC policy from the environment (`CRM_ABAC_POLICY` /
/// `CRM_ABAC_POLICY_FILE`, same rules and default-fallback as
...
/// `CRM_ABAC_POLICY_FILE` changes on disk — so operators can edit the
...
///
/// A no-op when `CRM_ABAC_POLICY_FILE` is unset — an inline
/// `CRM_ABAC_POLICY` (or the built-in default) has nothing to watch,
/// and env vars do not change at runtime. Call once at boot
/// (`app.rs::after_routes`).
pub fn spawn_policy_watcher() {
    let Some(path) = std::env::var("CRM_ABAC_POLICY_FILE")
        .ok()
    ...
    tracing::info!(
        ...
        "watching CRM_ABAC_POLICY_FILE for changes"
    );
}
...
/// Derive the **record-level resource attributes** of a stored deal
...
/// |---|---|---|
/// | `resource.owner` | `Deal::owner_ref` | the bare worker uuid (enables `$sub` ownership) |
/// | `resource.owner_ref` | `Deal::owner_ref` | the full `worker:` URN |
/// | `resource.status` | `won`/`closed_at` | `open`, `won`, `lost` |
///
/// A deployment can then write e.g. "allow write only when
/// `resource.owner = $sub`" (rep ownership, CRM-R15), or "allow a
/// **masked** read otherwise" (an `allow` rule with the `mask`
/// obligation) — entirely as policy, no code change.
#[must_use]
pub fn deal_resource_attrs(deal: &deals::Model) -> BTreeMap<String, Vec<String>> {
    ...
    let status = if deal.closed_at.is_none() {
        "open"
    } else if deal.won {
        "won"
    } else {
...
/// Record-level resource attributes of a **contact**: `resource.owner`
/// (bare worker uuid + full URN, when owned) and `resource.status`.
#[must_use]
pub fn contact_resource_attrs(contact: &contacts::Model) -> BTreeMap<String, Vec<String>> {
    ...
    attrs.insert("status".to_string(), vec![contact.status.clone()]);
    attrs
}
...
/// top-level key to `null` (CRM-R15 masks **amounts** — deal values,
/// forecast totals, ROI — while structure stays visible). Setting to
/// `null` rather than `0` keeps the family's honesty rule: a masked
/// amount is *absent*, never a fake zero.
#[must_use]
...
/// context.
///
/// Gated on the same `CRM_REQUIRE_AUTH` flag as the blanket guard:
...
fn build_from_env() -> Verifier {
    let issuer = env_or("CRM_TOKEN_ISSUER", DEFAULT_ISSUER);
    let audience = env_or("CRM_TOKEN_AUDIENCE", DEFAULT_AUDIENCE);
    let keys = std::env::var("CRM_PASETO_KEYS")
        .ok()
        .filter(|s| !s.trim().is_empty())
...
mod tests {
    ...
    /// SEC-G8 — the default-off **exposure pin**. With `CRM_REQUIRE_AUTH`
    ...
    fn a_deal(
        ...
        crate::models::_entities::deals::Model {
            ...
            account_pid: None,
            primary_contact_pid: None,
            owner_ref: owner.map(|o| format!("worker:{o}")),
            pipeline_pid: uuid::Uuid::new_v4(),
            stage_pid: uuid::Uuid::new_v4(),
            name: "Big Deal".to_string(),
            amount_minor: 5_000_000,
            currency: "GBP".to_string(),
            expected_close_on: None,
            kanban_position: 0,
            source_campaign_pid: None,
            closed_at: closed.then(|| chrono::Utc::now().into()),
            won,
            lost_reason: None,
            deleted_at: None,
        }
    ...
    /// The deal resource attributes map owner + open/won/lost status
    /// (`authorization-attributes.md` §9).
    #[test]
    fn deal_resource_attrs_map_owner_and_status() {
    ...
    /// The attributes drive rep ownership through the shared engine:
    /// a `$sub` owner-rule reads the own deal unmasked; anyone else
    /// falls through to the masked-read rule.
    #[test]
    fn deal_attrs_drive_ownership_and_masking() {
        ...
        let self_read = policy.evaluate_with_resource(
            ...
            Action::Read,
            ENTITY,
            &deal_resource_attrs(&a_deal(Some(me), false, false)),
        );
    ...
    /// `mask_json` nulls the named amount keys and leaves structure —
    /// never a fake zero.
    #[test]
    fn mask_json_nulls_amounts_keeps_structure() {
        let masked = mask_json(
            serde_json::json!({ "name": "Big Deal", "amount_minor": 5_000_000, "currency": "GBP" }),
            &["amount_minor"],
        );
        assert_eq!(masked["name"], "Big Deal");
        assert!(masked["amount_minor"].is_null());
