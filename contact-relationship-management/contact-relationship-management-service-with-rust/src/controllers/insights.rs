...
use crate::models::_entities::{
    activities, audit_logs, consent_events, contacts, deals, forecast_snapshots, leads,
    pipeline_stages, tickets,
};
...
/// `GET /api/insights/followups` — activities carrying a `due_on` and
/// not `done`: the overdue list (oldest first, with age) and the next
/// 30 days, plus per-recorder counts (`actor_ref` is the recorder —
/// said, not assumed to be an assignee).
#[debug_handler]
async fn followups(State(ctx): State<AppContext>, headers: HeaderMap) -> Result<Response> {
    let today = chrono::Utc::now().date_naive();
    let horizon = today + chrono::Days::new(30);
    let rows: Vec<activities::Model> = activities::Entity::find()
