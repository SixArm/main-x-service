# Main Person Service (MPI) - Project Index

## Overview

A high-performance Main Person Service system built with Rust for managing centralized person identity registries across healthcare providers.
...
├── models/          # Domain models (Person, Identifier, Document, etc.)
...
| `MergeRequest`        | `models::merge`             | Person merge operations                               |
...
| `find_matches`        | `matching::mod`        | Find matches for a person in a candidate list |
...
| `validate_person`    | `validation`           | Validate person fields                        |
| `normalize_phone`     | `validation`           | E.164-like phone normalization                 |
| `standardize_address` | `validation`           | Address standardization                        |
| `mask_person`        | `privacy`              | Mask sensitive fields                          |
| `export_person_data` | `privacy`              | GDPR data export                               |
