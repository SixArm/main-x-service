...
Main Person Service (MPI) - A healthcare person identification and matching system built with Rust. Production-ready with 15 API endpoints, 34 unit tests, and comprehensive feature set.
...
- Capacity: 10M persons ~ 40-60 GB with indexes and audit
...
- Methods: search, fuzzy_search, search_by_name_and_year, index_person, index_persons, delete_person
...
- Health: GET /health
- Person CRUD: POST /persons, GET /persons/{id}, PUT /persons/{id}, DELETE /persons/{id}
- Search: GET /persons/search (pagination, fuzzy, mask_sensitive)
- Matching: POST /persons/match
- Dedup: POST /persons/check-duplicates, POST /persons/merge, POST /persons/deduplicate
- Privacy: GET /persons/{id}/export, GET /persons/{id}/masked
- Audit: GET /persons/{id}/audit, GET /audit/recent, GET /audit/user
- OpenAPI/Swagger at /swagger-ui

## Phase 6: FHIR R5 Support (Partial)

- FhirPerson resource model with all standard fields
- Bidirectional conversion (to_fhir_person / from_fhir_person)
...
- DieselPersonRepository: full CRUD with transactions
...
- PersonEvent: Created, Updated, Deleted, Merged, Linked, Unlinked
...
- `tax_id` field on Person (CPF, SSN, TIN)
...
- Real-time during POST /persons (returns 409 Conflict with matches)
- POST /persons/check-duplicates (explicit check)
...
- mask_person(): masks SSN, tax ID, passport, DL, phone (shows last 4)
- GET /persons/{id}/export: GDPR data export (full JSON)
- GET /persons/{id}/masked: masked person view
