...
The Main Person Service (MPI) is a critical healthcare system that maintains a
...
│   │   └── mod.rs         # Event types
│   ├── models/
│   │   ├── person.rs     # Person model (with tax_id, documents, emergency_contacts)
...
**Create Person (with duplicate detection):**
...
**Search Persons (with pagination and masking):**

```bash
curl "http://localhost:8080/api/persons/search?q=Smith&limit=10&offset=0&fuzzy=true&mask_sensitive=true"
```
...
cargo test --lib test_person_matching        # Specific test
...
- GDPR Data Export: Full person data export endpoint
...
- **Person Create**: ~50ms (includes DB + search index + duplicate check)
- **Person Read**: ~5ms
