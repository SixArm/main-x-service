//! Benchmarks for person matching algorithms

use criterion::{criterion_group, criterion_main, Criterion, black_box};
use chrono::{NaiveDate, Utc};
use uuid::Uuid;

use main_person_service::models::*;
use main_person_service::matching::*;
use main_person_service::matching::algorithms::{
    name_matching, dob_matching, gender_matching,
    address_matching,
    tax_id_matching, document_matching,
};
use main_person_service::matching::phonetic;
use main_person_service::config::MatchingConfig;
...
fn bench_full_person_matching(c: &mut Criterion) {
    ...
    c.bench_function("match_persons_pair", |b| {
        ...
        b.iter(|| {
            matcher.match_persons(black_box(&person), black_box(&candidate)).unwrap()
        })
    });
    ...
    c.bench_function("find_matches_10_candidates", |b| {
        b.iter(|| {
            matcher.find_matches(black_box(&person), black_box(&candidates_10)).unwrap()
        })
    });
    ...
    c.bench_function("find_matches_100_candidates", |b| {
        b.iter(|| {
            matcher.find_matches(black_box(&person), black_box(&candidates_100)).unwrap()
        })
    });
    ...
    c.bench_function("find_matches_1000_candidates", |b| {
        b.iter(|| {
            matcher.find_matches(black_box(&person), black_box(&candidates_1000)).unwrap()
        })
    });
}
