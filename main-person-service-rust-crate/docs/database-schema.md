...
This document describes the PostgreSQL database schema for the Main Person Service (MPI) system. The schema is designed to support millions of person records with high performance, HIPAA compliance, and full audit trail capabilities.
...
CREATE TABLE persons (
    ...
    -- Indexes
    INDEX idx_persons_birth_date (birth_date),
    INDEX idx_persons_gender (gender),
    INDEX idx_persons_active (active),
    INDEX idx_persons_organization (managing_organization_id),
...
Stores multiple names per person (legal name, maiden name, aliases, etc.).
...
CREATE TABLE person_names (
    ...
    -- Indexes
    INDEX idx_person_names_person_id (person_id),
    INDEX idx_person_names_family (family),
    INDEX idx_person_names_is_primary (is_primary)
);
```

### person_identifiers

Stores person identifiers (MRN, SSN, driver's license, etc.).
...
CREATE TABLE person_identifiers (
    ...
    -- Indexes
    INDEX idx_person_identifiers_person_id (person_id),
    INDEX idx_person_identifiers_type (identifier_type),
    INDEX idx_person_identifiers_value (value),
    INDEX idx_person_identifiers_system_value (system, value)
);
```
...
CREATE TABLE person_addresses (
    ...
    INDEX idx_person_addresses_postal_code (postal_code),
    INDEX idx_person_addresses_city_state (city, state),
    INDEX idx_person_addresses_is_primary (is_primary)
);
```
...
CREATE TABLE person_contacts (
    ...
    -- Indexes
    INDEX idx_person_contacts_person_id (person_id),
    INDEX idx_person_contacts_system (system),
    INDEX idx_person_contacts_value (value),
    INDEX idx_person_contacts_is_primary (is_primary)
);
```

### person_links

Links between person records (duplicates, merges, references).

```sql
CREATE TABLE person_links (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    person_id UUID NOT NULL REFERENCES persons(id) ON DELETE CASCADE,
    other_person_id UUID NOT NULL REFERENCES persons(id) ON DELETE CASCADE,
    ...
    UNIQUE(person_id, other_person_id, link_type),

    -- Indexes
    INDEX idx_person_links_person_id (person_id),
    INDEX idx_person_links_other_person_id (other_person_id),
    INDEX idx_person_links_link_type (link_type)
);
```
...
CREATE TABLE audit_log (
    ...
    entity_type VARCHAR(50) NOT NULL,  -- person, organization, etc.
...
Stores calculated match scores between person records.

```sql
CREATE TABLE person_match_scores (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    person_id UUID NOT NULL REFERENCES persons(id) ON DELETE CASCADE,
    candidate_id UUID NOT NULL REFERENCES persons(id) ON DELETE CASCADE,
    ...
    -- Indexes
    INDEX idx_match_scores_person_id (person_id),
...
For very large deployments (10M+ persons), consider:

- Partitioning `audit_log` by timestamp (monthly partitions)
- Partitioning `person_match_scores` if storing all calculated scores
...
ANALYZE persons;
ANALYZE person_names;
ANALYZE person_identifiers;
```
...
CREATE POLICY person_access ON persons
    FOR ALL
...
1. Create tables in dependency order (organizations before persons)
...
1. **Cascading Deletes**: Child records (names, addresses) cascade when person deleted
...
5. **Soft Deletes**: Never hard delete persons (HIPAA requirement)
...
| ------------------- | ---- | ------------ | ---------- |
| persons            | 10M  | 500 bytes    | 5 GB       |
| person_names       | 15M  | 300 bytes    | 4.5 GB     |
| person_identifiers | 30M  | 200 bytes    | 6 GB       |
| person_addresses   | 20M  | 250 bytes    | 5 GB       |
| person_contacts    | 30M  | 200 bytes    | 6 GB       |
| **Total**           |      |              | **~27 GB** |
