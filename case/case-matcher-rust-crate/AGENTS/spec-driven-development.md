...
The case-matcher spec uses the §1–§25 matcher-crate shape (distinct
...
| How `case-service` consumes the crate | §20 |
...
| §19–§21 quality / consumption / compat | top-level docs, integration with `case-service` |
...
1. Tick the box: `[x]`.
2. Add a `CHANGELOG.md` entry under the latest version.
...
- **Adding a deterministic-identifier scheme without bridge-test
  coverage** in `case-service`. A false positive at score 1.0 is
  the worst-case bug.
...
- **Using `unwrap` / `expect` in library code.** Total functions
  only.
- **Scoring a `case_number` across agencies.** Per §11, the
  case-number component MUST be gated on agency-key equality
  (`CV-2024-001234` at one agency != the same number at another).
- **Scoring `priority` or `opened_date`.** Per §14 they are data only.
