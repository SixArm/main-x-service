...
//!
//! Interprets the fuzz bytes as a JSON `[case_a, case_b]` tuple,
//! then runs `MatchingEngine::match_cases`. Pins the two load-bearing
...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok((a, b)) = serde_json::from_slice::<(Case, Case)>(data) else {
        return;
    };

    let engine = MatchingEngine::new(MatchConfig::default());
    // Run both orderings so either argument position is exercised.
    for result in [engine.match_cases(&a, &b), engine.match_cases(&b, &a)] {
        assert!(
