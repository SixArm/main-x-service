...
impl Model {
    ...
    /// and double-shipping the same rows — the second instance simply skips
    /// the locked rows and moves on.
    ///
    /// # Errors
    ///
    /// When the query fails.
...
mod tests {
    ...
    fn from_envelope_maps_every_column_and_keeps_the_full_payload() {
        ...
        assert_eq!(row.payload["name"], "Housing benefit appeal");
        assert_eq!(row.payload["entity"], "case");
