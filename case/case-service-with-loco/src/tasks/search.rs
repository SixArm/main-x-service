...
pub async fn reindex(db: &DatabaseConnection, batch: u64) -> Result<ReindexReport> {
    let engine = crate::search::engine()
        .ok_or_else(|| Error::string("search index unavailable; check CASE_SEARCH_INDEX_PATH"))?;
    engine.clear()?;

    let mut report = ReindexReport::default();
    // Paginate rather than `all()`: the table can be far larger than the
    // process's memory budget.
    let mut pages = cases::Entity::find()
    ...
    while let Some(rows) = pages.fetch_and_next().await? {
        for row in rows {
            ...
            match row.to_case() {
                Ok(case) => {
                    engine.index_case(row.pid, &case)?;
                    report.indexed += 1;
                }
                Err(err) => {
                    report.skipped += 1;
...
pub const BOOT_REINDEX_ENV: &str = "CASE_SEARCH_BOOT_REINDEX";
