//! Prometheus metrics for the case service.
//!
...
//! `Metrics::global().case_created_total.inc()` in the cases controller).
...
//! | `case_updated_total` | counter | — |
//! | `case_deleted_total` | counter | — |
//! | `case_merged_total` | counter | — |
...
/// Process-wide Prometheus registry and the standard case-service metric
/// handles.
///
...
pub struct Metrics {
    ...
    /// Count of case records created (`POST /api/cases`).
    pub case_created_total: Counter,
    /// Count of case records updated (`PUT /api/cases/{pid}`).
    pub case_updated_total: Counter,
    /// Count of case records soft-deleted (`DELETE /api/cases/{pid}`).
    pub case_deleted_total: Counter,
    /// Count of case records merged (`POST /api/cases/merge`).
    pub case_merged_total: Counter,
...
impl Metrics {
    ...
    fn new() -> Self {
        ...
        let case_created_total = Counter::with_opts(Opts::new(
            "case_created_total",
            "Total case records created.",
        ))
        .expect("static counter opts are always valid");
        let case_updated_total = Counter::with_opts(Opts::new(
            "case_updated_total",
            "Total case records updated.",
        ))
        .expect("static counter opts are always valid");
        let case_deleted_total = Counter::with_opts(Opts::new(
            "case_deleted_total",
            "Total case records soft-deleted.",
        ))
        .expect("static counter opts are always valid");
        let case_merged_total =
            Counter::with_opts(Opts::new("case_merged_total", "Total case records merged."))
                .expect("static counter opts are always valid");
        ...
        for collector in [
            Box::new(case_created_total.clone()) as Box<dyn prometheus::core::Collector>,
            Box::new(case_updated_total.clone()),
            Box::new(case_deleted_total.clone()),
            Box::new(case_merged_total.clone()),
        ] {
            registry
                .register(collector)
    ...
    /// The process-wide case-service metrics, initialised on first access.
    ///
...
mod tests {
    ...
    fn render_yields_valid_prometheus_text() {
        ...
        metrics.case_created_total.inc();
        ...
        assert!(
            body.contains("# HELP case_created_total"),
            "missing HELP for case_created_total; got: {body}"
        );
        assert!(
            body.contains("# TYPE case_created_total counter"),
            "missing TYPE for case_created_total; got: {body}"
        );
        // The incremented sample is present and non-zero.
        assert!(
            body.contains("case_created_total"),
            "missing case_created_total sample; got: {body}"
        );
