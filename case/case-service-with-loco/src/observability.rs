...
//! OTLP export** (repo `tasks.md` PRO-H12, slice 6 of 7).
//!
//! Ported from [`care-pathway-service`]'s `src/observability.rs` (slice 5) —
//! itself a port of [`organization-service`]'s (slice 4), itself
//! [`course-service`]'s, itself [`person-service`]'s, itself
//! [`link-graph-service`]'s, the family's first working OTLP pipeline
//! (`agents/share/rust-tracing-opentelemetry-stack.md`). This crate
//! carried **no** `src/observability` module at all before this change,
//! and is the **third of the four loco-idiomatic registries**
//! (organization, care-pathway, case, portfolio — `src/controllers/`, not
//! `src/api/rest/`) to carry it: organization's slice 4 and
//! care-pathway's slice 5 both found exactly one router-construction
//! surface, and that shape is confirmed here too, not assumed (see "What
//! differs from the person-style ports" below).
//!
...
//! | `OTLP_SERVICE_NAME` | `case-service` | `service.name` resource attribute |
//! | `OTLP_ENDPOINT` | `http://localhost:4317` | OTLP/gRPC collector endpoint |
//!
//! These three variables are **deliberately not `CASE_`-prefixed**,
//! matching every other crate that carries this pipeline and
...
//! as organization's and care-pathway's. This crate also declares no
//! `tonic` dependency of its own (no gRPC stub —
...
//! course's, organization's, and care-pathway's did not.
//!
//! Like care-pathway, this crate carries its own IEC 62304 SOUP register
//! (`compliance/soup.tsv`, `src/compliance/soup.rs`'s
//! `every_direct_dependency_is_annotated` test), so this port needed the
//! same bookkeeping step: every new dependency below (main and dev) is
...
pub const DEFAULT_SERVICE_NAME: &str = "case-service";
...
fn http_metrics() -> &'static HttpMetrics {
    ...
    METRICS.get_or_init(|| {
        let meter = global::meter("case-service");
        HttpMetrics {
            duration: meter
...
/// embed record UUIDs (`/api/cases/{pid}`, …), so a per-path label would
/// be unbounded cardinality — the same reasoning link-graph-service
/// applied to its own `EntityRef`-shaped paths, here for plain UUID path
/// segments instead.
///
...
mod tests {
    ...
    fn exporter_diagnostics_are_added_to_a_default_filter() {
        ...
        let base = || tracing_subscriber::EnvFilter::new("case_service=info");
    ...
    fn an_operator_supplied_filter_is_left_alone() {
        ...
        assert_eq!(untouched.to_string(), "case_service=info");
    }
    ...
    fn explicit_service_name_and_endpoint_are_honoured() {
        let config = TelemetryConfig::resolve(
            Some("case-service-canary"),
            Some("http://collector.internal:4317"),
        );
        assert_eq!(config.service_name, "case-service-canary");
        assert_eq!(
    ...
    fn resource_carries_service_name_and_version() {
        ...
        let name = resource
            ...
            .expect("service.name");
        assert_eq!(name.as_str(), "case-service");
