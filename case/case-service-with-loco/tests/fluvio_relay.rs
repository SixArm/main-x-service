...
//! (BUS-1; `agents/share/event-bus.md` §10 "Fluvio-gated" tests).
//!
...
use case_service::models::_entities::event_outbox;
use case_service::relay::{self, FluvioSink};
use case_service::streaming;
...
async fn fluvio_relay_publishes_an_outbox_row_to_a_real_topic() {
    let endpoint = std::env::var("CASE_FLUVIO_ENDPOINT")
        .expect("set CASE_FLUVIO_ENDPOINT to the broker's SC address (e.g. 127.0.0.1:9103)");

    // Durable transport, so `create_and_emit` writes an outbox row.
    unsafe {
        std::env::set_var("CASE_EVENT_TRANSPORT", "outbox");
    }

    request::<App, _, _>(|_request, ctx| async move {
        let case = Case::new("Fluvio Relay Round Trip");
        let model = streaming::create_and_emit(&ctx.db, &case, None)
            .await
            .expect("create_and_emit under outbox transport");

        let sink = FluvioSink::connect(&endpoint, "mxi.case.events.test")
            .await
