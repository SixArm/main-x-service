...
//!
//! Ported from care-pathway-service's `tests/otlp_export.rs` (itself from
//! organization-service's, itself course-service's, itself
...
use case_service::observability::{TelemetryConfig, build_meter_provider, build_tracer_provider};
...
async fn a_tracing_span_reaches_a_real_otlp_collector() {
    ...
    let config = TelemetryConfig {
        service_name: "case-service-otlp-test".to_string(),
    ...
    {
        let layer = tracing_opentelemetry::layer().with_tracer(provider.tracer("case-service"));
        let subscriber = tracing_subscriber::registry().with(layer);
        tracing::subscriber::with_default(subscriber, || {
            let span = tracing::info_span!("cases.get", case.id = "0c4f1e2a-test");
            let _entered = span.enter();
            tracing::info!("serving a case read");
        });
    }
    ...
    assert!(
        service_names.contains(&"case-service-otlp-test".to_string()),
    ...
    let span = spans
        .iter()
        .find(|span| span.name == "cases.get")
        .unwrap_or_else(|| {
            panic!(
                "exported spans did not include `cases.get`: {:?}",
                spans.iter().map(|span| &span.name).collect::<Vec<_>>()
            )
        });
    assert!(
        span.attributes.iter().any(|kv| kv.key == "case.id"),
...
async fn a_metric_reaches_a_real_otlp_collector() {
    ...
    let meter = provider.meter("case-service");
    let histogram = meter
