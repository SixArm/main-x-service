...
This phase implements a comprehensive event streaming and audit logging system for the Main Place Index (MPI), providing critical infrastructure for production location management environments. The implementation enables real-time event notification, complete audit trails for compliance, and integration capabilities with downstream systems.

## Task Description

Implement event-driven architecture and audit logging to capture, publish, and persist all place data lifecycle events within the MPI system. This includes:

1. **Event Publisher Infrastructure**: Create a thread-safe event publishing system that broadcasts place lifecycle events
2. **Audit Log Repository**: Implement database-backed audit logging for compliance and forensic analysis
3. **Repository Integration**: Enhance the place repository to automatically publish events and log audits
...
1. **Event-Driven Architecture**: Enable real-time notification of place data changes to downstream systems
2. **Compliance**: Meet data protection regulatory requirements (GDPR, CCPA) for audit trails
3. **Observability**: Provide complete visibility into all data modifications
4. **System Integration**: Allow other systems to react to place data changes
...
Location management systems must maintain complete audit trails of all place data modifications:

- **GDPR Article 30**: Requires records of processing activities for location data
- **GDPR Article 5(2)**: Accountability principle - demonstrate compliance with data protection
- **CCPA**: California Consumer Privacy Act requirements for location data handling
- **LGPD**: Brazilian data protection requirements for geographic data
- **National data protection regulations**: Many countries have additional audit requirements for location data

### System Integration

The MPI acts as the authoritative source for place identity. Other systems need to know when:

- New places are registered
- Place attributes are updated (name, address, coordinates)
- Places are merged (duplicate resolution)
- Place records are linked/unlinked
- Records are closed or archived

Event streaming enables:

- Real-time synchronization with GIS systems and mapping platforms
- Analytics pipeline updates
- Cache invalidation in downstream systems
- Workflow triggering (e.g., notify data stewards of attribute changes)

### Operational Benefits

- **Debugging**: Trace the history of place record changes
...
- **Dispute Resolution**: Provide evidence for data accuracy disputes
...
Defined comprehensive place lifecycle events:

```rust
pub enum PlaceEvent {
    Created { place: Place, timestamp: DateTime<Utc> },
    Updated { place: Place, timestamp: DateTime<Utc> },
    Deleted { place_id: Uuid, timestamp: DateTime<Utc> },
    Merged { source_id: Uuid, target_id: Uuid, timestamp: DateTime<Utc> },
    Linked { place_id: Uuid, linked_id: Uuid, timestamp: DateTime<Utc> },
    Unlinked { place_id: Uuid, unlinked_id: Uuid, timestamp: DateTime<Utc> },
    Closed { place_id: Uuid, timestamp: DateTime<Utc> },
}
```

Each event includes:

- Event-specific data (place record or IDs)
- Timestamp for temporal ordering
- Helper methods: `timestamp()`, `place_id()`
...
- `get_logs_for_entity()`: Retrieve audit history for a specific place
...
Enhanced `DieselPlaceRepository` with event and audit capabilities:

**Builder Pattern:**

```rust
let repository = DieselPlaceRepository::new(pool)
...
fn create(&self, place: &Place) -> Result<Place> {
    ...
    self.publish_event(PlaceEvent::Created {
        place: result.clone(),
        timestamp: Utc::now(),
    });

    // 3. Log audit trail
    if let Ok(place_json) = serde_json::to_value(&result) {
        self.log_audit("CREATE", result.id, None, Some(place_json), &AuditContext::default());
    }
...
fn update(&self, place: &Place) -> Result<Place> {
    ...
    let old_place = self.get_by_id(&place.id)?;

    // 2. Execute database transaction
    let result = conn.transaction(|conn| { ... })?;

    // 3. Publish event
    self.publish_event(PlaceEvent::Updated { ... });

    // 4. Log audit with old and new values
    if let Some(old_json) = old_place.as_ref().and_then(|p| serde_json::to_value(p).ok()) {
...
fn delete(&self, id: &Uuid) -> Result<()> {
    // 1. Get old values for audit
    let old_place = self.get_by_id(id)?;
    ...
    self.publish_event(PlaceEvent::Deleted {
        place_id: *id,
        timestamp: Utc::now(),
    });

    // 4. Log audit
    if let Some(old_place) = old_place {
        if let Ok(old_json) = serde_json::to_value(&old_place) {
...
pub struct AppState {
    ...
    pub matcher: Arc<dyn PlaceMatcher>,
...
impl AppState {
    pub fn new(db_pool, search_engine, matcher, config) -> Self {
        ...
        // 3. Create place repository with dependencies
...
- Place data mutations should succeed even if audit/events fail
...
```rust
fn publish_event(&self, event: PlaceEvent) {
...
let repo = DieselPlaceRepository::new(pool);
...
4. **Event Compression**: Compress large place records in events
...
- Audit logs contain location data that may be subject to data protection regulations
- Must be secured with same rigor as place data
...
- Events may contain sensitive location data
...
- Article 30: Records of processing activities for location data
- Article 5(2): Accountability (demonstrate compliance with data protection principles)
- Recital 39: Processing records for demonstrating compliance
- Article 17: Right to erasure audit trail (track deletion requests)

### Additional Data Protection Standards Met

- CCPA: California Consumer Privacy Act audit requirements for location data
- LGPD: Brazilian data protection audit requirements
...
Phase 8 establishes production-grade observability infrastructure for the Main Place Index:

- **Event Streaming**: Real-time place lifecycle event publication
...
The implementation balances immediate functionality with long-term flexibility, using in-memory implementations that can be swapped for production-grade systems (Kafka, etc.) without code changes. The non-blocking design ensures core place operations remain performant while building complete observability.

This phase transforms the MPI from a standalone system into an integration-ready, auditable, compliant location data management platform.
