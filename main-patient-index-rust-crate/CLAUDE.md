...
The Main Patient Index (MPI) is a critical healthcare system that maintains a
...
curl -X POST http://localhost:8080/api/patients/merge \
  -H "Content-Type: application/json" \
  -d '{ "main_patient_id": "uuid-master", "duplicate_patient_id": "uuid-dup", "merge_reason": "Confirmed duplicate" }'
```
