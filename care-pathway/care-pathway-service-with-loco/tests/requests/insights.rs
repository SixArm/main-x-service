...
async fn insight_views_round_trip() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        let coverage: Value = request
            ...
            .await
            .json();
        let gap_rules: Vec<&str> = coverage["gaps"]
            .as_array()
            .unwrap()
            .iter()
        ...
        assert_eq!(
            gap_rules
                .iter()
                .filter(|r| **r == "no_emergency_pathway")
                .count(),
            2
        );
        assert_eq!(
            gap_rules
                .iter()
                .filter(|r| **r == "no_primary_care_pathway")
                .count(),
            1
        );

        // ── Variants: E11 offered by two providers.
        let variants: Value = request
            .get("/api/care-pathways/insights/variants")
            .await
            .json();
        let e11 = variants["variants"]
            .as_array()
            .unwrap()
            .iter()
            .find(|v| v["condition"] == "Icd10:E11")
            .expect("E11 variant")
            .clone();
        ...
        let languages: Value = request
            .get("/api/care-pathways/insights/languages")
            .await
            .json();
        assert_eq!(languages["by_language"]["en"], 3);
        assert_eq!(languages["by_language"]["fr"], 1);
        let single: Vec<&str> = languages["single_language_conditions"]
            .as_array()
            .unwrap()
            .iter()
            .filter_map(|c| c["condition"].as_str())
            .collect();
