...
//!
//! Ported from organization-service's `tests/otlp_middleware.rs`.
//! `tests/otlp_export.rs` proves the export pipeline; this proves the
//! thing the service actually runs — the exact
//! [`observability::trace_mw`](care_pathway_service::observability::trace_mw)
...
use care_pathway_service::observability::{TelemetryConfig, build_tracer_provider, trace_mw};
...
async fn a_served_request_exports_a_span_and_returns_its_traceparent() {
    ...
    let config = TelemetryConfig {
        service_name: "care-pathway-service-mw-test".to_string(),
    ...
    tracing_subscriber::registry()
        .with(tracing_opentelemetry::layer().with_tracer(provider.tracer("care-pathway-service")))
        .with(tracing_subscriber::EnvFilter::new(
            "care_pathway_service=trace,otlp_middleware=trace",
        ))
        .init();
