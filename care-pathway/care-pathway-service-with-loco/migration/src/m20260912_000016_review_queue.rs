...
//! duplicate-review queue, adopted here as part of the native bulk
//! import/export rollout (§13 T-10) to route a **keyless** bulk-import
//! row's likely duplicate to an operator
//! (`agents/share/bulk-import-export.md` §6) — mirroring the
//! person/worker/place/thing/organization registries and case's own BLK-5
//! precedent. Pairs are stored normalized (`record_id_a < record_id_b`)
//! under a UNIQUE constraint so re-scans upsert in place and decided rows
//! keep their decision.
//!
//! Like case (and unlike organization's original migration), `provenance`
//! is part of the initial table shape rather than a follow-up migration:
//! care-pathway has no pre-existing batch-deduplication scan to preserve
