...
/// `CARE_PATHWAY_EVENT_TRANSPORT=outbox`**; this is stated in the entity
...
impl Model {
    ...
    pub async fn record_with_context<C: ConnectionTrait>(
        ...
        // Keyed over the *SHA-256* pre-image, so the MAC is bound to the
        // same chain position the primary digest is.
    ...
    /// Disclosure entries for one care pathway, most-recent first — the
