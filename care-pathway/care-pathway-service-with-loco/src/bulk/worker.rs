//! The loco `BackgroundWorker` that drains **native bulk import/export**
//! `bulk_jobs` rows (§13 T-10; `agents/share/bulk-import-export.md` §3).
//!
//! Distinct from [`crate::workers::bulk_export`] (the FHIR Bulk Data
//! `$export` worker) even though both drain the same `bulk_jobs` table:
//! this worker is selected by `job.format` naming a native-bulk format
//! (`jsonl`/`csv`/`tsv`), the FHIR one by `format = "ndjson"`.
//!
...
use crate::bulk::{BulkFormat, BulkKind, MaskingProfile, error_report};
use crate::models::audit_logs::Model as AuditModel;
use crate::models::bulk_jobs;

/// The loco background worker that runs one native bulk job.
...
pub async fn run_bulk_job(ctx: &AppContext, job_id: Uuid) -> Result<()> {
    ...
    if bulk_jobs::is_terminal(&job.status) {
        // A client may have cancelled between enqueue and pickup.
        tracing::info!(%job_id, status = %job.status, "job already terminal");
        return Ok(());
    }
    let job = job.start(&ctx.db).await?;

    let result = match crate::bulk::store::from_env().await {
        Ok(store) => match BulkKind::parse(&job.kind) {
    ...
    if let Err(ref e) = result {
        ...
        // Best-effort: mark the job failed; keep the original error. The
        // job row is re-fetched since `job` above was consumed by
        // `run_import`/`run_export`.
        if let Ok(Some(current)) = bulk_jobs::Model::find_by_id(&ctx.db, job_id).await {
            let _ = current.fail(&ctx.db, &e.to_string()).await;
        }
    }
    result
}
...
async fn run_import(
    ...
    bulk_jobs::Model::find_by_id(db, job.id)
        .await?
        .ok_or_else(|| Error::Message(format!("bulk job {} vanished mid-run", job.id)))?
        .finish_import(
            db,
            outcome.rows_total,
            outcome.rows_created,
            outcome.rows_upserted,
            outcome.rows_to_review,
            outcome.rows_errored,
            error_report_url,
        )
        .await?;
...
async fn run_export(
    ...
    let rows_total_i64 = i64::try_from(rows_total).unwrap_or(i64::MAX);
    bulk_jobs::Model::find_by_id(db, job.id)
        .await?
        .ok_or_else(|| Error::Message(format!("bulk job {} vanished mid-run", job.id)))?
        .complete(db, &result_url, rows_total_i64)
        .await?;
    Ok(())
}
...
mod tests {
    ...
    fn export_params_from_json_falls_back_on_unknown_tokens() {
        ...
        assert_eq!(parsed.query.as_deref(), Some("stroke"));
