...
pub fn parse(
    ...
    let target = match vars.get("target").map_or("all", String::as_str) {
        ...
        "records" | "care_pathways" => Target::Records,
        other => {
            return Err(format!(
...
async fn resign_audit(
    ...
    let rows = audit_logs::Entity::find()
        .filter(audit_logs::Column::Mac.is_not_null())
...
async fn resign_records(
    ...
    let rows = care_pathways::Entity::find()
        .filter(care_pathways::Column::ContentMac.is_not_null())
        .order_by_asc(care_pathways::Column::Id)
        .limit(limit)
        .all(&ctx.db)
        .await?;

    for row in rows {
        let preimage = record_integrity::preimage(&record_integrity::input_for(&row));
        ...
        report.record(&decision, &row.pid.to_string());

        if apply && decision == Decision::Resign {
            let fresh = mac::tag(mac::Domain::Record, &preimage);
            let mut active: care_pathways::ActiveModel = row.into();
