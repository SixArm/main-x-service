...
pub async fn reindex(db: &DatabaseConnection, batch: u64) -> Result<ReindexReport> {
    let engine = crate::search::engine().ok_or_else(|| {
        Error::string("search index unavailable; check CARE_PATHWAY_SEARCH_INDEX_PATH")
    })?;
    engine.clear()?;
    ...
    let mut pages = care_pathways::Entity::find()
    ...
    while let Some(rows) = pages.fetch_and_next().await? {
        for row in rows {
            report.scanned += 1;
            match row.to_pathway() {
                Ok(pathway) => {
                    engine.index_pathway(row.pid, &pathway)?;
                    report.indexed += 1;
                }
                Err(err) => {
                    report.skipped += 1;
...
pub const BOOT_REINDEX_ENV: &str = "CARE_PATHWAY_SEARCH_BOOT_REINDEX";
...
impl Task for SearchReindex {
    ...
    async fn run(&self, ctx: &AppContext, vars: &task::Vars) -> Result<()> {
        ...
        println!(
            "reindexed {} of {} care_pathways ({} skipped)",
