...
//! OTLP export** (repo `tasks.md` PRO-H12, slice 5 of 7).
//!
//! Ported from [`organization-service`]'s `src/observability.rs` (slice 4) —
//! itself a port of [`course-service`]'s, itself [`person-service`]'s,
//! itself [`link-graph-service`]'s, the family's first working OTLP
//! pipeline (`agents/share/rust-tracing-opentelemetry-stack.md`). This
...
//! `src/api/rest/`) to carry it: organization's slice 4 was the first,
//! and its "exactly one router-construction surface" shape is confirmed
//! here too, not assumed (see "What differs from the person-style ports"
//! below).
//!
...
//! | `OTLP_SERVICE_NAME` | `care-pathway-service` | `service.name` resource attribute |
//! | `OTLP_ENDPOINT` | `http://localhost:4317` | OTLP/gRPC collector endpoint |
//!
//! These three variables are **deliberately not `CARE_PATHWAY_`-prefixed**,
...
//! as organization's. This crate also declares no `tonic` dependency of
//! its own (no gRPC stub — `agents/share/overview.md`'s capability
//! matrix), so the in-process OTLP collector tests below need no
//! `package = "…"` rename, exactly as course's and organization's did not.
...
pub const DEFAULT_SERVICE_NAME: &str = "care-pathway-service";
...
fn http_metrics() -> &'static HttpMetrics {
    ...
    METRICS.get_or_init(|| {
        let meter = global::meter("care-pathway-service");
        HttpMetrics {
            duration: meter
...
/// embed record UUIDs (`/api/instances/{pid}`, `/api/care-pathways/{pid}`,
...
mod tests {
    ...
    fn exporter_diagnostics_are_added_to_a_default_filter() {
        ...
        let base = || tracing_subscriber::EnvFilter::new("care_pathway_service=info");
    ...
    fn an_operator_supplied_filter_is_left_alone() {
        ...
        assert_eq!(untouched.to_string(), "care_pathway_service=info");
    }
    ...
    fn explicit_service_name_and_endpoint_are_honoured() {
        let config = TelemetryConfig::resolve(
            Some("care-pathway-service-canary"),
            Some("http://collector.internal:4317"),
        );
        assert_eq!(config.service_name, "care-pathway-service-canary");
        assert_eq!(
    ...
    async fn building_an_exporter_does_not_require_a_live_collector() {
        let config = TelemetryConfig {
            service_name: "care-pathway-service".to_string(),
    ...
    fn resource_carries_service_name_and_version() {
        ...
        let name = resource
            ...
            .expect("service.name");
        assert_eq!(name.as_str(), "care-pathway-service");
