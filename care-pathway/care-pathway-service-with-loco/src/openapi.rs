    ...
    json!({
        "/api/instances/{pid}/time-analysis": {
            "get": {
                ...
                "description": "Lead time, value time, process time, touch time, the value-adding ratio, and coverage. The denominator is elapsed calendar time, never the sum of recorded activity, so unrecorded time counts as non-value-adding; coverage_ratio and confidence say how much of the journey was mapped at all. `anchors` (T-14d) is this instance's first-reached timestamp per stage (null if never reached); `delays` is the elapsed time between each adjacent pair of stages, with a `reason` when either side was never reached. `conformance` (T-14i) scores the declared steps' completion order (instance_steps.position) against this instance's own enrolled template only, never a discovered model: declared_pairs/pairs_in_order/ratio, a null ratio with a reason when fewer than two steps are declared, per-pair verdicts (in_order/inverted/skipped -- a step with no done_on yet is skipped, never counted as an inversion), skipped_positions, completed_after_closure (steps done after the instance's own closed_on), and escalation_events (disclosed alongside the ratio, never subtracted from it -- a journey may need more than its template foresaw).",
                "parameters": [instance_pid],
