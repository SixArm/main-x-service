...
use crate::compliance::DataProtection;
...
impl AccessContext {
    ...
    /// Whether this access sends data outside the declared residency
    /// region — a GDPR Ch. V transfer.
    #[must_use]
    pub fn is_cross_border(&self) -> bool {
        DataProtection::get().is_cross_border(self.destination.as_deref())
    }
    ...
    pub fn to_json(&self) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "residency": dp.residency,
            "lawful_basis": dp.lawful_basis,
            "art9_condition": dp.art9_condition,
            "transfer_safeguard": dp.transfer_safeguard,
        })
    }
}
...
mod tests {
    ...
    fn context_json_carries_declaration_and_derivation() {
        ...
        // Standing declarations are present even when undeclared, so the
        // row records that nothing was configured rather than omitting it.
        for key in [
            "residency",
            "lawful_basis",
            "art9_condition",
