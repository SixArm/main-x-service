...
//! attacker with SQL access could edit a stored pathway and, as long as
...
//! it.
//!
//! Each `care_pathways` row carries a `content_hash` — a SHA-256 over its
...
/// The same record's digest under **BLAKE3**, over the byte-identical
/// pre-image as [`record_hash`] — see `spec/12-compliance.md` §12.4z for
/// why both algorithms are kept.
#[must_use]
...
#[must_use]
pub fn input_for(row: &care_pathways::Model) -> RecordInput<'_> {
    RecordInput {
        pid: row.pid,
...
#[must_use]
pub fn hash_of(row: &care_pathways::Model) -> String {
    record_hash(&input_for(row))
}

/// The BLAKE3 digest a stored row *should* carry.
#[must_use]
pub fn hash_of_blake3(row: &care_pathways::Model) -> String {
...
pub struct RecordMismatch {
    /// The record's public id.
    pub pid: String,
    /// The record's display name, to make the report actionable without a
    /// second lookup.
    pub name: String,
}
...
#[must_use]
pub fn verify(rows: &[care_pathways::Model]) -> RecordIntegrityReport {
...
mod tests {
    ...
    fn record_hash_is_deterministic() {
        let r = row(1, "Acute Stroke Care Pathway");
    ...
    fn version_tag_is_bound() {
        ...
        assert_eq!(RECORD_HASH_VERSION, "r1");
    }
}
