...
//! and justified. The FDA makes the machine-readable half a premarket
//! requirement for cyber devices (FD&C Act §524B): a **software bill of
//! materials**.
//!
...
//! cannot supply. `tests/traceability.rs` and the unit tests below pin the
//! two together, so an added dependency fails the build until it is
//! annotated.
//!
//! ## Determinism
//!
//! The rendered SBOM contains no timestamp and no random serial number, so
//! two builds of the same source produce byte-identical output — the
//! property `scripts/build-reproducible.sh` depends on. `CycloneDX` makes
//! both fields optional precisely for this case.
...
pub fn sbom() -> Sbom {
    ...
    let safety = super::SafetyClass::from_env();
    ...
    Sbom {
        ...
        metadata: SbomMetadata {
            component: SbomComponent {
                ...
                description: Some("Care-pathway registry service".to_string()),
                properties: Vec::new(),
            },
            properties: vec![
                SbomProperty::new("iec62304-safety-class", format!("{safety:?}")),
...
mod tests {
    ...
    fn lockfile_contains_known_dependencies() {
        let packages = locked_packages();
        for expected in ["loco-rs", "sea-orm", "axum", "sha2", "care-pathway-matcher"] {
            assert!(
    ...
    fn manifest_parses_direct_dependencies() {
        let direct = declared_dependencies();
        for expected in ["loco-rs", "serde", "sha2", "care-pathway-matcher"] {
    ...
    /// The SBOM carries the IEC 62304 classification and build provenance
    /// — the fields that make it evidence rather than an inventory.
    #[test]
