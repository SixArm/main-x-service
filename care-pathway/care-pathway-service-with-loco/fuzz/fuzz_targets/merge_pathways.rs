...
//!
//! `POST /api/care-pathways/merge` folds a duplicate into a survivor. The inputs are
...
use care_pathway_service::merge;
use libfuzzer_sys::fuzz_target;

/// Build a `CarePathway` from a byte slice, round-robining NUL-separated chunks
/// across the collections the fold unions.
fn build(data: &[u8]) -> CarePathway {
    let mut chunks = data.split(|b| *b == 0).map(String::from_utf8_lossy);
    let mut record = CarePathway::new(chunks.next().unwrap_or_default().into_owned());
    for (i, chunk) in chunks.enumerate() {
        let s = chunk.into_owned();
        match i % 4 {
            0 => record.alternate_names.push(s),
            1 => record.interventions.push(s),
            2 => record.keywords.push(s),
...
fuzz_target!(|data: &[u8]| {
    ...
    let first = merge::merge_pathways(&main, &duplicate);
    assert_eq!(first.merged.name, main.name, "merge renamed the survivor");
    assert_eq!(
        merge::merge_pathways(&main, &duplicate).merged.name,
        first.merged.name,
        "merge is not deterministic"
    );

    let second = merge::merge_pathways(&first.merged, &duplicate);
    for (field, a, b) in [
        (
            "alternate_names",
        ...
        (
            "interventions",
            first.merged.interventions.len(),
            second.merged.interventions.len(),
        ),
        (
            "keywords",
