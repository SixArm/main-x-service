...
//! random bytes are not a `CarePathway`, so the deep fields are reached only
//! once libFuzzer has learned the grammar. This target builds the `CarePathway`
...
use care_pathway_service::validation;
...
fuzz_target!(|data: &[u8]| {
    ...
    for (i, chunk) in chunks.enumerate() {
        ...
        match i % 6 {
            ...
            3 => record.same_as.push(s),
            4 => record.condition_codes.push(ConditionCode {
                system: CodeSystem::Icd10,
                code: s,
            }),
            _ => record.identifiers.push(PathwayIdentifier {
