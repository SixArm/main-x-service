...
//!
//! `POST /api/care-pathways` deserializes an attacker-supplied body into
//! `care_pathway_matcher::CarePathway` and runs `validation::problems` over it, so this
...
fuzz_target!(|data: &[u8]| {
    let Ok(record) = serde_json::from_slice::<care_pathway_matcher::CarePathway>(data) else {
        // A body that is not a `CarePathway` is rejected by the extractor long
