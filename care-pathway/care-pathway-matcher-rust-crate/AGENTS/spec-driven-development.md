...
The care-pathway-matcher spec uses the §1–§25 matcher-crate shape (distinct
...
| How `care-pathway-service` consumes the crate | §20 |
...
| §19–§21 quality / consumption / compat | top-level docs, integration with `care-pathway-service` |
...
- **Adding a deterministic-identifier scheme without bridge-test
  coverage** in `care-pathway-service`. A false positive at score 1.0 is
  the worst-case bug.
