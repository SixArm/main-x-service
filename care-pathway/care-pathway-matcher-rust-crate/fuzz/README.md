# Fuzzing — `care-pathway-matcher` (SEC-I2)
...
|---|---|
| `match_care_pathways` | Deserialize a JSON `[care_pathway_a, care_pathway_b]` tuple → `MatchingEngine::match_care_pathways`. The whole deserialize → normalize → score path; asserts finite score in `[0,1]` in both argument orders. |
| `normalize` | The pure `normalize` free functions (fold / pathway code / fold-set) over arbitrary UTF-8 — never-panic. |
...
cargo +nightly fuzz run match_care_pathways     # fuzz until a crash / Ctrl-C
