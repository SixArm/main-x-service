...
/// Blanket JWT enforcement (spec §13 T-7; family contract
/// `agents/share/jwt-enforcement.md`). With `CARE_PATHWAY_REQUIRE_AUTH`
/// on and no token, a protected `/api/*` route is `401`, while the public
/// OpenAPI doc still serves `200`. `#[serial]` because it mutates the
/// process env; the flag is read once via `OnceLock`, so this stays in
/// its own ignored test.
#[tokio::test]
#[serial]
#[ignore = "requires PostgreSQL (config/test.yaml); run with `cargo test -- --ignored`"]
async fn require_auth_gates_api_but_not_openapi() {
    // SAFETY: single-threaded test process; `#[serial]` prevents overlap.
    unsafe {
