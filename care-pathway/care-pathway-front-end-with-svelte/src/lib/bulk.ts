// Client-side rules for the native bulk import/export surface (T-10).
//
// Mirrors `care-pathway-service-with-loco/src/bulk/handlers.rs` (the REST
// layer of `agents/share/bulk-import-export.md` §4), so the operator
// learns each constraint from the form rather than from a `400`. The
// server stays authoritative — this is a pre-flight courtesy, never a
...
/**
 * File formats the service recognises for the **native** bulk surface
 * (`BulkFormat::parse`). `ndjson` also exists on the wire, but only for
 * the separate FHIR Bulk Data `$export` job sharing the same table — it
 * is never offered here.
 */
export const BULK_FORMATS = ["jsonl", "csv", "tsv"] as const;
/** One of {@link BULK_FORMATS}. */
export type BulkFormat = (typeof BULK_FORMATS)[number];

/**
 * Formats accepted for **import**. Unlike person's bulk surface (where
 * Parquet is export-only), care-pathway's native formats are symmetric —
 * every export format is also an import format — so this is the same
 * list as {@link BULK_FORMATS}, kept as its own name for parity with the
 * sibling front-ends and in case the two ever diverge.
 */
export const BULK_IMPORT_FORMATS = BULK_FORMATS;
