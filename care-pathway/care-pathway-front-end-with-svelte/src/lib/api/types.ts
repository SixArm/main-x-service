...
export interface DirectoryInsight {
  ...
  by_setting: Record<
    string,
    Array<{ pid: string; name: string; specialty?: string | null }>
  >;
...
export interface VariantsInsight {
  ...
  variants: Array<{
    ...
    by_provider: Record<
      string,
      Array<{ pid: string; name: string; jurisdiction?: string | null }>
    >;
  }>;
}
...
export interface LanguagesInsight {
  ...
  single_language_conditions: Array<{
    condition: string;
 ...
 * Rust `BulkJobView` (`src/bulk/handlers.rs`). This table is shared with
 * the FHIR Bulk Data `$export` operation, distinguished by `format`
 * (`ndjson` for FHIR; `jsonl`/`csv`/`tsv` for native).
 *
...
export interface BulkJobView {
  ...
  /** `"import"` or `"export"`. */
  kind: string;
  /** The entity — always `"care_pathway"` for this service. */
  entity: string;
   ...
   * fetchable URL despite the name.
   */
...
/** Body of `POST /api/care-pathways/export`; every field is optional. */
export interface BulkExportRequest {
  /** Output format; defaults to `jsonl` server-side. */
  format?: string;
  /** Optional full-text search query scoping the export. */
  q?: string;
  ...
  offset?: number;
  /**
   * `masked` (default) or `full`. `full` is privileged and answers
   * 401/403 when the caller lacks elevated authorisation.
   */
