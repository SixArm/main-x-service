...
// for a short-lived PASETO and forwards the request to the care-pathway
// service with `Authorization: Bearer <paseto>`. The browser thus never
// holds a token, and pages keep using the existing client unchanged
// (its base URL points here).

import type { RequestHandler } from "./$types";
import { CARE_PATHWAY_API_URL } from "$lib/server/config";
import { exchangeToken } from "$lib/server/auth";

const proxy: RequestHandler = async ({ request, params, url, locals, fetch }) => {
    const target = `${CARE_PATHWAY_API_URL}/${params.path}${url.search}`;
