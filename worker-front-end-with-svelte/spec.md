# worker-front-end-with-svelte — Living Specification
...
For the underlying service contract, see [`../worker-service-rust-crate/spec.md`](../worker-service-rust-crate/spec.md). For shared MXI guidance (REST conventions, observability, compliance), see [`../AGENTS.md`](../AGENTS.md) and [`../agents/share/`](../agents/share/).
...
Provide an operator-facing web UI for the Worker Service that exercises the full duplicate-handling workflow: search, create with real-time duplicate detection, score-based match check, manual merge, and per-record audit review.
...
- Workers list with full-text / fuzzy / phonetic search and SVAR DataGrid.
- Create worker with 409-duplicate handling that surfaces the match candidates inline.
- Detail view: identity, identifiers, addresses, telecom, emergency contacts.
- Edit form (full Worker record).
...
| **Match quality** | `definite` / `certain` / `probable` / `possible` / `unlikely` (per `worker-service-rust-crate/AGENTS/matching.md`). |
...
- **No PII logging**: never `console.log` Worker values.
...
- [x] T-2: Wire TypeScript types matching `worker-service-rust-crate/AGENTS/models.md`.
...
| Match check | ✅ |
| Merge UI | ✅ |
| Unit tests | ✅ (8 tests across `client.test.ts` + `workers.test.ts`) |
| E2E smoke | ✅ (6 tests) |
...
- **v0.3**: Auth integration (once Worker Service ships auth — §15 of service spec).
...
- [`../worker-service-rust-crate/spec.md`](../worker-service-rust-crate/spec.md)
- [`../worker-service-rust-crate/AGENTS/restful.md`](../worker-service-rust-crate/AGENTS/restful.md)
- [`../worker-service-rust-crate/AGENTS/models.md`](../worker-service-rust-crate/AGENTS/models.md)
- [`../worker-service-rust-crate/AGENTS/matching.md`](../worker-service-rust-crate/AGENTS/matching.md)
