# event-front-end-with-svelte — Living Specification
...
For the underlying service contract, see [`../event-service-rust-crate/spec.md`](../event-service-rust-crate/spec.md). For shared MXI guidance (REST conventions, observability, compliance), see [`../AGENTS.md`](../AGENTS.md) and [`../agents/share/`](../agents/share/).
...
Provide an operator-facing web UI for the Event Service that exercises the full duplicate-handling workflow: search, create with real-time duplicate detection, score-based match check, manual merge, and per-record audit review.
...
| ID | Requirement |
| --- | --- |
| FR-1 | List page MUST issue `GET /api/v1/events/search?q=…` on mount and on search submission. |
| FR-2 | List page MUST expose toggles for `fuzzy` and `phonetic`. |
| FR-3 | Create page MUST capture HTTP 409 from `POST /api/v1/events` and render the match candidates from `error.details`. |
...
| FR-8 | Match page MUST POST to `/api/v1/events/match` and render quality + score breakdown. |
| FR-9 | Merge page MUST issue a per-ID GET to render preview before POST `/api/v1/events/merge`. |
...
| Layer | Tool | Scope |
| --- | --- | --- |
| Unit | Vitest + jsdom | `ApiClient` envelope handling, `ApiError` mapping, `EventRepository` wiring. |
| E2E smoke | Playwright | Page-shell rendering for every MVP route without requiring a live service. |
| Live integration | (manual) | Run `pnpm dev` against a running `event-service-rust-crate`; click through CRUD/match/merge. |
...
- [x] T-2: Wire TypeScript types matching `event-service-rust-crate/AGENTS/models.md`.
...
| Match check | ✅ |
| Merge UI | ✅ |
| Unit tests | ✅ (8 tests across `client.test.ts` + `events.test.ts`) |
| E2E smoke | ✅ (6 tests) |
...
- **v0.3**: Auth integration (once Event Service ships auth — §15 of service spec).
