...
- The service's [`spec.md`](../event-service-rust-crate/spec.md) and [`AGENTS/`](../event-service-rust-crate/AGENTS/) describe the API contract. If a field disappears from `Event` in the service, fix `src/lib/api/types.ts` here — do not let the front-end drift.
