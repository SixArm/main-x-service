...
**No black-box output, family-wide** (repo `tasks.md` EV-5). Every
derived figure a TBA surface returns either discloses its own inputs
and reasons, or is `null` with a reason (§9's null-figure convention).
There is no third shape — no derived number is ever returned bare,
with the disclosure left to a separate call the caller has to know to
make. Portfolio's own spec is the reference statement of the property
(`project-portfolio-management/spec/time-based-analysis.md`, and the
...
**The corollary this rule exists to rule out**: a model-driven
assistant living *inside* a TBA service. It would be the one output
that could not disclose its inputs the way every other figure here
does — a forecast, a ranking, or a narrative summary generated by a
model has no `numerator`/`denominator` to report, only a claim to
trust. If a deployment ever wants one, it sits **outside** the
services, at a front-end BFF or a separate tool, calling the open API
and citing the specific endpoint and response it read — never inside
the boundary this contract governs.
