...
| [Project Portfolio Management Service](project-portfolio-management/project-portfolio-management-service-with-loco/) | Portfolio (umbrella container) + Project / Product / Program (distinct matchable record types under it) — loco.rs CRUD across **four collections** (embeds project-portfolio-management-matcher; DTO = matcher `WorkItem` type as JSONB; matching is within-kind via a hard **kind gate**) + **operational sub-resources** (goals / tasks / issues + derived timeline & burndown) + name search + audit + event streaming + OpenAPI/Swagger + record merge + PASETO verification + cross-service links + bulk import/export. Integrates auth (users/SSO), person/worker (people), organization (sponsor). | [spec](project-portfolio-management/project-portfolio-management-service-with-loco/spec/index.md) | [index](project-portfolio-management/project-portfolio-management-service-with-loco/index.md) |
...
| [Project Portfolio Management Matcher](project-portfolio-management/project-portfolio-management-matcher-rust-crate/) | Work item (Portfolio / Project / Product / Program) — **kind gate** (records of different kinds never match), name (Jaro-Winkler + Soundex), goal-title & keywords Jaccard, owner-scoped code, owner org, parent portfolio, timeframe proximity, relationships & tags Jaccard; deterministic short-circuits on Jira/Asana/Trello/MS-Project/GitHub/Linear ids / URI / UUID, same-owner code, sameAs URL | [spec](project-portfolio-management/project-portfolio-management-matcher-rust-crate/spec/index.md) | [index](project-portfolio-management/project-portfolio-management-matcher-rust-crate/index.md) |
...
| [human-capital-management](human-capital-management/spec/index.md) | All-in-one HR platform across the employee lifecycle — five pillars: talent acquisition & onboarding (requisitions/ATS pipeline, candidate pool, onboarding checklists), workforce management (time & attendance, leave, shift scheduling), HR service delivery (employee records, org chart, self-service, benefits), talent development (reviews, LMS via course-service, succession), payroll & compensation (payroll runs + payslips in minor units, benchmarking). Owns the employment relationship + operational HR state; references person / worker / organization / course identities by EntityRef. **Both editions implemented 2026-07-18 (service HCM-T1–T17: full Loco API, pure rules core, $sub self-service + salary/payslip masking, verified against Postgres 18; front-end HCM-T18/T19: SvelteKit SPA + BFF, 13-locale i18n, vitest + Playwright)** | [service-with-rust](human-capital-management/human-capital-management-service-with-rust/spec/index.md) (Loco JSON API) · [front-end-with-svelte](human-capital-management/human-capital-management-front-end-with-svelte/spec/index.md) (SvelteKit HR/self-service client) |
