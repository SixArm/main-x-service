...
| [Portfolio Service](portfolio/portfolio-service-with-loco/) | Portfolio (umbrella container) + Project / Product / Program (distinct matchable record types under it) — loco.rs CRUD across **four collections** (embeds portfolio-matcher; DTO = matcher `WorkItem` type as JSONB; matching is within-kind via a hard **kind gate**) + **operational sub-resources** (goals / tasks / issues + derived timeline & burndown) + name search + audit + event streaming + OpenAPI/Swagger + record merge + PASETO verification + cross-service links + bulk import/export. Integrates auth (users/SSO), person/worker (people), organization (sponsor). | [spec](portfolio/portfolio-service-with-loco/spec/index.md) | [index](portfolio/portfolio-service-with-loco/index.md) |
...
| [portfolio-front-end-with-svelte](portfolio/portfolio-front-end-with-svelte/) | [portfolio-service](portfolio/portfolio-service-with-loco/) | [spec](portfolio/portfolio-front-end-with-svelte/spec/index.md) | [CHANGELOG](portfolio/portfolio-front-end-with-svelte/CHANGELOG.md) |
