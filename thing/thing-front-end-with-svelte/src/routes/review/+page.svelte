<!--
  ...
  three ways over one selection:
    ...
    3. An inline comparison panel below both, fetching each side of the
       pair with its own `GET /api/things/{id}` (there is no combined
  ...
  merge. The panel therefore deep-links to `/things/merge` with both ids
  ...
  worker / place / organization. The queue/table below surface
  `detection_method` in place of a provenance column, and the breakdown
  table renders its documented empty state rather than a lie.
-->
<script lang="ts">
    ...
    let pair = $state<{ a: Thing | null; b: Thing | null } | null>(null);
    ...
    async function loadPair(item: ReviewQueueItem) {
        ...
        const [a, b] = await Promise.allSettled([
            repo.get(item.thing_id_a),
            repo.get(item.thing_id_b),
        ]);
    ...
    function pairLabel(item: ReviewQueueItem): string {
        return `${shortId(item.thing_id_a)} ↔ ${shortId(item.thing_id_b)}`;
    }

    // Render an identifier's scheme as a label, handling the Custom
    // variant (mirrors the [id] detail page's own helper).
    function identifierLabel(
        identifier: NonNullable<Thing["identifiers"]>[number],
    ): string {
        return typeof identifier.property_id === "string"
            ? identifier.property_id
            : `${translate("detail.customPrefix")}${identifier.property_id.Custom}`;
    }

    /** The first identifier as `scheme: value`; `null` when there is none. */
    function identifierLine(thing: Thing | null): string | null {
        const identifier = thing?.identifiers?.[0];
        return identifier
            ? `${identifierLabel(identifier)}: ${identifier.value}`
            : null;
    }

    /** The first same-as URL; `null` when there is none. */
    function sameAsLine(thing: Thing | null): string | null {
        return thing?.same_as?.[0] ?? null;
    }
    ...
    const compareRows = $derived(
        pair
            ? [
                  ...
                  {
                      labelKey: "detail.additionalType" as StringKey,
                      a: pair.a?.additional_type ?? null,
                      b: pair.b?.additional_type ?? null,
                  },
                  {
                      labelKey: "detail.description" as StringKey,
                      a: pair.a?.description ?? null,
                      b: pair.b?.description ?? null,
                  },
                  {
                      labelKey: "detail.url" as StringKey,
                      a: pair.a?.url ?? null,
                      b: pair.b?.url ?? null,
                  },
                  {
                      labelKey: "detail.owner" as StringKey,
                      a: pair.a?.owner ?? null,
                      b: pair.b?.owner ?? null,
                  },
                  {
                      labelKey: "detail.identifiers" as StringKey,
                      a: identifierLine(pair.a),
                      b: identifierLine(pair.b),
                  },
                  {
                      labelKey: "detail.sameAs" as StringKey,
                      a: sameAsLine(pair.a),
                      b: sameAsLine(pair.b),
                  },
              ]
    ...
    const breakdown = $derived(
        selectedItem ? breakdownRows(selectedItem.score_breakdown) : [],
    );
    const flags = $derived(
        selectedItem ? breakdownFlags(selectedItem.score_breakdown) : [],
    );
...
<p class="muted hint">{t("review.gap.provenance")}</p>
...
{#if selectedItem}
    ...
    >
            ...
            <table class="compare" data-testid="review-compare-table">
                <thead>
                    <tr>
                        ...
                        <th scope="col">
                            {t("review.compare.a")} · {shortId(item.thing_id_a)}
                        </th>
                        <th scope="col">
                            {t("review.compare.b")} · {shortId(item.thing_id_b)}
                        </th>
                    </tr>
                </thead>
                <tbody>
        ...
        {#if breakdown.length === 0 && flags.length === 0}
        ...
        {#if item.status === "confirmed"}
            ...
            <div class="row">
                <a
                    class="button"
                    href={mergeHref(item.thing_id_a, item.thing_id_b)}
                >
                    {t("review.merge.keepA")}
                </a>
                <a
                    class="button"
                    href={mergeHref(item.thing_id_b, item.thing_id_a)}
                >
