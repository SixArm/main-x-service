...
- [x] T-23: **E2E coverage for the BFF pages.** *(resolved 2026-09-06.)*
  `tests/e2e/things.spec.ts` had no Playwright smoke test for `/signin`
  or `/verify` — the outbound magic-link calls happen server-side
  (`src/lib/server/auth.ts`), so `page.route` cannot stub them.
  - **Resolved.** Ported `place-front-end-with-svelte`'s T-26 test
    infrastructure verbatim: `tests/e2e/auth-stub-server.ts` (a real
    in-process `http.Server` standing in for the authentication
    service's magic-link endpoints, with three fixed tokens driving
    the happy path / expired / unreachable-service scenarios),
    `tests/e2e/auth-stub-port.ts`, and `tests/e2e/global-setup.ts`
    (starts the stub before Playwright's own `webServer`, since
    `AUTH_API_URL` is read once at the preview server's startup).
    `playwright.config.ts` gained the `globalSetup` wiring and points
    `AUTH_API_URL` at the stub. New `tests/e2e/auth.spec.ts` (5 tests):
    the sign-in form + confirmation state, a missing-token verify, an
    expired-token verify, an unreachable-service verify, and a
    valid-token verify that establishes the session and redirects home.
  - **Found and fixed a real bug while porting the stub's third
    scenario**: `src/routes/verify/+page.server.ts` called
    `await verifyMagicLink(fetch, token)` with no `try`/`catch`. A
    network-level failure (the authentication service unreachable,
    timed out, connection reset) makes `fetch` throw rather than
    resolve — uncaught, that propagated out of `load` and SvelteKit
    rendered its generic 500 page instead of this route's own friendly
    UI. This is the exact same bug class `place-front-end-with-svelte`
    found and fixed as its own T-26; ported the identical fix here (a
    `try`/`catch` around the call, a new `"serviceUnavailable"` error
    variant, and its message in `+page.svelte`). Verified to **fail**
    with the `try`/`catch` reverted (SvelteKit's 500 page, not the
    friendly message) and **pass** with it restored.
  - **Acceptance:** 5 new Playwright tests, all green; the full suite
    (20 tests total, up from 15) unaffected. Three-part change: spec
    (here) + code (the bug fix) + test.
