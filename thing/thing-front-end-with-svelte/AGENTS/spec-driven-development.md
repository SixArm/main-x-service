...
1. **Service spec** —
   [`../../thing-service-rust-crate/spec.md`](../../thing-service-rust-crate/spec/index.md)
   — describes the API contract. If `Thing` loses a field server-side,
...
- **Changing the wire-format type to "fix" a server bug.** If
  `Thing` doesn't match `thing-service`'s `Thing`, the fix is in
