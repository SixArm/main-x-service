...
//! [`observability::trace_mw`](thing_service::observability::trace_mw)
...
use thing_service::observability::{TelemetryConfig, build_tracer_provider, trace_mw};
...
async fn a_served_request_exports_a_span_and_returns_its_traceparent() {
    ...
    let config = TelemetryConfig {
        service_name: "thing-service-mw-test".to_string(),
    ...
    tracing_subscriber::registry()
        .with(tracing_opentelemetry::layer().with_tracer(provider.tracer("thing-service")))
        .with(tracing_subscriber::EnvFilter::new(
            "thing_service=trace,otlp_middleware=trace",
        ))
        .init();
