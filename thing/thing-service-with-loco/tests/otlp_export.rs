...
use thing_service::observability::{TelemetryConfig, build_meter_provider, build_tracer_provider};
...
async fn a_tracing_span_reaches_a_real_otlp_collector() {
    ...
    let config = TelemetryConfig {
        service_name: "thing-service-otlp-test".to_string(),
    ...
    {
        let layer = tracing_opentelemetry::layer().with_tracer(provider.tracer("thing-service"));
        let subscriber = tracing_subscriber::registry().with(layer);
        tracing::subscriber::with_default(subscriber, || {
            let span = tracing::info_span!("things.get", thing.id = "0c4f1e2a-test");
            let _entered = span.enter();
            tracing::info!("serving a thing read");
        });
    }
    ...
    assert!(
        service_names.contains(&"thing-service-otlp-test".to_string()),
    ...
    let span = spans
        .iter()
        .find(|span| span.name == "things.get")
        .unwrap_or_else(|| {
            panic!(
                "exported spans did not include `things.get`: {:?}",
                spans.iter().map(|span| &span.name).collect::<Vec<_>>()
            )
        });
    assert!(
        span.attributes.iter().any(|kv| kv.key == "thing.id"),
...
async fn a_metric_reaches_a_real_otlp_collector() {
    ...
    let meter = provider.meter("thing-service");
    let histogram = meter
