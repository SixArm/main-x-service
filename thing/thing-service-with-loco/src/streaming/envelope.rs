...
//! [`ThingEvent`](super::ThingEvent) / [`EventPublisher`](super::EventPublisher)
...
//! `THING_EVENT_TRANSPORT=outbox` is set.
//!
//! [`Envelope::for_event`] builds the envelope from a domain
//! [`Thing`](crate::models::thing::Thing); [`EventView`] is the flat
//! operator projection; [`EventTransport`] / [`transport`] read the
//! configured transport once from the environment.
//!
//! Note: [`EventKind`] here (`Created`/`Updated`/`Deleted`/`Merged`) is the
//! canonical durable-bus change kind and is **distinct** from the legacy
//! [`EventKind`](super::EventKind) (`ThingCreated`/…) used by the in-memory
//! [`ThingEvent`](super::ThingEvent). To avoid a name clash, the parent
//! `streaming` module re-exports everything from here **except** `EventKind`;
//! reach it as `crate::streaming::envelope::EventKind`.
...
pub struct Envelope {
    ...
    /// The entity name (`snake_case`), e.g. `"thing"`.
    ///
...
impl Envelope {
    /// Build an envelope describing `kind` applied to `thing`, minting a
    ...
    #[must_use]
    pub fn for_event(thing: &Thing, kind: EventKind) -> Self {
        Self {
            event_id: Uuid::new_v4(),
            schema_version: SCHEMA_VERSION,
            entity: ENTITY,
            kind,
            pid: thing.id.to_string(),
            seq: next_seq(),
            actor: None,
    ...
    #[must_use]
    pub fn for_merge(survivor: &Thing, merged_from: &Uuid) -> Self {
        Self {
...
impl EventTransport {
    /// Parse the `THING_EVENT_TRANSPORT` value. `outbox` selects the
...
/// The process-wide transport, read once from `THING_EVENT_TRANSPORT` and
...
pub fn transport() -> EventTransport {
    ...
    *TRANSPORT.get_or_init(|| {
        std::env::var("THING_EVENT_TRANSPORT")
...
mod tests {
    ...
    fn for_event_leaves_merged_from_none() {
        let env = super::Envelope::for_event(&a_thing(), EventKind::Updated);
    ...
    fn seq_is_monotonic_across_envelopes() {
        let thing = a_thing();
        let a = super::Envelope::for_event(&thing, EventKind::Created);
        let b = super::Envelope::for_event(&thing, EventKind::Updated);
    ...
    fn event_view_projects_the_frozen_shape() {
        let thing = a_thing();
        let env = super::Envelope::for_event(&thing, EventKind::Deleted);
