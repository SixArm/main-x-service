...
pub struct AppState {
    ...
    /// PASETO bearer token on every route except the public
    /// allow-list (`super::auth::is_public_path`).
...
///
/// - `THING_PASETO_KEYS` — the Ed25519 key set (JSON, OKP/Ed25519 form)
...
/// - `THING_TOKEN_ISSUER` — expected `iss` (default
///   `authentication-service`).
/// - `THING_TOKEN_AUDIENCE` — expected `aud` (default
///   `main-x-service`).
///
/// This is the env-injection path. Prefer [`boot_verifier`], which
/// fetches the key set over HTTP at boot when `THING_PASETO_KEYS_URL`
/// is set and falls back to this path otherwise.
fn verifier_from_env() -> Verifier {
    let issuer = env_or("THING_TOKEN_ISSUER", DEFAULT_ISSUER);
    let audience = env_or("THING_TOKEN_AUDIENCE", DEFAULT_AUDIENCE);
    let keys = std::env::var("THING_PASETO_KEYS")
        .ok()
        .filter(|s| !s.trim().is_empty())
...
///
/// - `THING_PASETO_KEYS_URL` **set (non-blank)** — fetch the key-set
///   JSON over HTTP from the auth service (normally its
///   `/.well-known/paseto-keys` endpoint), once, at boot. A successful
///   fetch **wins** over any `THING_PASETO_KEYS` env value; a failed
///   fetch logs a warning and falls back to the env path. See
///   [`verifier_from_url_or_env`].
/// - `THING_PASETO_KEYS_URL` **unset / blank** — exactly the previous
///   behaviour: build from the `THING_PASETO_KEYS` env key set, else an
...
pub async fn boot_verifier() -> Verifier {
    match std::env::var("THING_PASETO_KEYS_URL")
        .ok()
        .filter(|s| !s.trim().is_empty())
    {
...
/// `THING_TOKEN_ISSUER` / `THING_TOKEN_AUDIENCE` claims (same defaults
    ...
    match Verifier::from_paseto_keys_url(url, &issuer, &audience).await {
        ...
        Err(error) => {
            tracing::warn!(
                url,
                error = %error,
                "boot-time PASETO key-set fetch failed; falling back to the THING_PASETO_KEYS env path"
            );
            verifier_from_env()
        }
    }
...
mod tests {
    ...
    fn test_env_or_falls_back_when_unset() {
        assert_eq!(
            env_or("THING_SERVICE_TEST_UNSET_VAR_XYZ", "fallback"),
            "fallback"
        );
    }
}
