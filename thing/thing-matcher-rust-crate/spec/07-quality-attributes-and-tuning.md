...
Since `0.4.0` shipped (the crate is now at `0.7.0`), the public types (`Thing`, `ThingBuilder`, `MatchConfig`, `MatchResult`, `MatchBreakdown`, `Identifier`, `Confidence`, `MatchingError`, `Result`, `SimilarityAlgorithm`, `MatchingEngine`, `Normalizer`, `Scorer`, and — new in v0.7.0 — `RelationKind`, `RelationshipRef`) and their semantics are stable under SemVer.

Only `Thing`, `MatchingError`, and — new in v0.7.0 — `RelationKind` carry `#[non_exhaustive]` (confirmed in `src/models.rs` and `src/error.rs`; see the `AGENTS.md` Quick orientation table) — field/variant additions to these are therefore non-breaking. `MatchBreakdown` does **not** carry `#[non_exhaustive]`: adding `relationships_score` / `tags_score` (§3.7, §5.9.1, §5.9.2) in v0.7.0 was a struct-literal-breaking change under strict SemVer, permitted by this crate's pre-1.0 "minor bumps may break" policy (`agents/release.md`).
