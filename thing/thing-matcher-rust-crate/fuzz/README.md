...
|---|---|
| `match_things` | Deserialize a JSON `[thing_a, thing_b]` tuple → `MatchingEngine::match_things`. The whole deserialize → normalize → score path; asserts finite score in `[0,1]` in both argument orders. |
| `normalizer` | The pure `Normalizer` string helpers (name / free text / URL / phonetic) over arbitrary UTF-8 — never-panic. |
...
cargo +nightly fuzz run match_things     # fuzz until a crash / Ctrl-C
