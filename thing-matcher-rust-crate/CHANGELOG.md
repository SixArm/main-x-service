...
- Corrected `repository` URL to `https://github.com/sixarm/thing-matcher-rust-crate`.
...
- `Confidence` enum on `MatchResult` (spec FR-37, §12.5, task T-2).
  - New public `Confidence { High, Medium, Low }` enum in `thing_matcher::matcher` (re-exported as `thing_matcher::Confidence`). `Confidence::from_score(f64)` exposes the banding function; it is total over `f64` (NaN/negative → `Low`, > 1.0 → `High`).
...
- Nickname-aware name scoring (spec FR-33/FR-34, task T-10).
  - New public `NicknameTable` type in `thing_matcher::nicknames` (re-exported as `thing_matcher::NicknameTable`). API: `empty()`, `english()`, `with_class(names)`, `are_equivalent(a, b)`, `is_empty()`, `len()`.
...
- Sophisticated address parsing (spec §14.4a, FR-31, task T-20).
  ...
  - `Normalizer::parse_address_line(line) -> ParsedAddressLine { house_number, unit, street }`. The struct is `Serialize + Deserialize` and is re-exported from the crate root as `thing_matcher::ParsedAddressLine`.
...
### Added
- Initial release of thing matcher crate
...
- Implemented based on findings from:
  - "Thing matcher within a Health Information Exchange" (Grannis et al., 2014)
