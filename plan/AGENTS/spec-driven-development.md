# Spec-Driven Development — Plan Entity

The plan entity practises **spec-driven development** at two levels.
Read the per-crate discipline first (the matcher's
[AGENTS/spec-driven-development.md](../plan-matcher-rust-crate/AGENTS/spec-driven-development.md)
...
- The entity-level [`../spec/`](../spec/index.md) is the single
  ...
  composition, the DTO contract (API body = matcher `Plan`,
  persisted as JSONB), the endpoint inventory the front-end consumes
  (plan CRUD + match/merge + sub-resources + derived views + links +
...
edit**. If a change crosses subprojects (e.g. a new `Plan` field or a
new `PlanType` variant), the one PR carries all the touched specs:
matcher spec §6, entity spec §5, front-end `types.ts` + its spec, and
CHANGELOGs.
...
|---|---|
| `Plan` fields or enums (PlanType, PlanStatus, RelationKind, IdentifierScheme) | §5.1 (+ matcher spec §6, front-end types) |
| Sub-resource shape (Goal, Task, Issue, Post, Comment, Member) | §5.2 (+ migration, front-end types) |
...
- Forking a service-side `Plan` DTO "for convenience" — the whole
  entity design is *one shape end to end* (matcher type = DTO =
  JSONB, no adapter).
- Short-circuiting matches on owner-scoped codes (`PlanCode` /
  `LocalId`) — they are not globally unique.
- Matching on sub-resources beyond goal *titles* — tasks / issues /
  posts / comments / members are operational, not identity.
- Embedding another service's data in a `Plan` — store only the
  opaque `*_ref` ids; resolution belongs to the front-end / link
  aggregator.
