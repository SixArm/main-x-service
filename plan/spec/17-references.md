...
The plan entity's standards and tooling landscape, plus the family's
standing references. The registry-of-identities-with-a-PM-tool posture
relative to this landscape is §8.7.

### 17.1 Standards and vocabularies

- [schema.org/Project](https://schema.org/Project) — nearest
  schema.org type for a plan / initiative; `same_as` follows
  schema.org [`sameAs`](https://schema.org/sameAs) semantics.
...
these tools expose, so a plan synced from a source tool deduplicates
against its registry twin (§8.7):
...
- **Issue / risk tracking** — `Issue { kind, severity, status }`
  ...
  level.
- **Portfolio / programme management** — the `ParentOf` / `ChildOf`
  relationships model portfolio → programme → project hierarchy;
  `DependsOn` / `BlockedBy` model cross-initiative dependencies.
...
- Subproject specs (to be scaffolded, T-1):
  [service](../plan-service-with-loco/spec/index.md),
  [matcher](../plan-matcher-rust-crate/spec/index.md),
  [front-end](../plan-front-end-with-svelte/spec/index.md).
