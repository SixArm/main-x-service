...
A plan's thin record (name, goals, plan code, type, timeframe,
identifiers) is **business data**, not personal data. Two angles keep
the data-protection posture in force:

- **People references are personal data.** `lead_ref`, members, task
  `assignee_ref`s, and post / comment `author_ref`s are `EntityRef`s
  to **people** (person / worker / authentication identities). Who
  leads, staffs, or comments on a plan — and the audit trail of who
  changed what — is personal data about identifiable individuals.
  **GDPR / UK DPA 2018 apply** to these references, to the membership
  table, and to the audit log.
- **Free-text fields can leak.** `keywords`, `tags`, goal titles, and
  post / comment Markdown MUST never carry special-category or
  unnecessary personal data; this is an operator-training point and is
  why export defaults to masked (§9.6).
...
|---|---|
| UK DPA 2018 / UK GDPR / EU GDPR | Apply to the people references (`lead_ref`, members, assignees, authors), the membership table, and operator identities in audit data; lawful-basis and accountability documentation deployment-side; soft delete supports retention policy; bulk export of these references is a compliance event (§9.6) |
...
| ISO/IEC 27001 | ISMS operational controls (deployment-side): access control, backups, logging; membership-scoped write access (roadmap) is an access-control control |
...
- **People-reference minimisation.** The model stores `EntityRef`s
  ...
  keeps the plan entity a *referrer*, not a second copy of personal
  data — re-assess masking the moment any denormalised personal field
  appears.
- **Masking on export.** Bulk export defaults to masked, with the
  ...
  the `case ↔ person`-style sensitivity does **not** apply (plans are
  not case data), but member / assignee references are still personal
...
- **Auditability.** Full audit log + event stream of every CRUD **and
  ...
  — including who joined / left a plan and who was assigned what — is
  the production bar; the durable event bus is roadmap (§15).
- **Access control.** Production deployments MUST sit behind SSO
  ...
  supersedes the RS256-JWT model) and TLS;
  membership + role gate who may write a plan's sub-resources
  (enforcement is roadmap once auth is enforced, §15).
