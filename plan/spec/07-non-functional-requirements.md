...
- **NFR-1 — Scale.** Hundreds of thousands of plan records (every
  project, programme, and portfolio across participating
  departments); each plan may own thousands of tasks, issues, and
  comments — hence the matchable/operational partition (§5.6), which
  keeps the high-volume sub-resources out of both the matcher payload
  and the cheap list/read projections. *(roadmap:
  `check-duplicates` must use search-blocked candidates rather than an
...
- **NFR-2 — Performance.** Read ≤ 5 ms p50; list ≤ 20 ms p50; pairwise
  ...
  are computed within p99 ≤ 200 ms over a single plan's sub-resources.
...
- **NFR-4 — i18n / locales.** Plan names, goal titles, keywords, and
  Markdown bodies are Unicode-correct end to end; the matcher folds
  with NFKC and **preserves diacritics** (never strips them).
  `in_language` records the plan's language. *(roadmap: multilingual
  operator UI per
...
- **NFR-5 — Security.** TLS at the edge; SSO via the central
  ...
  supersedes the RS256-JWT model); members /
  assignees / authors are user identities resolved by `EntityRef`; no
...
- **NFR-8 — Determinism and explainability.** Matching is a pure,
  ...
  replay. Date-proximity scoring is computed from the records'
  declared dates, never from "now".
