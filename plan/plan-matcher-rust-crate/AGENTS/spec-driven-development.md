...
- When the public surface of the matcher changes (re-exports from
  ...
  test in
  [`../plan-service-with-loco/tests/matching.rs`](../../plan-service-with-loco/tests/matching.rs)
  in the same PR.

## When To Update Which Section

The plan-matcher spec uses the §1–§25 matcher-crate shape (distinct
...
| Plan-code rule (same-owner gate, shape) | §11 Plan code |
| Owner-org score | §11a Owner org |
| Plan-type / timeframe score | §12 Plan type & timeframe |
...
| How `plan-service` consumes the crate | §20 |
...
| §19–§21 quality / consumption / compat | top-level docs, integration with `plan-service` |
...
- **Adding a deterministic-identifier scheme without bridge-test
  coverage** in `plan-service`. A false positive at score 1.0 is
  the worst-case bug.
...
- **Using `unwrap` / `expect` in library code.** Total functions
  only.
- **Scoring a `plan_code` across owners.** Per §11, the plan-code
  component MUST be gated on `owner_org_id` equality (PLAN-01 at one org
  != PLAN-01 at another).
