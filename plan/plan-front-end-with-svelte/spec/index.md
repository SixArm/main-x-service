# Plan Front-End — Specification

> **Single source of truth.** Code conforms to this spec. Behavioural
> change = spec + code + test. Live work queue is §13.
>
> Sibling service: [plan-service-with-loco](../../plan-service-with-loco/spec/index.md).
> Entity umbrella: [plan/spec](../../spec/index.md).
...
identities (project / product / programme / initiative / portfolio /
epic) **and** to run the plan as a live project workspace —
goals, a Kanban task board, issues, a timeline / Gantt, a burndown
chart, a posts feed with threaded comments, and member-role
management. It is a thin presentation layer over the plan service's
REST API (`/api/v1/plans/...`); the Rust service is the system of
record.

The entity has two faces that share one record (entity spec §1): a
**matchable identity registry** (portfolio-level dedup) and a
**project-management tool** (operational sub-resources). The front-end
surfaces both — identity CRUD + duplicate-check + merge on one side,
the project-management views on the other.

## 2. Scope

In scope:

- The **identity routes** (`/`, `/new`, `/[pid]`, `/[pid]/edit`): the
  plan list (SVAR DataGrid), create/edit form, detail page.
- The **API client** (`src/lib/api/{types,client,plans}.ts`), the
  plan form, a name-search box on the list, a duplicate-check / match
  screen with a per-component **MatchBreakdown** visual, a
  merge-duplicate action on the detail page, and a per-plan audit
  timeline.
- The **project-management views** on the detail page (or its
  sub-routes): a **Kanban task board** (drag = status change), an
  **Issues** list, a **Gantt / timeline** view, a **burndown** chart,
  a **Goals** panel, a **Posts feed with threaded Comments**, and a
  **Members** panel with role management.
...
consent-management UI; no finance / budgeting UI; no binary-attachment
upload (posts and comments are Markdown text). Authentication is **not**
implemented here as a login screen — sign-on is delegated to the
central [authentication-service](../../../authentication/authentication-service-with-loco/)
...
Portfolio managers and PMO analysts (dedup, roll-up), programme /
project leads (the workspace), team contributors (tasks, issues,
posts), and auditors (the audit timeline + match breakdown).

## 4. Glossary

- **pid** — the plan's public id (route param).
- **Plan** — the `plan_matcher::Plan` payload (the matchable identity
  header; the API DTO = the matcher type, persisted as JSONB).
- **Sub-resource** — a `Goal` / `Task` / `Issue` / `Post` / `Comment`
  / `Member` owned by a plan, reached under `/api/v1/plans/{pid}/…`.
...
- **MatchBreakdown** — the per-component score map returned by a match
  (name, goals, plan code, owner org, plan type, timeframe, keywords,
  relationships, tags).
...
```
/                 list of plans (SVAR DataGrid) + search + recent activity
/new              create form
/[pid]            detail + delete + check-duplicates + merge + audit timeline
/[pid]/edit       edit form
/[pid]/board      Kanban task board (Todo/InProgress/InReview/Done/Blocked)
...
(Project-management views MAY be implemented as detail-page tabs rather
than discrete sub-routes; the spec fixes the *capabilities*, not the
URL shape. If sub-routes are used they share the `[pid]` layout.)
...
- A chrome utility area in the top bar carries the **theme selector**
  (`lily-design-system-svelte-theme-select`) and the **locale selector**
  (`lily-design-system-svelte-locale-select`), plus the session
  affordance (Sign in / Sign out).
...
`<html>`. The choice persists to `localStorage` (key `plan:theme`).
...
The choice persists to `localStorage` (key `plan:locale`). UI strings
come from a per-locale catalogue under `$lib/i18n/`; missing keys fall
back to `en`.

## 6. Functional requirements

1. **List** active plans (`GET /api/v1/plans`) in a **SVAR DataGrid**
   with columns: name, plan type, status, owner org, lead, target date,
   tags. Sortable; client-side filter/search.
   - Search box (search-on-submit): a non-blank query calls
     `GET /api/v1/plans/search?q=` (URL-encoded) and renders the
   ...
   - Recent activity: a "Show recent activity" toggle lazy-loads
     `GET /api/v1/plans/events/recent` on first open and renders the
     events newest-first (highest `seq` first): the kind
     (created/updated/deleted/merged), the name (linked to the plan by
     pid), and the `seq`. Loading, empty, and error states; the panel
...
6. **Check-duplicates** posts the current record and lists matches
   (name, score, confidence), excluding the record itself, each with a
   visual **MatchBreakdown** (per-component bars for name / goals /
   plan code / owner org / plan type / timeframe / keywords /
   relationships / tags).
7. **Merge**: each duplicate row offers "Merge into this record" (the
   ...
   A two-step inline confirm calls `POST /api/v1/plans/merge` with
...
8. **Audit timeline**: a "Show audit trail" toggle lazy-loads
   `GET /api/v1/plans/{pid}/audit` on first open and renders the rows
   newest-first (action, actor or "—" when null, timestamp). Loading,
   empty, and error states; the panel does not auto-load on mount.
9. **The plan form** (create/edit) edits: `name` (required),
   `alternate_names`, `plan_type` (`<select>` unit variants), `plan_code`,
   `owner_org_id` (**org picker** into the organization entity),
   `lead_ref` (**person / worker picker**), `plan_status`, a **goals
   editor** (title + description + status + target date rows), `start_date`
   / `end_date`, `keywords`, `tags`, `identifiers` (scheme + value rows),
   `same_as`, and `relationships` (kind + target rows). Comma-list fields
   split on submit; blanks null; empty repeatable rows dropped.
10. **Kanban task board** (`/[pid]/board`): columns **Todo /
...
11. **Issues list** (`/[pid]/issues`): table of issues with kind,
    severity, status, reporter, assignee; create / edit; filter by
    status / severity.
12. **Gantt / timeline** (`/[pid]/timeline`): renders
    `GET …/{pid}/timeline` — goal milestones (target dates) + task date
    ranges (start / due) as Gantt-shaped rows over the plan timeframe.
13. **Burndown chart** (`/[pid]/burndown`): renders
    `GET …/{pid}/burndown` — remaining estimate over time as a series.
14. **Goals panel** (`/[pid]/goals`): list / create / edit / delete
    goals (title, description, status, target date). Goal titles feed
    the match `Goals` component (display-only note).
15. **Posts feed + threaded comments** (`/[pid]/posts`): a feed of
    posts (title, body Markdown, author, time) newest-first; each post
    expands to **threaded comments**; create post / add comment.
16. **Members panel** (`/[pid]/members`): list members with role
    (Owner / Manager / Contributor / Viewer); add a member (user / person
    / worker ref) with a role; **change role**; remove a member.
17. **Session / auth (BFF + httpOnly cookie)**: the top bar carries a
    ...
    attaches a short-lived PASETO server-side when calling the plan
    ...
    blanket enforcement (`PLAN_REQUIRE_AUTH`, off by default). Per
    [`../../../agents/share/authentication-sessions.md`](../../../agents/share/authentication-sessions.md).
18. **Layout shell**: global navigation is a full-width **top bar**
...
- **SVAR Svelte DataGrid** for the plan list and any tabular sub-view;
  native HTML for simple lists.
...
`PlanRepository` → routes. The service is loco.rs and returns **raw
JSON** (no envelope). Under the BFF model (§6.17) the browser carries
only the `__Host-mxi_session` cookie and the SvelteKit server attaches
the short-lived PASETO server-side when calling the service; no token is
read or attached in browser JS. `PlanForm`
builds a `Plan` from the inputs (comma lists split, blanks nulled,
goals / identifiers / relationships as editable rows; empty rows
dropped on submit; a seeded `Custom` enum collapses to "—"). The
project-management views call the sub-resource endpoints via the same
repository. Layout chrome (theme / locale selectors, hamburger nav,
session affordance) lives in `+layout.svelte`; `+layout.ts` sets the
SPA toggles.

`types.ts` hand-mirrors `plan_matcher::Plan` and the sub-resource /
DTO shapes: `Plan`, `PlanType`, `PlanStatus`, `Goal`, `GoalStatus`,
`Task`, `TaskStatus`, `Issue`, `Post`, `Comment`, `Member`,
`MemberRole`, `Relationship`, `RelationKind`, `PlanIdentifier`,
`IdentifierScheme`, `PlanRef`, `ScoredRef`, `MatchBreakdown`,
`MergeResult`, `AuditEntry`, `PlanEvent`, `TimelineRow`,
`BurndownPoint`. It MUST be updated in the same change cycle as any
matcher-type change (entity spec §18).
...
| `/[pid]` load | `GET /api/v1/plans/{pid}` |
| `/[pid]` delete | `DELETE /api/v1/plans/{pid}` |
| `/[pid]` duplicates | `POST /api/v1/plans/check-duplicates` (→ `ScoredRef[]` w/ `MatchBreakdown`) |
| `/[pid]` merge | `POST /api/v1/plans/merge` (`{main_pid, duplicate_pid, reason?}`) |
| `/[pid]` audit | `GET /api/v1/plans/{pid}/audit` (→ `AuditEntry[]`) |
| `/[pid]/edit` | `PUT /api/v1/plans/{pid}` |
...
`localStorage` keys for chrome preferences (`plan:theme` /
`plan:locale`). The session lives only in the `__Host-mxi_session`
httpOnly cookie — no auth token in `localStorage` (BFF model, §6.17).
...
- the `ApiClient` (verb / body / headers / bearer-token /
...
- `PlanRepository` (every method's path + verb, incl. `check-duplicates`,
  `search()` `/search?q=` URL-encoding, `merge()` body
  `{main_pid, duplicate_pid, reason?}` with `404` / `422` `ApiError`
  propagation, `audit()`, `recentEvents()`, and the sub-resource methods
  — task move `PATCH`, issues, goals, posts/comments, members,
  timeline, burndown);
- the **`PlanForm`** component (required-name guard blocks `onsubmit`
  on blank/whitespace; `build()` trims scalars, nulls blanks, splits
  comma lists, drops empty goal / identifier / relationship rows,
  collapses a `Custom` enum seed);
...
- a **`+layout` render test** asserting the **hamburger toggles the
  nav** (collapsed → expanded), and that the theme + locale selectors
  render.
...
the list search box (matching keeps the row, non-matching shows the
empty message); the detail-page merge action (check-duplicates →
confirm merge → success, asserting the merge endpoint fired); the
audit timeline; the recent-activity panel; the Kanban board (drag a
card → status PATCH fired); the issues list; the timeline / burndown
views render; the posts feed expands a thread; the members panel
changes a role; and the layout (hamburger toggles nav; theme + locale
selectors switch `data-theme` / `lang` + `dir`, incl. RTL for `ar`).

Run: `pnpm test` (vitest) and `pnpm test:e2e` (Playwright).

## 12. Compliance

Plans are government-portfolio artefacts; defer to the service's
controls for access / audit. Member / author refs are opaque
`EntityRef`s — the front-end displays them but does not resolve PII
beyond what the referenced services return. Cross-service links are
**never** a match signal (entity spec §1).
...
- [ ] `src/lib/api/types.ts` — mirror `plan_matcher::Plan` + sub-resource
  + DTO shapes (per §8).
...
- [ ] `src/lib/api/plans.ts` — `PlanRepository` (CRUD + search +
  checkDuplicates + merge + audit + recentEvents + sub-resource methods
  + timeline + burndown).
- [ ] Auth — adopt BFF + httpOnly cookie + CSRF: `hooks.server.ts` /
  server routes read the `__Host-mxi_session` cookie, exchange it for a
  short-lived PASETO server-side, and attach it when calling the plan
  service; CSRF on mutating browser→BFF calls. No `mxi_access_token` /
...
- [ ] `+layout.svelte` / `+layout.ts` — top-bar nav with **leftmost
  hamburger**, full-width content, theme + locale selectors, session
  affordance (Sign in / Sign out — BFF, no token paste); SPA toggles.
- [ ] Identity routes: `/` (SVAR DataGrid list + search + recent
  activity), `/new`, `/[pid]` (detail + delete + check-duplicates +
  MatchBreakdown + merge + audit timeline), `/[pid]/edit`.
- [ ] `PlanForm` component (incl. org picker, person/worker lead picker,
  goals / identifiers / relationships editors).
- [ ] `MatchBreakdown` visual component (per-component score bars incl.
  goals / relationships / tags).
- [ ] Kanban task board (`/[pid]/board`) — drag = status PATCH.
- [ ] Issues list (`/[pid]/issues`).
- [ ] Gantt / timeline view (`/[pid]/timeline`).
- [ ] Burndown chart (`/[pid]/burndown`).
- [ ] Goals panel (`/[pid]/goals`).
- [ ] Posts feed + threaded comments (`/[pid]/posts`).
- [ ] Members panel + role management (`/[pid]/members`).
- [ ] vitest unit suite (client, repository, BFF auth integration,
  signInUrl, PlanForm, i18n, `+layout` hamburger-toggle render test).
- [ ] Playwright e2e smoke (identity routes, search, merge, audit,
  recent activity, board drag, issues, timeline/burndown, posts thread,
  members role change, layout hamburger + theme/locale switch incl. RTL).
...
- **v0.2**: scaffold + identity routes (list / create / detail / edit)
  + lean client + repository + BFF auth (httpOnly cookie + CSRF, per
  [`../../../agents/share/authentication-sessions.md`](../../../agents/share/authentication-sessions.md))
...
- **v0.4**: project-management views — Kanban board, issues, timeline /
  Gantt, burndown, goals, posts / comments, members.
...
- How rich should the org / person / worker pickers be (typeahead
  against the sibling services vs. raw `pid` entry) for MVP?

## 17. References

- Sibling service spec: [plan-service-with-loco/spec](../../plan-service-with-loco/spec/index.md).
- Entity umbrella: [plan/spec](../../spec/index.md);
...
`src/lib/api/types.ts` in lockstep with the matcher `Plan` type — if a
field changes in the service / matcher, fix it here in the same cycle.
