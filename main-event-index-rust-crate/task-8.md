...
This phase implements a comprehensive event streaming and audit logging system for the Main Event Index (MPI), providing critical infrastructure for production healthcare environments. The implementation enables real-time event notification, complete audit trails for compliance, and integration capabilities with downstream systems.

## Task Description

Implement event-driven architecture and audit logging to capture, publish, and persist all event data lifecycle events within the MPI system. This includes:

1. **Event Publisher Infrastructure**: Create a thread-safe event publishing system that broadcasts event lifecycle events
2. **Audit Log Repository**: Implement database-backed audit logging for compliance and forensic analysis
3. **Repository Integration**: Enhance the event repository to automatically publish events and log audits
...
1. **Event-Driven Architecture**: Enable real-time notification of event data changes to downstream systems
...
4. **System Integration**: Allow other systems to react to event data changes
...
Healthcare systems must maintain complete audit trails of all event data modifications:
...
The MPI acts as the authoritative source for event identity. Other systems need to know when:

- New events are registered
- Event demographics are updated
- Events are merged (duplicate resolution)
- Event records are linked/unlinked
...
- **Debugging**: Trace the history of event record changes
...
Defined comprehensive event lifecycle events:
...
- Event-specific data (event record or IDs)
- Timestamp for temporal ordering
- Helper methods: `timestamp()`, `event_id()`
...
- `get_logs_for_entity()`: Retrieve audit history for a specific event
...
- `entity_type`: Type of entity (e.g., "event")
...
Enhanced `DieselEventRepository` with event and audit capabilities:

**Builder Pattern:**

```rust
let repository = DieselEventRepository::new(pool)
...
fn create(&self, event: &Event) -> Result<Event> {
    ...
    self.publish_event(EventEvent::Created {
        event: result.clone(),
        timestamp: Utc::now(),
    });
    ...
    if let Ok(event_json) = serde_json::to_value(&result) {
        self.log_audit("CREATE", result.id, None, Some(event_json), &AuditContext::default());
    }
...
fn update(&self, event: &Event) -> Result<Event> {
    ...
    let old_event = self.get_by_id(&event.id)?;

    // 2. Execute database transaction
    let result = conn.transaction(|conn| { ... })?;

    // 3. Publish event
    self.publish_event(EventEvent::Updated { ... });
...
fn delete(&self, id: &Uuid) -> Result<()> {
    // 1. Get old values for audit
    let old_event = self.get_by_id(id)?;
    ...
    self.publish_event(EventEvent::Deleted {
        event_id: *id,
        timestamp: Utc::now(),
    });
...
impl AppState {
    pub fn new(db_pool, search_engine, matcher, config) -> Self {
        ...
        // 3. Create event repository with dependencies
...
- Event data mutations should succeed even if audit/events fail
...
```rust
fn publish_event(&self, event: EventEvent) {
...
let repo = DieselEventRepository::new(pool);
...
4. **Event Compression**: Compress large event records in events
...
- Must be secured with same rigor as event data
...
Phase 8 establishes production-grade observability infrastructure for the Main Event Index:

✓ **Event Streaming**: Real-time event lifecycle event publication
...
The implementation balances immediate functionality with long-term flexibility, using in-memory implementations that can be swapped for production-grade systems (Kafka, etc.) without code changes. The non-blocking design ensures core event operations remain performant while building complete observability.
