...
Completed Phase 3 of the Main Event Index (MPI) implementation: Core MPI Logic. This phase implements the sophisticated event matching algorithms and scoring systems that form the heart of the MPI system.
...
- **Identify Duplicates**: Find potential duplicate event records across facilities
...
```
Event A: John Smith, 1980-01-15, Male, 123 Main St, 12345, MRN:12345
Event B: John Smith, 1980-01-15, Male, 123 Main St, 12345, MRN:12345
...
```
Event A: John Smith, 1980-01-15, Male, (no address), (no identifier)
Event B: John Smith, 1980-01-15, Male, (no address), (no identifier)
...
```
Event A: William Smith, 1980-01-15, Male
Event B: Bill Smyth, 1980-01-16, Male
...
pub trait EventMatcher {
    fn match_events(&self, event: &Event, candidate: &Event)
        -> Result<MatchResult>;

    fn find_matches(&self, event: &Event, candidates: &[Event])
...
- `match_events()`: Compare two events
...
pub struct MatchResult {
    pub event: Event,       // The matched event
...
Matchers don't modify event records:
...
- Typically small (1-3 identifiers per event)
- Space: O(n) for normalization
...
4. **Caching**: Cache frequently compared event pairs
...
```rust
use main_event_index::matching::{ProbabilisticMatcher, EventMatcher};
use main_event_index::config::MatchingConfig;
use main_event_index::models::{Event, HumanName, Gender};
...
// Match events
let result = matcher.match_events(&event1, &event2)?;
...
let new_event = create_event("John Smith", "1980-01-15");

// Get candidates from database (pseudo-code)
let candidates: Vec<Event> = database
    .search_by_name_soundex("Smith")
    .limit(1000)
    .collect()?;

// Find matches
let matches = matcher.find_matches(&new_event, &candidates)?;

for (idx, match_result) in matches.iter().enumerate() {
    println!("Match #{}: {} (score: {:.3})",
        idx + 1,
        match_result.event.full_name(),
        match_result.score
    );
...
```rust
use main_event_index::matching::DeterministicMatcher;
...
if matcher.is_match(result.score) {
    ...
    flag_for_review(&event1, &event2, result.score)?;
}
```
...
if auto_mode {
    let matches = auto_matcher.find_matches(&event, &candidates)?;
    // Only highest confidence matches
} else {
    let matches = search_matcher.find_matches(&event, &candidates)?;
...
impl EventRepository {
    fn find_duplicates(&self, event: &Event) -> Result<Vec<MatchResult>> {
        // 1. Use blocking strategy to narrow candidates
        let soundex = calculate_soundex(&event.name.family);
        ...
        matcher.find_matches(event, &candidates)
    }
}
```
...
```rust
impl SearchEngine {
    fn find_potential_matches(&self, event: &Event) -> Result<Vec<Event>> {
        // Use Tantivy for initial filtering
        let query = format!(
            "name:{} AND birth_year:{}",
            event.name.family,
            event.birth_date.map(|d| d.year()).unwrap_or(0)
        );
...
async fn match_event(
    ...
    let candidates = repo.find_potential_duplicates(&event)?;
...
4. **Multi-event Clustering**: Identify clusters of related records
...
Phase 3 successfully implemented a sophisticated event matching system with:
...
- Identify potential duplicate events
- Support manual review workflows
- Enable automated record linking
- Scale to millions of events (with future optimizations)

This foundation enables the MPI system to fulfill its core purpose: maintaining a unified, accurate view of event identities across healthcare organizations.
