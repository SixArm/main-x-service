...
COPY --from=builder /app/target/release/main_event_index /app/main_event_index-server
...
# Run the application
CMD ["/app/main_event_index-server"]
```
  ...
  test-runner:
    ...
    depends_on:
      postgres-test:
        condition: service_healthy
    environment:
      DATABASE_URL: postgresql://test_user:test_password@postgres-test:5432/main_event_index_test
...
```bash
# Database
DATABASE_URL=postgresql://main_event_index_user:mpi_password@localhost:5432/main_event_index
...
DATABASE_URL=postgresql://main_event_index_prod:STRONG_PASSWORD@db-host:5432/main_event_index_production?sslmode=require
...
RUST_LOG=info,main_event_index=info
RUST_BACKTRACE=0
```
  ...
  test:
    ...
    steps:
      ...
      - name: Push to Registry
        ...
        run: |
          docker tag mpi-server:${{ github.sha }} registry.io/main_event_index-server:latest
          docker push registry.io/main_event_index-server:latest
```
...
OTLP_SERVICE_NAME=main-event-index
```
