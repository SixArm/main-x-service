...
use crate::models::{Event, Address, ContactPoint, ContactPointSystem, IdentityDocument};
...
/// Validate a event record, returning all validation errors found.
pub fn validate_event(event: &Event) -> Vec<ValidationError> {
    let mut errors = Vec::new();

    // Required: family name
    if event.name.family.trim().is_empty() {
    ...
    if event.name.given.is_empty() || event.name.given.iter().all(|g| g.trim().is_empty()) {
        errors.push(ValidationError {
    ...
    if let Some(dob) = event.birth_date {
    ...
    if let Some(ref tid) = event.tax_id {
    ...
    for (i, cp) in event.telecom.iter().enumerate() {
        errors.extend(validate_contact_point(cp, &format!("telecom[{}]", i)));
    }

    // Validate addresses
    for (i, addr) in event.addresses.iter().enumerate() {
        errors.extend(validate_address(addr, &format!("addresses[{}]", i)));
    }

    // Validate documents
    for (i, doc) in event.documents.iter().enumerate() {
        errors.extend(validate_document(doc, &format!("documents[{}]", i)));
    }

    // Validate emergency contacts
    for (i, ec) in event.emergency_contacts.iter().enumerate() {
...
mod tests {
    ...
    fn test_validate_future_birth_date() {
        ...
        event.birth_date = Some(chrono::NaiveDate::from_ymd_opt(2099, 1, 1).unwrap());
        let errors = validate_event(&event);
    ...
    fn test_validate_invalid_email() {
        ...
        event.telecom.push(ContactPoint {
    ...
    fn test_validate_tax_id_format() {
        ...
        event.tax_id = Some("---".into()); // No alphanumeric chars
        let errors = validate_event(&event);
    ...
    fn test_validate_document_missing_number() {
        ...
        event.documents.push(IdentityDocument {
    ...
    fn test_validate_emergency_contact_missing_name() {
        ...
        event.emergency_contacts.push(EmergencyContact {
            name: "".into(),
