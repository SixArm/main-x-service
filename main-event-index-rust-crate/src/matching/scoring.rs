...
impl ProbabilisticScorer {
    ...
    pub fn calculate_score(
        ...
        let name_score = name_matching::match_names(&event.name, &candidate.name);
        ...
        let tax_id_score = tax_id_matching::match_tax_ids(event, candidate);
...
impl DeterministicScorer {
    ...
    pub fn calculate_score(
        ...
        if !event.addresses.is_empty() && !candidate.addresses.is_empty() {
...
mod tests {
    ...
    fn create_test_event(name: &str, dob: Option<NaiveDate>) -> Event {
        Event {
            id: uuid::Uuid::new_v4(),
            identifiers: vec![],
            active: true,
    ...
    fn test_exact_match_scores_high() {
        ...
        let event1 = create_test_event("Smith", dob);
        let event2 = create_test_event("Smith", dob);

        let result = scorer.calculate_score(&event1, &event2);
    ...
    fn test_fuzzy_match_scores_moderate() {
        ...
        let event1 = create_test_event("Smith", dob1);
        let event2 = create_test_event("Smyth", dob2); // Spelling variant
    ...
    fn test_no_match_scores_low() {
        ...
        let event1 = create_test_event("Smith", dob1);
        let event2 = create_test_event("Johnson", dob2);
    ...
    fn test_probabilistic_all_fields_match() {
        ...
        let dob = NaiveDate::from_ymd_opt(1980, 1, 15);
        let mut event1 = create_test_event("Smith", dob);
        let mut event2 = create_test_event("Smith", dob);
        ...
        event1.addresses = vec![addr.clone()];
        event2.addresses = vec![addr];

        // Add matching identifiers
        let id = crate::models::Identifier::mrn("hospital-a".into(), "MRN-001".into());
        event1.identifiers = vec![id.clone()];
        event2.identifiers = vec![id];
    ...
    fn test_probabilistic_no_fields_match() {
        ...
        let mut event1 = create_test_event("Smith", NaiveDate::from_ymd_opt(1980, 1, 15));
        event1.gender = Gender::Male;
        let mut event2 = create_test_event("Johnson", NaiveDate::from_ymd_opt(1995, 8, 22));
        event2.gender = Gender::Female;
    ...
    fn test_probabilistic_partial_match() {
        ...
        let event1 = create_test_event("Smith", NaiveDate::from_ymd_opt(1980, 1, 15));
        let event2 = create_test_event("Smith", NaiveDate::from_ymd_opt(1990, 6, 20));
    ...
    fn test_deterministic_tax_id_match_short_circuits() {
        ...
        let mut event1 = create_test_event("Smith", NaiveDate::from_ymd_opt(1980, 1, 15));
        event1.tax_id = Some("123-45-6789".into());
        let mut event2 = create_test_event("Jones", NaiveDate::from_ymd_opt(1995, 12, 1));
        event2.tax_id = Some("123-45-6789".into());
