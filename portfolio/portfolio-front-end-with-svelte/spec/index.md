...
>
> Sibling service: [portfolio-service-with-loco](../../portfolio-service-with-loco/spec/index.md).
...
- **WorkItem** — the `portfolio_matcher::WorkItem` payload (the matchable
...
15. **Session / auth (BFF + httpOnly cookie)**: the top bar carries a
    ...
    blanket enforcement (`PORTFOLIO_REQUIRE_AUTH`, off by default). Per
    [`../../../agents/share/authentication-sessions.md`](../../../agents/share/authentication-sessions.md).
...
`types.ts` hand-mirrors `portfolio_matcher::WorkItem` and the
...
- [ ] `src/lib/api/types.ts` — mirror `portfolio_matcher::WorkItem` +
...
- Sibling service spec: [portfolio-service-with-loco/spec](../../portfolio-service-with-loco/spec/index.md).
- Entity umbrella: [portfolio/spec](../../spec/index.md);
