...
// a collection, so every path is scoped to `/api/{collection}`.
...
describe("WorkItemRepository", () => {
  it("list() GETs the collection", async () => {
    ...
    expect(calls[0]?.url).toBe("http://svc.test/api/projects");
  });

  it("list(portfolioRef) adds the roll-up query", async () => {
    const { repo, calls } = spyClient();
    await repo.list("port-1");
    expect(calls[0]?.url).toBe("http://svc.test/api/projects?portfolio=port-1");
  });

  it("search() GETs the search endpoint with an encoded q", async () => {
    const { repo, calls } = spyClient();
    await repo.search("a b");
    expect(calls[0]?.url).toBe("http://svc.test/api/projects/search?q=a%20b");
  });
  ...
  it("get() GETs a single record by pid", async () => {
    ...
    await repo.get("p1");
    expect(calls[0]?.init.method).toBe("GET");
    expect(calls[0]?.url).toBe("http://svc.test/api/projects/p1");
  });

  it("get() URL-encodes the pid", async () => {
    const { repo, calls } = spyClient();
    await repo.get("a/b 1");
    expect(calls[0]?.url).toBe("http://svc.test/api/projects/a%2Fb%201");
  });
  ...
  it("checkDuplicates() POSTs to check-duplicates (not /duplicates)", async () => {
    ...
    expect(calls[0]?.init.method).toBe("POST");
    expect(calls[0]?.url).toBe("http://svc.test/api/projects/check-duplicates");
  });
  ...
  it("merge() POSTs the merge body", async () => {
    ...
    expect(calls[0]?.init.method).toBe("POST");
    expect(calls[0]?.url).toBe("http://svc.test/api/projects/merge");
  ...
  it("scopes paths to its bound collection", async () => {
    ...
    await repo.list();
    expect(calls[0]).toBe("http://svc.test/api/portfolios");
  });
});
