...
- [ ] **T-1 — Scaffold the trio.**
  - [ ] Create `portfolio-matcher-rust-crate/`,
    `portfolio-service-with-loco/`, and
    `portfolio-front-end-with-svelte/` from the care-pathway / plan
...
- [ ] **T-2 — Matcher crate: domain model + matching.**
  - [ ] The canonical `WorkItem` type + `WorkItemKind` discriminator +
    `Goal` + all enums (§5.1–§5.4), serde round-trip, NFKC-folding
    diacritic-preserving normalisation.
  - [ ] The **kind gate (R-GATE)**: `A.kind != B.kind` → 0.0 before any
    other rule (a project never matches a product).
  - [ ] Deterministic short-circuits (after the gate): R-0 (each
    deterministic identifier scheme — `Uri`, `Uuid`, `JiraProjectKey`,
    `AsanaGid`, `TrelloBoardId`, `MsProjectId`, `GitHubProjectId`,
    `LinearId`; owner-scoped `Code`/`LocalId`/`Custom` excluded), R-1
    (same `owner_org_id` + equal normalised `code`), R-2 (`same_as`
    overlap) → 1.0.
  - [ ] Probabilistic components + weights per §6.8 (name JW + Soundex
    bonus, goal-title Jaccard, code, owner org, parent-portfolio exact,
    timeframe date-proximity, keywords, relationships typed-set
    Jaccard, tags), renormalised over present components; `status`
    never scored; presets strict/default/lenient.
  - **Acceptance:** `cargo test` green; the FR-8 weight table sums to
    1.00 in a test; the kind gate, each rule, and each component have a
    unit test; public-API + doctest suites pass.
- [ ] **T-3 — Service crate: chassis + thin-record CRUD + matching.**
  - [ ] loco 0.16 / Axum 0.8 / SeaORM 1.1 chassis; `portfolios` /
    `projects` / `products` / `programs` migrations; `cargo loco
    start`; config yamls; port 5150.
  - [ ] The **identical** controller shape over the four collections:
    CRUD over the thin `WorkItem` (JSONB `data`), name search
    (`ILIKE`), `/match`, `/check-duplicates` (all within-kind),
    validation (§FR-1a incl. `kind`/collection match + `portfolio_ref`
    on child kinds).
  - **Acceptance:** DB-free matcher-embedding + JSON round-trip tests
    green; blank-name / kind-mismatch / malformed-`EntityRef` /
    malformed-`portfolio_ref` / malformed-deterministic-id → `422`
    pinned un-gated.
- [ ] **T-4 — Service crate: operational sub-resources + derived views.**
  - [ ] Tables + CRUD for tasks, issues (keyed by `(parent_kind,
    parent_pid)`); goals via `data.goals[]` mutation (the §5.3 bridge).
  - [ ] Derived `timeline` + `burndown` read endpoints; `task_snapshots`
    feeding burndown.
  - [ ] Real-time `409` duplicate detection on create (same-kind);
    record merge that **re-homes sub-resources** to the survivor; the
    portfolio child roll-up (`?portfolio=` filter + `portfolio_pid`
    column).
...
- [ ] **T-5 — Auditability + security.**
  - [ ] `audit_logs` (work item + sub-resource actions, `entity_kind`
    scoped) + read endpoints; in-memory `WorkItemEvent` stream +
    `…/events/recent`.
...
- [ ] **T-6 — Front-end: routes + sub-resource workspaces + tests.**
  - [ ] Per-collection `/{collection}`, `…/new`, `…/[pid]`,
    ...
    (`…/[pid]/{goals,tasks,issues}`) and derived views
    (`…/[pid]/{timeline,burndown}`); the portfolio child roll-up.
  - [ ] vitest units (`ApiClient` + per-collection repositories) +
...
- [ ] **T-8 — Bulk import / export.**
  ...
  - [ ] `bulk_jobs` migration (shared doc §3 schema, with
    `UNIQUE (entity, kind, idempotency_key)`; `entity` distinguishes the
    four collections).
  - [ ] The five endpoints per collection (§9.6); `bg_pg` worker
    draining `queued → running → completed | completed_with_errors |
    failed`.
  ...
  - [ ] Per-row pipeline reusing the single-create validators +
    ...
    a `kind`/collection mismatch is a per-row error; keyless / unmatched
    rows → duplicate detection (same-kind) → review queue with
  ...
  - **Acceptance:** integration tests cover idempotent re-import,
    ...
    (`provenance = import`), masked vs full export of a work item with
...
- [ ] **T-9 — OpenAPI / Swagger + richer validation.**
  ...
  - [ ] Validation of `kind`/collection match, deterministic identifier
    shapes (UUID, external-PM-id patterns), `EntityRef` syntax,
    `portfolio_ref` (a valid portfolio `pid`, child kinds only),
    `in_language` (BCP-47), and relationship integrity (no
    self-reference, inverse-consistency, acyclicity — §5.8); `422` on
    failure.
  - **Acceptance:** Swagger UI serves the documented endpoints; a
    malformed-identifier / self-referencing-relationship /
    kind-mismatch test returns `422`.
