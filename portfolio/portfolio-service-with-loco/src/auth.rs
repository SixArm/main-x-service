...
//!
//! - `PORTFOLIO_PASETO_KEYS_URL` — optional URL of the auth service's
...
//!   wins over `PORTFOLIO_PASETO_KEYS` (`tracing::info!`), on failure the
...
//! - `PORTFOLIO_PASETO_KEYS` — the Ed25519 key set (JSON, OKP/Ed25519
...
//!   still boots).
//! - `PORTFOLIO_TOKEN_ISSUER` — expected `iss` (default
//!   `authentication-service`).
//! - `PORTFOLIO_TOKEN_AUDIENCE` — expected `aud` (default
//!   `main-x-service`).
//!
//! ## Blanket enforcement
//!
//! When `PORTFOLIO_REQUIRE_AUTH` is truthy (`1`/`true`/`yes`/`on`,
...
//! `PORTFOLIO_REQUIRE_AUTH` is on — a verified token is further checked
...
//! [`policy_from_env`]) from `PORTFOLIO_ABAC_POLICY` (inline JSON) or
//! `PORTFOLIO_ABAC_POLICY_FILE` (path); unset or unparsable ⇒ the
...
/// (called from `App::after_routes`). When `PORTFOLIO_PASETO_KEYS_URL` is
/// set (non-blank) the published key set is fetched over HTTP **once** —
/// no refresh loop — and, on success, wins over the
/// `PORTFOLIO_PASETO_KEYS` env key set; on fetch failure, or with the URL
...
pub async fn init() {
    ...
    let verifier = match std::env::var("PORTFOLIO_PASETO_KEYS_URL")
        .ok()
        .filter(|s| !s.trim().is_empty())
    {
        Some(url) => {
            let issuer = env_or("PORTFOLIO_TOKEN_ISSUER", DEFAULT_ISSUER);
            let audience = env_or("PORTFOLIO_TOKEN_AUDIENCE", DEFAULT_AUDIENCE);
...
/// `PORTFOLIO_REQUIRE_AUTH` and cached. Off by default — see the
...
pub fn require_auth() -> bool {
    ...
    *REQUIRE_AUTH
        .get_or_init(|| parse_bool(&std::env::var("PORTFOLIO_REQUIRE_AUTH").unwrap_or_default()))
}

/// The process-wide ABAC policy, read once from
/// `PORTFOLIO_ABAC_POLICY` / `PORTFOLIO_ABAC_POLICY_FILE` and cached
...
/// Load the ABAC policy: `PORTFOLIO_ABAC_POLICY` (inline JSON) wins,
/// then `PORTFOLIO_ABAC_POLICY_FILE` (path to a JSON file), else the
...
pub fn policy_from_env() -> Policy {
    let source = std::env::var("PORTFOLIO_ABAC_POLICY")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .or_else(|| {
            let path = std::env::var("PORTFOLIO_ABAC_POLICY_FILE")
                .ok()
                .filter(|v| !v.trim().is_empty())?;
...
fn build_from_env() -> Verifier {
    let issuer = env_or("PORTFOLIO_TOKEN_ISSUER", DEFAULT_ISSUER);
    let audience = env_or("PORTFOLIO_TOKEN_AUDIENCE", DEFAULT_AUDIENCE);
    let keys = std::env::var("PORTFOLIO_PASETO_KEYS")
        .ok()
        .filter(|s| !s.trim().is_empty())
