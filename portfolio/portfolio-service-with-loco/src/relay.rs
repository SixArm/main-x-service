...
//! (`PORTFOLIO_EVENT_TRANSPORT=outbox`) **and** `PORTFOLIO_EVENT_RELAY`
...
/// Whether the relay background loop should run: `PORTFOLIO_EVENT_RELAY`
/// truthy (`1`/`true`/`yes`/`on`). Off by default.
#[must_use]
pub fn relay_enabled() -> bool {
    matches!(
        std::env::var("PORTFOLIO_EVENT_RELAY")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase()
            .as_str(),
...
/// Relay poll interval in seconds (`PORTFOLIO_EVENT_RELAY_INTERVAL_SECS`,
/// default 5, floored at 1).
#[must_use]
pub fn interval_secs() -> u64 {
    std::env::var("PORTFOLIO_EVENT_RELAY_INTERVAL_SECS")
        .ok()
...
/// Outbox retention in days (`PORTFOLIO_EVENT_RETENTION_DAYS`, default 7).
#[must_use]
pub fn retention_days() -> i64 {
    std::env::var("PORTFOLIO_EVENT_RETENTION_DAYS")
        .ok()
...
mod tests {
    ...
    fn logging_sink_sends_ok() {
        ...
        let out = rt.block_on(async {
            LoggingSink
                .send("work_item", "pid-1", &serde_json::json!({"kind": "created"}))
                .await
        });
        assert!(out.is_ok());
    }
    ...
    fn capturing_sink_records_entity_and_key() {
        ...
        assert_eq!(sink.0.lock().unwrap().as_slice(), &[("work_item".to_string(), "pid-9".to_string())]);
    }
    ...
    fn config_defaults_are_safe() {
        ...
        if std::env::var("PORTFOLIO_EVENT_RELAY").is_err() {
            assert!(!relay_enabled());
        }
        if std::env::var("PORTFOLIO_EVENT_RELAY_INTERVAL_SECS").is_err() {
            assert_eq!(interval_secs(), 5);
        }
        if std::env::var("PORTFOLIO_EVENT_RETENTION_DAYS").is_err() {
