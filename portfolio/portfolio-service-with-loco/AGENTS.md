...
Entry point for AI coding agents working in the `portfolio-service` crate:
...
the canonical [`portfolio-matcher`](../portfolio-matcher-rust-crate). The
API DTO **is** `portfolio_matcher::WorkItem` — stored verbatim (JSONB) and
...
CRUD + `ILIKE` name search + matching (embed `portfolio-matcher`,
