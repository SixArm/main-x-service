...
- [x] T-31: *(resolved 2026-09-25.)* Close the ten flat FHIR fidelity
  gaps that T-30 `TODO`-marked. `to_fhir_basic` now emits, and
  `from_fhir_basic` recovers, `description`, `about`, `url`, `same_as`,
  `assesses`, `competency_required`, `number_of_credits`, `status`,
  `active`, and `provider_id` as documented `urn:mxi:course:*`
  extensions (`description`, `about`, `url`, `same-as`, `assesses`,
  `competency-required`, `number-of-credits`, `status`, `active`,
  `provider-id`; the list-valued ones repeat one extension per value).
  `FhirExtension` gained FHIR-typed `valueBoolean` (for `active`) and
  `valueUnsignedInt` (for `number_of_credits`) beside `valueString`.
  `status` and `active` are always emitted, since both have non-`None`
  defaults; an absent one inbound keeps the `Course::new` default.
  **Malformed typed values are a `400`, not a silent drop** — an
  unknown `status` tag, a non-UUID `provider-id`, or a
  `number-of-credits` / `active` extension missing its typed value —
  consistent with `agents/share/fhir.md` §2 ("never silent").
  - **Still open (still `TODO(fhir)`-marked):** credentials, syllabus
    sections, and the `instances` sub-resource — each is structured
    and needs a complex (nested) extension or a contained resource,
    not a flat one.
  - **Acceptance met:** `cargo test --lib` 139/139 (was 135; +4 in
    `fhir::tests`: full round-trip of all ten fields with
    non-default `status`/`active`, typed `value[x]` serialization,
    absent-extension defaults, malformed-value rejection);
