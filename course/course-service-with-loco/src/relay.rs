...
//!
//! Phase 2 ([`crate::streaming::envelope`] + [`crate::db::outbox`])
//! durably enqueues each event into the `course_outbox` table inside the
//! entity mutation's transaction. Phase 3 is the **relay**: a background
//! loop that drains unpublished outbox rows to an [`EventSink`], stamps
//! `published_at`, and periodically purges old published rows. Delivery is
//! **at-least-once** (a crash between the sink send and the `published_at`
//! stamp re-sends), so sinks/consumers dedupe on the envelope `event_id`
//! (see `agents/share/event-bus.md` §6).
//!
...
//! `FluvioSink` (BUS-3; only compiled under this crate's `fluvio` cargo
...
//! (`COURSE_EVENT_TRANSPORT=outbox`) **and** `COURSE_EVENT_RELAY` is truthy
...
//! [`LoggingSink`] when `COURSE_FLUVIO_ENDPOINT` (§7) is configured — see
...
use crate::db::models::course_outbox::{Column as OutboxColumn, Entity as OutboxEntity};
...
/// The real-broker sink (BUS-3; behind the `fluvio` cargo feature): ships
...
pub async fn purge_published(db: &DatabaseConnection, retention_days: i64) -> crate::Result<u64> {
    let cutoff = time::OffsetDateTime::now_utc() - time::Duration::days(retention_days.max(0));
    let res = OutboxEntity::delete_many()
...
/// Whether the relay background loop should run: `COURSE_EVENT_RELAY`
/// truthy (`1`/`true`/`yes`/`on`). Off by default.
#[must_use]
pub fn relay_enabled() -> bool {
    matches!(
        std::env::var("COURSE_EVENT_RELAY")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase()
            .as_str(),
...
/// Relay poll interval in seconds (`COURSE_EVENT_RELAY_INTERVAL_SECS`,
/// default 5, floored at 1).
#[must_use]
pub fn interval_secs() -> u64 {
    std::env::var("COURSE_EVENT_RELAY_INTERVAL_SECS")
        .ok()
...
/// Outbox retention in days (`COURSE_EVENT_RETENTION_DAYS`, default 7).
#[must_use]
pub fn retention_days() -> i64 {
    std::env::var("COURSE_EVENT_RETENTION_DAYS")
        .ok()
        .and_then(|v| v.trim().parse::<i64>().ok())
        .unwrap_or(7)
}

/// Fluvio broker endpoint (`COURSE_FLUVIO_ENDPOINT`, §7). Unset/blank ⇒ no
...
pub fn fluvio_endpoint() -> Option<String> {
    std::env::var("COURSE_FLUVIO_ENDPOINT")
        .ok()
        .filter(|s| !s.trim().is_empty())
}

/// The topic events publish to (`COURSE_EVENT_TOPIC`, default
/// `mxi.course.events` per §7's `mxi.<entity>.events` convention).
#[must_use]
pub fn event_topic() -> String {
    std::env::var("COURSE_EVENT_TOPIC")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| "mxi.course.events".to_string())
}
...
///
/// Sink selection: no `COURSE_FLUVIO_ENDPOINT` ⇒ [`LoggingSink`]. An
...
pub fn spawn(db: DatabaseConnection) {
    ...
    if endpoint.is_some() && !cfg!(feature = "fluvio") {
        tracing::error!(
            "COURSE_FLUVIO_ENDPOINT is set but this binary was built without the `fluvio` \
...
mod tests {
    ...
    fn logging_sink_sends_ok() {
        ...
        let out = rt.block_on(async {
            LoggingSink
                .send("course", "pid-1", &serde_json::json!({"kind": "created"}))
                .await
        });
        assert!(out.is_ok());
    }
    ...
    fn capturing_sink_records_entity_and_key() {
        ...
        rt.block_on(async {
            sink.send("course", "pid-9", &serde_json::json!({}))
                .await
                .unwrap();
        });
        assert_eq!(
            sink.0.lock().unwrap().as_slice(),
            &[("course".to_string(), "pid-9".to_string())]
        );
    }
    ...
    fn config_defaults_are_safe() {
        ...
        if std::env::var("COURSE_EVENT_RELAY").is_err() {
            assert!(!relay_enabled());
        }
        if std::env::var("COURSE_EVENT_RELAY_INTERVAL_SECS").is_err() {
            assert_eq!(interval_secs(), 5);
        }
        if std::env::var("COURSE_EVENT_RETENTION_DAYS").is_err() {
