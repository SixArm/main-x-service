...
//! Nocourse in a row can attest to its own continued existence. Catching
...
//! a silent edit to holding the key; it does nocourse about `DELETE`.
...
pub const CAVEAT: &str = "A verified result attests that no examined row's content was altered \
     without the key. It does NOT attest that no row was deleted: nocourse in a row can \
