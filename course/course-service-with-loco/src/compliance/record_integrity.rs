//! Row-level integrity over the assembled `Course` record.
//!
//! ## Why the assembled record, not the root row
//!
//! A course's data lives across several tables — the root `courses` row
...
//! the same value `GET /api/courses/{id}` returns.
//!
...
//!
//! ## Soft-deleted courses carry no digest
//!
//! `apply_course_soft_delete` performs a **partial** update — it sets
//! `active`/`deleted_at` without loading the record — and reassembling
//! the full course there would add several queries to a path that merge
//! also uses. Rather than leave a digest that no longer describes the
//! row, the soft-delete clears all three columns.
//!
//! The consequence is stated rather than hidden: a soft-deleted course
//! reports as `unhashed`, not as verified and not as tampered. That is
//! the honest reading — we hold no attestation for that state — and it
//! avoids the worse outcome, which would be every soft delete showing up
//! as a false tampering finding.
...
pub struct RecordInput<'a> {
    ...
    pub course: &'a Course,
    /// Whether the row is live. In the pre-image because reactivating a
    /// deleted record is a content change.
    pub active: bool,
}
...
pub fn preimage(input: &RecordInput<'_>) -> Vec<u8> {
    ...
    field(
        &serde_json::to_string(input.course).unwrap_or_else(|_| "\u{0}unserializable".to_string()),
    );
    ...
    for row in rows {
        ...
        if !sha256_ok || !sha3_ok || !mac_ok {
            report.mismatched.push(RecordMismatch {
                id: row.course.id.to_string(),
                name: row.course.name.clone(),
            });
        }
    }
...
mod tests {
    ...
    fn course(name: &str) -> Course {
        let mut t = Course::new(name);
        t.id = Uuid::from_u128(1);
        t
    }

    fn input(t: &Course) -> RecordInput<'_> {
        RecordInput {
            id: t.id,
            course: t,
            active: true,
    ...
    fn spec_documents_this_version_tag() {
        assert_eq!(RECORD_HASH_VERSION, "c-r1");
    }
    ...
    /// The live flag is bound in, so reactivating a deleted record by
    /// flipping the column is caught.
    #[test]
    fn the_active_flag_is_bound_in() {
        let t = course("Widget");
