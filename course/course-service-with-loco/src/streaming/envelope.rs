...
//! [`CourseEvent`](super::CourseEvent) / [`EventPublisher`](super::EventPublisher)
//! in-memory publisher. The envelope is the shape that is persisted to
//! `course_outbox` (Phase 2) and later relayed to Fluvio (Phase 3); the
//! in-memory publisher stays exactly as before for the `memory` transport,
//! so shipping this module is behaviour-neutral until
//! `COURSE_EVENT_TRANSPORT=outbox` is set.
//!
//! [`Envelope::for_event`] builds the envelope from a domain
//! [`Course`](crate::models::Course); [`EventView`] is the flat operator
//! projection; [`EventTransport`] / [`transport`] read the configured
//! transport once from the environment.
//!
//! Note: this module's [`EventKind`] (the lowercase `created` /
//! `updated` / `deleted` / `merged` wire tokens the outbox stores) is
//! distinct from the legacy PascalCase [`super::EventKind`] used by the
//! in-memory [`CourseEvent`]; it is intentionally **not** re-exported at
//! the `streaming` root, so refer to it as
...
/// The kind of change that occurred (durable-bus wire tokens).
...
pub struct Envelope {
    ...
    /// The entity name (`snake_case`), e.g. `"course"`.
    ///
...
impl Envelope {
    /// Build an envelope describing `kind` applied to `course`, minting a
    ...
    #[must_use]
    pub fn for_event(course: &Course, kind: EventKind) -> Self {
        Self {
            event_id: Uuid::new_v4(),
            schema_version: SCHEMA_VERSION,
            entity: ENTITY,
            kind,
            pid: course.id.to_string(),
            seq: next_seq(),
            actor: None,
    ...
    #[must_use]
    pub fn for_merge(survivor: &Course, merged_from: &Uuid) -> Self {
        Self {
...
pub enum EventTransport {
    /// In-memory publisher; no DB, no transaction (default).
    Memory,
    /// Transactional outbox: a `course_outbox` row per event, written on
    /// the entity mutation's transaction (Phase 2).
    Outbox,
}

impl EventTransport {
    /// Parse the `COURSE_EVENT_TRANSPORT` value. `outbox` selects the
...
/// The process-wide transport, read once from `COURSE_EVENT_TRANSPORT` and
...
pub fn transport() -> EventTransport {
    ...
    *TRANSPORT.get_or_init(|| {
        std::env::var("COURSE_EVENT_TRANSPORT")
...
mod tests {
    ...
    fn for_event_leaves_merged_from_none() {
        let env = super::Envelope::for_event(&a_course(), EventKind::Updated);
    ...
    fn seq_is_monotonic_across_envelopes() {
        let course = a_course();
        let a = super::Envelope::for_event(&course, EventKind::Created);
        let b = super::Envelope::for_event(&course, EventKind::Updated);
    ...
    fn event_view_projects_the_frozen_shape() {
        let course = a_course();
        let env = super::Envelope::for_event(&course, EventKind::Deleted);
