...
//! (`/api/courses`, never `/api/v1/courses`), and a client selects the
//! representation version with `Accepts-version: 1.0`. This module mirrors
//! the **reference implementation** in the event service.
//!
...
//!   orthogonal to any auth guard and a near-noop when the header is
...
/// (`/api` and `/api/...`). The FHIR surface (`/fhir/*`) is exempt — it
/// advertises `fhirVersion` in its `CapabilityStatement` instead.
...
mod tests {
    ...
    fn only_api_paths_are_versioned() {
        ...
        assert!(is_versioned_path("/api/courses"));
        assert!(!is_versioned_path("/fhir/Basic"));
