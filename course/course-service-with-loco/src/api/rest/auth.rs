...
//! ## Blanket enforcement
//!
//! When `COURSE_REQUIRE_AUTH` is truthy (`1`/`true`/`yes`/`on`,
//! case-insensitive), the [`enforce`] decision — wired as the
//! [`require_auth_mw`] middleware on both router surfaces — requires a
//! valid bearer token on **every** route except the small public
//! allow-list ([`is_public_path`]: health/ping, OpenAPI/Swagger,
//! Prometheus metrics, `/api/health`, `/fhir/metadata`). It is
//! deny-by-default: a route outside the allow-list is guarded even if it
//! is outside `/api` and `/fhir` (SEC-G5). It is **off by default**:
...
//! ## Authorization (ABAC)
//!
//! Inside the same guard — so it applies only when `COURSE_REQUIRE_AUTH`
...
//! construction ([`policy_from_env`]) from `COURSE_ABAC_POLICY` (inline
//! JSON) or `COURSE_ABAC_POLICY_FILE` (path); unset or unparsable ⇒ the
...
/// Read the blanket-enforcement flag from `COURSE_REQUIRE_AUTH` via
...
pub fn require_auth_from_env() -> bool {
    parse_bool(&std::env::var("COURSE_REQUIRE_AUTH").unwrap_or_default())
}

/// Paths that stay public even when blanket enforcement is on (SEC-G5,
/// deny-by-default): loco's liveness/readiness probes (`/_health`,
/// `/_ping`), the `OpenAPI` doc (`/api-docs/openapi.json`) and Swagger UI
/// (`/swagger-ui*`), the Prometheus scrape (`/metrics.prom`), the API
/// health check (`/api/health`), and the FHIR `CapabilityStatement`
/// (`/fhir/metadata`, capability discovery — fhir.md §8). This is the
/// **complete** allow-list; every other route requires a valid bearer
/// token when enforcement is on, whether or not it is under `/api` or
/// `/fhir`.
...
pub fn derive_action(method: &Method, path: &str) -> Action {
    ...
    // check, so `POST /api/courses/merge/` (and `//`) is still classified
    // as `Destructive` rather than silently downgraded to `Write` — which
    // would let an `access=write` (non-admin) caller reach a destructive op.
...
/// Load the ABAC policy: `COURSE_ABAC_POLICY` (inline JSON) wins, then
/// `COURSE_ABAC_POLICY_FILE` (path to a JSON file), else the built-in
...
pub fn policy_from_env() -> Policy {
    let source = std::env::var("COURSE_ABAC_POLICY")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .or_else(|| {
            let path = std::env::var("COURSE_ABAC_POLICY_FILE")
                .ok()
                .filter(|v| !v.trim().is_empty())?;
...
/// # Errors
///
/// `401` when enforcement is on, the path is not in the [`is_public_path`]
/// allow-list, and the request carries no valid bearer
...
///
/// Built lazily from `COURSE_PASETO_KEYS` on first use; [`init`] swaps in
...
/// boot). With `COURSE_PASETO_KEYS_URL` set, the published key set is
...
pub async fn init() {
    if std::env::var("COURSE_PASETO_KEYS_URL")
        .ok()
...
/// `COURSE_PASETO_KEYS_REFRESH_SECS` is unset. One hour: rotation is
...
pub fn spawn_key_refresh() {
    let Some(url) = std::env::var("COURSE_PASETO_KEYS_URL")
        .ok()
    ...
    let secs = std::env::var("COURSE_PASETO_KEYS_REFRESH_SECS")
        .ok()
    ...
    let issuer = super::state::env_or("COURSE_TOKEN_ISSUER", super::state::DEFAULT_ISSUER);
    let audience = super::state::env_or("COURSE_TOKEN_AUDIENCE", super::state::DEFAULT_AUDIENCE);
    ...
    tracing::info!(secs, "polling COURSE_PASETO_KEYS_URL for key rotation");
}
...
/// `COURSE_ABAC_POLICY_FILE` changes on disk, so an operator can edit the
/// policy and have it take effect without a restart. Polls the file's
/// mtime (not an OS notify — this stays dependency-light).
///
/// A no-op when the variable is unset: an inline `COURSE_ABAC_POLICY` (or
...
pub fn spawn_policy_watcher() {
    let Some(path) = std::env::var("COURSE_ABAC_POLICY_FILE")
        .ok()
    ...
    tracing::info!(
        ...
        "watching COURSE_ABAC_POLICY_FILE for changes"
    );
}
...
mod tests {
    ...
    /// `parse_bool` (the `COURSE_REQUIRE_AUTH` semantics) accepts the
    ...
    fn test_derive_action_matrix() {
        for method in [Method::GET, Method::HEAD, Method::OPTIONS] {
            assert_eq!(derive_action(&method, "/api/courses"), Action::Read);
        }
        assert_eq!(
            derive_action(&Method::DELETE, "/api/courses/1"),
            Action::Delete
        );
        for path in [
            "/api/courses/merge",
        ...
        assert_eq!(derive_action(&Method::POST, "/api/courses"), Action::Write);
        assert_eq!(
            derive_action(&Method::POST, "/api/courses/check-duplicates"),
            Action::Write
        );
        assert_eq!(derive_action(&Method::PUT, "/api/courses/1"), Action::Write);
        assert_eq!(
            derive_action(&Method::PATCH, "/api/courses/1"),
            Action::Write
        );
        // GET on a destructive-suffixed path is still a read — only
        // POST consults the suffix list.
        assert_eq!(
            derive_action(&Method::GET, "/api/courses/merge"),
            Action::Read
        );
        // SEC-G6: a trailing slash must NOT downgrade a destructive POST
        // to Write (a non-admin `access=write` caller must not reach the
        // merge/dedup/import ops via a `/`-suffixed path).
        for path in [
            "/api/courses/merge/",
            "/api/courses/merge//",
            "/api/courses/deduplicate/",
    ...
    /// SEC-G5: with enforcement on, a route **outside** the public
    /// allow-list is guarded even when it is outside `/api` and `/fhir` —
    /// the guard is deny-by-default, not allow-unless-in-prefix. Each of
    /// these out-of-prefix paths rejects with `401` when no token is
    /// presented (previously they slipped through unguarded).
    #[test]
    ...
    fn test_abac_access_admin_allows_destructive() {
        ...
        for path in ["/api/courses/merge", "/api/courses/deduplicate"] {
            assert!(
                enforce(
                    true,
                    &Method::POST,
    ...
    fn test_abac_svc_true_allows_everything() {
        ...
        for (method, path) in [
            ...
            (Method::PUT, "/api/courses/1"),
            (Method::DELETE, "/api/courses/1"),
            (Method::POST, "/api/courses/merge"),
            (Method::POST, "/api/courses/deduplicate"),
        ] {
            assert!(
    ...
    /// without panicking. `COURSE_PASETO_KEYS` is unset in the test
