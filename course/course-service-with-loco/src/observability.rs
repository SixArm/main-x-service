...
//! (unlike person/worker/event, which had a dead stub to replace — PRO-H9)
//! — this is new from scratch, following the proven port rather than
//! re-deriving it.
//!
...
//! | `OTLP_SERVICE_NAME` | `course-service` | `service.name` resource attribute |
//! | `OTLP_ENDPOINT` | `http://localhost:4317` | OTLP/gRPC collector endpoint |
//!
//! These three variables are **deliberately not `COURSE_`-prefixed**,
...
pub const DEFAULT_SERVICE_NAME: &str = "course-service";
...
fn http_metrics() -> &'static HttpMetrics {
    ...
    METRICS.get_or_init(|| {
        let meter = global::meter("course-service");
        HttpMetrics {
            duration: meter
...
/// embed record UUIDs (`/api/courses/{id}`, `/api/courses/{id}/instances/{instance_id}`,
...
///
/// This is a **second**, complementary middleware to
/// [`crate::metrics::track_http_requests_mw`] (T-18's Prometheus
/// `http_requests_total{path,status}` counter) — the two are independent
/// sinks (`OTel` export vs. `/metrics.prom` scraping) layered side by side,
...
mod tests {
    ...
    fn exporter_diagnostics_are_added_to_a_default_filter() {
        ...
        let base = || tracing_subscriber::EnvFilter::new("course_service=info");
    ...
    fn an_operator_supplied_filter_is_left_alone() {
        ...
        assert_eq!(untouched.to_string(), "course_service=info");
    }
    ...
    fn explicit_service_name_and_endpoint_are_honoured() {
        let config = TelemetryConfig::resolve(
            Some("course-service-canary"),
            Some("http://collector.internal:4317"),
        );
        assert_eq!(config.service_name, "course-service-canary");
        assert_eq!(
    ...
    fn resource_carries_service_name_and_version() {
        ...
        let name = resource
            ...
            .expect("service.name");
        assert_eq!(name.as_str(), "course-service");
