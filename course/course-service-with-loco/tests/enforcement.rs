...
//! with `COURSE_REQUIRE_AUTH` **on** — the "activation proof" (AU-1).
//!
...
use course_service::api::rest::{AppState, create_router};
use course_service::config::Config;
use course_service::db::create_connection;
...
/// A valid course payload with a unique name, so a create that reaches
/// the handler is not turned away as a duplicate.
///
/// Built by serializing `Course::new`, not by hand. The wire type
...
fn course_body() -> Value {
    ...
    serde_json::to_value(Course::new(format!("{unique} Enforcement Course"))).expect("serialize")
}
...
/// `POST` a course payload, returning the status **and** the body, so a
/// refusal reports the server's reason rather than only a status
/// mismatch.
async fn post_course_verbose(app: &axum::Router, token: Option<&str>) -> (StatusCode, String) {
    ...
    let response = app
        ...
        .oneshot(
            builder
                .body(Body::from(serde_json::to_vec(&course_body()).unwrap()))
                .unwrap(),
        )
        .await
        .unwrap();
...
/// `POST` a course payload with an optional bearer, returning the status.
async fn post_course(app: &axum::Router, token: Option<&str>) -> StatusCode {
    let mut builder = Request::builder()
        .method("POST")
        .uri("/api/courses")
        .header("content-type", "application/json");
...
async fn enforcement_on_gates_the_real_router() {
    ...
    unsafe {
        std::env::set_var("COURSE_REQUIRE_AUTH", "1");
        std::env::set_var("COURSE_PASETO_KEYS", keys.to_string());
        // No COURSE_ABAC_POLICY ⇒ the built-in default policy: read
    ...
    assert_eq!(
        get(&app, "/api/courses/search?q=anyone", None).await,
        StatusCode::UNAUTHORIZED,
        "a protected read without a token is 401"
    );
    assert_eq!(
        post_course(&app, None).await,
        StatusCode::UNAUTHORIZED,
    ...
    assert_eq!(
        get(&app, "/api/courses/search?q=anyone", Some("not-a-token")).await,
        StatusCode::UNAUTHORIZED,
    ...
    let reader = mint(&kid, &[]);
    assert_eq!(
        get(&app, "/api/courses/search?q=anyone", Some(&reader)).await,
        StatusCode::OK,
        "any authenticated caller may read"
    );
    assert_eq!(
        post_course(&app, Some(&reader)).await,
    ...
    let (status, body) = post_course_verbose(&app, Some(&writer)).await;
    assert_eq!(
        status,
        StatusCode::CREATED,
    ...
    unsafe {
        std::env::remove_var("COURSE_REQUIRE_AUTH");
        std::env::remove_var("COURSE_PASETO_KEYS");
    }
}
