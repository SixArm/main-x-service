...
//!
//! Ported from person-service's `tests/otlp_middleware.rs`.
//! `tests/otlp_export.rs` proves the export pipeline; this proves the
//! thing the service actually runs — the exact
//! [`observability::trace_mw`](course_service::observability::trace_mw)
...
//! crate's router-construction surfaces). It layers `trace_mw` onto a
//! minimal, real Axum router — rather than booting the full application,
//! which would need a database — serves it over a real socket, makes a
//! real HTTP request, and then asserts that
//!
...
use course_service::observability::{TelemetryConfig, build_tracer_provider, trace_mw};
...
async fn a_served_request_exports_a_span_and_returns_its_traceparent() {
    ...
    let config = TelemetryConfig {
        service_name: "course-service-mw-test".to_string(),
    ...
    tracing_subscriber::registry()
        .with(tracing_opentelemetry::layer().with_tracer(provider.tracer("course-service")))
        .with(tracing_subscriber::EnvFilter::new(
            "course_service=trace,otlp_middleware=trace",
        ))
        .init();
