...
// `$app/environment`. Stub it false so the store stays on its default
// locale (English) and never touches localStorage during the test.
...
describe("+layout top-bar navigation", () => {
    it("hamburger toggles nav visibility (aria-expanded + .open)", async () => {
        const { getByLabelText, container } = render(Layout, {
            children,
            // The layout now reads `data.signedIn` (server-resolved session).
            data: { signedIn: false },
        });
