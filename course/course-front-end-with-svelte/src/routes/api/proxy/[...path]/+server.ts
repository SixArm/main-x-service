...
// for a short-lived PASETO and forwards the request to the course service
...
// reaches the course service.

import type { RequestHandler } from "./$types";
import { COURSE_API_URL } from "$lib/server/config";
...
const proxy: RequestHandler = async ({
  ...
  const target = `${COURSE_API_URL}/${params.path}${url.search}`;
