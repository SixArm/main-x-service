...
<script lang="ts">
    ...
    // SSR). `lang` must read BCP 47 hyphenated form ("en-US"), but
    // `i18n.locale` uses an underscore for a region subtag ("en_US") — this
    // must agree with what PickerBar's LocalePicker itself writes via its
    // own `bcp47LocaleTag`, since both write the same attribute. RTL
    // locales (ar / ur) flip the document direction to "rtl".
    $effect(() => {
        if (browser) {
