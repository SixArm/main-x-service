...
| NFR-8 | **Security:** SSO via the [authentication entity](../../authentication/) — service verifies short-lived PASETO v4.public tokens offline against the auth-service's published Ed25519 key; front-end BFF attaches the token server-side (browser holds only the httpOnly session cookie) per [`agents/share/authentication-sessions.md`](../../agents/share/authentication-sessions.md). Not yet enforced (service T-15) | Roadmap §15 |
