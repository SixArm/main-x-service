...
The course-matcher spec uses the §1–§25 matcher-crate shape (distinct
...
| How `course-service` consumes the crate | §20 |
...
| §19–§21 quality / consumption / compat | top-level docs, integration with `course-service` |
...
- **Adding a deterministic-identifier scheme without bridge-test
  coverage** in `course-service`. A false positive at score 1.0 is
  the worst-case bug.
...
- **Using `unwrap` / `expect` in library code.** Total functions
  only.
- **Scoring `course_code` across providers.** Per §10–§11, the
  course-code component MUST be gated on `provider_id` equality
