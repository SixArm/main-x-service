...
|---|---|
| `match_courses` | Deserialize a JSON `[course_a, course_b]` tuple → `MatchingEngine::match_courses`. The whole deserialize → normalize → score path; asserts finite score in `[0,1]` in both argument orders. |
| `normalize` | The pure `normalize` free functions (fold / course code / fold-set) over arbitrary UTF-8 — never-panic. |
...
cargo +nightly fuzz run match_courses     # fuzz until a crash / Ctrl-C
