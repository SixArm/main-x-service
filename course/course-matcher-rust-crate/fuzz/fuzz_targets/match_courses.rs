...
//!
//! Interprets the fuzz bytes as a JSON `[course_a, course_b]` tuple,
//! then runs `MatchingEngine::match_courses`. Pins the two load-bearing
...
use course_matcher::{MatchConfig, MatchingEngine, Course};
...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok((a, b)) = serde_json::from_slice::<(Course, Course)>(data) else {
        return;
    };

    let engine = MatchingEngine::new(MatchConfig::default());
    // Run both orderings so either argument position is exercised.
    for result in [engine.match_courses(&a, &b), engine.match_courses(&b, &a)] {
        assert!(
