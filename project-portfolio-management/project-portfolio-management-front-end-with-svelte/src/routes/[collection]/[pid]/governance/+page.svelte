...
<script lang="ts">
  ...
  const collection = $derived(page.params.collection as Collection);
  ...
  async function refresh() {
    try {
      [summary, journey, risks, budget, benefits, mappings, milestones, allocations, objectives] =
        await Promise.all([
          ppm.governance(collection, pid),
          ppm.gateJourney(collection, pid),
          ppm.listRisks(collection, pid),
          ppm.budget(collection, pid),
          ppm.benefits(collection, pid),
          ppm.itemObjectives(collection, pid),
          ppm.milestones(collection, pid),
          ppm.allocations(collection, pid),
          ppm.listObjectives(),
        ]);
      error = null;
    } catch (err) {
...
<details open>
              ...
              <form
                ...
                onsubmit={(e) => {
                  ...
                  if (Number.isFinite(minor)) {
                    act(() => ppm.recordActual(collection, pid, line.pid, minor));
                    input.value = "";
                  }
                }}
              >
  ...
  <form
    ...
    onsubmit={(e) => {
      ...
      act(() =>
        ppm.createBudgetLine(collection, pid, {
...
<details open>
            ...
            <td>
              <form
                ...
                onsubmit={(e) => {
                  ...
                  if (Number.isFinite(minor)) {
                    act(() =>
                      ppm.realizeBenefit(collection, pid, benefit.pid, {
                        amount_minor: minor,
                        status: "on_track",
                      }),
                    );
  ...
  <form
    ...
    onsubmit={(e) => {
      ...
      act(() =>
        ppm.createBenefit(collection, pid, {
          title: benefitTitle,
...
<details>
  ...
  <form
    ...
    onsubmit={(e) => {
      e.preventDefault();
      if (mapObjective) act(() => ppm.linkObjective(collection, pid, mapObjective, mapWeight));
    }}
  >
...
<details>
  ...
  <form
    ...
    onsubmit={(e) => {
      e.preventDefault();
      act(() => ppm.createMilestone(collection, pid, { name: milestoneName, due: milestoneDue })).then(
        () => (milestoneName = ""),
      );
    }}
  >
...
<details>
  ...
  <form
    ...
    onsubmit={(e) => {
      e.preventDefault();
      act(() =>
        ppm.createAllocation(collection, pid, {
