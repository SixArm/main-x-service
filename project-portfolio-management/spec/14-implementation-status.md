...
| service | Controls register (T-26 / FR-38–39, partial) | Pure `src/controls.rs` (three timings, four comparators, verdict derivation, coverage); migration `m20260825_000002_controls` (the `verdict` CHECK keeps `unmeasured` a real third value); eight routes under `/api/plans/{pid}/controls`, `/api/controls`, `/api/readings`; feedforward may block, feedback may only record |
...
| Controls: no action → task / issue conversion (the `converted_*_pid` columns exist, always `NULL`), and the controls that already exist in all but name (gate readiness, WIP limits, the SLE, retrospectives) are not registered, so coverage reports only newly-authored controls | T-26 |
