...
impl Default for MatchResult {
    ...
    /// [`Confidence::Low`], and an empty breakdown.
    fn default() -> Self {
        Self {
            score: 0.0,
            is_match: false,
...
impl Confidence {
    ...
    ///
    /// ```
    /// use project_portfolio_management_matcher::Confidence;
    ///
...
/// Per-component scores. `None` means the component was skipped because
/// one or both records lacked the data, so it did not contribute to the
/// weighted sum.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct MatchBreakdown {
    /// Name similarity (Jaro-Winkler + Soundex bonus).
    pub name_score: Option<f64>,
    /// Goal-title (Jaccard) overlap.
    pub goals_score: Option<f64>,
    /// Owner-scoped code similarity.
    ...
    /// Relationships (typed-set Jaccard) similarity.
...
mod tests {
    ...
    fn weighted_average_renormalises_over_present_weights() {
        // name 1.0 @ 0.30 and code 0.0 @ 0.15; others absent.
