...
| Engineering | `POST`/`GET /{collection}/{pid}/tasks` (+ `PUT`/`PATCH`(move; WIP-limit env caps)/`DELETE /{t_pid}`; story points) · `/{pid}/sprints` (+ `/{s_pid}/notes` retro/feedback + `convert`) · `GET /{pid}/burndown?sprint=` (honest, done_at-only) · `GET /{pid}/velocity` · `GET /{pid}/standup` · `GET /engineering/{blocked,moscow,delivery-links,milestone-calendar}` |
