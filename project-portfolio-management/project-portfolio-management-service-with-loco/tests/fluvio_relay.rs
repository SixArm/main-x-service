...
//! (BUS-3, following BUS-1's case-service reference;
...
//! PROJECT_PORTFOLIO_MANAGEMENT_FLUVIO_ENDPOINT=127.0.0.1:9103 \
...
use project_portfolio_management_service::models::_entities::event_outbox;
use project_portfolio_management_service::relay::{self, FluvioSink};
...
async fn fluvio_relay_publishes_an_outbox_row_to_a_real_topic() {
    let endpoint = std::env::var("PROJECT_PORTFOLIO_MANAGEMENT_FLUVIO_ENDPOINT").expect(
        "set PROJECT_PORTFOLIO_MANAGEMENT_FLUVIO_ENDPOINT to the broker's SC address (e.g. 127.0.0.1:9103)",
    );

    // Durable transport, so `create_and_emit` writes an outbox row.
    unsafe {
        std::env::set_var("PROJECT_PORTFOLIO_MANAGEMENT_EVENT_TRANSPORT", "outbox");
    }

    request::<App, _, _>(|_request, ctx| async move {
        let wi = Plan::new("Fluvio Relay Round Trip");
        let model = streaming::create_and_emit(&ctx.db, &wi, None)
            .await
            .expect("create_and_emit under outbox transport");

        let sink = FluvioSink::connect(&endpoint, "mxi.plan.events.test")
            .await
