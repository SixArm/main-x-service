...
//!
//! Ported from case-service's `tests/otlp_export.rs` (itself from
//! care-pathway-service's, itself organization-service's, itself
//! course-service's, itself link-graph-service's, the family's first
//! working exporter). Each case stands up a **real OTLP/gRPC collector
//! in-process** ([`otlp_collector`]), points the exporter at it, and
//! asserts on the decoded protobuf that arrives over the socket.
//!
...
use project_portfolio_management_service::observability::{
...
async fn a_tracing_span_reaches_a_real_otlp_collector() {
    ...
    let config = TelemetryConfig {
        service_name: "project-portfolio-management-service-otlp-test".to_string(),
...

    {
        let layer = tracing_opentelemetry::layer()
            .with_tracer(provider.tracer("project-portfolio-management-service"));
        let subscriber = tracing_subscriber::registry().with(layer);
        tracing::subscriber::with_default(subscriber, || {
            let span = tracing::info_span!("plans.get", plan.id = "0c4f1e2a-test");
            let _entered = span.enter();
            tracing::info!("serving a plan read");
        });
    }
    ...
    assert!(
        service_names.contains(&"project-portfolio-management-service-otlp-test".to_string()),
    ...
    let span = spans
        .iter()
        .find(|span| span.name == "plans.get")
        .unwrap_or_else(|| {
            panic!(
                "exported spans did not include `plans.get`: {:?}",
                spans.iter().map(|span| &span.name).collect::<Vec<_>>()
            )
        });
    assert!(
        span.attributes.iter().any(|kv| kv.key == "plan.id"),
...
async fn a_metric_reaches_a_real_otlp_collector() {
    ...
    let meter = provider.meter("project-portfolio-management-service");
    let histogram = meter
