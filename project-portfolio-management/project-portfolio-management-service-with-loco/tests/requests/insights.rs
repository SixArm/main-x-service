...
async fn executive_financial_and_technology_views() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        let portfolio: Value = request
            ...
            .await
            .json();
        let parent_pid = portfolio["pid"].as_str().expect("pid").to_string();
        let project: Value = request
            .post("/api/plans")
            .json(&json!({
                "kind": "Project", "name": "Platform rebuild",
                "parent_ref": parent_pid,
        ...
        let line: Value = request
            .post(&format!("/api/plans/{project_pid}/budget-lines"))
        ...
        request
            .post(&format!(
                "/api/plans/{project_pid}/budget-lines/{line_pid}/actual"
            ))
        ...
        request
            .post(&format!("/api/plans/{product_pid}/budget-lines"))
        ...
        request
            .post(&format!("/api/plans/{project_pid}/gate-reviews"))
        ...
        let benefit: Value = request
            .post(&format!("/api/plans/{product_pid}/benefits"))
        ...
        request
            .post(&format!(
                "/api/plans/{product_pid}/benefits/{benefit_pid}/realize"
            ))
        ...
        let bucket = health["portfolios"]
            ...
            .expect("portfolios")
            .iter()
            .find(|b| b["portfolio"]["pid"] == json!(parent_pid))
        ...
        let capex = by_category
            .iter()
            .find(|c| c["category"] == "capex")
            .expect("capex");
        ...
        let postgres = techs
            .iter()
            .find(|t| t["technology"] == "postgres")
            .expect("pg");
        assert_eq!(postgres["ring"], "unclassified");
        let rust = techs
            .iter()
            .find(|t| t["technology"] == "rust")
            .expect("rust");
...
async fn moderate_fits_tranches_debt_flow_alignment_compare() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        request
            .post(&format!("/api/plans/{pid}/risks"))
        ...
        request
            .post(&format!("/api/plans/{pid}/objectives"))
        ...
        let roomy: Value = request
            .post("/api/scenarios")
            .json(&json!({ "name": "Roomy", "plan_pids": [pid],
                            "budget_cap_minor": 10_000, "currency": "GBP" }))
            .await
            .json();
        let tight: Value = request
            .post("/api/scenarios")
            .json(&json!({ "name": "Tight", "plan_pids": [pid],
        ...
        assert_eq!(
            compare["deltas"]["planned_by_currency"][0]["delta_minor"],
            0
        );
    ...
    request::<App, _, _>(|request, _ctx| async move {
        ...
        for (title, category, prob, impact) in [
            ...
            request
                .post(&format!("/api/plans/{pid}/risks"))
                .json(
                    &json!({ "title": title, "probability": prob, "impact": impact,
                                "category": category }),
                )
                .await
                .assert_status_ok();
        ...
        let benefit: Value = request
            ...
            .json(
                &json!({ "title": "Postage saved", "category": "cost_saving",
        ...
        request
            .post(&format!("/api/plans/{pid}/benefits/{b_pid}/realize"))
            .json(&json!({ "amount_minor": 500_i64 }))
            .await
            .assert_status_ok();
        let scenario: Value = request
            .post("/api/scenarios")
            .json(&json!({ "name": "Commit case", "plan_pids": [pid],
        ...
        assert_eq!(
            pack["tranches_released"]["per_currency"][0]["currency"],
            "GBP"
        );
        ...
        request
            ...
            .json(&json!({ "kind": "Product", "name": "Stale product",
                            "parent_ref": pf_pid, "target_date": "2020-01-01" }))
            .await
            .assert_status_ok();
        ...
        assert!(
            findings["findings"]
                ...
                .expect("findings")
                .iter()
                .any(|f| f["rule"] == "overdue_item_without_recent_gate_review"),
...
async fn engineering_tasks_sprints_and_views() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        let task_a: Value = request
            .post(&format!("/api/plans/{pid}/tasks"))
        ...
        request
            .patch(&format!("/api/plans/{pid}/tasks/{a_pid}"))
        ...
        let done_task: Value = request
            .patch(&format!("/api/plans/{pid}/tasks/{b_pid}"))
            .json(&json!({ "status": "done" }))
            .await
            .json();
        assert!(
            done_task["done_at"].is_string(),
            "first done stamps done_at"
        );

        // PUT refuses status changes (flow stamps stay true).
        let sneaky = request
            .put(&format!("/api/plans/{pid}/tasks/{a_pid}"))
            .json(&json!({ "title": "Wire the API", "status": "done" }))
            .await;
        assert_eq!(sneaky.status_code(), 422);

        // List + counts.
        let listed: Value = request.get(&format!("/api/plans/{pid}/tasks")).await.json();
        ...
        let burndown: Value = request
            .get(&format!("/api/plans/{pid}/burndown?sprint={sprint_pid}"))
            .await
            .json();
        ...
        assert!(
            burndown["derivation"]
                .as_str()
                .expect("d")
                .contains("no ideal line")
        );

        // ── Standup digest: creations + moves + the current blocker.
        let standup: Value = request
            .get(&format!("/api/plans/{pid}/standup"))
            .await
            .json();
        ...
        let all: Value = request
            .get("/api/engineering/milestone-calendar")
            .await
            .json();
...
async fn engineering_moderate_points_notes_wip_devops() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        request
            ...
            .assert_status_ok();
        request
            .patch(&format!("/api/plans/{pid}/tasks/{pointed_pid}"))
            .json(&json!({ "status": "done" }))
            .await
            .assert_status_ok();
        let velocity: Value = request
            .get(&format!("/api/plans/{pid}/velocity"))
            .await
            .json();
        ...
        let note: Value = request
            .post(&format!("/api/plans/{pid}/sprints/{s_pid}/notes"))
        ...
        let converted: Value = request
            .post(&format!(
                "/api/plans/{pid}/sprints/{s_pid}/notes/{n_pid}/convert"
            ))
            .await
            .json();
        ...
        assert_eq!(
            request
                .post(&format!(
                    "/api/plans/{pid}/sprints/{s_pid}/notes/{o_pid}/convert"
                ))
                .await
                .status_code(),
            422,
        ...
        assert_eq!(
            request
                ...
                .json(&json!({ "plan_pid": pid, "kind": "deploy" }))
                .await
                .status_code(),
            422,
        ...
        let incident: Value = request
            .post("/api/devops/events")
            .json(&json!({ "plan_pid": pid, "kind": "incident",
        ...
        assert_eq!(
            request
                .post("/api/devops/events")
                .json(&json!({ "plan_pid": pid, "kind": "recovery" }))
                .await
                .status_code(),
            422,
...
async fn wip_limits_refuse_overfull_columns() {
    ...
    request::<App, _, _>(|request, _ctx| async move {
        ...
        request
            .patch(&format!("/api/plans/{pid}/tasks/{}", task_pids[0]))
            .json(&json!({ "status": "in_progress" }))
            .await
            .assert_status_ok();
        let refused = request
            .patch(&format!("/api/plans/{pid}/tasks/{}", task_pids[1]))
            .json(&json!({ "status": "in_progress" }))
            .await;
