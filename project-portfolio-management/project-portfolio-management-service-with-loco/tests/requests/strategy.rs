...
//! the idea funnel end-to-end (idea → proposal → work item), scenario
...
async fn idea_funnel_flows_into_the_intake_pipeline() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        let loud: Value = request
            .post("/api/ideas")
            .json(&json!({ "title": "Popular idea", "pitch": "Everyone wants this",
                            "tags": ["platform", "q3"] }))
            .await
            .json();
        let loud_pid = loud["pid"].as_str().expect("pid").to_string();
        for _ in 0..3 {
            request.post(&format!("/api/ideas/{loud_pid}/vote")).await.assert_status_ok();
        }
        ...
        let proposal: Value = request.get(&format!("/api/proposals/{proposal_pid}")).await.json();
        ...
        assert_eq!(
            request.post(&format!("/api/ideas/{loud_pid}/vote")).await.status_code(),
            422
        );
        ...
        let done: Value = request.get(&format!("/api/proposals/{proposal_pid}")).await.json();
...
async fn scenarios_evaluate_and_commit_feasibly() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        request
            .post(&format!("/api/projects/{alpha}/risks"))
        ...
        assert_eq!(
            request
                .post("/api/scenarios")
                .json(&json!({ "name": "Ghost", "work_item_pids": [uuid::Uuid::new_v4()] }))
                .await
                .status_code(),
            422
        );
        ...
        assert_eq!(
            request.post(&format!("/api/scenarios/{tight_pid}/commit")).await.status_code(),
            422,
        ...
        assert_eq!(
            request.post(&format!("/api/scenarios/{fits_pid}/commit")).await.status_code(),
            422,
            "already committed"
        );
    })
...
async fn okr_alignment_rolls_up() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        assert_eq!(
            request
                .post(&format!("/api/projects/{project_pid}/objectives"))
        ...
        request
            ...
            .assert_status_ok();
        request
            .post(&format!("/api/programs/{program_pid}/objectives"))
        ...
        let item_view: Value = request
            .get(&format!("/api/projects/{project_pid}/objectives"))
            .await
            .json();
...
async fn benefits_track_realization_and_roi() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        assert_eq!(
            request
                .post(&format!("/api/products/{pid}/benefits"))
        ...
        let benefit: Value = request
            .post(&format!("/api/products/{pid}/benefits"))
            .json(&json!({ "title": "Support cost saving", "category": "cost_saving",
        ...
        let line: Value = request
            .post(&format!("/api/products/{pid}/budget-lines"))
        ...
        request
            .post(&format!("/api/products/{pid}/budget-lines/{line_pid}/actual"))
            .json(&json!({ "amount_minor": 1_000_000 }))
            .await
            .assert_status_ok();
        request
            .post(&format!("/api/products/{pid}/benefits/{benefit_pid}/realize"))
            .json(&json!({ "amount_minor": 1_500_000, "status": "on_track" }))
            .await
            .assert_status_ok();

        let listed: Value = request.get(&format!("/api/products/{pid}/benefits")).await.json();
