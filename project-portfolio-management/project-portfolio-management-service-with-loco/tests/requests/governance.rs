...
async fn intake_pipeline_promotes_into_the_registry() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        // Promote mints the plan in `projects`.
        let promoted: Value = request
            .post(&format!("/api/proposals/{pid}/promote"))
            .await
            .json();
        assert_eq!(promoted["status"], "promoted");
        let item_pid = promoted["plan_pid"].as_str().expect("plan pid");
        let item: Value = request.get(&format!("/api/plans/{item_pid}")).await.json();
        ...
        assert_eq!(final_state["promoted_plan_pid"].as_str(), Some(item_pid));
    })
    .await;
}
...
async fn duplicate_demand_is_flagged_at_intake() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        assert!(
            hits.iter().any(|h| h["source"] == "plan"),
            "the live plan is flagged: {hits:?}"
        );
        assert!(
            hits.iter()
                .any(|h| h["source"] == "proposal" && h["pid"] == first["pid"]),
...
async fn gate_journey_advances_stage_in_order() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        let listed: Value = request
            .get(&format!("/api/plans/{pid}/gate-reviews"))
            .await
            .json();
...
async fn risks_score_and_escalate() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        let high: Value = request
            .post(&format!("/api/plans/{pid}/risks"))
            .json(
                &json!({ "title": "Vendor collapse", "probability": 4, "impact": 5,
                            "mitigation": "second-source the contract" }),
            )
            .await
            .json();
        let listed: Value = request.get(&format!("/api/plans/{pid}/risks")).await.json();
        ...
        let closed: Value = request
            .put(&format!("/api/plans/{pid}/risks/{low_pid}"))
...
async fn budgets_track_variance_and_the_summary_aggregates() {
    request::<App, _, _>(|request, _ctx| async move {
        ...
        let line: Value = request
            .post(&format!("/api/plans/{pid}/budget-lines"))
            .json(
                &json!({ "category": "capex", "description": "Platform build",
        ...
        let listed: Value = request
            .get(&format!("/api/plans/{pid}/budget-lines"))
            .await
            .json();
        ...
        let summary: Value = request
            .get(&format!("/api/plans/{pid}/governance"))
            .await
            .json();
