...
//!
//! `POST /api/plans/merge` folds a duplicate into a survivor. The inputs are
...
use project_portfolio_management_service::merge;

/// Build a `Plan` from a byte slice, round-robining NUL-separated chunks
/// across the collections the fold unions.
fn build(data: &[u8]) -> Plan {
    let mut chunks = data.split(|b| *b == 0).map(String::from_utf8_lossy);
    let mut record = Plan::new(chunks.next().unwrap_or_default().into_owned());
    for (i, chunk) in chunks.enumerate() {
        let s = chunk.into_owned();
        match i % 4 {
            0 => record.alternate_names.push(s),
            1 => record.keywords.push(s),
            2 => record.tags.push(s),
...
fuzz_target!(|data: &[u8]| {
    ...
    let first = merge::merge_plans(&main, &duplicate);
    assert_eq!(first.merged.name, main.name, "merge renamed the survivor");
    assert_eq!(
        merge::merge_plans(&main, &duplicate).merged.name,
        first.merged.name,
        "merge is not deterministic"
    );

    let second = merge::merge_plans(&first.merged, &duplicate);
    for (field, a, b) in [
        (
            "alternate_names",
        ...
        ("tags", first.merged.tags.len(), second.merged.tags.len()),
        (
            "same_as",
