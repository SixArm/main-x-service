...
| Address | `0.10` | Weight-renormalised average across postcode (`0.5`), city (`0.3`), line 1 (`0.2`), plus (OQ-L) line 2 (`0.1`), county (`0.1`), country (`0.05`), additive on top so the first three keep their original weights unchanged. Postcode is exact-equality on normalised form. City / line 2 / county / country use Jaro-Winkler on the normalised name (the OQ-L trio also require the normalised form to be non-empty on both sides, so a shared blank never scores a spurious `1.0`). Line 1 runs `parse_address_line` on both sides; with house numbers present on both, `0.6 × jaro_winkler(street) + 0.4 × (house_number_a == house_number_b)`, else street similarity alone. An empty address scores neutral `0.5`. |
