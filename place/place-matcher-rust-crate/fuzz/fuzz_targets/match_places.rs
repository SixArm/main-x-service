...
//!
//! Interprets the fuzz bytes as a JSON `[place_a, place_b]` tuple,
//! then runs `MatchingEngine::match_places`. Pins the two load-bearing
...
use place_matcher::{MatchConfig, MatchingEngine, Place};
...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok((a, b)) = serde_json::from_slice::<(Place, Place)>(data) else {
        return;
    };

    let engine = MatchingEngine::new(MatchConfig::default());
    // Run both orderings so either argument position is exercised.
    for result in [engine.match_places(&a, &b), engine.match_places(&b, &a)] {
        assert!(
