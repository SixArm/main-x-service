...
//! with `PLACE_REQUIRE_AUTH` **on** — the "activation proof" (AU-1).
//!
...
/// `POST` a place payload, returning the status **and** the body, so a
/// refusal reports the server's reason rather than only a status
/// mismatch.
async fn post_place_verbose(app: &axum::Router, token: Option<&str>) -> (StatusCode, String) {
    ...
    let response = app
        ...
        .oneshot(
            builder
                .body(Body::from(serde_json::to_vec(&place_body()).unwrap()))
                .unwrap(),
        )
        .await
        .unwrap();
...
/// `POST` a place payload with an optional bearer, returning the status.
async fn post_place(app: &axum::Router, token: Option<&str>) -> StatusCode {
    let mut builder = Request::builder()
        .method("POST")
        .uri("/api/places")
        .header("content-type", "application/json");
...
async fn enforcement_on_gates_the_real_router() {
    ...
    unsafe {
        std::env::set_var("PLACE_REQUIRE_AUTH", "1");
        std::env::set_var("PLACE_PASETO_KEYS", keys.to_string());
        // No PLACE_ABAC_POLICY ⇒ the built-in default policy: read
    ...
    assert_eq!(
        get(&app, "/api/places/search?q=anyone", None).await,
        StatusCode::UNAUTHORIZED,
        "a protected read without a token is 401"
    );
    assert_eq!(
        post_place(&app, None).await,
        StatusCode::UNAUTHORIZED,
    ...
    assert_eq!(
        get(&app, "/api/places/search?q=anyone", Some("not-a-token")).await,
        StatusCode::UNAUTHORIZED,
    ...
    let reader = mint(&kid, &[]);
    assert_eq!(
        get(&app, "/api/places/search?q=anyone", Some(&reader)).await,
        StatusCode::OK,
        "any authenticated caller may read"
    );
    assert_eq!(
        post_place(&app, Some(&reader)).await,
    ...
    let (status, body) = post_place_verbose(&app, Some(&writer)).await;
    assert_eq!(
        status,
        StatusCode::CREATED,
    ...
    unsafe {
        std::env::remove_var("PLACE_REQUIRE_AUTH");
        std::env::remove_var("PLACE_PASETO_KEYS");
    }
}
