...
//! [`observability::trace_mw`](place_service::observability::trace_mw)
...
use place_service::observability::{TelemetryConfig, build_tracer_provider, trace_mw};
...
async fn a_served_request_exports_a_span_and_returns_its_traceparent() {
    ...
    let config = TelemetryConfig {
        service_name: "place-service-mw-test".to_string(),
    ...
    tracing_subscriber::registry()
        .with(tracing_opentelemetry::layer().with_tracer(provider.tracer("place-service")))
        .with(tracing_subscriber::EnvFilter::new(
            "place_service=trace,otlp_middleware=trace",
        ))
        .init();
