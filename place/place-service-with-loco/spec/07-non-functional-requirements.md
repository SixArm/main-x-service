...
| Security | Offline PASETO v4.public bearer verification against the auth-service published Ed25519 keys — opt-in per handler, plus blanket `/api/*` enforcement middleware (default-off, `PLACE_REQUIRE_AUTH`; public allow-list: health, OpenAPI/Swagger, `/metrics.prom`; roles are the T-8 remainder); TLS at the edge |
