//! `event_outbox` write + relay surface for the durable event bus
...
//! [`PlaceRepository`](crate::db::PlaceRepository) trait (whose methods
//! cannot be generic and stay object-safe).
//!
//! [`Model::recent`] projects the last N rows to the frozen operator
...
use crate::streaming::envelope::{Envelope, EventKind, EventView};

pub use event_outbox::Model;
...
impl OutboxInsert {
    ...
    /// Convenience: build the row for `kind` applied to `place`, stamping
    /// `occurred_at` to now. Wraps [`Envelope::for_place`](crate::streaming::Envelope::for_place)
    /// + [`Self::from_envelope`].
    ///
    /// # Errors
    ///
    /// As [`Self::from_envelope`].
    pub fn for_place(place: &crate::models::place::Place, kind: EventKind) -> Result<Self> {
        let env = Envelope::for_place(place, kind);
    ...
    ///
    /// As [`Self::from_envelope`].
    pub fn for_merge(survivor: &crate::models::place::Place, merged_from: &Uuid) -> Result<Self> {
    ...
    /// can take `&self`.
    fn active_model(&self) -> event_outbox::ActiveModel {
...
impl Model {
    ...
    /// `/events/recent` endpoint when the transport is `outbox`; a row
    /// whose payload cannot be parsed is skipped rather than failing the
    /// whole read.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn from_envelope_maps_every_column_and_keeps_the_full_payload() {
        ...
        assert_eq!(row.payload["name"], "Central Park");
        assert_eq!(row.payload["entity"], "place");
