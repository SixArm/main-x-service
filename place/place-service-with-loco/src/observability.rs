...
//! | `OTLP_SERVICE_NAME` | `place-service` | `service.name` resource attribute |
//! | `OTLP_ENDPOINT` | `http://localhost:4317` | OTLP/gRPC collector endpoint |
//!
//! These three variables are **deliberately not `PLACE_`-prefixed**,
...
pub const DEFAULT_SERVICE_NAME: &str = "place-service";
...
fn http_metrics() -> &'static HttpMetrics {
    ...
    METRICS.get_or_init(|| {
        let meter = global::meter("place-service");
        HttpMetrics {
            duration: meter
...
/// embed record UUIDs (`/api/places/{id}`, …), so a per-path label would be
/// unbounded cardinality — the same reasoning link-graph-service applied to
/// its own `EntityRef`-shaped paths, here for plain UUID path segments
/// instead.
///
...
mod tests {
    ...
    fn exporter_diagnostics_are_added_to_a_default_filter() {
        ...
        let base = || tracing_subscriber::EnvFilter::new("place_service=info");
    ...
    fn an_operator_supplied_filter_is_left_alone() {
        ...
        assert_eq!(untouched.to_string(), "place_service=info");
    }
    ...
    fn explicit_service_name_and_endpoint_are_honoured() {
        let config = TelemetryConfig::resolve(
            Some("place-service-canary"),
            Some("http://collector.internal:4317"),
        );
        assert_eq!(config.service_name, "place-service-canary");
        assert_eq!(
    ...
    fn resource_carries_service_name_and_version() {
        ...
        let name = resource
            ...
            .expect("service.name");
        assert_eq!(name.as_str(), "place-service");
