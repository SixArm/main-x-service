...
//! `FluvioSink` (BUS-3, porting BUS-1's case-service reference; only
...
//! [`LoggingSink`] when `PLACE_FLUVIO_ENDPOINT` is configured — see
...
/// The real-broker sink (BUS-3, porting BUS-1's case-service reference;
...
/// Fluvio broker endpoint (`PLACE_FLUVIO_ENDPOINT`, §7). Unset/blank ⇒ no
...
pub fn fluvio_endpoint() -> Option<String> {
    std::env::var("PLACE_FLUVIO_ENDPOINT")
        .ok()
        .filter(|s| !s.trim().is_empty())
}

/// The topic events publish to (`PLACE_EVENT_TOPIC`, default
/// `mxi.place.events` per §7's `mxi.<entity>.events` convention).
#[must_use]
pub fn event_topic() -> String {
    std::env::var("PLACE_EVENT_TOPIC")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| "mxi.place.events".to_string())
}
...
///
/// Sink selection: no `PLACE_FLUVIO_ENDPOINT` ⇒ [`LoggingSink`]. An
...
pub fn spawn(db: DatabaseConnection) {
    ...
    if endpoint.is_some() && !cfg!(feature = "fluvio") {
        tracing::error!(
            "PLACE_FLUVIO_ENDPOINT is set but this binary was built without the `fluvio` \
