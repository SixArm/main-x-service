//! Row-level integrity over the assembled `Place` record.
//!
//! ## Why the assembled record, not the root row
//!
//! A place's data lives across several tables — the root `places` row
...
//! the same value `GET /api/places/{id}` returns.
//!
...
pub const RECORD_HASH_VERSION: &str = "pl-r1";
...
pub fn preimage(input: &RecordInput<'_>) -> Vec<u8> {
    ...
    field(
        &serde_json::to_string(input.place).unwrap_or_else(|_| "\u{0}unserializable".to_string()),
    );
...
/// simply had nowhere to go, and noplace pointed that out.
#[derive(Debug, Clone)]
    ...
    for row in rows {
        ...
        if !sha256_ok || !sha3_ok || !mac_ok {
            report.mismatched.push(RecordMismatch {
                id: row.place.id.to_string(),
                name: row.place.name.clone(),
            });
        }
    }
...
mod tests {
    ...
    fn place(name: &str) -> Place {
        let mut t = Place::new(name);
        t.id = Uuid::from_u128(1);
        t
    }

    fn input(t: &Place) -> RecordInput<'_> {
        RecordInput {
            id: t.id,
            place: t,
            is_deleted: false,
    ...
    fn spec_documents_this_version_tag() {
        assert_eq!(RECORD_HASH_VERSION, "pl-r1");
    }
