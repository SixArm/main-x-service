...
<script lang="ts">
    ...
    function typeLabel(type: MatchResult["place"]["place_type"]): string {
        ...
        return typeof type === "string"
            ? type
            : translate("matchResults.typeOther").replace(
                  "{value}",
                  type.Other,
              );
    }
            ...
            {#each results as r}
                <li class="result">
                    ...
                    <div class="meta small muted">
                        {#if r.place.place_type}{typeLabel(
                                r.place.place_type,
                            )}{/if}
                        {#if r.place.address?.address_locality}
                            · {r.place.address.address_locality}{/if}
                        <!-- Link to the candidate's detail page, showing a short id prefix. -->
                        {#if r.place.id}
                            · <a href={`/places/${r.place.id}`}
                                >{r.place.id.slice(0, 8)}…</a
                            >{/if}
                    </div>
                    ...
                    {#if r.breakdown}
                        <details>
                            <summary class="small"
                                >{t("matchResults.scoreBreakdown")}</summary
                            >
                            <ul class="breakdown small">
                                {#if r.breakdown.name_score != null}<li>
                                        {t("matchResults.nameScore")}: {(
                                            r.breakdown.name_score * 100
                                        ).toFixed(0)}%
                                    </li>{/if}
                                {#if r.breakdown.geo_score != null}<li>
                                        {t("matchResults.geoScore")}: {(
                                            r.breakdown.geo_score * 100
                                        ).toFixed(0)}%
                                    </li>{/if}
                                {#if r.breakdown.address_score != null}<li>
                                        {t("matchResults.addressScore")}: {(
                                            r.breakdown.address_score * 100
                                        ).toFixed(0)}%
                                    </li>{/if}
                                {#if r.breakdown.identifier_score != null}<li>
                                        {t("matchResults.identifierScore")}: {(
                                            r.breakdown.identifier_score * 100
                                        ).toFixed(0)}%
                                    </li>{/if}
                                {#if r.breakdown.phonetic_match}<li>
                                        {t("matchResults.phoneticMatch")}
                                    </li>{/if}
                                {#if r.breakdown.deterministic_match}<li>
                                        {t("matchResults.deterministicMatch")}
                                    </li>{/if}
                            </ul>
                        </details>
                    {/if}
