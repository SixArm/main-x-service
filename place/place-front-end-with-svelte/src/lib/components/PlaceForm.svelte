    ...
    const form = createForm<Place>({
        ...
        validate(value) {
            ...
            if (value.geo) {
                ...
                if (
                    value.geo.latitude_as_decimal_degrees < -90 ||
                    value.geo.latitude_as_decimal_degrees > 90
                )
                    errors.latitude_as_decimal_degrees =
                        translate("form.latRange");
                if (
                    value.geo.longitude_as_decimal_degrees < -180 ||
                    value.geo.longitude_as_decimal_degrees > 180
                )
                    errors.longitude_as_decimal_degrees =
                        translate("form.lonRange");
            }
            // GLN, when supplied, must be exactly 13 digits (GS1).
            if (
                value.global_location_number &&
                !/^\d{13}$/.test(value.global_location_number)
            ) {
...
<form onsubmit={handleSubmit} class="stack">
    ...
    <FieldRow>
        ...
        <LabeledField
            label={t("form.gln")}
            for="gln"
            error={form.errors.gln}
            hint={t("form.glnHint")}
        >
            <input
                id="gln"
                bind:value={form.value.global_location_number}
                maxlength="13"
            />
        </LabeledField>
    </FieldRow>
        ...
        <header class="row" style="justify-content: space-between">
            ...
            <label class="row small">
                <input
                    ...
                    checked={hasAddress}
                    onchange={(e) =>
                        toggleAddress((e.target as HTMLInputElement).checked)}
                />
        ...
        <header class="row" style="justify-content: space-between">
            ...
            <label class="row small">
                <input
                    ...
                    checked={hasGeo}
                    onchange={(e) =>
                        toggleGeo((e.target as HTMLInputElement).checked)}
                />
