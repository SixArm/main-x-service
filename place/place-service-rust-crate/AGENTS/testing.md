...
| `validation`            | 23    | Valid place, empty/whitespace name, invalid lat/lon, valid coords, invalid/valid GLN + check-digit helper, valid/invalid opening-hours times + `time_is_valid` helper, invalid/valid URL, invalid/valid telephone, address missing fields, address with locality, multiple errors, normalization |
